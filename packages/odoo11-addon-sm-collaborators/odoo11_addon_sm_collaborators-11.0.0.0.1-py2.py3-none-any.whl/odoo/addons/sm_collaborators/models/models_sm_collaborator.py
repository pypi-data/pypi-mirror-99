...
class sm_collaborator(models.Model):
  ...
  related_batch_orders_id = fields.Many2many('sommobilitat.sm_batch_payment_order',
    string=_("Rel orders"),
    compute="_get_rel_batch_orders", store=False)

  _order = "date desc"

  @api.depends('related_member_id')
  def _get_rel_batch_orders(self):
    for record in self:
      record.related_batch_orders_id = record.related_member_id.batch_orders_id
