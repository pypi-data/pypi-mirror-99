...
from algorith.arrays.birth_death.cad_as_violations import complete_intervals, system_downs


def l_pod_k_of_n(l,k,n,p_obj,p_pod):
    return 1

def l_pod_n_quorum(l,n,p_obj,p_pod):
    return l_pod_k_of_n(l,n//2+1,n,p_obj,p_pod)

def k_of_n_av(k,n,p):
    return binom.sf(k-1,n,p)

def k_of_n_rate(k,n,lmb,mu):
    p=mu/(lmb+mu)
    return n*lmb*p*binom.pmf(k-1,n-1,p)/binom.sf(k-1,n,p)

def k_of_n_sim(k,n,lmb,mu):
    nodes = []
    for i in range(n):
        node1 = birth_death_gen(lmb,mu,1e4)
        node1 = node1[node1.start>7e3 and node1.state=="down"]
        nodes.append(node1)
    dat = pd.concat(nodes)
    dat = dat.sort_values(by=['start'])
    dd = system_downs(dat.start,dat.end,n-k+1)
