           ...
           r_total = np.array(Image.open(BytesIO(r.content)), dtype = 'uint8')
            ...
            for tileY in y_tiles:
                for tileX in x_tiles:
                    ...
                    r = s.get(url + '/tileService/' + mapId + '/' + str(timestamp) + '/' + layerName + '/' + str(zoom) + '/' + str(tileX) + '/' + str(tileY) + token_inurl ,
                                 timeout = 10 )
...
def newMap(name,  token, bands, dataType = 'float32' , dryRun = True):


    body = {'name':name, 'bands':bands, 'dataType':dataType, 'dryRun':dryRun }
