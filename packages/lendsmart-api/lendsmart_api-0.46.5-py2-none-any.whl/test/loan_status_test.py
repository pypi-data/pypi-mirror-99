import sys
sys.path.append('/home/lendsmart/code/lendsmart/workspace/py_lscommon/core_api/')
from ramda import path_or,is_empty,head
from lendsmart_api.lendsmart_client import LendsmartClient
from lendsmart_api.jwt.access_token.service_account import ServiceAccount

api_endpoint = "https://devapi.lendsmart.ai/api/v1"
service_account_key = """-----BEGIN PRIVATE KEY-----

PtKjZcbK+onu2Q9pkljc0rig
-----END PRIVATE KEY-----"""

l = LendsmartClient(ServiceAccount(service_account_key),api_endpoint)
loan_app_id = '1719596660911153152'
loan_statuses = l.loan_status.describe_loan_status(loan_app_id)
print(loan_statuses)

if not is_empty(path_or([], ['items'], loan_statuses)):
    status=head(path_or([], ['items'], loan_statuses))
    print('got success', status)
    status['metadata']['testing']='To test py_lendsmart_api'
    updatedstatus = l.loan_status.update_loan_status(data=status)
    print('successfully update loan status', updatedstatus)
else:
    print('failed to get loan_statuses for loan_app_id', loan_app_id)
