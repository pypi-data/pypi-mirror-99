...
class FacileToolkit2:
    ...
    def i_see(self, *locators):
        ...
        if not_visible_elements:
            ...
            BuiltIn().run_keyword_and_continue_on_failure("Should Be Empty", not_visible_elements, "Following elements are not visible: %s" % errors)
    ...
    def i_dont_see(self, *locators):
        ...
        if visible_elements:
            ...
            BuiltIn().run_keyword_and_continue_on_failure("Should Be Empty", visible_elements, "Following elements are visible: %s" % errors)
    ...
    def error_message_should_be(self, message,
        ...
        self.set_selenium_library()
        self._wait_until(
            lambda: self._error_message_should_be(message, error_locator, case_insensitive, match),
            "Error finding message '%s' in <TIMEOUT>." % message, timeout, None
        )

    def _error_message_should_be(self, message, error_locator, case_insensitive, match):
        """
        Checks if the given error message is present in DOM
        """
        self.set_selenium_library()
...
if __name__ == "__main__":
    robot.run('%s/atests/keyword_tests.robot' % os.getcwd())
