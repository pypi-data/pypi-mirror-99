...
class LinearOperatorLowRankUpdate(linear_operator.LinearOperator):
  ...
  ```
  tensor_shape.TensorShape(operator.shape) = [B1,...,Bb] + [M, N],  with b >= 0
  tensor_shape.TensorShape(x.shape) =        [B1,...,Bb] + [N, R],  with R >= 0.
  ```
  ...
  `x` having `tensor_shape.TensorShape(x.shape) = [N, R]` with `O(L_matmul*N*R)` complexity (and similarly
  for `solve`, `determinant`.  Then, if `tensor_shape.TensorShape(x.shape) = [N, R]`,
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def __init__(self,
    ...
    if diag_update is not None:
      if is_diag_update_positive and np.issubdtype(dtype, np.complexfloating):
    ...
    if base_operator.is_self_adjoint and v is None and not np.issubdtype(dtype, np.complexfloating):
  ...
  def _matmul(self, x, adjoint=False, adjoint_arg=False):
    ...
    if adjoint:
      uh_x = _linalg.matmul(u, x, adjoint_a=True, adjoint_b=adjoint_arg)
      d_uh_x = d.matmul(uh_x, adjoint=adjoint)
      v_d_uh_x = _linalg.matmul(v, d_uh_x)
      return leading_term + v_d_uh_x
    else:
      vh_x = _linalg.matmul(v, x, adjoint_a=True, adjoint_b=adjoint_arg)
      d_vh_x = d.matmul(vh_x, adjoint=adjoint)
      u_d_vh_x = _linalg.matmul(u, d_vh_x)
  ...
  def _make_capacitance(self):
    ...
    # V^H L^{-1} U
    vh_linv_u = _linalg.matmul(self.v, linv_u, adjoint_a=True)
