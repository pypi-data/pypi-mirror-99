...
class MCP23017_I2C:
    ...
    def invert_input(self, pin, invert = False):
        ...
        reg, bit = self.register_bit_select(pin, reg8A=0x01,reg16A=0x02, reg8B=0x11, reg16B=0x03)
        regValue = self.single_access_read(reg)
        if invert == True:
            mask = 0x00 | (1<<bit)           
            regValue = regValue | mask                
    ...
    def input(self, pin):
        ...
        reg, bit = self.register_bit_select(pin, reg8A=0x09,reg16A=0X12, reg8B=0X19, reg16B=0x13)
        regValue = self.single_access_read(reg)
        mask = 0x00 | (1<<bit)
        value = regValue & mask
        value = value >> bit              
    ...
    def input_at_interrupt(self, pin):
        ...
        INTCAPA/B registers"""    
        reg, bit = self.register_bit_select(pin, reg8A=0x08,reg16A=0X10, reg8B=0X18, reg16B=0x11)
        regValue = self.single_access_read(reg)
        mask = 0x00 | (1<<bit)
        value = regValue & mask
        value = value >> bit       
