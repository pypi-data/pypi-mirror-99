...
class OptionalParam(Decorator):
    ...
    async def target(self, interface: DecoratorInterface) -> Optional[Any]:
        try:
            return Force(await interface.dispatcher_interface.lookup_param(
                interface.dispatcher_interface.name,
                    interface.dispatcher_interface.annotation.__args__[0]\
                    if isinstance(interface.dispatcher_interface.annotation, typing._GenericAlias) and\
                        type(None) in interface.dispatcher_interface.annotation.__args__\
