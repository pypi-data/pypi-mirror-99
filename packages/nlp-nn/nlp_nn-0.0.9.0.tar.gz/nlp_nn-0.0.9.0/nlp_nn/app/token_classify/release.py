...
import logging
import argparse
from ...model.token_classifier import DeviceSettings
...
def main():
    ...
    token_classify = TokenClassify(device_settings=DeviceSettings(gpu_idx=-1))
    if token_classify.load_model_ckpt(model_path_ckpt=ckpt_path) is False:
        logging.error("Failed to load ckpt model")
        return -1
    if token_classify.release_model(model_path_ckpt=ckpt_path, model_path_script=release_path):
