...
import logging
import argparse
from ...model.sent_multi_classifier import DeviceSettings
from ...model.sent_multi_classifier import SentMultiClassify
...
def main():
    ...
    sent_multi_classify = SentMultiClassify(device_settings=DeviceSettings(gpu_idx=-1))
    if sent_multi_classify.load_model_ckpt(model_path_ckpt=ckpt_path) is False:
        logging.error("Failed to load ckpt model")
        return -1
    if sent_multi_classify.release_model(model_path_ckpt=ckpt_path, model_path_script=release_path):
