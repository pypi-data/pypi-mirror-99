...
import logging
import argparse
from ...model.sent_similarity import DeviceSettings
...
def main():
    ...
    sent_similarity = SentSimilarity(device_settings=DeviceSettings(gpu_idx=-1))
    if sent_similarity.load_model_ckpt(model_path_ckpt=ckpt_path) is False:
        logging.error("Failed to load ckpt model")
        return -1
    if sent_similarity.release_model(model_path_ckpt=ckpt_path, model_path_script=release_path):
