...
class Image(object):
    ...
    def draw_contour(self, resolution, contour, color, thickness, bg_color):
        '''
        Draw only the outline of contour of color and thickness against bg.
        ...
        if color.shape[0] > 1:
            resolution = resolution + color.shape
        img_contour = cv2.drawContours(
            image=np.ones(resolution) * bg_color,
            contours=[contour], contourIdx=0,
