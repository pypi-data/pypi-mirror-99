...
def surv_likelihood(n_intervals):
    ...
    def loss(y_true, y_pred):
        ...
        uncens = (
            1.0 - y_true[:, n_intervals : 2 * n_intervals] * y_pred
        )  # component for only uncensored individuals
        return K.sum(
            -K.log(K.clip(K.concatenate((cens_uncens, uncens)), K.epsilon(), None)),
            axis=-1,
...
def surv_likelihood_rnn(n_intervals):
    ...
    def loss(y_true, y_pred):
        cens_uncens = 1.0 + y_true[0, :, 0:n_intervals] * (
            y_pred - 1.0
        )  # component for all patients
        uncens = (
            1.0 - y_true[0, :, n_intervals : 2 * n_intervals] * y_pred
