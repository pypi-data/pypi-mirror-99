...
class convert_string_to_datetime(AbstractRowProcessableDecorator):

    def __init__(self, date_format: str, columns_to_convert: dict):
        super().__init__()
    ...
    def _process_row(self, row: tuple) -> Generator:
        ...
        for column_to_convert, to_timezone in self.__columns_to_convert.items():
            ...
            date_str = handled_row[column_index]
            utc = datetime.strptime(date_str, self.__date_format).replace(tzinfo=timezone('UTC'))
            dt = utc.astimezone(tz=timezone(to_timezone))
