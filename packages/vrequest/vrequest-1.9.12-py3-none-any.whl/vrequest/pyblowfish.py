...
def PKCS7_padding(data, num):
    pad = num - (len(data) % num)
...
if __name__ == '__main__':
    key         = b'12345678'           # 4-56 个字符
    plaintext   = b'12345678'       # 如无pkcs则这里必须为 8 的倍数
    iv          = b'12345678'       # 8 字符 # 部分模式不使用该参数
    # ecb, ecb_cts 这两种模式可以不使用 iv， 默认是None

    mode = 'cbc'
    print('mode:', mode)
    s = encrypt(key, plaintext, iv, mode)
    print(s)
    v = decrypt(key, s, iv, mode)
    print(v)