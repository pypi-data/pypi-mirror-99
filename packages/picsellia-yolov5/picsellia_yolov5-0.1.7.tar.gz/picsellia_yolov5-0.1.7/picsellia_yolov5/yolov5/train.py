...
def train(hyp, opt, device, tb_writer=None, wandb=None, pxl=None):
    
    opt = generate_opt(opt)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            verified_tags = ['metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95',
                    'val/box_loss', 'val/obj_loss', 'val/cls_loss',  # val loss
                    'x/lr0', 'x/lr1', 'x/lr2']  # params
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                ...
                if pxl:
                    if tag in verified_tags:
                        try:
                            name = str(tag).replace('/', '_')
                            pxl.log(name=name, type='line', data=[x])
                        except Exception as e:
                            logger.info(str(e))
                            try:
                                #name = tag.cpu()
                                name = str(tag).replace('/', '_')
                                pxl.log(name=name, type='line', data=x.cpu().numpy())
                            except:
                                print(tag, "can't be uploaded")
            # Update best mAP
...
if __name__ == '__main__':
    ...
    if "experiment_id" in os.environ:
        ...
        exp = experiment.checkout(experiment_id, tree=False, with_file=False)
    else:
        if "experiment_name" in os.environ and "project_token" in os.environ:
            ...
            exp = experiment.checkout(experiment_name, tree=False, with_file=TrueFalse)
        else:
    ...
    if not opt.evolve:
        ...
        train(hyp, opt, device, tb_writer=None, wandb=None, pxl=exp)
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, device, pxl=exp)
