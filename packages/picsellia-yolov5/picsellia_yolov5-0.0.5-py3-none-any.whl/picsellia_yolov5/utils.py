...
def to_yolo(pxl_annotations_dict=None,labelmap=None, base_imgdir=None, targetdir=None, prop=0.7, copy_image=True):
    ...
    for i, path in tqdm.tqdm(enumerate(os.listdir(base_imgdir))):
        ...
        if copy_image:
            img.save(os.path.join(targetdir,"images", step, img_id.split('.')[0] + '.png', ), 'PNG')
        else:
        ...
        if success:
            ...
            label_name = "{}.txt".format(img_id.split('.')[0])
...
def setup_hyp(exp_name, cfg, data_yaml, checkpoint_dir, params={}, label_map=[]):
    YOLOSIZE = cfg.split('/')[-2].split('5')[-1]
    YOLODIR = 'YOLO-DS-{}'.format(exp_name)
    experiment_name = exp_name.replace('-', '_')

    model_exp_name = "yolov5{}_{}".format(YOLOSIZE, experiment_name)
    data_yaml = 'data/{}.yaml'.format(experiment_name.replace('_', '-'))

    tmp = os.listdir(checkpoint_dir)[0]

    pt_path = '../{}/'.format(os.path.join(checkpoint_dir, tmp))
    weight_path = '../{}'.format(pt_path)
    ...
    opt.hyp = 'data/hyp.scratch.yaml' if not 'hyperparams' in params.keys() else params["hyperparams"]

    opt.resume = False
    opt.bucket = ''
    opt.device = torch.device('0') if torch.cuda.is_available() else 'cpu'
    opt.local_rank = -1 
    opt.log_imgs = 4
    opt.workers = 4
