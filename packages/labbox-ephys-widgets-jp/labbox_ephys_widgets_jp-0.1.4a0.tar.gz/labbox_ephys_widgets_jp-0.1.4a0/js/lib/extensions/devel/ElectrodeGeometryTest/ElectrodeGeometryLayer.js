...
export const paintTestLayer = (painter, props) => {
    painter.wipe();
    const pen = { color: 'rgb(22, 22, 22)', width: 3 };
...
export const reportMouseMove = (e, layer) => {
    if (e.type !== ClickEventType.Move)
        return;
    console.log(`Mouse moved to relcoords at ${JSON.stringify(e.point)}`);
};
export const reportMouseClick = (e, layer) => {
    if (e.type !== ClickEventType.Press && e.type !== ClickEventType.Release)
        return;
    console.log(`Mouse ${e.type === ClickEventType.Press ? 'down' : 'up'} at ${JSON.stringify(e.point)}`);
};
export const reportMouseDrag = (layer, drag) => {
