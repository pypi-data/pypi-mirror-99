...
import { getHeight, getWidth } from '../../common/CanvasWidget/Geometry';
import { getArrayMax, getArrayMin } from '../../common/Utility';
import { handleAnimatedClick, paintAnimationLayer } from './AnimatedLayer';
import { handleClickTrail, paintClickLayer } from './ClickHistoryLayer';
import { paintDragLayer, setDragLayerStateFromProps, updateDragRegion } from './DragLayer';
import { paintTestLayer } from './ElectrodeGeometryLayer';
const computeElectrodeCoordinates = (electrodes) => {
...
const computeRadius = (electrodes, scaledCoordinates, electrodeRect) => {
    ...
    let leastNorm = Math.min(electrodeRect.xmin - scaledCoordinates.xmin, electrodeRect.ymin - scaledCoordinates.ymin, scaledCoordinates.xmax - electrodeRect.xmax, scaledCoordinates.ymax - electrodeRect.ymax);
...
export const setCanvasFromProps = (layer, layerProps) => {
    // layer.setBasePixelTransformationMatrix(layerProps.scaledCoordinates)
};
const createTestLayer = () => {
    return new CanvasWidgetLayer(paintTestLayer, setCanvasFromProps, {}, {
        discreteMouseEventHandlers: [],
        dragHandlers: [] //reportMouseDrag],
    });
};
const createDragLayer = () => {
    return new CanvasWidgetLayer(paintDragLayer, setDragLayerStateFromProps, {
        dragRegion: null,
...
const createClickLayer = () => {
    return new CanvasWidgetLayer(paintClickLayer, setCanvasFromProps, {
        clickHistory: []
    }, {
...
const createAnimatedLayer = () => {
    return new CanvasWidgetLayer(paintAnimationLayer, setCanvasFromProps, {
        points: [],
        newQueue: []
    }, {
...
const ElectrodeGeometry = (props) => {
    ...
    const layerProps = Object.assign(Object.assign({}, props), { width: width, height: height, scaledCoordinates: scaledCoordinates, electrodeRect: electrodeRect, electrodeRadius: radius });
    ...
    return (React.createElement(CanvasWidget, Object.assign({ key: 'canvas', layers: layers }, { width, height })));
};
