...
import { getBoundingBoxForEllipse, getHeight, getWidth, transformDistance } from '../../common/CanvasWidget/Geometry';
import { getArrayMax, getArrayMin } from '../../common/Utility';
const computeRadiusCache = new Map();
const computeRadius = (electrodeLocations) => {
...
const getElectrodesBoundingBox = (electrodeLocations, radius) => {
    return {
...
export const getElectrodesAspectRatio = (electrodeLocations) => {
...
const setupVerticalElectrodes = ({ width, height, electrodeIds }) => {
    ...
    const electrodeBoxes = electrodeIds.map((eid, ii) => {
        ...
        const transform0 = funcToTransform((p) => {
...
const fixDegenerateCase = (electrodeLocations) => {
    const box = {
...
const setupElectrodes = (args) => {
    ...
    const electrodeBoxes = realizedElectrodeLocations.map((loc, i) => {
        const x = loc[0];
        const y = loc[1];
...
//# sourceMappingURL=setupElectrodes.js.map