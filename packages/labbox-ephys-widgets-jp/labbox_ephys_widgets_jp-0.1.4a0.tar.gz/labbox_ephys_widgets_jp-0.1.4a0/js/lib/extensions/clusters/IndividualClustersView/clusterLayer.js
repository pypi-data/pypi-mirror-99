...
const paintLayer = (painter, props, state) => {
...
const getIndexAtPoint = (props, p) => {
    const { x, y } = props;
    const inds = x.map((x, ii) => (ii));
    const radius = 10 * (props.rect.xmax - props.rect.xmin) / props.width;
    const inds2 = inds.filter((ii) => (pointIsInEllipse(p, [x[ii], y[ii]], radius))).sort((i1, i2) => {
...
const handleHover = (event, layer) => {
    ...
    layer.setState(Object.assign(Object.assign({}, state), { hoverIndex: index }));
    layer.scheduleRepaint();
};
...
const createClusterLayer = () => {
    return new CanvasWidgetLayer(paintLayer, onUpdateLayerProps, initialClusterLayerState, {
        dragHandlers: [],
