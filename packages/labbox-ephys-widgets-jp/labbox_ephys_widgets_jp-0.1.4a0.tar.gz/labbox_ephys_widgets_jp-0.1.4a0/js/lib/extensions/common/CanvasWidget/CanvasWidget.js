...
import { ClickEventType, formClickEventFromMouseEvent, KeyEventType, MousePresenceEventType } from './CanvasWidgetLayer';
...
const dragReducer = (state, action) => {
    ...
    switch (type) {
        ...
        case COMPUTE_DRAG:
            ...
            if (!point)
                throw Error('Unexpected: no point');
            ...
            if (dragging && !dragAnchor)
                throw Error('Invalid state in computing drag: cannot have drag set with no dragAnchor.');
            ...
            if (dragging && dragAnchor)
                return {
                    ...
                    dragAnchor: dragAnchor,
                    dragPosition: point,
                    dragRect: [Math.min(dragAnchor[0], point[0]),
...
const CanvasWidget = (props) => {
    ...
    const divRef = React.useCallback((elmt) => {
    ...
    useEffect(() => {
        ...
        if (!layers)
            return;
        const stopScrollEvent = (e) => {
            if (divElement['_hasFocus']) {
                e.preventDefault();
            }
        };
        layers.forEach((L, i) => {
    ...
    const scheduledDispatchDragAction = useRef(null);
    const scheduleDispatchDrag = (a) => {
    ...
    const [prevDragState, setPrevDragState] = useState(null);
    ...
    const _handleDiscreteMouseEvents = useCallback((e, type) => {
        if (dragState.dragging)
            return;
        for (let l of layers) {
            if (l) {
    ...
    const _handleMousePresenceEvents = useCallback((e, type) => {
        if (!layers)
            return;
    ...
    const _handleMouseMove = useCallback((e) => {
    ...
    const _handleMouseDown = useCallback((e) => {
    ...
    const _handleMouseUp = useCallback((e) => {
        setHasFocus(true);
        const { point, mouseButton, modifiers } = formClickEventFromMouseEvent(e, ClickEventType.Move);
    ...
    return (React.createElement("div", { ref: divRef, style: { position: 'relative', width, height, left: 0, top: 0 }, onKeyDown: _handleKeyPress, tabIndex: 0 }, (layers || []).map((L, index) => (React.createElement("canvas", { key: 'canvas-' + index, style: { position: 'absolute', left: 0, top: 0 }, width: width, height: height, onMouseMove: _handleMouseMove, onMouseDown: _handleMouseDown, onMouseUp: _handleMouseUp, onMouseEnter: _handleMouseEnter, onMouseLeave: _handleMouseLeave, onWheel: _handleWheel })))));
};
