...
export const homogenizeVec2 = (v) => {
    return [v[0], v[1], 1];
};
export const Vec2HToVector = (v) => {
...
export const pointIsInEllipse = (pt, center, xRadius, yRadius) => {
    yRadius = yRadius || xRadius;
    const dist = Math.pow(((pt[0] - center[0]) / xRadius), 2) + Math.pow(((pt[1] - center[1]) / yRadius), 2);
    return dist <= 1;
};
export const isRectangularRegion = (x) => {
    ...
    if (x.xmin > x.xmax)
        return false;
    if (x.ymin > x.ymax)
        return false; // TODO: Delete this? It doesn't hold for pixelspace regions!
    return true;
};
export const rectsAreEqual = (a, b) => {
    if (!isRectangularRegion(a) || !isRectangularRegion(b))
        return false;
...
export const pointSpanToRegion = (pointSpan) => {
...
export const rectangularRegionsIntersect = (r1, r2) => {
    ...
    if ((r1.xmax < r2.xmin) || (r1.xmin > r2.xmax))
        return false;
    if ((r1.ymax < r2.ymin) || (r1.ymin > r2.ymax))
        return false;
    return true;
};
...
export const getBoundingBoxForEllipse = (point, xRadius, yRadius) => {
    return {
...
export const isTransformationMatrix = (x) => {
    if ((x) && (Array.isArray(x)) && (x.length === 3)) {
        for (let row of x) {
            if (!isVec3(row))
                return false;
        }
        if (JSON.stringify(x[2]) === JSON.stringify([0, 0, 1]))
            return true;
    }
    return false;
};
export const toTransformationMatrix = (x) => {
    if (JSON.stringify(x.size()) === JSON.stringify([3, 3])) {
        const asArray = x.toArray();
...
export const getBasePixelTransformationMatrix = (pixelSpaceWidth, pixelSpaceHeight, newTargetSystem) => {
    if (!pixelSpaceWidth || !pixelSpaceHeight)
        throw Error('Cannot build default transformation matrix with dimensions unset.');
    ...
    if (shape !== 'square') {
        if (shape === 'landscape')
            coordRange = Object.assign(Object.assign({}, coordRange), { xmax: aspectRatio });
        if (shape === 'portrait')
            coordRange = Object.assign(Object.assign({}, coordRange), { ymax: aspectRatio });
    }
...
export const getTransformationMatrix = (newSystem, targetRangeInCurrentSystem) => {
...
export const getInverseTransformationMatrix = (t) => {
    const tmatrix = matrix(t);
    const inverse = inv(tmatrix).toArray();
    return inverse;
};
export const getUpdatedTransformationMatrix = (newSystem, targetRangeInCurrentSystem, oldTransform) => {
...
export const transformXY = (tmatrix, x, y) => {
    return transformPoint(tmatrix, homogenizeVec2([x, y]));
};
export const transformPoint = (tmatrix, point) => {
...
export const transformDistance = (tmatrix, xyDist) => {
    ...
    const b = multiply(A, x);
    const scaled = abs(b).toArray(); // take absolute value to ensure we don't get negative distances
