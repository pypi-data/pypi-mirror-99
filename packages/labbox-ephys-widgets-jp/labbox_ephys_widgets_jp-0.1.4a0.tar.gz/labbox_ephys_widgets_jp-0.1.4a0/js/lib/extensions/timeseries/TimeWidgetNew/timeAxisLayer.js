...
import { getInverseTransformationMatrix } from "../../common/CanvasWidget/Geometry";
const initialLayerState = {
    transformation: [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
    inverseTransformation: [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
};
const onPaint = (painter, layerProps, state) => {
    ...
    const brush = { color: 'rgb(22, 22, 22)' };
    ...
    if (si.label) {
        ...
        let rect = { xmin: si.t1, ymin: 0, xmax: si.t2 - si.t1, ymax: 0.35 };
        let alignment = { Horizontal: "AlignCenter", Vertical: "AlignTop" };
        painter2.drawText({
...
const onPropsChange = (layer, layerProps) => {
    ...
    layer.setState(Object.assign(Object.assign({}, layer.getState()), { transformation,
        inverseTransformation }));
    layer.repaintImmediate();
};
const get_ticks = (t1, t2, width, samplerate) => {
    const ret = {
        ticks: [],
        scale_info: {
            t1: 0,
    ...
    ret.ticks = ret.ticks.filter((tick, i) => {
        if (i > 0) {
            if (Math.abs(ret.ticks[i - 1].t - tick.t) < 1)
                return false;
            else
                return true;
        }
...
//# sourceMappingURL=timeAxisLayer.js.map