...
import TimeSpanWidget from './TimeSpanWidget';
...
const TimeWidgetNew = (props) => {
    ...
    const [spanWidgetInfo, setSpanWidgetInfo] = useState({ numTimepoints });
    ...
    const handleTimeZoom = useCallback((a) => {
        selectionDispatch({ type: 'ZoomTimeRange', direction: a.direction });
    }, [selectionDispatch]);
    const handleTimeShiftFrac = useCallback((frac) => {
        // selectionDispatch({type: 'TimeShiftFrac', frac})
    }, []);
    const handleCurrentTimeChanged = useCallback((t) => {
        selectionDispatch({ type: 'SetCurrentTimepoint', currentTimepoint: t });
    }, [selectionDispatch]);
    const handleTimeRangeChanged = useCallback((timeRange) => {
    ...
    const handleRepaintTimeEstimate = useCallback((ms) => {
    ...
    allLayers.forEach(L => {
        if (L)
            L.setProps(layerProps);
    });
    return (React.createElement("div", { className: "TimeWidget", style: { position: 'relative', left: 0, top: 0, width, height } },
        React.createElement(TimeWidgetToolbarNew, { width: toolbarWidth, height: height, top: 0, onZoomIn: handleZoomTimeIn, onZoomOut: handleZoomTimeOut, onShiftTimeLeft: handleShiftTimeLeft, onShiftTimeRight: handleShiftTimeRight, customActions: customActions }),
        React.createElement("div", { style: { position: 'relative', left: toolbarWidth, top: 0, width: width - toolbarWidth, height: height } },
            React.createElement(TimeSpanWidget, { key: 'timespan', width: width - toolbarWidth, height: spanWidgetHeight, info: spanWidgetInfo, onCurrentTimeChanged: handleCurrentTimeChanged, onTimeRangeChanged: handleTimeRangeChanged }),
            React.createElement(CanvasWidget, Object.assign({ key: 'canvas', layers: allLayers, preventDefaultWheel: true }, { width: width - toolbarWidth, height: layerProps.height })),
            React.createElement(TimeWidgetBottomBar, { key: 'bottom', width: width - toolbarWidth, height: bottomBarHeight, info: bottomBarInfo, onCurrentTimeChanged: handleCurrentTimeChanged, onTimeRangeChanged: handleTimeRangeChanged }))));
};
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const shiftTimeRange = (timeRange, shift) => {
    return {
...
//# sourceMappingURL=TimeWidgetNew.js.map