...
import { CanvasWidgetLayer, ClickEventType, useLayer, useLayers } from '../../common/CanvasWidget/CanvasWidgetLayer';
...
const createTimeSpanLayer = () => {
    ...
    const onMouseEvent = (e, layer) => {
        ...
        if (e.type === ClickEventType.Press) {
            layer.setState(Object.assign(Object.assign({}, layer.getState()), { dragging: false }));
        }
    ...
    const onDrag = (layer, drag) => {
        ...
        if (!numTimepoints)
            return;
        const { anchorCurrentTime, anchorTimeRange, dragging } = layer.getState();
        if (!dragging) {
            const timeRange = props.info.timeRange;
            if (!timeRange)
                return;
            layer.setState(Object.assign(Object.assign({}, layer.getState()), { dragging: true, anchorCurrentTime: props.info.currentTime || null, anchorTimeRange: timeRange }));
        }
...
//# sourceMappingURL=TimeSpanWidget.js.map