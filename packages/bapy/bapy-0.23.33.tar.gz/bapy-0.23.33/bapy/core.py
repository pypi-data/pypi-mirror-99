...
SSH_CONFIG_TEXT = '-o '.join([f'{key}={value}' for key, value in SSH_CONFIG.items()])
SSH_DIR = '.ssh'
STACK = inspect.stack()
TEMPLATES = 'templates'
TESTS = 'tests'
...
class Line(Enum):
    ...
    def echo(cls, **kwargs: Optional[Union[bool, dict, Line, str]]) -> NoReturn:
        ...
        Args:
            **kwargs: defaults(dict(blink=False, data=dict(), err=False, even=' ', odd=' ', reset=True, rc=None, separator=' ',
                underline=False)
        ...
        """
        globs = globals().copy()
        defaults = dict(blink=False, data=dict(), err=False, even=', ', odd=': ', reset=True, rc=None, underline=False)
        kw = {var: kwargs.pop(var, default) for var, default in defaults.items()}
        count = 0
        for last, msg, func in yield_last(kw['data'] | kwargs):
            count += 1
            if not isinstance(func, Line):
                raise TypeError(f'Value must be an instance of Line: {[item for item in Line]}')
            globs[func.lower](f'{msg}{str() if last else kw["even"] if even(count) else kw["odd"]}', blink=kw['blink'],
                              err=kw['err'], nl=last, reset=kw['reset'], rc=kw['rc'] if last else None,
                              underline=kw['underline'])
...
class Env(environs.Env):
    ...
    def __post_init__(self, file: Union[Path, str]):
        ...
        f = Path.cwd() / PathSuffix.ENV.lowerdot
...
class Git(git.Repo, BaseData):
    ...
    _origin: Optional[Url] = None
    _path: Optional[Path, pathlib.Path, str] = None
    _previous: Optional[VersionInfo] = datafield(default=None, init=False)
    _version: Optional[VersionInfo] = datafield(default=None, init=False)

    def __post_init__(self, log: Optional[Log]):
        if not self._name and not self._path:
            top = self.top(self._path)
            self._name = top.name
            self._origin = top.origin
            self._path = top.path
        if not self._origin and self._name:
            self._origin = Url.lumenbiomics(repo=self._name)
        if self._path and not self._path.exists:
            self.clone_from(self._origin, self._path.text)
    ...
    def bump(self, value: Bump = Bump.PATCH, msg: str = str()):
        self.previous = self.version
        getattr(self.version, f'bump_{value.value}')()
        new = getattr(self.version, f'bump_{value.value}')()
        self.create_tag(f'v{str(new)}', message=msg)
        self._version = new
        Line.echo(data={'repo': Line.BWHITE, self.name: Line.GREEN, 'version': Line.BWHITE,
                        str(self._version): Line.GREEN, 'previous': Line.BWHITE, self.previous: Line.GREEN}, err=True)
    ...
    def ls(self) -> list:
        self.path.c()
        # noinspection PyArgumentList
        rv = cmd(f'{GitCmd().ls} {self.branch}').stdout
        self.path.c_()
        return rv

    @property
    def name(self) -> str:
        return self.origin.gitrepo if self.origin else self.path.name if self.path else self._name

    @property
    def origin(self) -> Url:
        # noinspection PyArgumentList
        return urls[0] if (urls := self.remote_urls) else Url(rv.stdout[0]) \
            if (rv := cmd(GitCmd().url)).stdout else self._origin
    ...
    @previous.setter
    def previous(self, value: VersionInfo):
    ...
    # noinspection PyArgumentList
    @staticmethod
    def top(path: Union[Path, pathlib.Path, str] = None) -> GitTop:
        path = Path(path if path else Path.cwd())
        path.c()
        p = Path(rv.stdout[0]) if (rv := cmd(GitCmd().path)).stdout else None
        origin = Url(rv.stdout[0]) if p and (rv := cmd(GitCmd().url)).stdout else None
        name = origin.gitrepo if origin else p.name if p else None
        path.c_()
        return GitTop(name, origin, p)

    @property
    def version(self) -> VersionInfo:
        self._version = self.versions[-1]
        return self._version

    def version_delete(self, keep: int = GIT_VERSIONS) -> int:
        keep = keep if keep >= GIT_VERSIONS else GIT_VERSIONS
        versions = self.versions
        if (delete := len(versions) - keep) > 0:
            self.delete_tag(*map(lambda x: f'v{str(x)}', versions[:delete]))
        green(delete if delete > 0 else int())
        Line.echo(data={
            'repo': Line.BWHITE, self.name: Line.GREEN, 'version': Line.BWHITE,
            str(self._version): Line.GREEN, 'previous': Line.BWHITE, self.previous: Line.GREEN
        }, err=True)

        return delete if delete > 0 else int()

    @property
    def versions(self):
        return sorted([VersionInfo.parse(tag.name.removeprefix('v')) for tag in self.tags])
...
class Path(pathlib.Path, pathlib.PurePosixPath):
    ...
    __slots__ = ('bapy', 'file', 'env', '_frame', 'git', 'ic', 'icc', 'init', 'kill', 'log', 'logconf', 'package',
                 'path', 'prefix', 'previous', 'project', 'repo', 'sem', 'semfull', 'setup_kwargs', 'tests', 'work')

    def __call__(self, name: str = None, t: PathCall = PathCall.DIR) -> Path:
        return getattr(self, t.value)(name if name else self)
    ...
    def c_(self) -> Path:
        ...
        if not hasattr(self, 'previous'):
            self.previous = self.cwd()
        os.chdir(self.previous.text)
        return self.previous

    def cd(self, p: Union[Path, pathlib.Path, str] = '-') -> Path:
        """
        ...
        if not hasattr(self, 'previous'):
            self.previous = self.cwd()
        p = self.previous if p == '-' else path if (path := Path(p)).is_dir() else path.parent
        previous = self.cwd()
        os.chdir(p.text)
        p = self.cwd()
        p.previous = previous
    ...
    @property
    def readme(self) -> str:
        if hasattr(self, 'project') and self.project and self.project.is_dir():
    ...
    def requirements(self) -> dict:
        ...
        } if hasattr(self, 'project') and self.project and self.project.is_dir() else {}
    ...
    def _setup(self) -> Path:
        ...
        log_dir = self.user.home.mkdir_add(PathSuffix.LOG.lowerdot)
        ...
        self.env.call()
        # ic(self.cwd(), self.path, self.env)
        self.ic = deepcopy(ic)
        ...
        self.work = self.user.home.mkdir_add(f'.{self.path.name}')
        return self
    ...
    def setuptools(self) -> dict:
        ...
        if top.path:
            ...
            color = Line.BRED
        self.project = top.path if top.path else self.user.home / self.repo
        Line.echo(data={'repo': Line.BWHITE, self.repo: color, 'path': Line.BWHITE, self.project: color}, err=True)
        self.git = None
    ...
    def setuppy(self) -> FindUp:
        # noinspection PyArgumentList
        return FindUp() if self.installed else self.find_up(name='setup.py')
...
class Url(furl.furl, Base):
    ...
    def lumenbiomics(repo: str = None, http: bool = False, username: str = None, password: str = None,
                     suffix: bool = False, stdout: bool = False) -> Url:
        """
        ...
        if http:
            ...
            if stdout:
                cprint(rv.url)
            return rv
        rv = Url(f'{Url.id}@{Url.github().host}:{p}')  # org-4379404@github.com:lumenbiomics/repos.git
        if stdout:
            cprint(rv.url)
        return rv
...
class User:
    ...
    def __post_init__(self):
        ...
        self.GITHUB_USERNAME = self.process.GITHUB_USERNAME if self.SUDO else self.actual.GITHUB_USERNAME
        os.environ['GIT_SSH_COMMAND'] = f'ssh -i {self.id_rsa} {SSH_CONFIG_TEXT}'
...
@app.command(context_settings=appcontext, name=CMD_DELETE)
def _(keep: int = Argument(GIT_VERSIONS, help='Tags to keep.')):
    """
    Delete git tags.
...
@app.command(context_settings=appcontext, name=CMD_INSTALL)
def _(post: bool = Option(CMD_INSTALL_POST_DEFAULT,
      help='Execute post install commands.')):
    """
...
@app.command(context_settings=appcontext, name=CMD_UPLOAD)
def _(bump: Bump = Option(Bump.PATCH, autocompletion=Git.bump_values, case_sensitive=False,
