...
class Git(git.Repo, BaseData):
    path: Optional[Path, pathlib.Path, str] = None
    name: Optional[str] = None
    origin: Optional[Url] = None

    def __post_init__(self, log: Optional[Log]):
        if not self.name and not self.path:
            top = self.top(self.path)
            self.name = top.name
            self.origin = top.origin
            self.path = top.path
        if not self.origin and self.name:
            self.origin = Url.lumenbiomics(repo=self.name)
        if self.path and not self.path.exists:
            self.clone_from(self.origin, self.path.text)
        super().__init__(path=self.path.text)
    ...
    def top(path: Union[Path, pathlib.Path, str] = None) -> GitTop:
        ...
        path.c()
        p = Path(rv.stdout[0]) if (rv := cmd('git rev-parse --show-toplevel')).stdout else None
        origin = Url(rv.stdout[0]) if p and (rv := cmd('git config --get remote.origin.url')).stdout else None
...
class Path(pathlib.Path, pathlib.PurePosixPath):
    ...
    __slots__ = ('bapy', 'file', 'env', '_frame', 'github', 'ic', 'icc', 'init', 'kill', 'log', 'logconf', 'package',
                 'path', 'prefix', 'previous', 'project', 'repo', 'semfull', 'sem', 'setup_kwargs', 'tests', 'work')
    ...
    def c(self):
        ...
        os.chdir((self if self.is_dir() else self.parent).text)
