...
from sysconfig import get_paths as get_sys_paths
...
PYTHON_SITE = PYTHON_SYS.resolve()
...
class Py(Enum):
    PACKAGES = f'{PYTHON_SITE} -c "import site; _ = [print(i) for i in site.getsitepackages()]"'
    PATHS = f'{PYTHON_SITE} -c "import sysconfig; print(sysconfig.get_paths())"'

    @property
    def run(self) -> CompletedProcess:
        # noinspection SubprocessShellMode
        return subprocess.run(self.value, capture_output=True, check=True, shell=True, text=True)

    @property
    def lines(self) -> list:
        return self.run.stdout.splitlines()

    @property
    def load(self) -> Union[dict, list]:
        return json.loads(self.run.stdout.replace("'", '"'))
...
class Git(git.Repo, BaseData):
    ...
    clone_if_not: InitVar[bool] = False
    ...
    def __post_init__(self, log: Optional[Log], clone_if_not: bool, fallback: Optional[str], file: Optional[Path, str],
                      frame: Optional[FrameInfo, FrameType], github: [Optional[Url]],
        ...
        if self._path and not self._path.exists():
            ...
            self.clone(self._url.text, self._path.text) \
                if self.is_url(self._url) and clone_if_not else self.init(self._path.text)
    ...
    def _alt(self, stdout: bool = STDOUT, **kwargs):
        if stdout:
            Line.alt(repo=self.name, function=f'{self.__class__.__name__}.{inspect.stack()[1].function}', **kwargs)

    def all(self, bump: Bump = Bump.PATCH, delete: bool = GIT_VERSIONS_DELETE, message: str = None,
            stdout: bool = STDOUT) -> VersionInfo:
    ...
    def add(self, force: bool = False, write: bool = True, stdout: bool = STDOUT):
        """
    ...
    def bump(self, bump: Bump = Bump.PATCH, delete: bool = GIT_VERSIONS_DELETE, message: str = None,
             stdout: bool = STDOUT) -> Optional[VersionInfo]:
        if self.git_dir:
            self.path.c()
    ...
    def clone(self, url: Optional[Url, str] = None, path: Optional[Path, PathLib, str] = None,
              clone_rm: bool = clone_rm, stdout: bool = STDOUT, **kwargs) -> Git:
    ...
    def commit_super(self, rev: str = 'HEAD') -> Optional[git.Commit]:
        if self.git_dir:
            return super(Git, self).commit(rev)

    def commit(self, bump: Bump = Bump.PATCH, message: str = None, stdout: bool = STDOUT) -> Optional[git.Commit]:
        if self.git_dir:
    ...
    def commit_cmd(self, bump: Bump = Bump.PATCH, message: str = None, stdout: bool = STDOUT) -> Optional[str]:
        if self.git_dir:
    ...
    def dirs(self) -> dict[str, Path]:
        return self.path.scan(option=PathOption.DIRS) if self.path and self.git_dir else dict()

    def fetch(self, stdout: bool = STDOUT, **kwargs) -> Optional[IterableList]:
        if self.origin:
    ...
    def ls(self) -> list:
        self.path.c()
        rv = cmd(f'git ls-tree --name-only -r {self.branch}').stdout if self.git_dir else list()
        self.path.c_()
        return rv
    ...
    def origin(self) -> Optional[git.Remote]:
        if self.is_url(self.url) and self.is_git(self.path):
            with suppress(ValueError):
                return self.remote()
    ...
    def pull(self, stdout: bool = STDOUT, rebase: bool = False, **kwargs) -> git.util.IterableList:
        if self.origin and isinstance(self.origin, git.Remote):
            rv = self.origin.pull(rebase=rebase, **kwargs)
            self._alt(stdout=stdout)
            self.m(rv)
            return rv

    def push(self, stdout: bool = STDOUT, tags: bool = True, **kwargs) -> list:
    ...
    def remote_urls(self) -> list[Url]:
        ...
        if self.git_dir:
            with suppress(ValueError):
                rv = list(map(Url, self.remote().urls))
        return rv
    ...
    @property
    def url(self) -> Url:
        return self.remote_urls[0] if self.remote_urls else self._url
    ...
    def version_delete(self, keep: int = GIT_VERSIONS, stdout: bool = STDOUT) -> Optional[int]:
        if self.git_dir:
    ...
    def versions(self) -> list[VersionInfo]:
        self.path.c()
        rv = sorted([self.to_vers(tag.name) for tag in self.tags]) if self.git_dir else list()
        self.path.c_()
        return rv
    ...
    def versions_remote(self) -> list[VersionInfo]:
        self.path.c()
        rv = sorted(map(VersionInfo.parse, cmd(f'git ls-remote --exit-code --refs --tags {str(self.url)} | '
                                               f'sed "s/^.*\/v//g"').stdout)) if self.is_url(self.url) else list()
        self.path.c_()
        return rv
...
class Path(PathLib, pathlib.PurePosixPath):
    ...
    def installed(self) -> Optional[Path]:
        ...
        Returns:
            Optional[Path]:
        """
        for s in list({*getsitepackages(), *[USER_SITE, ],
...
class Package(Path):
    """Setup Class."""
    exclude_dirs: ClassVar[tuple] = (PYCACHE, BACKUP, VENV_DIR, BYTECODE_SUFFIXES, '.pyo',)
    ...
    def all(self, bump: Bump = Bump.PATCH, delete: bool = GIT_VERSIONS_DELETE, message: str = None,
        ...
        if self.git.git_dir:
            self.project.c()
            self.clean(stdout=stdout)
            self.project.c_()

    def clean(self, stdout: bool = STDOUT):
        if self.git.git_dir:
            with Shell(stdout=sys.stdout, stderr=sys.stderr, blocking=False) as sh:
                sh(
                    """
                    sudo /bin/rm -fr build/;
                    sudo /bin/rm -fr dist/;
                    sudo /bin/rm -fr .eggs/;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "*.egg-info" -exec /bin/rm -fr {} +;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "*.egg" -exec /bin/rm -f {} +;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "*.pyc" -exec /bin/rm -f {} +;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "*.pyo" -exec /bin/rm -f {} +;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "*~" -exec /bin/rm -f {} +;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name "__pycache__" -exec /bin/rm -fr {} +;
                    sudo /bin/rm -fr .tox/;
                    sudo /bin/rm -fr .pytest_cache;
                    sudo find . -not \( -path "*/venv/*" -prune \) -name ".mypy_cache" -exec /bin/rm -rf {} +;
                    """
                )
            self._alt(stdout=stdout)
    ...
    @property
    def importlib_module_from_file(self) -> Optional[ModuleType]:
        if (file := self.file) and self._file != __file__:
            with suppress(ModuleNotFoundError):
                spec = importlib.util.spec_from_file_location(self.name, file.text)
    ...
    def _init_obj(cls, file: str, frame: FrameInfo) -> Package:
        ...
        obj._path = _path
        obj.frame = frame
        obj.package = '.'.join(obj.file.relative(obj.parent).stemfull.parts).removesuffix('.__init__')
        name = obj.name
    ...
    def path(self) -> Optional[Package]:
        if self.file:
            return self(p) if (p := slot(self, '_path')) else None
    ...
    def scripts(self) -> list:
        if self.project and self.project.is_dir():
            ...
            if directory.is_dir():
                return [item.chmod(cast(int, '+x')) for item in directory.iterdir() if directory.is_dir()]
        return list()
    ...
    def setuptools(self, bump: Bump = Bump.PATCH, delete: bool = GIT_VERSIONS_DELETE, message: str = None,
        ...
        if self.project:
            ...
            return slot(self, 'kwargs', dict(
                author=user.gecos, author_email=Url.email(), description=self.description,
                entry_points=dict(console_scripts=[f'{p} = {p}:{CLI}' for p in self.packages_upload]),
                include_package_data=True, install_requires=self.requirements.get('requirements', list()),
                name=self.repo,
                package_data={
                    self.repo: [f'{p}/{d}/*' for p in self.packages_upload
                                for d in (self.project / p).scan(PathOption.DIRS)
    ...
    def version(self) -> Version:
        if self.distribution and self.repo:
            return Version(VersionInfo.parse(self.distribution.version), self.git.version, )


@dataclass
class Python:
    """Python Server Information Class."""
    version: VersionInfo = VersionInfo(*python_version_tuple())
    PY2: bool = True if version.major == 2 else False
    PY3: bool = True if version.major == 3 else False
    PY310: bool = True if PY3 and version.minor == 10 else False
    PY39: bool = True if PY3 and version.minor == 9 else False
    PY38: bool = True if PY3 and version.minor == 8 else False
    PY37: bool = True if PY3 and version.minor == 7 else False
    site_executable: Union[Path, PathLib] = datafield(default=PYTHON_SITE, init=False)
    sys_executable: Union[Path, PathLib] = datafield(default=PYTHON_SYS, init=False)
    site_executable_name: str = datafield(default=None, init=False)
    sys_executable_name: str = datafield(default=None, init=False)
    base_prefix: Union[Path, str] = datafield(default=sys.base_prefix, init=False)
    prefix: Union[Path, str] = datafield(default=sys.prefix, init=False)
    site_paths: dict[str, Path] = datafield(default_factory=get_sys_paths, init=False)
    sys_paths: dict[str, Union[Path, str]] = datafield(default_factory=get_sys_paths, init=False)
    _site_packages: list[str] = datafield(default_factory=list, init=False)
    site_packages: list[Path] = datafield(default_factory=list, init=False)
    sys_packages: list[Union[PathLib, Path]] = datafield(default_factory=getsitepackages, init=False)
    user_packages: Optional[list[Path], str] = datafield(default=USER_SITE, init=False)
    packages: list[Path] = datafield(default_factory=list, init=False)
    exception: tuple = True

    def __post_init__(self):
        self.sys_executable = Path(self.sys_executable)
        self.site_executable = self.sys_executable.resolved if VENV else self.sys_executable
        self.sys_executable_name = self.sys_executable.name
        self.site_executable_name = self.site_executable.name
        self.base_prefix = Path(self.base_prefix)
        self.prefix = Path(self.prefix)
        self.site_paths = {key: Path(value) for key, value in Py.PATHS.load.items()}
        self.sys_paths = {key: Path(value) for key, value in self.sys_paths.items()}
        self._site_packages = Py.PACKAGES.lines
        self.site_packages = [Path(item) for item in self._site_packages]
        self.sys_packages = [Path(item) for item in self.sys_packages]
        self.user_packages = [Path(self.user_packages)]
        self.packages = self.sys_packages + self.user_packages

        if self.exception and (self.version < PYTHON_VERSIONS[0] or self.version > PYTHON_VERSIONS[1]):
            raise RuntimeError('Invalid python version', f'{self.version=}')

    def extend(self):
        sys.path.extend(self._site_packages)

    def remove(self):
        _ = [sys.path.remove(i) for i in self._site_packages]
...
@dataclass
class System(Distro, Executable, Machine, Python):
...
def asdict(obj: Any, depth: Optional[int] = obj_defaults.depth, ignore: bool = obj_defaults.ignore,
           swith: str = obj_defaults.swith) -> dict:
    return Obj(data=obj, depth=depth, ignore=ignore, swith=swith).asdict()
...
def cmd(command: Iterable, exc: bool = False, lines: bool = True, shell: bool = True,
        py: bool = False, site: bool = True) -> Union[Cmd, int, list, str]:
    """
    Runs a cmd.
    ...
    Args:
        ...
        site: run on site python if running on a VENV.
    ...
    if py:
        ...
        if isinstance(command, str) and command.startswith('/'):
            m = str()
        command = f'{python.sys_executable.text if site else python.sys_executable.text} {m} {command}'
    elif not shell:
...
def _info(dist: str = None, executable: bool = False, linux: bool = False,
    ...
    if py:
        cprint('[bold blue]Python: ', dataasdict(Python()), str())
    if u:
