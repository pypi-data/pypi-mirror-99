...
class Path(pathlib.Path, pathlib.PurePosixPath):
    ...
    stack: ClassVar[list[FrameInfo]] = inspect.stack()
    ...
    def _setup_file(self) -> Optional[Path]:
        for frame in self.stack:
            if all([frame.function == FUNCTION_MODULE, frame.index == 0, 'PyCharm' not in frame.filename,
                    type(self)(frame.filename).suffix, 'setup.py' not in frame.filename,
    ...
    def setuptools(self) -> dict:
        ...
        return dict(
            ...
            package_data={name: [f'{d}/*' for d in self.project.scan(PathOption.DIRS)
...
class Py:
    ...
    VERSION: float = float(f'{python_version[0]}.{python_version[1]}')
