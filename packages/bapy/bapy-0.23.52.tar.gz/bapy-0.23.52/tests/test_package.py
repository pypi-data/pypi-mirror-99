...
def test_installed(tmp_path, pathtest):
    ...
    var = 'prefix'
    ic(cmd(f'{new_project_init} {var}', sysexec=True, exc=True))
    assert f'{new_project_path.name.upper()}_' in cmd(f'{new_project_init} {var}', sysexec=True, exc=True).stdout[0]
    var = 'path'
    assert new_project_path.text in cmd(f'{new_project_init} {var}', sysexec=True).stdout[0]
    var = 'package'
    assert (User().home / new_project_path.name).text in cmd(f'{new_project_init} {var}', sysexec=True).stdout[0]

    var = 'repo'
    assert new_project_path.name in cmd(f'{new_project_init} {var}', sysexec=True).stdout[0]
    new_project_init.write_text(_test_installed(r=TESTS))
    assert TESTS in cmd(f'{new_project_init} {var}', sysexec=True).stdout[0]
