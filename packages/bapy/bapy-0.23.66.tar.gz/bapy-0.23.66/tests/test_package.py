...
def test_bapy(console_script):
    ...
    assert core.file.text in cmd(f'{cli} {TESTS}', sysexec=True).stdout
    cli.rm()
...
def test_installed(tmp_path, pathtest):
    ...
    var = 'prefix'
    assert f'{new_project_path.name.upper()}_' in cmd(f'{new_project_init} {var}', sysexec=True, exc=True).stdout
    var = 'path'
    assert new_project_path.text in cmd(f'{new_project_init} {var}', sysexec=True).stdout
    var = 'package'
    assert new_project_path.name in cmd(f'{new_project_init} {var}', sysexec=True).stdout
    var = 'project'
    assert (User().home / new_project_path.name).text in cmd(f'{new_project_init} {var}', sysexec=True).stdout

    var = 'repo'
    assert new_project_path.name in cmd(f'{new_project_init} {var}', sysexec=True).stdout
    new_project_init.write_text(_test_installed(r=TESTS))
    g.rm()
    ic(cmd(f'{new_project_init} {var}', sysexec=True))
    assert TESTS in cmd(f'{new_project_init} {var}', sysexec=True).stdout
