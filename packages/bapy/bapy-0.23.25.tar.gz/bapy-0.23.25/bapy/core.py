...
class Git(git.Repo, BaseData):
    ...
    def version_delete(self, keep: int = GIT_VERSIONS):
        ...
        self.delete_tag(*map(lambda x: f'v{str(x)}', versions[:len(versions) - keep]))
...
@app.command(context_settings=appcontext)
def upload(version: Bump = typer.Option(Bump.PATCH, case_sensitive=False, help='Version part to be increased.',
                                        autocompletion=Git.bump_values)):
    """
