...
class DataA(BaseData):
    ...
    logger: Log = package.log
    path: Path = Path()


def factory():
    return [package.log, DataA(), Path(), environs.Env()]
...
def test_annotation():
    obj = Obj(package.semfull)
    assert obj.initvars == ['log']
    assert isinstance(obj.initvarsdict['log'], Log)
    assert 'log' in package.semfull.kwargs
    assert sorted(package.semfull.keys) == sorted(obj.attrs_cls + ['log'])
    assert 'log' not in package.semfull.defaults()
    assert package.semfull._initvars == obj.initvarsdict
    assert SemFull._annotation('http')[0] == package.semfull._annotation('http')[0] == int
    assert GitHub._annotation('config')[0] == package.github._annotation('config')[
        0] == git.config.GitConfigParser
    assert MongoFullDoc._annotation('_id')[0] == str
    assert isinstance(package.semfull._initvars['log'], Log)
...
def test_asdict():
    assert DictA().asdict == {
        ...
        'logger': dict(name=package.log.name,
                       level=package.log.level)
    }
    assert DictB().asdict == {
        '_b': 'b',
        'dict_a': {
            ...
            'a': 'a',
            'clsvar': 3,
            'logger': dict(name=package.log.name, level=package.log.level)
        },
        'env': {},
        'path': '.'
    }
    ...
    assert DictC().asdict == {
        '_a': 'a', 'clsvar': None, 'f': 2, 'logger': dict(name=package.log.name, level=package.log.level),
    ...
    assert DataB().asdict == {
        ...
        'integer': 0,
        'logger': dict(name=package.log.name, level=package.log.level),
        'lst': [dict(name=package.log.name, level=package.log.level),
                {
                    'a': 'a',
                    'dict_a': {
                        '_a': 'a',
...
def test_defaults():
    ...
    assert DictA.defaults(nested=False) == {'_a': 'a', 'clsvar': 3, 'logger': package.log}

    assert DataA.defaults() == {
        'a': 'a',
        'dict_a': {'_a': 'a', 'clsvar': 3, 'logger': dict(name=package.log.name, level=package.log.level)},
        'dictionary': {},
        'env': {},
        'integer': 0,
        'logger': dict(name=package.log.name, level=package.log.level),
        ...
        'lst': [dict(name=package.log.name, level=package.log.level),
                {
                    'a': 'a',
                    'dict_a': {
                        '_a': 'a', 'clsvar': 3, 'logger': dict(name=package.log.name,
                                                               level=package.log.level)
                    },
                    'dictionary': {},
                    'env': {},
...
def test_public():
    assert DataC().public == {
        ...
        'dict_a': {'a': 'a', 'clsvar': 3, 'logger': dict(name=package.log.name, level=package.log.level)},
        'dictionary': {'a': {'a': '__a'}},
        'env': {},
        'integer': 0,
        'logger': dict(name=package.log.name, level=package.log.level),
