...
class TBUpdater(Thread):
    def __init__(self):
        super().__init__()
        self.__version = {"current_version": get_distribution('ndu_gateway').version,
                          "latest_version": get_distribution('ndu_gateway').version}
    ...
    def update(self):
        if self.__version["latest_version"] != self.__version["current_version"]:
            result = TBUtility.install_package("ndu-gateway", self.__version["latest_version"])
        else:
