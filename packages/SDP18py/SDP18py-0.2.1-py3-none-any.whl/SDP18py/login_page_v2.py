...
def login_page():
    ...
    label3 = Label(frame1, text='Please input runtime :')
    label3.place(relx=0.35, rely=0.45)
    label3.config(font=('', 10))

    entry_runtime = Entry(frame1, width=22, justify='center')
    entry_runtime.place(relx=0.35, rely=0.48)
    entry_runtime.insert(END, '180')

    label_runtime = Label(frame1, text='s')
    label_runtime.place(relx=0.7, rely=0.48)
    ...
    def generate_schedule():
        ...
        time_taken = int(entry_runtime.get())
        ...
        if optimizer_dict[optimizer_selected] == 1:
            ...
            run_moshcr_1(current_schedule, to_schedule, mas_full, disc_selected, time_taken)
        if optimizer_dict[optimizer_selected] == 2:
            # run_ts(current_schedule, to_schedule, mas_full, disc_selected)
            run_tamoco_1(current_schedule, to_schedule, mas_full, disc_selected, time_taken)
        if optimizer_dict[optimizer_selected] == 3:
            # run_sa(current_schedule, to_schedule, mas_full, disc_selected)
            run_umosa_1(current_schedule, to_schedule, mas_full, disc_selected, time_taken)
        if optimizer_dict[optimizer_selected] == 4:
            # run_ga(current_schedule, to_schedule, mas_full, disc_selected)
            run_nsga2_1(current_schedule, to_schedule, mas_full, disc_selected, time_taken)
