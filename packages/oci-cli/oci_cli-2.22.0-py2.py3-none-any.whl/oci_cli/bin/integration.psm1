...
function GetOciSubcommands_integration() {
    $ociSubcommands = @{
        ...
        'integration integration-instance' = 'change-compartment change-network-endpoint create delete get list start stop update'
...
function GetOciCommandsToLongParams_integration() {
    $ociCommandsToLongParams = @{
        ...
        'integration integration-instance create' = 'alternate-custom-endpoints compartment-id consumption-model custom-endpoint defined-tags display-name freeform-tags from-json help idcs-at is-byol is-file-server-enabled is-visual-builder-enabled max-wait-seconds message-packs network-endpoint-details type wait-for-state wait-interval-seconds'
