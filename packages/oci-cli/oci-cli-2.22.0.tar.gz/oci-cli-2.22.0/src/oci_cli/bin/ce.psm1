...
function GetOciCommandsToLongParams_ce() {
    $ociCommandsToLongParams = @{
        'ce cluster create' = 'compartment-id dashboard-enabled endpoint-nsg-ids endpoint-public-ip-enabled endpoint-subnet-id from-json help kms-key-id kubernetes-version max-wait-seconds name pods-cidr service-lb-subnet-ids services-cidr tiller-enabled vcn-id wait-for-state wait-interval-seconds'
