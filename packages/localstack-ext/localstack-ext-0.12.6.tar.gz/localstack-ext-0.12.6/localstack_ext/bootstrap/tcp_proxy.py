...
class AttrDict(LKTwC):
 def __init__(self,*args,**kwargs):
  LKTwp(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and LKTwg(socket,'IPV6_V6ONLY'):
...
class Half(LKTwb):
 def __init__(self,opt,sock,addr,LKTws):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.LKTws=LKTws
  self.name='peer' if self.LKTws else 'client'
  self.queue=[]
  self.dest=LKTwF
  self.err=LKTwF
  self.ready=LKTwS
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':LKTwS,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':LKTwS,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':LKTwS,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':LKTwS,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
