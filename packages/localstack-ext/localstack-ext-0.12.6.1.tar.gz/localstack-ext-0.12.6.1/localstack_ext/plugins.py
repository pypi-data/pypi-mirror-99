...
def register_localstack_plugins():
 ...
 if EFJOy([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in EFJOL(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return EFJOx(os.environ.get('LOCALSTACK_API_KEY'))
