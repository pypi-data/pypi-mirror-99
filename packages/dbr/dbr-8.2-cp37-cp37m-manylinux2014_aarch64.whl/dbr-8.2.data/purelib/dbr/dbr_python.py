...
class ImageData:
    ...
    - stride <int> : The image data content in a byte array

    - image_pixel_format <EnumImagePixelFormat> : The image data content in a byte array
    """
...
class LineSegment:
    ...
    def _convert_to_innner(self):
        line_segment = {}
        line_segment["StartPoint"] = self.start_point._convert_to_innner()
        line_segment["EndPoint"] = self.end_point._convert_to_innner()
...
class RegionOfInterest:
    ...
    def _convert_to_innner(self):
        ...
        region_of_interest["Point"] = self.point._convert_to_innner()
...
class Quadrilateral:
    ...
    Attributes:
    -----------
    - point <class Point> : Four vertexes in a clockwise direction of a quadrilateral. Index 0 represents the left-most vertex.
        ...
        if self.results is not None:
            if self.data_type == EnumIMResultDataType.IMRDT_IMAGE:
                for result in self.results:
                    im_results.append(result._convert_to_innner())
            elif self.data_type == EnumIMResultDataType.IMRDT_CONTOUR:
                for result in self.results:
                    im_results.append(result._convert_to_innner())
            elif self.data_type == EnumIMResultDataType.IMRDT_LINESEGMENT:
...
#region License Functions
    ...
    def init_lts_connection_parameters():
        cp_parameters = BarcodeReader.__dbr__.InitLTSConnectionParameters()
    ...
    def init_license_from_lts(lts_connection_parameters):
        ...
        error = BarcodeReader.__dbr__.InitLicenseFromLTS(cp_parameters)
...
#region Image Decoding Functions
    ...
    def decode_file(self, image_file_name, template_name=""):
        ...
        if error_code == EnumErrorCode.DBR_OK or error_code == EnumErrorCode.DBRERR_LICENSE_EXPIRED or error_code == EnumErrorCode.DBRERR_LICENSE_INVALID or error_code == EnumErrorCode.DBRERR_RECOGNITION_TIMEOUT or \
            ...
            error_code == EnumErrorCode.DMERR_FAILED_TO_REACH_LTS:
    ...
    def decode_intermediate_results(self, intermediate_results, template_name=""):
        ...
        for intermediate_result in intermediate_results:
            inner_intermediate_result = intermediate_result._convert_to_innner()
