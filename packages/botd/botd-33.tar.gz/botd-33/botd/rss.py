...
from botl.utl import debug, gettinyurl, geturl, striphtml
...
class Fetcher(Object):
    ...
    def fetch(self, rssobj):
        ...
        if not rssobj.rss:
            return 0
        for o in reversed(list(getfeed(rssobj.rss))):
            if not o:
                continue
            f = Feed()
            update(f, rssobj)
...
def dpl(event):
    ...
    for fn, o in lastmatch("botd.rss.Rss", {"rss": event.args[0]}):
        edit(o, setter)
        save(o)
        event.reply("ok")
