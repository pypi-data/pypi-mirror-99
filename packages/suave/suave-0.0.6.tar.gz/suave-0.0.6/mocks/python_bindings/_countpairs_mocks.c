...
// Used in compute_amplitudes function
static int64_t check_dims_and_datatype_paircounts(PyObject *module, PyArrayObject *w1_obj, PyArrayObject *x1_obj, PyArrayObject *y1_obj, PyArrayObject *z1_obj, PyArrayObject *q1_obj, size_t *element_size)
{
    char msg[1024];

    /* All the position arrays should be 1-D*/
    const int nwdims = PyArray_NDIM(w1_obj);
    const int nxdims = PyArray_NDIM(x1_obj);
    const int nydims = PyArray_NDIM(y1_obj);
    const int nzdims = PyArray_NDIM(z1_obj);
    ...
    /* All the arrays should be floating point (only float32 and float64 are allowed) */
    const int w_type = PyArray_TYPE(w1_obj);
    const int x_type = PyArray_TYPE(x1_obj);
    const int y_type = PyArray_TYPE(y1_obj);
    const int z_type = PyArray_TYPE(z1_obj);
    const int q_type = PyArray_TYPE(q1_obj);
    if( ! ((w_type == NPY_FLOAT || w_type == NPY_DOUBLE) &&
           (x_type == NPY_FLOAT || x_type == NPY_DOUBLE) &&
           (y_type == NPY_FLOAT || y_type == NPY_DOUBLE) &&
           (z_type == NPY_FLOAT || z_type == NPY_DOUBLE) &&
           (q_type == NPY_FLOAT || q_type == NPY_DOUBLE))
        ) {
        PyArray_Descr *w_descr = PyArray_DescrFromType(w_type);
        PyArray_Descr *x_descr = PyArray_DescrFromType(x_type);
        PyArray_Descr *y_descr = PyArray_DescrFromType(y_type);
        PyArray_Descr *z_descr = PyArray_DescrFromType(z_type);
        PyArray_Descr *q_descr = PyArray_DescrFromType(q_type);
        if(w_descr == NULL || x_descr == NULL || y_descr == NULL || z_descr == NULL || q_descr == NULL) {
            /* Generating the dtype descriptor failed somehow. At least provide some information */
            snprintf(msg, 1024, "TypeError: Expected floating point arrays (allowed types = %d or %d). Instead found type-nums (%d, %d, %d, %d, %d)\n",
                     NPY_FLOAT, NPY_DOUBLE, w_type, x_type, y_type, z_type, q_type);
        } else {
            snprintf(msg, 1024, "TypeError: Expected floating point arrays (allowed types = %d or %d). Instead found type-nums (%d, %d, %d, %d, %d) "
                     "with type-names = (%s, %s, %s, %s, %s)\n",
                     NPY_FLOAT, NPY_DOUBLE, w_type, x_type, y_type, z_type, q_type, w_descr->typeobj->tp_name, x_descr->typeobj->tp_name, y_descr->typeobj->tp_name, z_descr->typeobj->tp_name,
                     q_descr->typeobj->tp_name);
        }
        Py_XDECREF(w_descr);Py_XDECREF(x_descr);Py_XDECREF(y_descr);Py_XDECREF(z_descr);Py_XDECREF(q_descr);
        countpairs_mocks_error_out(module, msg);
        return -1;
    }

    // Current version of the code only supports all the same dtype
    if( w_type != x_type || x_type != y_type || y_type != z_type || z_type != q_type ) {
        PyArray_Descr *w_descr = PyArray_DescrFromType(w_type);
        PyArray_Descr *x_descr = PyArray_DescrFromType(x_type);
        PyArray_Descr *y_descr = PyArray_DescrFromType(y_type);
        PyArray_Descr *z_descr = PyArray_DescrFromType(z_type);
        ...
        if(w_descr == NULL || x_descr == NULL || y_descr == NULL || z_descr == NULL || q_descr == NULL) {
            /* Generating the dtype descriptor failed somehow. At least provide some information */
            snprintf(msg, 1024, "TypeError: Expected *ALL* 5 floating point arrays to be the same type (allowed types = %d or %d). Instead found type-nums (%d, %d, %d, %d, %d)\n",
                     NPY_FLOAT, NPY_DOUBLE, w_type, x_type, y_type, z_type, q_type);
        } else {
            snprintf(msg, 1024, "TypeError: Expected *ALL* 5 floating point arrays to be the same type (allowed types = %d or %d). Instead found type-nums (%d, %d, %d, %d, %d) "
                     "with type-names = (%s, %s, %s, %s, %s)\n",
                     NPY_FLOAT, NPY_DOUBLE, w_type, x_type, y_type, z_type, q_type, w_descr->typeobj->tp_name, x_descr->typeobj->tp_name, y_descr->typeobj->tp_name, z_descr->typeobj->tp_name, q_descr->typeobj->tp_name);
        }
        Py_XDECREF(w_descr);Py_XDECREF(x_descr);Py_XDECREF(y_descr);Py_XDECREF(z_descr);Py_XDECREF(q_descr);
        countpairs_mocks_error_out(module, msg);
        return -1;
    ...
    /* Check if the number of elements in the 4 first count arrays are identical */
    const int64_t nw1 = (int64_t)PyArray_SIZE(w1_obj);
    const int64_t nx1 = (int64_t)PyArray_SIZE(x1_obj);
    const int64_t ny1 = (int64_t)PyArray_SIZE(y1_obj);
    const int64_t nz1 = (int64_t)PyArray_SIZE(z1_obj);

    if(nw1 != nx1 || nx1 != ny1 || ny1 != nz1) {
      snprintf(msg, 1024, "ERROR: Expected arrays to have the same number of elements in all 3-dimensions.\nFound (ndd, ndr, nrd, nrr) = (%"PRId64", %"PRId64", %"PRId64", %"PRId64") instead",
               ny1, nx1, ny1, nz1);
      countpairs_mocks_error_out(module, msg);
      return -1;
    }
...
static PyObject *countpairs_countpairs_s_mu_mocks(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if ( ! PyArg_ParseTupleAndKeywords(args, kwargs, "iiidisO!O!O!|O!O!O!O!O!bbbbbbbhbbbissis", kwlist,
