...
from anyscale.sdk.anyscale_client import CreateSession, StartSessionOptions, StopSessionOptions, TerminateSessionOptions, UpdateSession  # type: ignore
...
@sessions.command(name="stop", short_help="Sets session goal state to Stopped.")
@click.argument("session_id", required=True)
@click.argument("workers_only", required=False, default=False)
@click.argument("keep_min_workers", required=False, default=False)
def stop_session(session_id: str, workers_only: bool, keep_min_workers: bool) -> None:
    """Sets session goal state to Stopped.
    A session with goal state Stopped will eventually
    transition from its current state to Stopped, or
    remain Stopped if its current state is already Stopped.
    Retrieves the corresponding session operation.
    Warning: A session with goal state Terminated may not
    have its goal state changed to Stopped.
    ...
    stop_session_options = StopSessionOptions(
        workers_only=workers_only, keep_min_workers=keep_min_workers
    )
    response = api_client.stop_session(session_id, stop_session_options)
    print(common_formatter.prettify_json(response.to_dict()))
