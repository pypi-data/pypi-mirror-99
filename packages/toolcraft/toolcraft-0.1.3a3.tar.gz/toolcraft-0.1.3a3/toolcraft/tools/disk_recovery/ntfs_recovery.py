...
def parse_attr(f, bpc, chunk):
    ...
    stype, sname, sparser = ATTR_INFO.get(type, (
    'unk_%d' % type, str(type), parseRaw))
    if sparser is None:
        sparser = parseRaw
    sname = sname.strip()
    ...
    if nonres:
        ...
        while rlpos < len(chunk):
            ...
            if rlpos + lenlen + offlen > len(chunk):
                print("Warning: invalid runlist header %02x (runlist %s)" % (
                header, codecs.encode(chunk[rloff:], 'hex')), file=sys.stderr)
                break
...
def usa_fixup(chunk, chunkoff, usa_ofs, usa_count):
    ...
    for i in range(len(chunk) // 512):
        ...
        if chunk[cpos:cpos + 2] != usa_num:
            print("Warning: bad USA data at MBR offset %d - disk corrupt?" % (
                    chunkoff + cpos), file=sys.stderr)
        else:
...
def parse_file(f, chunkoff, bpc, chunk):
    ...
    try:
        ...
        print("File at offset %d: failed to perform USA fixup: %s" % (
        chunkoff, e), file=sys.stderr)
    ...
    while 1:
        if pos > len(chunk) - 12:
            # Uhoh, corruption?
            break
        type, size, nonres, namelen, nameoff = struct.unpack('<iIBBH', chunk[
                                                                       pos:pos + 12])
        if type == -1:
            break
        ...
        try:
            ...
            print(
                ...
                chunkoff, type, pos, e), file=sys.stderr)
...
def read_mft(f, bpc, mft_cluster, clusters_per_mft):
    ...
    try:
        ...
        print(
            "WARNING: Failed to load $MFT (%s), proceeding with partial MFT." % e,
            file=sys.stderr)
...
def parse_args(argv):
    ...
    parser.add_argument('disk',
                        help='NTFS partition (e.g. /dev/disk*, \\\\.\\Harddisk*Partition*)')
...
def main(argv):
    ...
    mft_clust, mftmirr_clust, clust_per_mft = struct.unpack('<QQB',
                                                            readat(f, 0x30, 17))
    ...
    for i, file in enumerate(mft):
        ...
        # print(fullpath.startswith("GaMeS"), fullpath)
        ...
        #     if fnmatch.fnmatch(fn.lower().encode('utf8'),
        #                        pat) or fnmatch.fnmatch(
        #         fullpath.lower().encode('utf8'), pat):
        #         print("Recovering", fullpath, end=' ')
        #         try:
        #             save_file(file, fullpath)
        #         except Exception as e:
