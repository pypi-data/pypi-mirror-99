...
def get_modules(url,headers,project_id,module=None,page=None,object=None,attribute=None):
    ...
    if tmp.status_code == 200:
        ...
        modules["error"]="Invalid Token or project ID"
        flag3=True
...
class OMDriver:
    def __init__(self,headers,project_id):
    ...
    def getData(self,module=None,page=None,object=None,attribute=None):
        key=""
        page_url="http://172.104.33.173:4000/api/v1/project"
        if module!=None:
            key=key+module
        if page!=None:
            key=key+page
        ...
        if key in self.localstorage.keys():
            ...
            response=get_modules(page_url,self.headers,self.project_id,module,page,object,attribute)
