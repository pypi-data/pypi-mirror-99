...
from pollination.ladybug_comfort.map import PmvMap, MapResultInfo, Tcp
...
class PmvComfortMapEntryPoint(DAG):
    ...
    def run_comfort_map(
        ...
        air_speed=air_speed, met_rate=met_rate, clo_value=clo_value,
        solarcal_par=solarcal_parameters, comfort_par=comfort_parameters,
        run_period=run_period, write_set_map=write_set_map
    ) -> List[Dict]:
        return [
            {
                'from': PmvMap()._outputs.temperature_map,
                'to': 'temperature/{{item.id}}.csv'
            },
            {
                'from': PmvMap()._outputs.condition_map,
                'to': 'condition/{{item.id}}.csv'
            },
            {
                'from': PmvMap()._outputs.pmv_map,
