...
def get_siagie_por_anio(anio,id_grado=None,modalidad='EBR',dtypes_columns={'ID_PERSONA':int},
                        ...
                        id_persona_df=None,id_nivel=None,reduce_mem_usage=False):       
    ...
    if (modalidad=='EBR'):    
        if(anio<2019):
            ...
            df = procesar_chunk_siagie(iter_pd,id_grado,id_persona_df,id_nivel)
        else:
            list_df = []
...
def procesar_chunk_siagie(iter_pd,id_grado,id_persona_df,id_nivel):
    chunk_list = []
