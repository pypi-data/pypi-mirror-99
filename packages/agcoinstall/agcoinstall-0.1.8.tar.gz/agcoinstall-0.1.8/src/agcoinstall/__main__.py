    ...
    def ready_to_install(self):
        if os.path.isfile(r"C:\ProgramData\AGCO Corporation\AGCO Update\config.ini"):
            try:
                ...
                # self.ready_to_install = ready_to_install_value
...
class InstallState:
    ...
    @property
    def pause_download_set(self):
        return self._config_ini.pause_downloads_set
    ...
    @property
    def get_state(self):
        if self.ready_to_install and not self.edt_update:
        ...
        elif self.ready_to_install and self.voucher and self.active_packages != 0 and self._updategroup in \
                ('EDTUpdates', 'InternalTestPush', 'TestPush'):
        ...
        elif not self.ready_to_install and self.edt_update and self.active_packages == 0 and not self.voucher:
            logging.info("Changing current_state to EDTNeedsVouchered")
            self._state = 'EDTNeedsVouchered'
            return 'EDTNeedsVouchered'

        elif self.pause_download_set and self.active_packages > 1:
            logging.info("Changing current_state to DownloadsPaused")
            self._state = 'DownloadsPaused'
            return 'DownloadsPaused'

        elif self.active_packages == 0 and not self.ready_to_install and self.mtapi and self._updategroup in \
            ('EDTUpdates', 'InternalTestPush', 'TestPush') and self.models_installed:
        ...
        elif self.active_packages == 0 and not self.ready_to_install and self.mtapi and self._updategroup not in \
            ('EDTUpdates', 'InternalTestPush', 'TestPush') and not self.models_installed:
            logging.info("Changing current_state to AllDownloadsCompleted")
            self._state = 'AllDownloadsCompleted'
            return 'AllDownloadsCompleted'
