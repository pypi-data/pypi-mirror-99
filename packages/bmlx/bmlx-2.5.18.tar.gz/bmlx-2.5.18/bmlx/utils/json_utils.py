...
# original type.
_BMLX_OBJECT_TYPE_KEY = "__bmlx_object_type__"
_MODULE_KEY = "__module__"
_CLASS_KEY = "__class__"
_PROTO_VALUE_KEY = "__proto_value__"

RUNTIME_PARAMETER_PATTERN = (
    r'({\\*"__class__\\*": \\*"RuntimeParameter\\*", ' r".*?})"
)
...
JsonableValue = Union[
    bool, bytes, float, int, Jsonable, message.Message, Text, Type
]
...
class _DefaultEncoder(json.JSONEncoder):
    ...
    def default(self, obj: Any) -> Any:
        ...
        if obj.__class__.__name__ == "RuntimeParameter" and obj.ptype == Text:
            dict_data = {
                _BMLX_OBJECT_TYPE_KEY: _ObjectType.JSONABLE,
        ...
        if isinstance(obj, Jsonable):
            ...
            for k, v in data_patch.items():
                if (
                    v.__class__.__name__ == "RuntimeParameter"
                    and v.ptype == Text
                ):
        ...
        if inspect.isclass(obj):
            return {
                _BMLX_OBJECT_TYPE_KEY: _ObjectType.CLASS,
        ...
        if isinstance(obj, message.Message):
            return {
                _BMLX_OBJECT_TYPE_KEY: _ObjectType.PROTO,
                _MODULE_KEY: obj.__class__.__module__,
                _CLASS_KEY: obj.__class__.__name__,
                _PROTO_VALUE_KEY: json_format.MessageToJson(
                    message=obj,
                    sort_keys=True,
...
class _DefaultDecoder(json.JSONDecoder):
    ...
    def _dict_to_object(self, dict_data: Dict[Text, Any]) -> Any:
        ...
        if _BMLX_OBJECT_TYPE_KEY not in dict_data:
            return dict_data

        object_type = dict_data.pop(_BMLX_OBJECT_TYPE_KEY)
        ...
        if object_type == _ObjectType.JSONABLE:
            ...
            if not issubclass(jsonable_class_type, Jsonable):
                raise ValueError(
                    "Class %s must be a subclass of Jsonable"
                    % jsonable_class_type
                )
        ...
        if object_type == _ObjectType.PROTO:
            ...
            return json_format.Parse(
                dict_data[_PROTO_VALUE_KEY], proto_class_type()
            )
