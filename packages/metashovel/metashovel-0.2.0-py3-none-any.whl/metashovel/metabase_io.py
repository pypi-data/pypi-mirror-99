        ...
        if only_model == "all":
            ...
            if len(mapper.missing_mapping_cards) > 0:
                ...
                for card in mapper.missing_mapping_cards:
                    c = next(
                        filter(
                            lambda r: r["id"] == mapper.mappings["collections"][card["collection_id"]],
                            result,
                        )
                    )
                    self.add_items(
...
class Mapper:
    ...
    def add_version_to(self, item):
        source_desc = self.source_map[item["model"] + "s"][int(item["id"])]
    ...
    def has_changed(self, item):
        model = item["model"] + "s"
        source_h = self.source_map[model][int(item["id"])]["content_hash"]
    ...
    def deref_json_key(self, key):
        keyobj = json.loads(key)
        self.deref_fields(keyobj)
        return json.dumps(keyobj)

    def deref_column_settings(self, col_settings, for_card):
        if col_settings is not None and "click_behavior" in col_settings:
            col_settings = col_settings.copy()
            click_behavior = col_settings["click_behavior"]
            if click_behavior["type"] == "link":
                if click_behavior["linkType"] == "dashboard":
                    objectType = "dashboards"
                elif click_behavior["linkType"] == "question":
                    objectType = "cards"
                    if "parameterMapping" in click_behavior:
                        click_behavior["parameterMapping"] = {
                            self.deref_json_key(k): self.deref_fields(v)
                            for k, v in click_behavior["parameterMapping"].items()
                        }
                        from pprint import pprint
                        pprint(click_behavior)

                if click_behavior["targetId"] in self.mappings[objectType]:
                    click_behavior["targetId"] = self.mappings[objectType][click_behavior["targetId"]]
    ...
    def deref_visualization_settings(self, vs, original_card):
        if "column_settings" in vs:
            vs["column_settings"] = {
                self.deref_json_key(k): self.deref_column_settings(
                    v, original_card
                )
    ...
    def deref_card(self, card):
        ...
        if "visualization_settings" in card:
            card["visualization_settings"] = self.deref_visualization_settings(card["visualization_settings"], original_card)
    ...
    def deref_dashboard(self, dashboard):
        ...
        for c, card in enumerate(dashboard["ordered_cards"]):
            ...
            for pm in card["parameter_mappings"]:
                pm["card_id"] = self.mappings["cards"][pm["card_id"]]
                for target_spec in pm["target"]:
                    if isinstance(target_spec, list):
                        self.deref_fields(target_spec)

            if "visualization_settings" in card:
                card["visualization_settings"] = self.deref_visualization_settings(card["visualization_settings"], card)
