...
def stringify_keys(node):
    """
    Turns all the keys of node intro strings, recursively
    """
    if isinstance(node, dict):
        return { str(k): stringify_keys(v) for k, v in node.items() }
    else:
        return node
...
class Splitter:
    ...
    def store_datamodel(self, node):
        ...
        for db_spec in dbs.values():
            ...
            self.store_to(stringify_keys(db_spec), "databases/{}".format(db_file))

        self.store_to(stringify_keys(node["datamodel"]["segments"]), "segments")
    ...
    def load_datamodel(self):
        ...
        return { "databases": databases, "segments": self.load_from("segments.yaml") }
