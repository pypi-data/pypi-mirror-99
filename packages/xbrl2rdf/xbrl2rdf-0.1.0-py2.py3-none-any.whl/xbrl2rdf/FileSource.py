...
def openFileSource(filename, sourceZipStream=None, reloadCache=False, base=None):
    if sourceZipStream:
        filesource = FileSource(POST_UPLOADED_ZIP)
        ...
        if archivepathSelection is not None:
            ...
            filesource = FileSource(archivepath)
...
class FileSource:
    ...
    def open(self, reloadCache=False):
        if not self.isOpen:
            ...
            if self.isZip:
                try:
                    self.fs = zipfile.ZipFile(openFileStream(self.basefile, 'rb'), mode="r")
                    self.isOpen = True
    ...
    def loadTaxonomyPackageMappings(self):
        if not self.mappedPaths and self.taxonomyPackageMetadataFiles:
            ...
            taxonomyPackage = PackageManager.parsePackage(self, metadata,
    ...
    def fileSourceContainingFilepath(self, filepath):
        ...
        if referencedFileParts is not None:
            ...
            if referencedArchiveFile in self.referencedFileSources:
                ...
                referencedFileSource = openFileSource(filepath)
                ...
                if filepath.startswith(archiveFileSource.basefile):
                    ...
                    for f in TAXONOMY_PACKAGE_FILE_NAMES:
                        if filepath[l - len(f):l] == f:
                            ...
                            break
        if binary:
            return (openFileStream(filepath, 'rb'), )
        elif encoding:
            return (openFileStream(filepath, 'rt', encoding=encoding), )
        else:
            return openXmlFileStream(filepath, stripDeclaration)
...
def openFileStream(filepath, mode='r', encoding=None):
    ...
    if archiveFilenameParts(filepath): # file is in an archive
        return openFileSource(filepath).file(filepath, binary='b' in mode, encoding=encoding)[0]
...
def openXmlFileStream(filepath, stripDeclaration=False):
...
def saveFile(filepath, contents, encoding=None, mode='wt'):
    # file path may be server (or memcache) or local file system
