...
from .utilfunctions import getLanguageCode, isAbsolute
...
class Taxonomies:
    ...
    def __init__(self, path):
        ...
        self.mapping = dict()
        try:
            self.jsonFile = join(path, "taxonomyPackages.json")
            with io.open(self.JsonFile, 'rt', encoding='utf-8') as f:
                self.config = json.load(f)
        except Exception:
            pass
        if self.config is None:
            self.config = {"packages": list(), "remappings": dict()}


    def addPackage(self, url):
        newPackageInfo = packageInfo(url, packageManifestName=None)
        if newPackageInfo and newPackageInfo.get("name"):
            name = newPackageInfo.get("name")
            version = newPackageInfo.get("version")
            j = -1
            packagesList = self.config["packages"]
            ...
            for j in range(i-1, -1, -1):
                ...
                if (_packageURL != _packageURL2 and _prefix and _prefix2 and 
                    ...
                    if _url1 == _url2: # use full file names
                        ...
                        _url2 = _packageURL2
                    if _prefix != _prefix2:
                        print("Packages overlap rewrite start strings " + _prefix + " and " + _prefix2)
                    # addToLog(_("Packages overlap the same rewrite start string %(rewriteStartString)s")
    ...
    def save(self):
        with io.open(self.jsonFile, 'wt', encoding='utf-8') as f:
            jsonStr = json.dumps(self.orderedPackagesConfig(), ensure_ascii=False, indent=2)
            f.write(jsonStr)

    def orderedPackagesConfig(self):
        return OrderedDict(
            (('packages', [OrderedDict(sorted(_packageInfo.items(), 
                           ...
                           for _packageInfo in self.config['packages']]),
             ('remappings',OrderedDict(sorted(self.config['remappings'].items())))))
...
def parsePackage(filesource, metadataFile, fileBase, errors=[]):
        ...
        for n in root.iter(tag=nsPrefix + "*"):
            for eltName in ("name", "description", "publisher"):
                ...
                for lang, elts in langElts.items():
                    if not lang:
                        addToLog(_("Multi-lingual element %(element)s has no in-scope xml:lang attribute"),
                        ...
                        addToLog(_("Multi-lingual element %(element)s has multiple (%(count)s) in-scope xml:lang %(lang)s elements"),
        ...
        for m in rewriteTree.iter(tag=tag):
            ...
            if prefixValue and replaceValue is not None:
                if prefixValue not in remappings:
                    ...
                    _normedValue = replaceValue
                    if replaceValue.endswith(os.sep) and not _normedValue.endswith(os.sep):
                        _normedValue += os.sep
                    remappings[prefixValue] = _normedValue
                else:
                    addToLog(_("Package catalog duplicate rewrite start string %(rewriteStartString)s"),
...
def packageInfo(URL, reload=False, packageManifestName=None, errors=[]):
    packageFilename = URL
    if packageFilename:
        filesource = None
        try:
            filesource = openFileSource(packageFilename)
            ...
            if filesource.isZip:
                ...
                if len(topLevelDirectories) != 1:
                    addToLog(_("Taxonomy package contains %(count)s top level directories:  %(topLevelDirectories)s"),
                    ...
                    addToLog(_("Taxonomy package does not contain a subdirectory META-INF"),
                    ...
                    addToLog(_("Taxonomy package does not contain a metadata file */META-INF/taxonomyPackage.xml"),
                ...
                addToLog(_("Taxonomy package is not a zip file."),
            ...
            for packageFileUrl, packageFilePrefix, packageFile in packages:
                parsedPackage = parsePackage(filesource, packageFileUrl, packageFilePrefix, errors)
                ...
                for prefix, remapping in parsedPackage["remappings"].items():
                    if prefix not in remappings:
                        ...
                        addToLog("Package mapping duplicate rewrite start string %(rewriteStartString)s",
