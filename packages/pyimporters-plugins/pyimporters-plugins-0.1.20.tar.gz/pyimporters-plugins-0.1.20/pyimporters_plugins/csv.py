...
class CSVKnowledgeParser(KnowledgeParserBase):
    def parse(self, source, options : Dict[str,Any]):
        ...
        lines = pd.read_csv(codecs.iterdecode(source,'utf-8'), sep=options.separator, quotechar=options.quotechar, header=options.header, skip_blank_lines=options.skip_blank_lines, encoding=options.encoding)
