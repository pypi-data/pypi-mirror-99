...
class Concept(BaseModel):
    identifier: str = Field(..., description="Unique identifier of the concept", example="http://www.example.com/rocks")
    prefLabel: str = Field(..., description="The preferred label of the concept", example="rocks")
    altLabel: Optional[List[str]] = Field(None, description="The alternative labels of the concept", example=["basalt", "granite", "slate"])
...
class KnowledgeParserBase(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def parse(self, source : IO, options: Dict[str,Any]):
