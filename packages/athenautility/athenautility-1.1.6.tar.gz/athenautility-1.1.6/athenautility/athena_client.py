...
class AthenaClient:
    ...
    def __init__(self):
        self._user_name = os.getenv(self._USER_NAME, None) or 'Karthik'
        self._access_token = os.getenv(self._ACCESS_TOKEN, None) or '1'
    ...
    def databases(self, result_as_df=True):
        ...
        try:
            payload = json.dumps({"user_name": self._user_name, "password": self._access_token})
    ...
    def tables(self, schema_name, result_as_df=True):
        ...
        try:
            payload = json.dumps({"user_name": self._user_name, "password": self._access_token, "schema_name": schema_name})
    ...
    def run_query(self, query_string, result_as_df=True):
        ...
        try:
            payload = json.dumps(
                {"user_name": self._user_name, "password": self._access_token, "query_string": query_string})
