...
from skyalmanac.lib import alma_skyfield as alma_skyfield
from skyalmanac.lib import alma_ephem as alma_ephem
...
'''.format(date.strftime("%a"))
        aGHA             = alma_skyfield.ariesGHA(date)
        vGHA, vDEC, vDEG = alma_skyfield.venusGHA(date)
        mGHA, mDEC, mDEG = alma_skyfield.marsGHA(date)
        jGHA, jDEC, jDEG = alma_skyfield.jupiterGHA(date)
        sGHA, sDEC, sDEG = alma_skyfield.saturnGHA(date)
        h = 0
...
'''
        ...
        RAc_v, Dc_v = alma_skyfield.vdm_Venus(date)
        RAc_m, Dc_m = alma_skyfield.vdm_Mars(date)
        RAc_j, Dc_j = alma_skyfield.vdm_Jupiter(date)
        RAc_s, Dc_s = alma_skyfield.vdm_Saturn(date)
        mag_v, mag_m, mag_j, mag_s = alma_ephem.magnitudes(date)
        tab = tab + r'''\hline
...
\hline
\multicolumn{{10}}{{c}}{{}}\\
'''.format(alma_skyfield.ariestransit(date+datetime.timedelta(days=1)),RAc_v,Dc_v,mag_v,RAc_m,Dc_m,mag_m,RAc_j,Dc_j,mag_j,RAc_s,Dc_s,mag_s)
        n += 1
...
\hline\rule{0pt}{2.6ex}\noindent
'''
    stars = alma_skyfield.stellar_info(date+datetime.timedelta(days=1))
...
'''.format(datestr)
        ...
        p = alma_skyfield.planetstransit(datex)
        m = m + r'''Venus & {} & {} \\
'''.format(p[0],p[1])
        m = m + r'''Mars & {} & {} \\
'''.format(p[2],p[3])
...
'''.format(date.strftime("%a"))
        ...
        gham, decm, degm, HPm, GHAupper, GHAlower, ghaSoD, ghaEoD = alma_skyfield.moonGHA(date)
        vmin, dmin = alma_skyfield.moonVD(date0,date)
...
'''
        ...
        sdmm = alma_skyfield.moonSD(date)
        tab = tab + r'''\hline
...
'''
        ...
        twi = alma_ephem.twilight(date+datetime.timedelta(days=1), i, hemisph)
...
'''
        lasthemisph = hemisph
        moon, moon2 = alma_ephem.moonrise_set(date,i)
...
# Equation of Time section ...........................................
    ...
    if config.moonimg:
        ...
        age0, pct0 = alma_skyfield.moonage(d, d1)
        phase = alma_skyfield.moonphase(d1)       # moon phase (0:new to π:full to 2π:new)
        age2, pct2 = alma_skyfield.moonage(d2, d3)
...
'''.format(pcts)
        ...
        d = date
        for k in range(3):
            eq = alma_skyfield.equation_of_time(d,d + datetime.timedelta(days=1),UpperLists[k],LowerLists[k], False)
            if k == 0:
...
'''
        ...
        d = date
        for k in range(3):
            eq = alma_skyfield.equation_of_time(d,d + datetime.timedelta(days=1),UpperLists[k],LowerLists[k], True)
            if k == 2:
                tab = tab + r'''{} & {} & {} & {} & {} & {} & {}({}\%) \\[0.3ex]
...
'''.format(timedelta, date.strftime("%Y %B %d"),(date+datetime.timedelta(days=2)).strftime("%b. %d"),rightindent)
    ...
    page = page + twilighttab(date,dfcm)
    page = page + r'''
    ...
    for i in range(p):
        if p == 122:	# if Full Almanac for a year...
            ...
            if cmth != pmth:
                ...
                sys.stdout.flush()
            pmth = cmth
        out = out + doublepage(first_day,page1,dfcm)
        page1 = False
...
def almanac(first_day, pagenum, df180, df360, dfcm):
...
\restoregeometry    % so it does not affect the rest of the pages'''
    ...
    alm = alm + pages(first_day,pagenum,dfcm)
    alm = alm + '''
\end{document}'''
    return alm