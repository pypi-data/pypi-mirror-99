...
ns = DANamespace('{module}', description='{doc}', path='/')
"""
...
def create_module(path, module_name, module_doc):
    os.makedirs(op.join(path, module_name), exist_ok=False)

    for file, content in empties.items():
        if file == 'views.py':
            content = content.format(module=module_name, doc=module_doc)
...
def auto_view(config, init=False):
    ...
    for route in routes['path_list']:
        ...
        if not op.exists(op.join(config['app_path'], config['app_name'], 'api', route['module'])):
            create_module(op.join(config['app_path'], config['app_name'], 'api'), route['module'], route.get('doc', ''))
