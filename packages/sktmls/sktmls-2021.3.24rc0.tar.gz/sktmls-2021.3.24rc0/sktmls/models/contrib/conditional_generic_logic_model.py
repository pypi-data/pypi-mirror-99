...
class ConditionalGenericLogicModel(MLSGenericModel):
    ...
    def __init__(
        ...
        - model_selection_logic: (dict) `models`의 모델 중 하나를 선택하는 로직. 정수(int)값을 반환하는 로직이어야 합니다. (기본값: {"abs": 0} (models[0] 모델 사용))
    ...
    def predict(self, x: List[Any], **kwargs) -> Dict[str, Any]:
        ...
        model_index = self._select_model(x, kwargs.get("keys", []), pf_client)
        if not isinstance(model_index, int) or model_index < 0 or model_index >= len(self.models):
            raise MLSModelError(f"ConditionalGenericLogicModel: model_index{model_index}가 유효하지 않습니다.")
    ...
    def _select_model(self, x: List[Any], additional_keys: List[Any], pf_client: MLSProfileAPIClient) -> int:
