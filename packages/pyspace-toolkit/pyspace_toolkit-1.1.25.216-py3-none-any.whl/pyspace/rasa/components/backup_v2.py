...
class LMTokenizer(Tokenizer, SparseFeaturizer):
    ...
    #    _lm_feat_ohe: Any = None,
        ...
        #if _lm_feat_ohe is not None:
        #    self._lm_feat_ohe = _lm_feat_ohe
        #else:
        #    self._lm_feat_ohe = {
        #        TEXT : OneHotEncoder(handle_unknown='ignore'),
        #        RESPONSE : OneHotEncoder(handle_unknown='ignore')
        #    }
    ...
    def _lm_tokenize(self, message: Message, attribute: Text, doc:Any) -> Any:

        tokens = self._get_lm_tokens(doc)
    ...
    def _lm_featurize(self, doc:Any, attribute:Any, train_process:Any) -> Any:
        ...
        ##features = [ [token['upos'], token['xpos'], token['deprel'], *expand_feats(token),  ]
        ##    for token in lm_tokens]
        #features = [ [token['upos'],  ]
        #    for token in lm_tokens]
        ## append empty features for cls token
        #features.append( [none_value]*len(features[0]) )

        #if train_process == 'train':
        #    features = self._lm_feat_ohe[attribute].fit_transform(features).tocoo()
        #elif train_process == 'process':
        #    features = self._lm_feat_ohe[attribute].transform(features).tocoo()

        ##################################################################################

        seq_length = len(lm_tokens) + 1

        vec = np.zeros([seq_length, len(['NOUN', 'PROPN'])])

        for pattern_index, pattern in enumerate(['NOUN', 'PROPN']):
    ...
    def _set_lm_features(self, message:Any, doc:Any, attribute:Any, train_process:Any) -> Any:

        features = self._lm_featurize(doc, attribute, train_process) # if message.get(attribute) else None
            ...
            for attribute in MESSAGE_ATTRIBUTES:
                if message.get(attribute) is not None:
                    if attribute == INTENT:
                        ...
                        words, text = self._lm_tokenize(message, attribute, doc)
                        ...
                        if attribute == TEXT:
                            self._set_lm_features(message, doc, attribute, 'train')
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        words, text = self._lm_tokenize(message, TEXT, doc)
        ...
        self._set_lm_features(message, doc, TEXT, 'process')
    ...
    #    cached_component: Optional["LMTokenizer"] = None,
    #    **kwargs: Any,
    #) -> "LMTokenizer":
    #    ohe_file_name = meta.get("ohe")
    #    ohe_file = os.path.join(model_dir,ohe_file_name)
    #    import pickle
    #    if os.path.exists(ohe_file):
    #        with open(ohe_file, 'rb') as f:
    #            _lm_feat_ohe = pickle.load(f)
    #        return LMTokenizer(meta, _lm_feat_ohe)
    #    else:
    #        print('Warning: LMTokenizer ohe_file does not exist.')
    #        return LMTokenizer(meta)

    #def persist(self, file_name: Text, model_dir: Text) -> Optional[Dict[Text, Any]]:
    #    """Persist this model into the passed directory.
    #    Return the metadata necessary to load the model again."""
    #    ohe_file_name = file_name + "_ohe.pkl"
    #    ohe_file = os.path.join(model_dir, ohe_file_name)
    #    import pickle
    #    with open(ohe_file, 'wb') as f:
    #        pickle.dump(self._lm_feat_ohe, f)
    #    return {"ohe": ohe_file_name}
