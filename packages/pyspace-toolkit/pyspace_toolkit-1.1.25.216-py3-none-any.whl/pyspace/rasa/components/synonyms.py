    ...
    def train(
        ...
        **kwargs: Any,
        ...
        for key, value in list(training_data.entity_synonyms.items()):
            ...
            # self.add_entities_if_synonyms(xNormalizer.tr_normalize(key).lower(), value)
            self.synonyms[xNormalizer.tr_normalize(key).lower()] = value
            if xNormalizer.tr_normalize(value).lower() != value:
        ...
        for example in training_data.entity_examples:
            for entity in example.get(ENTITIES, []):
                ...
                if entity_val != str(entity.get("value")):
                    entity_val = xNormalizer.tr_normalize(entity_val).lower()
                    if entity_val not in self.synonyms:
                        self.synonyms[entity_val] = str(entity.get("value"))
