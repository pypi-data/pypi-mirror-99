...
class RasaSpacyTokenizer(Tokenizer, SparseFeaturizer, EntityExtractor):
    ...
    def __init__(
        ...
        # nlp = SpacyTurkish()
        # tnlp = SpacyTurkish()
        # tnlp.tokenizer = lambda x: x
        # tnlp.add_pipe(SpacyWhitespaceNormalizer())
        # tnlp.add_pipe(SpacyRegexReplaceNormalizer(replace_patterns))
        # tnlp.add_pipe(SpacyEndPunctuationNormalizer())
        # tnlp.add_pipe(SpacyTokenizer(tnlp))
        # tnlp.vocab = nlp.vocab
        # nlp.tokenizer = tnlp
        ...
        if self.custom_tokenizer:
            ...
            if self.custom_tokenizer__token_patterns:
                tnlp.add_pipe(SpacyTokenizer(tnlp, token_patterns=token_patterns))
            else:
                tnlp.add_pipe(SpacyTokenizer(tnlp, ))
