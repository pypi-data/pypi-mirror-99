...
from .logging_messages import OPTIMIZER_KWARGS_CONSTRUCTOR_DEPRACETED
...
class Optimizer(object):
    ...
    def _fill_defaults_predictor(self, predictor_spec):
        ...
        if predictor_spec is not None:
            # Keep in sync with Predictor.__init__ default values
            defaults = {
                "loss_name": "loss",
                "mode": "global",
    ...
    def _create_experiment(self, parameters, pid, trial, count, experiment_kwargs):
        ...
        if self.predictor_spec:
            ...
            try:
                exp.set_predictor(
                    Predictor(exp, optimizer_id=self.id, mode=mode, **predictor_spec)
                )
            except Exception:
                LOGGER.warning(
