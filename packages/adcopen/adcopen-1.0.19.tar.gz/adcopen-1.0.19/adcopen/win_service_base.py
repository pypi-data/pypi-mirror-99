...
Adapted from SMWinService
by Davide Mastromatteo

edits by Thomas C. Bitsky Jr.
Automated Design Corp.

- Adds command line option "runmain" for testing and development without
...
class WinServiceBase(win32serviceutil.ServiceFramework):
    '''Base class to create winservice in Python'''
    ...
    def parse_command_line(cls):
        ...
        if len(sys.argv) > 1 and sys.argv[1] == "runmain":
            logging.getLogger().setLevel(logging.DEBUG)
            logging.debug("Running main method of program for debugging.")
            cls.start(cls)
            cls.main(cls)
        else:
...
if __name__ == '__main__':
    WinServiceBase.parse_command_line()