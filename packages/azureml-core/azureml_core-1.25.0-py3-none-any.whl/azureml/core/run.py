...
class Run(_RunBase):
    ...
    * `How to manage runs <https://docs.microsoft.com/azure/machine-learning/how-to-manage-runs>`_
    ...
    def fail(self, error_details=None, error_code=None, _set_status=True):
        """DEPRECATED. Use :func:`tag` to mark run as failed or use :func:`cancel` to mark the run as canceled.
        ...
        :type _set_status: bool
        """
        module_logger.warning("Run.fail() is deprecated, use Run.tag() to mark run as failed "
                              "or use Run.cancel() to mark the run as canceled")
        module_logger.warning("Run.fail() will be removed shortly")
