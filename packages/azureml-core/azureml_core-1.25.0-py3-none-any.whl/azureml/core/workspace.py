...
class Workspace(object):
    ...
    * `Azure Machine Learning
    ...
    def get(name, auth=None, subscription_id=None, resource_group=None):
        ...
        workspace = _commands.get_workspace(
            auth,
            subscription_id=subscription_id,
    ...
    def update_dependencies(self, container_registry=None, force=False):
        ...
        try:
            cmk_keyvault = attrgetter(
                "encryption.key_vault_properties.key_vault_arm_id")(workspace)
        except AttributeError:
            cmk_keyvault = None
        try:
            resource_cmk_uri = attrgetter(
                "encryption.key_vault_properties.key_identifier")(workspace)
        except AttributeError:
            resource_cmk_uri = None
        try:
            user_assigned_identity_for_cmk_encryption = attrgetter(
                "encryption.identity.user_assigned_identity")(workspace)
        except AttributeError:
            user_assigned_identity_for_cmk_encryption = None
        return _commands.create_or_update_workspace(
            auth=self._auth,
            resource_group_name=self._resource_group,
            workspace_name=self._workspace_name,
            subscription_id=self._subscription_id,
            ...
            app_insights=workspace.application_insights,
            containerRegistry=container_registry,
            adb_workspace=workspace.adb_workspace,
            primary_user_assigned_identity=workspace.primary_user_assigned_identity,
            cmk_keyvault=cmk_keyvault,
            resource_cmk_uri=resource_cmk_uri,
            hbi_workspace=workspace.hbi_workspace,
            exist_ok=True,
            user_assigned_identity_for_cmk_encryption=user_assigned_identity_for_cmk_encryption,
            is_update_dependent_resources=True)
