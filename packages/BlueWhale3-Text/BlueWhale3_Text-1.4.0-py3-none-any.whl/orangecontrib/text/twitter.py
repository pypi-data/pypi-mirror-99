...
def __(key):
    return i18n.t('text.twitter.' + key)
...
class TwitterAPI:
    ...
    metas = [
        (
            StringVariable(__('content')),
            lambda doc: doc.full_text if not doc.retweeted else doc.text,
        ),
        (tv, lambda doc: TwitterAPI.tv.parse(doc.created_at.isoformat())),
        (DiscreteVariable(__('language')), lambda doc: doc.lang),
        (
            DiscreteVariable(__('location')),
            lambda doc: getattr(doc.place, "country_code", None),
        ),
        (
            ContinuousVariable(__('likes'), number_of_decimals=0),
            lambda doc: doc.favorite_count,
        ),
        (
            ContinuousVariable(__('retweets'), number_of_decimals=0),
            lambda doc: doc.retweet_count,
        ),
        (
            DiscreteVariable(__('reply_to')),
            lambda doc: "@" + doc.in_reply_to_screen_name
            if doc.in_reply_to_screen_name
            else "",
        ),
        (DiscreteVariable(__('author.name')), lambda doc: doc.author.name),
        (
            StringVariable(__('author.description')),
            lambda doc: doc.author.description,
        ),
        (
            ContinuousVariable(__('author.status_count'), number_of_decimals=0),
            lambda doc: doc.author.statuses_count,
        ),
        (
            ContinuousVariable(__('author.favourites_count'), number_of_decimals=0),
            lambda doc: doc.author.favourites_count,
        ),
        (
            ContinuousVariable(__('author.friends_count'), number_of_decimals=0),
            lambda doc: doc.author.friends_count,
        ),
        (
            ContinuousVariable(__('author.followers_count'), number_of_decimals=0),
            lambda doc: doc.author.followers_count,
        ),
        (
            ContinuousVariable(__('author.listed_count'), number_of_decimals=0),
            lambda doc: doc.author.listed_count,
        ),
        (
            DiscreteVariable(__('author.verified')),
            lambda doc: str(doc.author.verified),
        ),
        (
            ContinuousVariable(__('longitude')),
            lambda doc: coordinates_geoJSON(doc.coordinates)[0],
        ),
        (
            ContinuousVariable(__('latitude')),
