...
def __(key):
    return i18n.t('text.wikipedia.' + key)
...
class WikipediaAPI:
    ...
    """
    metas = [
        (data.StringVariable(__('title')), lambda doc: getattr(doc, 'title')),
        (data.StringVariable(__('content')), lambda doc: getattr(doc, 'content')),
        (data.StringVariable(__('summary')), lambda doc: getattr(doc, 'summary')),
        (data.StringVariable('Url'), lambda doc: getattr(doc, 'url')),
        (data.ContinuousVariable(__('page_id'), number_of_decimals=0),
         lambda doc: int(getattr(doc, 'pageid'))),
        (data.ContinuousVariable(__('revision_id'), number_of_decimals=0),
         lambda doc: int(getattr(doc, 'revision_id'))),
        (data.DiscreteVariable(__('query')), lambda doc: getattr(doc, 'query')),
    ]
