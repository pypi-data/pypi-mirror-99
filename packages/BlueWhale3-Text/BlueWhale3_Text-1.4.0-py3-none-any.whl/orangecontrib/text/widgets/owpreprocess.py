...
def __(key):
    return i18n.t('text.owpreprocess.' + key)


_DEFAULT_NONE = __("row.none")
...
class FileLoader(QWidget):
    ...
    def browse(self):
        ...
        formats = [__("dialog.text_file"), __("dialog.all_file")]
        file_name, _ = QFileDialog.getOpenFileName(
            None, __("dialog.open"), start_file, ";;".join(formats), formats[0])
        if not file_name:
            return
...
class TokenizerModule(SingleMethodModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addWidget(QLabel(__("row.pattern")), 3, 1)
...
class NgramsModule(PreprocessorModule):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.layout().addRow(__("row.range"), self.__range_spins)
...
PREPROCESS_ACTIONS = [
    PreprocessAction(
        ...
        Description(__("row.transform"), icon_path("Transform.svg")),
        TransformationModule
    ),
    PreprocessAction(
        "Tokenization", "preprocess.tokenize", "",
        Description(__("row.tokenize"), icon_path("Tokenize.svg")),
        TokenizerModule
    ),
    PreprocessAction(
        "Normalization", "preprocess.normalize", "",
        Description(__("row.normalize"), icon_path("Normalize.svg")),
        NormalizationModule
    ),
    PreprocessAction(
        "Filtering", "preprocess.filter", "",
        Description(__("row.filter"), icon_path("Filter.svg")),
        FilteringModule
    ),
    PreprocessAction(
        "N-grams Range", "preprocess.ngrams", "",
        Description(__("row.ngrams"), icon_path("NGrams.svg")),
        NgramsModule
    ),
    PreprocessAction(
        "POS Tagger", "tag.pos", "",
        Description(__("row.pos"), icon_path("POSTag.svg")),
        POSTaggingModule
    )
]
...
class OWPreprocess(Orange.widgets.data.owpreprocess.OWPreprocess,
    ...
    class Error(Orange.widgets.data.owpreprocess.OWPreprocess.Error):
        unknown_error = Msg("{}")
        udpipe_offline_no_models = Msg(__("msg.udpipe_offline_no_models"))
        file_not_found = Msg(__("msg.file_not_found"))
        invalid_encoding = Msg(__("msg.invalid_encoding"))
        stanford_tagger = Msg(__("msg.stanford_tagger"))

    class Warning(Orange.widgets.data.owpreprocess.OWPreprocess.Warning):
        no_token_left = Msg(__("msg.no_token_left"))
        udpipe_offline = Msg(__("msg.udpipe_offline"))
        udpipe_offline_no_models = Msg(__("msg.udpipe_offline_no_model"))
        tokenizer_propagated = Msg(__("msg.tokenizer_propagated"))
        tokenizer_ignored = Msg(__("msg.tokenizer_ignored"))
        filtering_ignored = Msg(__("msg.filtering_ignored"))

    UserAdviceMessages = [
        Message(__("msg.preprocess_method").format({nltk_data_dir()}), "nltk_data")]
    ...
    def __init__(self):
        ...
        box = gui.vBox(self.controlArea, __("box_preview"))
        self.preview = ""
    ...
    def get_corpus_info(corpus: Corpus) -> str:
        return __("tooltip_info").format(len(corpus), sum(map(len, corpus.tokens)), len(corpus.dictionary))
