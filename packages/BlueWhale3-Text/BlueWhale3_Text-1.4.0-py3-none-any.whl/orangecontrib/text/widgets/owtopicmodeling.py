...
def __(key):
    return i18n.t('text.owtopicmodeling.' + key)
...
class LdaWidget(TopicWidget):
    ...
    parameters = (
        ('num_topics', __("row_num_of_topic"), 1, 500, 1, int),
    )
...
class HdpWidget(TopicWidget):
    ...
    spin_format = '{description}:'
    parameters = (
        ('gamma', __("gbox.first_level_concentration"), .1, 10, .5, float),
        ('alpha', __("gbox.second_level_concentration"), 1, 10, 1, int),
        ('eta', __("gbox.the_topic_direchlet"), 0.001, .5, .01, float),
        ('T', __("gbox.top_level_truncation"), 10, 150, 1, int),
        ('K', __("gbox.second_level_truncation"), 1, 50, 1, int),
        ('kappa', __("gbox.learn_rate"), .1, 10., .1, float),
        ('tau', __("gbox.slow_down_parameter"), 16., 256., 1., float),
    )
...
class OWTopicModeling(OWWidget):
    ...
    class Outputs:
        corpus = Output("Corpus", Table, label=i18n.t("text.common.corpus"))
        selected_topic = Output("Selected Topic", Topic, label=i18n.t("text.common.select_topic"))
        all_topics = Output("All Topics", Table, label=i18n.t("text.common.all_topic"))
    ...
    class Warning(OWWidget.Warning):
        less_topics_found = Msg(__("msg_less_topics_found"))
    ...
    def __init__(self):
        ...
        # Commit button
        gui.auto_commit(self.buttonsArea, self, 'autocommit', __("btn_commit"), box=False)
        ...
        for i, (method, attr_name) in enumerate(self.methods):
            widget = method(self, title=__("box_option"))
    ...
    def send_report(self):
        ...
        if self.corpus is not None:
            self.report_items(__("dialog.topic"), self.topic_desc.report())
...
class TopicViewer(QTreeWidget):
    ...
    columns = [__("label_topic"), __("label_topic_key")]
