...
def __(key):
    return i18n.t('text.owdocumentembedding.' + key)
...
class OWDocumentEmbedding(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        new_corpus = Output('Embeddings', Corpus, default=True, label=i18n.t('text.common.embeddings'))
        skipped = Output('Skipped documents', Corpus, label=i18n.t('text.common.skip_document'))

    class Error(OWWidget.Error):
        no_connection = Msg(__("msg_no_connection"))
        unexpected_error = Msg(__("msg_unexpected_error"))

    class Warning(OWWidget.Warning):
        unsuccessful_embeddings = Msg(__("msg_unsuccessful_embeddings"))
    ...
    def _setup_layout(self):
        ...
        self.aggregator_cb = comboBox(widget=widget_box,
                                      master=self,
                                      value='aggregator',
                                      label=__("row_aggregator"),
                                      orientation=Qt.Horizontal,
                                      # items=self.aggregators,
                                      items=[__('mean'), __('sum'), __('max'), __('min')],
                                      callback=self._option_changed)
    ...
    def set_input_corpus_summary(self, corpus):
        if corpus is None:
            ...
            self.info.set_input_summary(str(len(corpus)), __("tooltip_document")
                                        .format(len(corpus)))
