...
def __(key):
    return i18n.t('text.owduplicates.' + key)
...
class OWDuplicates(widget.OWWidget):
    ...
    class Inputs:
        distances = Input("Distances", DistMatrix, label=i18n.t("text.common.distance"))

    class Outputs:
        corpus_without_duplicates = Output("Corpus Without Duplicates", Corpus,
                                           label=i18n.t("text.common.corpus_without_duplicates"))
        duplicates = Output("Duplicates Cluster", Corpus, label=i18n.t("text.common.duplicates"))
        corpus = Output("Corpus", Corpus, label=i18n.t("text.common.corpus"))

    resizing_enabled = True

    class Error(OWWidget.Error):
        dist_matrix_invalid_shape = Msg(__("msg_dist_matrix_invalid_shape"))
        too_little_documents = Msg(__("msg_too_little_documents"))

    LINKAGE = [__("gbox.single"), __("gbox.average"), __("gbox.complete"), __("gbox.weight"), __("gbox.ward")]
    ...
    CLUSTER_ROLES = [__("gbox.attr"), __("gbox.class"), __("gbox.meta")]
    ...
    def __init__(self):
        ...
        self.n_unique = ''
        self.n_duplicates = ''
        info_box = gui.widgetBox(self.controlArea, box=__("box.info"))
        gui.label(info_box, self, __("row.document"))
        gui.label(info_box, self, __("row.unique"))
        gui.label(info_box, self, __("row.duplicate"))
        ...
        self.table_model.setHorizontalHeaderLabels([__("label_cluster"), __("label_size")])
        ...
        # Controls
        gui.comboBox(self.controlArea, self, 'linkage_method', items=self.LINKAGE, box=__("box.linkage"),
        ...
        self.threshold_spin = gui.doubleSpin(self.controlArea, self, 'threshold',
                                             ...
                                             label=__("row.distance_threshold"), box=__("box.distance"),
        ...
        # Output
        gui.comboBox(self.controlArea, self, "cluster_role", box=__("box.output"),
                     label=__("row.append_cluster_id"), callback=self.send_corpus,
    ...
    def recalculate_linkage(self):
        if self.distances is not None:
            self.linkage = dist_matrix_linkage(self.distances,
                                               ['Single', 'Average', 'Complete', 'Weighted', 'Ward'][self.linkage_method].lower())
    ...
    def send_report(self):
        self.report_items([
            (__("dialog.linkage"), self.LINKAGE[self.linkage_method]),
            (__("dialog.distance_threshold"), '{:.2f}'.format(self.threshold)),
            (__("dialog.document"), self.n_documents),
            (__("dialog.unique"), self.n_unique),
            (__("dialog.duplicates"), self.n_duplicates),
        ])
