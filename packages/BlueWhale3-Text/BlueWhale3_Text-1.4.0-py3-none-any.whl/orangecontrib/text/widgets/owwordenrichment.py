...
def __(key):
    return i18n.t('text.owwordenrichment.' + key)
...
class OWWordEnrichment(OWWidget, ConcurrentWidgetMixin):
    ...
    # Input/output
    class Inputs:
        selected_data = Input("Selected Data", Table, label=i18n.t("text.common.select_data"))
        data = Input("Data", Table, label=i18n.t("text.common.data"))

    want_main_area = True

    class Error(OWWidget.Error):
        no_bow_features = Msg(__("msg.no_bow_features"))
        no_words_overlap = Msg(__("msg.no_words_overlap"))
        empty_selection = Msg(__("msg.empty_selection"))
        all_selected = Msg(__("msg.all_selected"))
    ...
    def __init__(self):
        ...
        # info box
        fbox = gui.widgetBox(self.controlArea, __("box_info"))
        self.info_fil = gui.label(fbox, self, __("row_word_display"))

        # Filtering settings
        fbox = gui.widgetBox(self.controlArea, __("box_filter"))
        hbox = gui.widgetBox(fbox, orientation=0)

        self.chb_p = gui.checkBox(hbox, self, "filter_by_p", __("checkbox_p_value"),
                                  callback=self.filter_and_display,
                                  tooltip=__("tooltip_filter_p_value"))
        self.spin_p = gui.doubleSpin(hbox, self, 'filter_p_value',
                                     1e-4, 1, step=1e-4, labelWidth=15,
                                     callback=self.filter_and_display,
                                     tooltip=__("tooltip_max_p_value"))
        self.spin_p.setEnabled(self.filter_by_p)

        hbox = gui.widgetBox(fbox, orientation=0)
        self.chb_fdr = gui.checkBox(hbox, self, "filter_by_fdr", __("checkbox_fdr"),
                                    callback=self.filter_and_display,
                                    tooltip=__("tooltip_filter_fdr"))
        ...
        # Word's list view
        self.cols = [__("label_word"), __("label_p_value"), __("label_fdr")]
    ...
    def set_input_info(self) -> None:
        ...
        self.info.set_input_summary(
            ...
            __("msg.data_info").format(cluster_words, selected_words)
        )

    def set_displayed_info(self, count: int) -> None:

        self.info_fil.setText(__("data_word").format(count))
    ...
    def send_report(self):
        if self.results.words:
            self.report_table(__("dialog.enriche_word"), self.tree_to_table())
