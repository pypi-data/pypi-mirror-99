...
def __(key):
    return i18n.t('text.owtweetprofiler.' + key)
...
class OWTweetProfiler(OWWidget, ConcurrentWidgetMixin):
    ...
    class Error(OWWidget.Error):
        server_down = Msg(__("msg_server_down"))
    ...
    def __init__(self):
        ...
        buttons_layout.addWidget(
            gui.auto_commit(None, self, 'auto_commit', __("btn_commit"), box=False)
        )
    ...
    def generate_grid_layout(self):
        box = QGroupBox(title=__("box_option"))
        ...
        self.tweet_attr_combo = gui.comboBox(None, self, 'tweet_attr',
                                             callback=self.apply)
        layout.addWidget(QLabel(__("row_attr")))
    ...
    def send_report(self):
        self.report_items([
            (__("dialog.attr"), self.strings_attrs[self.tweet_attr]
            if len(self.strings_attrs) > self.tweet_attr else ''),
            (__("dialog.emotion"), self.model_name),
            (__("dialog.output"), self.output_mode),
        ])
