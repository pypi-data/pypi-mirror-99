...
def __(key):
    return i18n.t('text.owwordcloud.' + key)
...
class OWWordCloud(OWWidget, ConcurrentWidgetMixin):
    ...
    class Inputs:
        corpus = Input("Corpus", Corpus, default=True, label=i18n.t("text.common.corpus"))
        topic = Input("Topic", Topic, label=i18n.t("text.common.topic"))

    class Outputs:
        corpus = Output("Corpus", Corpus, label=i18n.t("text.common.corpus"))
        selected_words = Output("Selected Words", Topic, dynamic=False, label=i18n.t("text.common.select_word"))
        word_counts = Output("Word Counts", Table, label=i18n.t("text.common.word_count"))
    ...
    class Warning(widget.OWWidget.Warning):
        topic_precedence = widget.Msg(
            __("msg.topic_precedence")
        )

    class Info(widget.OWWidget.Information):
        bow_weights = widget.Msg(__("msg.bow_weight"))
...
<script src="resources/wordcloud-script.js"></script>
    ...
    def _create_layout(self):
        box = gui.widgetBox(self.controlArea, __("box_cloud_preference"))
        gui.checkBox(
            box,
            self,
            "words_color",
            __("checkbox_color_word"),
        ...
        gui.valueSlider(
            ...
            self,
            "words_tilt",
            label=__("row_words_tilt"),
            values=list(range(len(TILT_VALUES))),
            callback=self.on_cloud_pref_change,
            labelFormat=lambda x: TILT_VALUES[x],
        )

        box = gui.widgetBox(self.controlArea, __("box_word"))
        ...
        model.setHorizontalHeaderLabels([__("label_weight"), __("label_word")])
    ...
    def update_input_summary(self) -> None:
        if self.corpus is None and self.topic is None:
            ...
            if self.corpus is not None:
                input_string += (
                    __("msg.data_info").format(len(self.corpus), len(self.corpus_counter))
                )
                input_numbers += str(len(self.corpus_counter))
            if self.topic is not None:
                input_string += __("msg.word_in_topic").format(self.n_topic_words)
                input_numbers += (
    ...
    def update_output_summary(
        ...
        if (
            ...
            input_numbers = f"{cor_output_len or 0} | {n_selected or 0} | " \
                            f"{cc_len}"
            input_string = (
                __("msg.datas_info").format((cor_output_len or 0), (n_selected or 0), (cc_len))
            )
