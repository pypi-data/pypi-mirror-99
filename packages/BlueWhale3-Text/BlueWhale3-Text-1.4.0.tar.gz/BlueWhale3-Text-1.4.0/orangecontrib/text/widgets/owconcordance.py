...
def __(key):
    return i18n.t('text.owconcordance.' + key)
...
class OWConcordance(OWWidget):
    ...
    class Inputs:
        ...
        query_word = Input("Query Word", Topic, label=i18n.t("text.common.query_word"))

    class Outputs:
        selected_documents = Output("Selected Documents", Corpus, label=i18n.t("text.common.selected_document"))
        concordances = Output("Concordances", Corpus, label=i18n.t("text.common.concordance"))
    ...
    class Warning(OWWidget.Warning):
        multiple_words_on_input = Msg(__("msg_multiple_words_on_input"))
    ...
    def __init__(self):
        ...
        # Info attributes
        info_box = gui.widgetBox(self.controlArea, __("box_info"))
        ...
        gui.spin(self.controlArea, self, 'context_width', 3, 10, box=True,
                 label=__("row.num_of_word"), callback=self.set_width)
        ...
        self.input = gui.lineEdit(
            ...
            sizePolicy=QSizePolicy(QSizePolicy.MinimumExpanding,
                                   QSizePolicy.Fixed),
            label=__("row.query"), callback=self.set_word, callbackOnType=True)
        self.input.setFocus()
    ...
    def send_report(self):
        ...
        self.report_items(__("dialog.name"), (
            (__("dialog.label_query"), model.word),
            (__("dialog.label_token"), model.n_tokens),
            (__("dialog.label_type"), model.n_types),
            (__("dialog.label_match"), self.n_matching),
        ))
