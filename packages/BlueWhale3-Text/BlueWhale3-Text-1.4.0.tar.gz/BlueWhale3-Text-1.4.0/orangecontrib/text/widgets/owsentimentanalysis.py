...
def __(key):
    return i18n.t('text.owsentimentanalysis.' + key)
...
class OWSentimentAnalysis(OWWidget):
    ...
    LANG = [__("gbox_english"), __("gbox_slovenian")]
    MULTI_LANG = MultiSentiment.LANGS.keys()
    SENTI_LANG = SentiArt.LANGS.keys()
    DEFAULT_NONE = None

    class Warning(OWWidget.Warning):
        senti_offline = Msg(__("msg.senti_offline"))
        senti_offline_no_lang = Msg(__("msg.senti_offline_no_lang"))
        one_dict_only = Msg(__("msg.one_dict_only"))
        no_dicts_loaded = Msg(__("msg.no_dicts_loaded"))
    ...
    def __init__(self):
        ...
        self.method_box = box = gui.radioButtonsInBox(
            self.controlArea, self, "method_idx", [], box=__("box.method"),
            orientation=self.form, callback=self._method_changed)
        self.liu_hu = gui.appendRadioButton(box, __("btn.liu_hu"), addToLayout=False)
        self.liu_lang = gui.comboBox(None, self, 'liu_language',
                                     sendSelectedValue=True,
                                     contentsLength=10,
                                     items=self.LANG,
                                     callback=self._method_changed)
        self.vader = gui.appendRadioButton(box, __("btn.vader"), addToLayout=False)
        self.multi_sent = gui.appendRadioButton(box, __("box.mulitilingua_sent"),
                                                addToLayout=False)
        ...
        self.senti_box = gui.comboBox(None, self, 'senti_language',
                                      ...
                                      contentsLength=10, items=[''],
                                      callback=self._method_changed)
        self.custom_list = gui.appendRadioButton(box, __("box.custom_list"),
                                                 addToLayout=False)
        ...
        self.form.addWidget(QLabel(__("row.language")), 0, 1, Qt.AlignRight)
        ...
        self.form.addWidget(QLabel(__("row.language")), 2, 1, Qt.AlignRight)
        ...
        self.filegrid.addWidget(QLabel(__("row.positive")), 0, 0, Qt.AlignRight)
        ...
        self.filegrid.addWidget(QLabel(__("row.negative")), 1, 0, Qt.AlignRight)
        ...
        ac = gui.auto_commit(self.controlArea, self, 'autocommit', __("btn.commit"),
                             __("btn.auto_commit"))
    ...
    def send_report(self):
        self.report_items((
            (__("dialog.method"), self.METHODS[self.method_idx].name),
        ))
