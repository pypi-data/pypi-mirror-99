...
def __(key):
    return i18n.t('text.owcorpustonetwork.' + key)
...
class OWCorpusToNetwork(OWWidget, ConcurrentWidgetMixin):
    ...
    class Outputs:
        network = Output("Network", "orangecontrib.network.Network", label=i18n.t("text.common.network"))
        items = Output("Node Data", Table, label=i18n.t("text.common.node_data"))

    class Error(OWWidget.Error):
        unexpected_error = Msg(__("msg_unknown_error"))
        no_network_addon = Msg(__("msg_no_network_addon"))

    class Information(OWWidget.Information):
        params_changed = Msg(__("msg_params_changed"))
    ...
    def __init__(self):
        ...
        self.corpus = None
        self.node_types = [__("gbox_document"), __("gbox_word")]
    ...
    def _setup_layout(self):
        ...
        widget_box = widgetBox(self.controlArea, __("box_set"))

        self.node_type_cb = comboBox(
            widget=widget_box,
            master=self,
            value="node_type",
            label=__("row.node_type"),
    ...
    def set_data(self, data):
        ...
        self.button.setText(__("btn_stop"))
        if not data:
            self._corpus_to_network = None
        ...
        details = __("detail.corpus_document").format(len(self.corpus))
    ...
    def cancel(self):
        self._task_state = "waiting"
        self.button.setText(__("btn_start"))
        super().cancel()
    ...
    def on_done(self, result: Any) -> None:
        ...
        directed = __("detail.directed") if network.edges[0].directed else __("detail.undirected")
        details = __("detail.summary").format(
