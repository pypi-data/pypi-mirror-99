...
import ibmdbpy4nps
from ibmdbpy4nps import IdaDataBase
...
class Test_OpenDataFrameObject(object):
    ...
    def test_idadf_attr_loc(self, idadf):
        assert isinstance(idadf.loc, ibmdbpy4nps.indexing.Loc)

    def test_idadf_attr_internalstate(self, idadf):
        assert isinstance(idadf.internal_state, ibmdbpy4nps.internals.InternalState)
    ...
    def test_idadf_empty(self, idadb, df):
        ...
        to_test = ibmdbpy4nps.IdaDataFrame(idadb, "TEST_EMPTY_3496593727406047264076")
...
class Test_IdaDataFrameBehavior(object):
    def test_idadf_getitem_1_col_idadf(self, idadf):
        if len(idadf.columns) >= 1:
            ...
            assert(isinstance(newidadf, ibmdbpy4nps.IdaDataFrame) is True)
    ...
    def test_idaseries_getitem_slice(self, idadb, idadf, idadf_tmp):
        ...
        if len(idadf.columns) >= 1:
            ...
            assert(isinstance(idadf, ibmdbpy4nps.IdaDataFrame))
            assert(isinstance(idadf_tmp, ibmdbpy4nps.IdaSeries))
