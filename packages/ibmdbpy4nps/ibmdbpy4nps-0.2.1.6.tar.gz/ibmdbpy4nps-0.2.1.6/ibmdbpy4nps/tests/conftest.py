...
def get_data(request):
    ...
    if table == "titanic":
        from ibmdbpy4nps.sampledata.titanic import titanic
        return titanic
    elif table == "iris":
        from ibmdbpy4nps.sampledata.iris import iris
        return iris
    elif table == "swiss":
        from ibmdbpy4nps.sampledata.swiss import swiss
        return swiss
    else:
...
def idageodf_county(idadb):
    ...
    """
    idageodf = ibmdbpy4nps.IdaGeoDataFrame(idadb, 'SAMPLES.GEO_COUNTY', indexer='OBJECTID')
    return idageodf
...
def idageodf_customer(idadb):
    ...
    Don't use it for destructive nor non-destructive methods (modify columns)
    """
    idageodf = ibmdbpy4nps.IdaGeoDataFrame(idadb, 'SAMPLES.GEO_CUSTOMER', indexer='OBJECTID')
    return idageodf
...
def idageodf_tornado(idadb):
    ...
    Don't use it for destructive nor non-destructive methods (modify columns)
    """
    idageodf = ibmdbpy4nps.IdaGeoDataFrame(idadb, 'SAMPLES.GEO_TORNADO')
    return idageodf
...
def idaview(request, idadb, idadf):
    ...
    return ibmdbpy4nps.IdaDataFrame(idadb, "TEST_VIEW_ibmdbpy")
...
def idaview_tmp(request, idadb, idadf):
    ...
    return ibmdbpy4nps.IdaDataFrame(idadb, "TEST_VIEW_ibmdbpy_TMP")
...
def idageodf_county_view(request, idadb):
    ...
    idageodf = ibmdbpy4nps.IdaGeoDataFrame(idadb, 'GEO_COUNTY_VIEW', indexer='objectid')
    return idageodf
