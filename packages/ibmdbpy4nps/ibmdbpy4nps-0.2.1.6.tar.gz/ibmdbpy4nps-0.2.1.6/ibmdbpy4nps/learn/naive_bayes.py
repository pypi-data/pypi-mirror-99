...
import ibmdbpy4nps
from ibmdbpy4nps.exceptions import IdaNaiveBayesError
import six
...
class NaiveBayes(object):
    ...
    def __init__(self, modelname = None, disc = None, bins = None):
        ...
        self.modelname = ibmdbpy4nps.utils.check_modelname(modelname)
        self.disc = disc
        self.bins = bins
    ...
    def fit(self, idadf, target, column_id="ID", incolumn=None, coldeftype=None,
        ...
        if not (colpropertiestable is None or isinstance(colpropertiestable, ibmdbpy4nps.frame.IdaDataFrame)):
            colpropertiestable = ibmdbpy4nps.utils.check_tablename(colpropertiestable)
        ...
        if self.modelname is None:
            ...
            self.modelname = ibmdbpy4nps.utils.check_modelname(self.modelname)
    ...
    def predict(self, idadf, column_id=None, outtable=None, outtableProb=None,
        ...
        if outtable is None:
            ...
            outtable = ibmdbpy4nps.utils.check_tablename(outtable)
            if idadf._idadb.exists_table(outtable):
                idadf._idadb.drop_table(outtable)

        if outtableProb is not None:
            outtableProb = ibmdbpy4nps.utils.check_tablename(outtableProb)
        ...
        self.labels_ = ibmdbpy4nps.IdaDataFrame(idadf._idadb, self.outtable)
        return self.labels_
