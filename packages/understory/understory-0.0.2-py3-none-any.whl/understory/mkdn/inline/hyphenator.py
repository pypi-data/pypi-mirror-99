"""
hyphenate text using existing hyphenation dictionaries

Caches dict files and hyphenated words. Supports non-standard
...
class Hyphenator(object):
    ...
    def positions(self, word):
        ...
        See also HyphenationDictionary.positions. The points that are too
        far to the left or right are removed.
    ...
    def inserted(self, word, hyphen='-'):
        ...
        for p in reversed(self.positions(word)):
            if p.data:
                ...
                if word.isupper():
                    change = change.upper()
                letters[p+index:p+index+cut] = change.replace("=", hyphen)
            else:
                letters.insert(p, hyphen)
        return "".join(letters)
