...
"""
This module helps you convert an AST back to readable Python code.

The module is adapted from https://github.com/JelleZijlstra/ast_decompiler.
"""
...
def ast2code(ast_node, indent=4, line_length=100):
    ...
    ----------
    ast_node : ast.Node
        Code to decompile, using AST objects as generated by the standard library ast module.
    indent : int
        Indent level of lines.
    line_length : int
        If the line become longer than "line_length", this module will break them up
    ...
    code : str
        Python code string.
    """
    decompiler = Transformer(indentation=indent,
...
class Transformer(ast.NodeVisitor):
    ...
    def write_expression_list(self, nodes, separator=', ', allow_newlines=True, need_parens=True,
        ...
        """Write a list of nodes, separated by separator.
    ...
    def visit_IfExp(self, node):
        ...
        if isinstance(
            ...
            should_parenthesize = True
        elif isinstance(parent_node, ast.IfExp) and (node is parent_node.test or node is parent_node.body):
            should_parenthesize = True
        else:
            should_parenthesize = False
    ...
    def visit_GeneratorExp(self, node):
        ...
        if (isinstance(parent_node, _CallArgs) and len(parent_node.args) == 1 and node == parent_node.args[0]):
            start = end = ''
        else:
            start = '('
            end = ')'
    ...
    def visit_Await(self, node):
        with self.parenthesize_if(not isinstance(self.get_parent_node(), (ast.Expr, ast.Assign, ast.AugAssign))):
            self.write('await ')
            self.visit(node.value)
