...
def build_model(params):
    ...
    if params.encoder_only:
        # build
        model = TransformerModel(params, is_encoder=True, is_crossModal=params.is_cross_modal,with_output=True)
        ...
        if params.reload_model != '':
            ...
            #for alignment weight not in org weight
        ...
        # build
        encoder = TransformerModel(params, is_encoder=True, is_crossModal=params.is_cross_modal, with_output=True)  # TODO: only output when necessary - len(params.clm_steps + params.mlm_steps) > 0
        decoder = TransformerModel(params, is_encoder=False, is_crossModal=params.is_cross_modal, with_output=True)
            ...
            if enc_path != '':
                ...
                for k in encoder.state_dict().keys():
                    if k not in enc_reload:
                        enc_reload[k] = encoder.state_dict()[k]
            ...
            if dec_path != '':
                ...
                for k in decoder.state_dict().keys():
                    if k not in dec_reload:
                        dec_reload[k] = decoder.state_dict()[k]
