...
def _validate_bulk_prediction(
    ...
    if prediction_timestamp is not None:
        ...
        if isinstance(prediction_timestamp, pd.Series):
            if prediction_timestamp.shape[0] != expected_count:
                raise ValueError(
                    f"time_overwrite has {prediction_timestamp.shape[0]} elements, but must have same number of "
                    f"elements as prediction_ids: {expected_count}. "
                )
            # if time_overwrite is a Series, convert to list for canonical access
        elif isinstance(prediction_timestamp, list):
            if len(prediction_timestamp) != expected_count:
                raise ValueError(
                    f"time_overwrite has length {len(prediction_timestamp)} but must have same number of elements as "
                    f"prediction_ids: {expected_count}. "
                )
        else:
            raise TypeError(
                f"time_overwrite is type {type(prediction_timestamp)}, but expected one of: pd.Series, list<int>"
            )
...
class Client:
    ...
    def log(
        ...
        if prediction_timestamp is not None and not isinstance(prediction_timestamp, int):
            raise TypeError(
                f"time_overwrite {prediction_timestamp} is type {type(prediction_timestamp)} but expected int"
            )
        ...
        if actual_label is not None:
            ...
            )  # TODO: double check that this makes sense with score categorical
        ...
        if shap_values is not None and bool(shap_values):
            for k, v in shap_values.items():
                ...
                public__pb2.FeatureImportances(feature_importances=shap_values)
    ...
    def bulk_log(
        ...
        prediction_timestamp: Optional[Union[List[int], pd.Series]] = None,
    ) -> List[cf.Future]:
        ...
        if shap_values is not None:
            if not isinstance(shap_values, pd.DataFrame):
                raise TypeError(
                    f"feature_importances is type {type(shap_values)}, but expect type pd.DataFrame."
                )
            if shap_values.shape[0] != prediction_ids.shape[0]:
                raise ValueError(
                    f"feature_importances has {shap_values.shape[0]} sets of values, but must match size of "
                    f"predictions_ids: {shap_values.shape[0]}. "
                )
            if isinstance(shap_values.columns, pd.core.indexes.numeric.NumericIndex):
                raise TypeError(
                    f"features.columns is of type {type(shap_values.columns)}, but expect elements to be str."
                )
        ...
        for row, v in enumerate(prediction_ids):
            ...
            if prediction_labels is not None:
                ...
                if features is not None:
                    ...
                    p.MergeFrom(feats)
                if prediction_timestamp is not None:
                    p.timestamp.MergeFrom(get_timestamp(prediction_timestamp[row]))
