...
class BasicDataApi(metaclass=Singleton):
    ...
    def get_asset_price(self, asset_list: Tuple[str] = ()):
        # asset_list in [ 'asset-big_cap',
        #                 'asset-btc',
        #                 'asset-chdebt',
        #                 'asset-comdy',
        #                 'asset-dollar',
        ...
        try:
            ...
            if len(_asset_list_0) > 0:
                ...
                _df = raw.get_em_macroeconomic_daily(codes=real_ids,).pivot_table(index='datetime',columns='codes',values='value')
                result.append(_df)
            if len(_asset_list_1) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_1]
                _df = raw.get_btc(asset_ids=real_ids).pivot_table(index='date',columns='codes',values='close')
                result.append(_df)
            if len(_asset_list_2) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_2]
                _df = raw.get_em_future_price(future_ids=real_ids).pivot_table(index='datetime',columns='future_id',values='close')
                result.append(_df)
            if len(_asset_list_3) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_3]
                _df = raw.get_hf_fund_nav(fund_ids=real_ids).pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            if len(_asset_list_4) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_4]
                _df = raw.get_hf_index_price(index_ids=real_ids).pivot_table(index='index_date',columns='index_id',values='close')
                result.append(_df)
            if len(_asset_list_5) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_5]
                _df = self.get_index_price(index_list=real_ids).pivot_table(index='datetime',columns='index_id',values='close')
                result.append(_df)
    ...
    def asset_recent_rate(self, year:int=None):
        #如果year为空 返回年度收益
        #如果year为整数 返回当年月度收益 year从2010开始
        try:
            index_list = ['hs300','sse','gem','sme','sse50','csi500','csi800','szse_a']
            index_info = self.get_index_info(index_list=index_list)
            desc_name_dic = index_info.set_index('index_id').to_dict()['desc_name']
            index_price = self.get_index_price_dt(start_date='20091231',index_list=index_list).pivot_table(index='datetime',columns='index_id',values='close')
            if year is None:
                # 年度
    ...
    def get_main_index_future_diff_yearly(self, begin_date, end_date, time_para, codes=['EMM00597141','EMM00597142','EMM00597143']):
        ...
        df.loc[:,'exchange'] = df.index.map(lambda x: is_third_monday(x))
        _df = df[['exchange']].copy() * 1
        _df.loc[:,'adv'] = _df.cumsum().shift(1).fillna(0)['exchange']
        _max = _df.adv.max()
        _df1 = _df[_df.adv != _max].copy()
        l = list(_df1.groupby('adv').apply(lambda x: ((x.shape[0]-1) - np.array(x.reset_index().index)).tolist()))
        res = []
        for i in l:
            res.extend(i)
        _df1.loc[:,'year_date'] = res
        _df2 = _df[_df.adv == _max].copy()
        b_d = _df2.index[0]
        next_dt = 0
        for i in range(40):
            _d = b_d + datetime.timedelta(i)
            if is_third_monday(_d):
                next_dt = _d

        _df2.loc[:,'year_date'] = _df2.index.map(lambda x: (next_dt - x).days)
        _df = pd.concat([_df1, _df2])
        df = df.join(_df[['year_date']])
        df.loc[:,'EMM00597141'] = df['EMM00597141']*250/(df['year_date'] + 4)
        df.loc[:,'EMM00597142'] = df['EMM00597142']*250/(df['year_date'] + (30+4))
        df.loc[:,'EMM00597143'] = df['EMM00597143']*250/(df['year_date'] + (60+4))
        dic = {
            'EMM00597141':'IC主力年化贴水率',
            'EMM00597142':'IC次主力年化贴水率',
            'EMM00597143':'IC当季年化贴水率',
        }
        return df[codes].rename(columns=dic)