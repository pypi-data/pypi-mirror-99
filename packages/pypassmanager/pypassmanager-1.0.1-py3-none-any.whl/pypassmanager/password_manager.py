...
class PasswordManager:
    ...
    def _destroy_safe(self):
        confirmation = input("You are about to delete all the saved password including the master password-->choose [Y/n] ").strip().lower()
        ...
        if confirmation == 'y' or confirmation == 'yes':
            ...
            print(self._text.SUCCESS + "All password data destroyed. Restart the program to set up a new safe." + self._text.DEFAULT)
    ...
    def _display_options(self):
        ...
        print("copy <" + self._text.PARAM + "entry" + self._text.DEFAULT + ">       - Copy the password of an entry ")
        print("list               - Display all the entries stored in the safe.")
        print("help               - Display available options and their usage details.")
        print("destroy            - Delete all data including the master password.")
    ...
    def _auth_user(self):
        ...
        while True:
            pin = getpass(prompt = "Enter your master password: ")
            pin = bytes(pin, 'ascii')
            ...
            if bcrypt.checkpw(pin, hashed_pin):
                ...
                print(self._text.SUCCESS + "Success! Password Manager unlocked." + self._text.DEFAULT)
                return
            else:
                print(self._text.ERROR + "Incorrect password. Please try again." + self._text.DEFAULT)


    def _validate_pin(self, pin, pin_confirmation):
        # validate that input is not blank
        if pin == '':
            print(self._text.ERROR + "No password was entered. Please try again." + self._text.DEFAULT)
            return False 
        ...
        try:
            ...
            print(self._text.ERROR + "password was not a combination of digits 0-9. Please try again." + self._text.DEFAULT)
            return False 

        # validate that pin is 4-digits long
        if len(pin) != 4:
            print(self._text.ERROR + "password must be 4-digits long. Please try again." + self._text.DEFAULT)
            return False 

        # validate that pin was confirmed
        if pin != pin_confirmation:
            print(self._text.ERROR + "The password numbers entered did not match. Please try again." + self._text.DEFAULT)
            return False
    ...
    def _create_user(self):
        print(self._text.INFO + "Please create a master password and remember it" + self._text.DEFAULT)

        while True: 
            pin = getpass(prompt = "Enter a 4-digit password: ") 
            pin_confirmation = getpass(prompt = "Confirm your password: ")
            ...
            if self._validate_pin(pin, pin_confirmation):
                ...
                print(self._text.SUCCESS + "Success! Password Manager configured. Restart the program and enter your master password to begin." + self._text.DEFAULT)
    ...
    def _display_banner(self):
        print("\n===========================================================")
        print("\t" + self._text.BANNER + self._text.LOCK + "PYTHON PASSWORD MANAGER CLI" + self._text.DEFAULT)
        print("===========================================================\n")
