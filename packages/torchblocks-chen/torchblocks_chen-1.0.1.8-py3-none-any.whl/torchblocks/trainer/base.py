...
class BaseTrainer:
    ...
    def train_update(self, model, optimizer, loss, scheduler, n=1):
        '''
        Tranining update
        '''
        if self.args.fp16:
        ...
        self.global_step += 1
        self.records['loss_meter'].update(loss, n=n)
    ...
    def train(self, model, train_dataset, eval_dataset):
        ...
        if self.args.logging_steps < 0:
            self.args.logging_steps = math.ceil(len(train_dataloader)/self.args.gradient_accumulation_steps)
        if self.args.save_steps < 0:
            self.args.save_steps = math.ceil(len(train_dataloader)/self.args.gradient_accumulation_steps)
        for epoch in range(0, int(self.args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (step + 1)==len(train_dataloader):
                    self.train_update(model, optimizer, accumulation_loss, scheduler)
                    if self.args.do_ema:
                        ema.update(model)
                    pbar(step, {'loss': tr_loss/(step + 1)})
                    # 累积的loss清零
                    accumulation_loss = 0
