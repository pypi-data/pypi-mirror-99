    ...
    def attack(self, model, inputs, gradient_accumulation_steps=1):
        ...
        if self.adv_init_mag > 0:
            ...
            if self.adv_norm_type == "l2":
                delta = torch.zeros_like(embeds_init).uniform_(-1, 1) * input_mask.unsqueeze(-1)
...
class ALUM(object):
    ...
    def attack(self, model, inputs, gradient_accumulation_steps=1):
        ...
        if isinstance(model, torch.nn.DataParallel):
            ...
            embeds_init = getattr(model, self.base_model).embeddings.word_embeddings(input_ids)
        input_mask = inputs['attention_mask'].to(embeds_init)
        delta = torch.zeros_like(embeds_init).normal_(0, 1) * self.adv_var * input_mask.unsqueeze(-1)
