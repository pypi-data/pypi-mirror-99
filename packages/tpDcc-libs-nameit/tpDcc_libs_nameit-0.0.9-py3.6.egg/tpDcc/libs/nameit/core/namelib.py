...
class Token(Serializable, object):
    ...
    def solve(self, rule, name=None):
        ...
        if 'iterator' in items:
            ...
            solved_token = items.get(name)
            if not solved_token and name in list(items.values()):
                return name
    ...
    def _get_default(self):
        ...
        if items and python.is_number(self.default) and self.default >= 0:
            default_value = list(self.get_items().values())[self.default - 1]
...
class NameLib(object):
    ...
    def naming_file(self, naming_file_path):
        if naming_file_path == self._naming_file:
            return
    ...
    def load_session(self, repo=None):
        ...
        if self.has_valid_naming_file():
            LOGGER.debug('Loading session from Naming File: {}'.format(self._naming_file))
            ...
            LOGGER.debug('Loading session from directory files: {}'.format(repo))
