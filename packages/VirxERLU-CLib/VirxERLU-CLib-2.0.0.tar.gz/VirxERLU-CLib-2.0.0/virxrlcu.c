...
{
    return offset_target_z < hitbox_height * 1.5 && !airborne && T > 0; // && slope >= get_adjusted_min_slope(&T, &ground_shot_jump_time, &ground_shot_min_slope);
}
...
_Bool jump_shot_is_viable(double T, double gravity_z, double velocity_z, double hitbox_height, _Bool airborne, double offset_target_z, _Bool safe)
{
    ...
    if (!safe)
        jump_time *= 0.5;
    return T >= jump_time + 0.05; // && slope >= get_adjusted_min_slope(&T, &jump_time, &jump_shot_min_slope);
}
...
_Bool double_jump_shot_is_viable(double T, double gravity_z, double velocity_z, double half_hitbox_height, _Bool airborne, double offset_target_z, _Bool safe)
{
    ...
    if (!safe)
        jump_time *= 0.5;
    return T >= jump_time + 0.05; // && slope >= get_adjusted_min_slope(&T, &jump_time, &double_jump_shot_min_slope);
}
...
static PyObject *method_ground_shot_is_viable(PyObject *self, PyObject *args)
{
    ...
    const _Bool shot_viable = T_r != -1 && ground_shot_is_viable(T_r + 0.1, me.hitbox.height, me.airborne, offset_target_z - 10);
