...
class Rediscli(Bash):
    def get_parser(self):
        ...
        parser.add_option(
            "-p", action="store", type="int", dest="port", default=6379, help="Server port (default: 6379)."
        )
        parser.add_option(
            "-a", action="store", dest="password", default=None, help="Password to use when connecting to the server."
        )
        parser.add_option(
            "-r", action="store", type="int", dest="repeat", default=1, help="Execute specified command N times."
        )
        parser.add_option(
            "-i",
            action="store",
