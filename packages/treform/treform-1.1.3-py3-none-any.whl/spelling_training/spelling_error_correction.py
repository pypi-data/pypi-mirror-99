...
from treform.utility.base_util import is_server, is_my_pc, is_my_gpu_pc
from treform.utility.datafile_util import DataFileUtil
from treform.utility.dataset import DataSet
from treform.utility.hangul_util import HangulUtil
from treform.utility.num_util import NumUtil
from treform.utility.char_one_hot_vector import CharOneHotVector
from treform.utility.string_util import StringUtil
from treform.utility.watch_util import WatchUtil
...
if __name__ == '__main__':
    ...
    try:
        ...
        total_epoch = max(10, 1000 // window_size)  # 10 ~ 100 # window_size 가 클 수록 total_epoch는 작아도 됨.
        ...
        if not os.path.exists(model_file + '.index') or not os.path.exists(model_file + '.meta'):
            if n_train > int('100,000'.replace(',', '')):
                ...
                print('%s start (max_sentences=%s, window_size=%s, noise_rate=%.1f)' % (sys.argv[0], n_train, window_size, noise_rate))
            ...
            if n_train > int('100,000'.replace(',', '')):
                ...
                print('%s end (max_sentences=%s, window_size=%s, noise_rate=%.1f)' % (sys.argv[0], n_train, window_size, noise_rate))
        ...
        with gzip.open(sentences_file, 'rt', encoding='utf-8') as f:
            for i, line in enumerate(f, 1):
