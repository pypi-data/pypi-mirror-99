...
from treform.abbreviations.candidate import Candidate
...
class BaseAbbreviationExtraction:
    IN_TYPE = [str]
    OUT_TYPE = [tuple]

class SchwartzHearstAbbreviationExtraction(BaseAbbreviationExtraction):
    def __init__(self):
    ...
    def yield_lines_from_doc(self, doc_text):
        for line in doc_text.split("\n"):
            yield line.strip()


    def best_candidates(self, sentence):
        """
    ...
    def get_definition(self, candidate, sentence):
        """
    ...
    def select_definition(self, definition, abbrev):
        """
    ...
    def extract_abbreviation_definition_pairs(self,
                                              file_path=None,
                                              doc_text=None,
        ...
        written = 0
        if file_path:
            sentence_iterator = enumerate(self.yield_lines_from_file(file_path))
        elif doc_text:
            sentence_iterator = enumerate(self.yield_lines_from_doc(doc_text))
        else:
            return abbrev_map
        ...
        for i, sentence in sentence_iterator:
            ...
            try:
                for candidate in self.best_candidates(clean_sentence):
                    try:
                        definition = self.get_definition(candidate, clean_sentence)
                    except (ValueError, IndexError) as e:
                        self.log.debug("{} Omitting candidate {}. Reason: {}".format(i, candidate, e.args[0]))
                        omit += 1
                    else:
                        try:
                            definition = self.select_definition(definition, candidate)
                        except (ValueError, IndexError) as e:
    ...
    def __call__(self, *args, **kwargs):
        pairs = self.extract_abbreviation_definition_pairs(doc_text=args[0], most_common_definition=True)
