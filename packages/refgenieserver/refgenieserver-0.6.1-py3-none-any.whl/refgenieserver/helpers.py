...
def build_parser():
    ...
    for cmd, desc in list(msg_by_cmd.items()):
...
def preprocess_attrs(attrs):
    ...
    for new_key in list(CHANGED_KEYS.keys()):
...
def purge_nonservable(rgc):
    ...
    for genome_name, genome in list(rgc[CFG_GENOMES_KEY].items()):
        for asset_name, asset in list(genome[CFG_ASSETS_KEY].items()):
            try:
                for tag_name, tag in list(asset[CFG_ASSET_TAGS_KEY].items()):
