...
class UNet(rimneuralnet):
    ...
    def fit(self) -> bool:
        ...
        self.classifier.fit([self.X_train, wmap], ycat, batch_size=UNet.BATCH_SIZE, epochs=UNet.EPOCHS)
    ...
    def _process_training_data(self) -> bool:
        ...
        self.X_train = numpy.zeros((len(train_ids), self.train_image_size[0], self.train_image_size[1], self.train_image_size[2]), dtype=numpy.uint16)
    ...
    def predict(self, image: numpy.ndarray) -> (numpy.ndarray, numpy.ndarray):
        ...
        self.object_array = numpy.asarray(rimutils.extract_contours(sm.dilation(sm.label(sm.binary_erosion(testLabel, sm.square(UNet.EROSION_WIDTH)), connectivity=1), sm.square(UNet.EROSION_WIDTH))))
    ...
    def save(self, filename: str) -> bool:
        ...
        theclassifier = {
            ...
            'classifier': classifier.get_weights(),
