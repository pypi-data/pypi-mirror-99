...
class Model:
    ...
    def remerge(self, sizetables):
        ...
        for sizetable in sizetables:
            for range in self.mergedcells:
                incell = [range.split(':')[0],range.split(':')[1]]
                outcell = []
                for cell in incell:
                    cell_column = cell[0]
                ...
                self.variable(range).name = outcell[0]+':'+str(outcell[1]))
                self.variable(range).value = outcell[0]+':'+str(outcell[1]))
    return None
