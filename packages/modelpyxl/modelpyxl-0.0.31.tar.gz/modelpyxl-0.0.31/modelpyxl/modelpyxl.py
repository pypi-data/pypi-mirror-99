        ...
        for sizetable in sizetables:
            ...
            for style in self.styles:
                if style.row == sizetable[0]:
                    for n in range(sizetable[1]):
                        self.__styles.append(Style(style.name,style.row+n,style.column,style.data_type, style.font, style.alignment,style.fill, style.border))
                elif style.row > sizetable[0]:
                ...
                for table in data['table']:
                    ...
                    if variable.name in table:
                        if n == 0:
                            ...
                            self.__variables.append(Variable(variable.name,varible.row+n,variable.column,table[variable.name]))
        return None
