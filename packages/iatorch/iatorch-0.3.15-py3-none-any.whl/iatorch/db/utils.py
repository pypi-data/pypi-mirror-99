...
def migrate_experiments(root, subdir=None, migration=MIGRATIONS['experiments']):
    '''
    ...
    root : str
        project root path
    subdir : str
        dir where db and artifacts are saved
    migration : dict()
        use default, no need to change
    '''
    
    if subdir is not None:
        migration['subdir'] = subdir
    
    artifact_root = os.path.join(root, migration['subdir'])
