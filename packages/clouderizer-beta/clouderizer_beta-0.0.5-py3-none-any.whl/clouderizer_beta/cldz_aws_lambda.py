...
from clouderizer_beta.operator_this import create_aws_lambda_cloudformation_template_file
from clouderizer_beta.utils import call_sam_command,call_bash,cleanup_build_files
import shutil
import os
...
def lambda_deploy(base_dir,namespace,project_name,stackName, projectConfig, model_path,infra,pipPackages):
		...
		if projectConfig["subtype"] in ["h2o","pmml","dai"]:
			# shutil.copy(base_dir+'/dockerfiles/AutomlDockerfile',project_dir+"/Dockerfile")
			shutil.copy('dockerfiles/AutomlDockerfile',project_dir+"/Dockerfile")
		...
		if projectConfig["subtype"] in ["python","pythonscore","onnx","train"]:
			...
			shutil.copy('dockerfiles/PythonDockerfile',project_dir+"/Dockerfile")
