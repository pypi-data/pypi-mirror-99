...
DOCKERFILES_LOC = LOCAL_DIR+"/dockerfile/"
...
class Clouderizer(object):
	...
	def dlambda(self,*args,**kwargs):
		lambda_commands = ['deploy','delete']

		if len(args)==0:
			print("No argument given")
			return

		if args[0] not in lambda_commands:
			print("lambda command not supported, available commands : ",end = " ")
			for i in lambda_commands:
				print(i, end=" ")
			return
		...
		if args[0]=="deploy":
			...
			self.ldeploy(file_path,model_type)


	def ldeploy(self,file_path,model_type=None,notebook_type=None,requirements=None,predict=None,preprocess=None,postprocess=None,infra="standard"):
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					...
					if type(servingmodel)==dict and "id" in servingmodel:
						...
						lambda_deploy(LOCAL_DIR,"nb", projectName, projectName+"-stack", projectConfig, notebook_path,infra,pipPackages)
					else:
			...
			if type(servingproject)==dict and "id" in servingproject:		
				...
				if uploadmodel_resp=="Success":
					try:
						...
						if subtype in ["python","onnx","pythonscore"]:
							deployment_model_type="regular"
						servingmodel=create_serving_model(file_name,s3Path,servingproject,servingproject["id"],pipPackages,auth_token,"false",deployment_model_type,subtype,"standard",preprocess=preprocess_path,postprocess=postprocess_path,predict=predict_path)
					except Exception as e:
						print(e)
						print("Some exception occurred while preparing your model, please try again!")						
						return
					...
					lambda_deploy(LOCAL_DIR,"model", projectName, projectName+"-stack", projectConfig, model_path,infra,pipPackages)
				else:
