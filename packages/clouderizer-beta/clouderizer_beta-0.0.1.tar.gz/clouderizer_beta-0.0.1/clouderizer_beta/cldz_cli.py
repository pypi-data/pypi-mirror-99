...
def create_serving_project(filename,companyid,auth_token,projectName=None,retry=0):
	if not projectName:
...
def create_serving_model(file_name,s3Path,servingproject,servingprojectid,pipPackages,auth_token,training,model_type,subtype,preprocess=None,postprocess=None,predict=None,retry=0):
	...
	if(model_resp.status_code==200):
		...
		sleep(5*retry)
		return create_serving_model(file_name,s3Path,servingproject,servingprojectid,pipPackages,auth_token,training,model_type,subtype,preprocess,postprocess,predict,retry)
...
class Clouderizer(object):
	...
	def deploy(self,file_path=None,requirements=None,predict=None,preprocess=None,postprocess=None,model_type=None,notebook_type=None,infra="standard"):
	# def deploy(self,first_path=None,second_path=None,third_path=None,model_type=None,notebook_type=None,enable_gpu=None):
		...
		if execution_type=="notebook":
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					try:
						servingmodel=create_serving_model(file_name,s3Path,servingproject,servingproject["id"],pipPackages,auth_token,"true","regular",subtype)
					except Exception:
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					...
					try:
						...
						servingmodel=create_serving_model(file_name,s3Path,servingproject,servingproject["id"],pipPackages,auth_token,"false",deployment_model_type,subtype,preprocess=preprocess_path,postprocess=postprocess_path,predict=predict_path)
					except Exception:
	...
	def ldeploy(self,file_path,model_type):
		from cldz_aws_lambda import lambda_deploy
		from utils import check_aws_cli_exists, check_sam_cli_exists

		check_aws_cli_exists()

		check_sam_cli_exists()
		...
		servingproject=create_serving_project(file_path.split("/")[-1],creds['cid'],auth_token,projectName=projectName)
		# print(type(servingproject))
		model_path=file_path
		if type(servingproject)==dict and "id" in servingproject:		
			update_local_project_list(servingproject["name"],"add")
			...
			if uploadmodel_resp=="Success":
				...
				accessUrl = lambda_deploy(LOCAL_DIR,"lambda", name, name+"-stack", projectConfig, model_path)
