...
def weight_emfisher_vmat(y, xmat, zmat_lst, gmat_lst, init=None, maxiter=100, cc_par=1.0e-8, step=0.01):
    """
    :param y:
    :param xmat:
    :param zmat_lst:
    ...
    while iter < maxiter:
        ...
        fisher_mat = np.zeros((num_var, num_var))
        for i in range(len(pzgzmat_lst)):
            for j in range(i + 1):
                fisher_mat[i, j] = 0.5*np.sum(pzgzmat_lst[i] * pzgzmat_lst[j].T)
        ...
        wemfisher_mat = np.array(em_mat)
        for weight in weight_vec:
            wemfisher_mat = weight * em_mat + (1 - weight) * fisher_mat
            try:
                linalg.cholesky(wemfisher_mat)
                logging.info('EM weight value: ' + str(weight))
                break
            except Exception as e:
                del e
                continue
        delta = np.dot(linalg.inv(wemfisher_mat), fd_mat)
