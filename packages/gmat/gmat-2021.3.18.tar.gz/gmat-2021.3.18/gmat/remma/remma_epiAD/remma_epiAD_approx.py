...
from gmat.remma.remma_epiAD.remma_epiAD_eff import remma_epiAD_eff, remma_epiAD_eff_parallel


def remma_epiAD_approx(pheno_file, bed_file, gmat_lst, var_com, p_cut=1.0e-5, num_random_pair=100000, out_file='epiAD_approx'):
    """
    additive by dominance epistasis test by random SNP-BLUP model based on approximate test
    ...
    :param out_file: The prefix for output file. Default value is 'epiAD_approx'
    :return: 0
    """
    ...
    random_pairAD(num_snp, out_file=out_file + '.random_pair', num_pair=num_random_pair)
    remma_epiAD_pair(pheno_file, bed_file, gmat_lst, var_com, snp_pair_file=out_file + '.random_pair', p_cut=1,
    ...
    remma_epiAD_eff(pheno_file, bed_file, gmat_lst, var_com, var_app=var_median, p_cut=p_cut,
...
def remma_epiAD_approx_parallel(pheno_file, bed_file, gmat_lst, var_com, parallel, p_cut=1.0e-5, num_random_pair=100000, out_file='epiAD_approx'):
    """
    ...
    logging.info("\n\n#####Screen the epistatic effects and select top SNP pairs based on approximate test#####")
    remma_epiAD_eff_parallel(pheno_file, bed_file, gmat_lst, var_com, parallel=parallel, var_app=var_median, p_cut=p_cut,
                    out_file=out_file + '.approx_p')
    logging.info("\n\n#####Calculate exact p values for top SNP pairs#####")
