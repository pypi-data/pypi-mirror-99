...
def _remma_epiDD_maf_eff(y, xmat, zmat, gmat_lst, var_com, bed_file, snp_lst_0=None, freq=None, freq_deno=None, p_cut=1.0e-5, out_file='epiDD_maf_eff'):
    """
    ...
    lib.remma_epiDD_maf_eff_cpu(pbed_file, pnum_id, pnum_snp, psnp_lst_0, plen_snp_lst_0, ppymat, pfreq, peff_cut, pout_file)
...
def remma_epiDD_maf_eff(pheno_file, bed_file, gmat_lst, var_com, snp_lst_0=None, freq=None, freq_deno=None, p_cut=1.0e-5, out_file='epiDD_maf_eff'):
    """
    Estimate additive by additive epistasis effects by random SNP-BLUP model.
    ...
    res = _remma_epiDD_maf_eff(y, xmat, zmat, gmat_lst, var_com, bed_file,
                           snp_lst_0=snp_lst_0, freq=freq, freq_deno=freq_deno, p_cut=p_cut, out_file=out_file)
    return res


def _remma_epiDD_maf_eff_parallel(y, xmat, zmat, gmat_lst, var_com, bed_file, parallel,
                                  freq=None, freq_deno=None, p_cut=1.0e-5, out_file='epiDD_maf_eff_parallel'):
    """
    ...
    res = _remma_epiDD_maf_eff(y, xmat, zmat, gmat_lst, var_com, bed_file, snp_lst_0=snp_list_0, freq=freq,
                               freq_deno=freq_deno, p_cut=p_cut, out_file=out_file + '.' + str(parallel[1]))
    return res


def remma_epiDD_maf_eff_parallel(pheno_file, bed_file, gmat_lst, var_com, parallel,
                                   freq=None, freq_deno=None,  p_cut=1.0e-5, out_file='epiDD_maf_eff_parallel'):
    """
    Parallel version. Additive by additive epistasis test by random SNP-BLUP model.
    :param pheno_file: phenotypic file. The fist two columns are family id, individual id which are same as plink *.fam
    ...
    """
    y, xmat, zmat = design_matrix_wemai_multi_gmat(pheno_file, bed_file)
    res = _remma_epiDD_maf_eff_parallel(y, xmat, zmat, gmat_lst, var_com, bed_file, parallel,
