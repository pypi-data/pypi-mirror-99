...
def unbalance_condition_longwas_fixed(data_file, id, tpoint, trait, bed_file, agmat_file, var_com, condition_snp,
    ...
    :param condition_snp: conditional snp added to the time fixed effect.
    ...
    if tfix is not None:
        mat = []
        _, dmat_t, _, _ = _design_matrix(tfix, data_df)
    ...
    xmat = xmat_t
    if fix is not None:
        _, xmat_nt, _, _ = _design_matrix(fix, data_df)
    ...
    condition_snp_val = snp_on_disk[id_order_index, condition_snp_index].read().val
    ...
    snp_condition = list(map(lambda x, y: x * y, leg_lst, list(condition_snp_val[:, -1])))
    ...
    np.savetxt(prefix_outfile + '.tpoint', res_tpoint, header='\t'.join(head_tpoint), delimiter="\t", comments='')
