...
def _remma_epiAA_pair(y, xmat, zmat, gmat_lst, var_com, bed_file, snp_pair_file, max_test_pair=50000, p_cut=1.0e-4, out_file='epiAA_pair'):
    """
    ...
    :param out_file: output file. default value is 'epiAA_pair'.
    :return: 0
    """
    ...
    while True:
        ...
        res = pd.DataFrame({0: snp_pair[:, 0], 1: snp_pair[:, 1], 2: eff_vec[:, -1], 3: var_vec[:, -1], 4: chi_vec[:, -1], 5: p_vec[:, -1]})
        res = res[res[5] < p_cut]
...
def remma_epiAA_pair(pheno_file, bed_file, gmat_lst, var_com, snp_pair_file, max_test_pair=50000, p_cut=1.0e-4, out_file='epiAA_pair'):
    """
    Given a SNP pair file, perform additive by additive epistasis test by random SNP-BLUP model.
    ...
    res = _remma_epiAA_pair(y, xmat, zmat, gmat_lst, var_com, bed_file, snp_pair_file,
