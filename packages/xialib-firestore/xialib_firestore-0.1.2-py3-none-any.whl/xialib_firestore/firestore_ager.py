...
class FirestoreAger(Ager):
    ...
    def get_ready_key(self, key: str, start_key: int = 0, end_key: int = 0, ager_ref=None) -> int:
    ...
    def get_update_tasks(self, start_frame: int = 0, end_frame: int = 0):
        ...
        for key, frame_id in update_dict.copy().items():
            if not frame_id or ctrl_dict["tasks"].get(key, None) == frame_id:
                update_dict.pop(key)
        ctrl_dict["tasks"].update(update_dict)
