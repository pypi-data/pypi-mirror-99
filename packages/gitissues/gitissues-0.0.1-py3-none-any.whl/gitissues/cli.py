...
def reopen(repo, number):
    ...
    g = Github(token)
    repo = g.get_repo(repository)
    for issue in repo.get_issues(params={'state':'closed'}):
        if issue.number == number:
            issue.reopen_issue()
...
def comment(body, repo, number):
    ...
    g = Github(token)
    repo = g.get_repo(repository)
    for issue in repo.get_issues(params={'state':'all'}):
        if issue.number == number:
...
@click.option("-s", "--state", help = "state of the issue. can be one of open, closed or all",
                type=click.Choice(['open', 'closed', 'all'], case_sensitive=False), default=None)
...
def list(repo, state, author):
    ...
    repo = g.get_repo(repository)
    table = []
    for issue in repo.get_issues(params={'state':state, 'creator':author}):
