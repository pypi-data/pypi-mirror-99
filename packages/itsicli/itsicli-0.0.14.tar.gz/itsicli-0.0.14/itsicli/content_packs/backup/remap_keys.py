...
class KeysUpdater(object):
    ...
    def update_foreign_key(self, model, field_name, field):
        ...
        if field.key_regex:
            ...
            if not match:
                logging.warning('WARNING: Not able to find match for field_name={} using regex={}'.format(
        ...
        if not new_key or new_key == old_key:
            logging.warning('WARNING: Not able to find any new key to map to old key={} for model={} field_name={}'.format(
    ...
    def update_foreign_key_list_str(self, model, field_name, field):
        ...
        if not match:
            logging.warning('WARNING: Not able to find any foreign keys for field_name={} using regex={}'.format(
                field_name, key_regx
            ))
            return
