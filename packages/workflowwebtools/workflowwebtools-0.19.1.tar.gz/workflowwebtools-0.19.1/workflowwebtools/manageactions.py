...
def get_actions_summary():
    """ Return the number of actions per week per user
    ...
    for action_ in coll.find():
        timestamp = action_['timestamp']
        user = action_['parameters']['user']
        output[ len(output) ] = [timestamp , user]
