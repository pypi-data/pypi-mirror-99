...
def _confirm_valid_dict(dict_in: dict, version: int = None) -> bool:
    # TODO: drop fallback keys when customers' legacy files are converted    
    if not dict_in.get("sourceTable", dict_in.get("table")):
        print("Missing table")
        return False
    ...
    for f in dict_in.get("features"):
        if not f.get("columnName", f.get("name")) or not f.get("dataType", f.get("data_type")):
    ...
    for d in dict_in.get("dimensions"):
        if not d.get("columnName", d.get("name")) or not d.get("dataType", d.get("data_type")) or not d.get("granularity"):
