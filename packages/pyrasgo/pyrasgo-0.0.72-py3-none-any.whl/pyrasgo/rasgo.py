...
# ------------------    
    ...
    def publish_features_from_df(self, df: pd.DataFrame, dimensions: List[str], features: List[str],
        ...
        self.data_warehouse.grant_table_ownership(table=featureset_name, role=self.data_warehouse.reader_role)
    ...
    def publish_features_from_source(self, data_source_id: int,
        ...
        tags.append(data_source.sourceType)
    ...
    def publish_features_from_yml(self, yml_file: str, sandbox: Optional[bool] = True, 
        ...
        :return: description of the featureset created
        """
        # TODO: Enforce 1 fs per yml when customers' legacy files are converted
        with open(yml_file) as fobj:
            contents = yaml.load(fobj, Loader=yaml.SafeLoader)
        if isinstance(contents, list):
            for featureset in contents:
                feature_set = self._save_feature_set_dict(featureset)
        else:
    ...
    def publish_source_data(self, source_type: str, 
        ...
        if source_type.lower() in ["csv", "dataframe"]:
            ...
            self.data_warehouse.grant_table_ownership(table=table_name, role=self.data_warehouse.reader_role)
...
# -------------------------------
    ...
    def _save_feature_set_dict(self, feature_set_dict: dict):
        ...
        org_id = self._profile.get('organizationId', None)
        source_table = feature_set_dict.get("sourceTable", feature_set_dict.get("table"))
        if isinstance(feature_set_dict.get("datasource"), dict):
            ds_name = feature_set_dict["datasource"].get("name", source_table)
        elif isinstance(feature_set_dict.get("datasource"), str):
            ds_name = feature_set_dict["datasource"]
        else:
            ds_name = source_table
        ...
        for dim in feature_set_dict["dimensions"]:
            name = dim.get("columnName", dim.get("name"))
            display_name = dim.get("displayName")
            data_type = dim.get("dataType", dim.get("data_type"))
        ...
        for feature in feature_set_dict["features"]:
            name = feature.get("displayName", feature.get("display_name"))
            code = feature.get("columnName", feature.get("name"))
            data_type = feature.get("dataType", feature.get("data_type"))
            # if we get an enum, convert it to str so pydantic doesn't get mad
            if isinstance(data_type, api.v1.DataType):
                data_type = data_type.value
