    ...
    def __init__(self, api_client=None):
        ...
        def __nodes_create(
            ...
            **kwargs
            ...
            Receives a request with the `node` (as JSON) and possibly other information e.g. the `project` the node is associated with, the `app` it was  Returns the URL of the node.  # noqa: E501
        ...
        def __nodes_read(
            ...
            **kwargs
            ...
            Performs content negotiation and authorization based on the `Accept` header.  - For `application/json` returns 403 if the request user is not authorized to read the project.  - For `text/html` (and others) returns a HTML rendering of the of the node without authorization checking; this is to allow scrapers such as Google Docs link preview generator to be able to fetch basic information on the node. For private projects node URLs should be kept private to avoid unauthorized access.  # noqa: E501
            This method makes a synchronous HTTP request by default. To make an
            asynchronous HTTP request, please pass async_req=True
