# Modified for the purposes of illustrating NSDF file format
# Author of function save_nsdf is Chintaluri Chaitanya
# Originally from STEPS-2.2.0/examples/tutorial/diffusion_boundary.py
...
def save_nsdf(mesh, resX, resY):
    ...
    #attach DS
    tie_data_map(h['/data/static/mesh/tet'], h['/map/static/tet_idx'], 'source', axis=0)
    tie_data_map(h['/data/static/mesh/vertex'], h['/map/static/vertex_idx'], 'source', axis=0)

    h['/data/uniform/molecules/X'] = resX.T
    ...
    x.attrs.create('field', 'X') #ADD FIELD attrb
    x.attrs.create('dt', tpnts[1]-tpnts[0])
    x.attrs.create('tunit', 's')
    x.attrs.create('tstart', tpnts[0])
    x.attrs.create('unit', 'count')
    y.attrs.create('field', 'Y') #ADD FIELD attrb
    y.attrs.create('dt', tpnts[1]-tpnts[0])
    y.attrs.create('tunit', 's')
    y.attrs.create('tstart', tpnts[0])
    y.attrs.create('unit', 'count')
    h.close()
...
def plot_binned(t_idx, bin_n = 100):
    ...
    for c in range(bin_n): 
        ...
        if bin_vols[c] != 0: bin_concs[c]/=(bin_vols[c]*1.0e18)
