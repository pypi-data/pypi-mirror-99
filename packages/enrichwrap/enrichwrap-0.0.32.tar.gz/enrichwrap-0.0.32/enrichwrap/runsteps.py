...
def call_specific_enrichment(which_one):
    ...
    try:
        ...
        print('ERROR calling %s [%s]' % (which_one, err.args))
...
def call_enrichment(which_one, depth):
    ...
    if which_one in enrich_targets.keys():
        ...
        try:
            ...
            print("Json decode error on %s to %s: %s" % (r_dict['mechanism'], r_dict['url'], err))
        ...
        print('No such target for enrichment %s' % which_one)
...
def serial_enrich_with_failover(serial_steps, failover_steps, depth):
    ...
    try:
        ...
        print("Exception raised in serial steps %s, failed on %s. Calling failover steps %s" %
                      (serial_steps, on_which_step, failover_steps))
        try:
            for on_which_step in failover_steps:
                call_enrichment(on_which_step, depth + 1)
        except CustomCallException:
            print('Failed on fail-over calls - enrichment data will be lost %s' % on_which_step)
    ...
    if lower <= ran <= upper:
        ...
        if 'serialsteps' in a_step.keys():
            ...
            try:
                ...
                print('Failed on enrichment step %s\n\t%s' % (what, cce.message.args[0].reason))
        else:
