...
def titanic(refresh=False):
  ...
  if (not os.path.exists('titanic.csv')) or (refresh==True):
    ...
    titanic_request = requests.get("https://github.com/vkreat-tech/ctrl4ai/raw/master/sample_datasets/titanic.csv.gz", allow_redirects=True)
...
def trip_fare(refresh=False):
  ...
  if (not os.path.exists('trip_fare.csv')) or (refresh==True):
    ...
    trip_fare_request = requests.get("https://github.com/vkreat-tech/ctrl4ai/raw/master/sample_datasets/trip_fare.csv.gz", allow_redirects=True)
