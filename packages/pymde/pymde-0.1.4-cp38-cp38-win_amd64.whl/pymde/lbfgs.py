...
class LBFGS(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        callback (fn): called after first closure evaluation (loss, grad args)
        project_callback (fn): takes candidate iterate and returns a
           candidate
        use_cached_loss (bool): whether the use the loss/gradient computed
    ...
    def __init__(self,
        ...
        self._use_cached_loss = use_cached_loss
    ...
    def _directional_evaluate(self, closure, x, t, d, project_callback):
        self._add_grad(t, d)
        if project_callback is not None:
            with torch.no_grad():
                project_callback(self._params[0])
        loss = float(closure())
    ...
    def step(self, closure):
        ...
        if (self._use_cached_loss and state['n_iter'] > 0 and
                line_search_fn is not None):
            orig_loss = self._cached_loss
        else:
            orig_loss = closure()
        if self._callback is not None:
            self._callback(orig_loss, self._params[0].grad)
                ...
                if line_search_fn != "strong_wolfe":
                    ...
                    def obj_func(x, t, d):
                        return self._directional_evaluate(
                            closure, x, t, d, self._project_callback)

                    loss, flat_grad, t, ls_func_evals = _strong_wolfe(
                        obj_func, x_init, t, d, loss, flat_grad, gtd)
                self._cached_loss = torch.tensor(loss, device=flat_grad.device)
                self._add_grad(t, d)
                opt_cond = flat_grad.abs().max() <= tolerance_grad
            else:
