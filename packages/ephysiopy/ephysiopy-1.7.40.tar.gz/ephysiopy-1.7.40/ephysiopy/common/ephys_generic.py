...
class MapCalcsGeneric(object):
	...
	* speed vs rate plots
	...
	def makeSpikePathPlot(self, cluster, ax, **kwargs):
		...
		ax.plot(self.xy[0,idx], self.xy[1,idx],'s',ms=1, c=spk_colour,mec=spk_colour, **kwargs)
		plt.setp(ax.get_xticklabels(), visible=False)
		plt.setp(ax.get_yticklabels(), visible=False)
		ax.axes.get_xaxis().set_visible(False)
		ax.axes.get_yaxis().set_visible(False)
...
class FieldCalcs:
	...
	def getGridFieldMeasures(self, A, maxima='centroid', field_extent_method=2, allProps=True, **kwargs):
		"""
...
#            fieldsLabel = skimage.segmentation.random_walker(-A, peaksLabel)
		...
		# get orientation
		try:
			orientation = self.getGridOrientation(xyCoordPeaksCentral, closestPeaks)
		except:
			orientation = np.nan
		# calculate gridness
	...
	def getGridOrientation(self, peakCoords, closestPeakIdx):
		...
		peak_orientation : float
			The first value in an array of the angles of the peaks in the SAC working counter-clockwise
			from a line extending from the middle of the SAC to 3 o'clock.
		"""
	...
	def __findPeakExtent__(self, A, peakID, peakCoord):
		...
		aboveHalfHeightLabel = ndimage.label(A > halfHeight, structure=np.ones((3,3)))[0]
