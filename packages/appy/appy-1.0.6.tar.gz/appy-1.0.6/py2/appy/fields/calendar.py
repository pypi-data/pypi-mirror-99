...
from appy.gen import Field, Layouts, Show
    ...
    def getEventsInfoAt(self, res, calendar, date, eventNames, inTimeline,
        ...
        for event in events:
            ...
            if inTimeline:
                ...
                # else.
                bgColor = calendar.getColorFor(self.obj, eventType, preComputed)
                if bgColor:
...
class Legend:
    ...
    def getEntries(self, field, obj, allEventTypes, allEventNames, url, _,
        ...
        for eventType in allEventTypes:
            ...
            eventColor = field.getColorFor(obj, eventType, preComputed)
        ...
        if self.update: self.update(obj, res)
        return res
...
class Calendar(Field):
    ...
    pxViewTimeline = Px('''
     <table cellpadding="0" cellspacing="0" class="list timeline"
            ...
            var="monthsInfos=field.getTimelineMonths(grid, zobj, preComputed);
                 gradients=field.getGradients(obj)">
      ...
      <tbody>
       ...
       <x for="otherGroup in others">
        <tr for="other in otherGroup" id=":other.obj.id"
            var2="tlName=field.getTimelineName(obj, other, month, grid);
    ...
    def __init__(self, eventTypes=None, eventNameMethod=None,
      ...
      createEventLabel='which_event', style='calTable', strictMonths=False):
        ...
        Field.__init__(self, validator, (0,1), default, defaultOnEdit, show,
          page, group, layouts, move, False, True, None, False,
          ...
          label, None, None, None, None, True, False, view, cell, edit, xml,
          translations)
    ...
    def getTimelineName(self, obj, other, month, grid):
    ...
    def getTimelineMonths(self, grid, obj, preComputed):
        ...
        for date in grid:
            if not res:
                ...
                m = Object(month=date.aMonth(), colspan=1,
    ...
    def getGradients(self, obj):
        '''Gets the gradients possibly defined in addition to p_self.colors'''
        if callable(self.gradients): return self.gradients(obj)
    ...
    def mayEdit(self, obj, raiseError=False):
        '''May the user edit calendar events ?'''
        # Check the security-based condition
        if not obj.mayEdit(self.writePermission, raiseError=raiseError): return
        # Check the field-specific condition
        return self.getAttribute(obj, 'editable')

    def deleteEvent(self, obj, date, timeslot, handleEventSpan=True,
                    log=True, executeMethods=True):
        ...
        if executeMethods and self.beforeDelete:
            r = self.beforeDelete(appyObj, date, timeslot)
    ...
    def process(self, obj):
        ...
        # Security check
        self.mayEdit(obj, raiseError=True)
    ...
    def getCellStyle(self, obj, date, render, events):
        ...
        if not events: return
            ...
            if event.bgColor:
                if event.gradient:
                    r = event.gradient.getStyle(event.bgColor)
                else:
                    r = 'background-color:%s' % event.bgColor
                return r
