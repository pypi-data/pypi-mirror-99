...
class TestDecorators(TestCase):
    ...
    def test_log_on_start(self):
        dec = log_on_start(
            logging.INFO, "test message {arg1:d}, {arg2:d}", logger=self.logger
        )
        fn = dec(test_func)
        fn(1, 2)
    ...
    def test_log_on_end(self):
        ...
        fn = dec(test_func)
        fn(1, 2)
        self.assertIn("test message 1, 2 => 3", self.log_handler.messages["info"])
    ...
    def test_log_on_error(self):
        ...
        fn(1, 2)
        self.assertIn(
            "test message TestException('test exception',)",
    ...
    def test_log_exception(self):
        ...
        dec = log_exception(
            logging.ERROR, "test message", logger=self.logger, on_exceptions=TypeError
        )
        fn = dec(test_func)
        fn(2, "asd")
