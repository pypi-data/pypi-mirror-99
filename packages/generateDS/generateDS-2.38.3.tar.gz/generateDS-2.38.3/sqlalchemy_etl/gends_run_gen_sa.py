...
"""
Synopsis:
    Generate SQLAlchemy sa_models.py from XML schema.
Usage:
...
Options:
    ...
    --extra-gds-args
                    Extra arguments to be passed as command line
                    arguments to generateDS.py.  Example:
                        --extra-gds-args='["-c", "mycatalog.xml"]'
Examples:
    python gends_run_gen_sa.py my_schema.xsd
...
class GenerateSAError(Exception):
    pass
...
def generate(options, schema_file_name):
    ...
    args = [
        ...
        '--member-specs=list',
        #        '--root-element=institutionInfo',
    ]
    if options['extra_gds_args']:
        extra_gds_args = json.loads(options['extra_gds_args'])
        args.extend(extra_gds_args)
    args.append(schema_file_name)
    if not run_cmd(options, args):
        return
    args = (
        'python',
        os.path.join('.', 'gends_extract_simple_types.py'),
        '-f',
        schema_file_name,
    )
    if not run_cmd(options, args):
    ...
    if options['class_suffixes']:
        args = (
            'python',
            os.path.join('.', 'gends_generate_sqlalchemy.py'),
            '-f',
            bindings_file_stem,
        )
    else:
...
def run_cmd(options, args):
    ...
    args1 = ' '.join(args)
    process = Popen(args1, stderr=PIPE, stdout=PIPE, shell=True)
...
def main():
    ...
    for opt, val in opts:
        ...
        elif opt == '--extra-gds-args':
            options['extra_gds_args'] = val
