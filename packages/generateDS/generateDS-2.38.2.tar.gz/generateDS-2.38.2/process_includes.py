...
def collect_inserts(node, params, inserts, ns_dict, schema_ns_dict,
    ...
    for child in node.iterchildren():
        if child.tag in _IGNORE_TAGS or isinstance(child, etree._Comment): continue
...
def collect_inserts_aux(
    ...
    if string_content is not None:
        ...
        if child.tag == '{http://www.w3.org/2001/XMLSchema}include':
            assert root.attrib['targetNamespace'] == master_schema_root.attrib['targetNamespace']
        # uniquify
        ...
        for child1 in root:
            if child1.tag in _IGNORE_TAGS or isinstance(child1, etree._Comment): continue
            namespace = child1.nsmap[child1.prefix]
            _log.debug('Processing "%s", attribs "%s".', repr(child1), repr(child1.attrib))
            ...
            _log.debug('Node "%s", attribs: "%s"', repr(child2), repr(child2.attrib))
            # copy targetNamespace from right source
            if child.tag == '{%s}import' % namespace:
                child2.attrib['targetNamespace'] = root.attrib['targetNamespace']
                child2.attrib['mustOverrideTargetNamespace'] = 'true'
            elif child.tag == '{%s}include' % namespace:
                child2.attrib['targetNamespace'] = master_schema_root.attrib['targetNamespace']

            _log.debug('Saving node "%s", attribs: "%s"', repr(child2), repr(child2.attrib))
