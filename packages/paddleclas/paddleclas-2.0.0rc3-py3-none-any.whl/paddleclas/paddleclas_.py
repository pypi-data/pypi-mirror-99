...
def load_label_name_dict(path):
    if not os.path.exists(path):
        logger.warning("If want to use your own label_dict, please input legal path!\nOtherwise label_names will be empty!")
        return None
    else:
        result = {}
...
class PaddleClas(object):
    ...
    def postprocess(self, output):
        ...
        label_names = [self.label_name_dict[c] for c in class_ids] if self.label_name_dict else []
        return {"class_ids": class_ids, "scores": scores, "label_names": label_names}
    ...
    def predict(self, input_data):
        ...
        Args:
            input_data: input image for clas, support single image , internet url, folder path containing series of images
        Returns:
        ...
        if isinstance(input_data, np.ndarray):
            ...
            for idx, img_path in enumerate(image_list):
                ...
                if cnt % self.args.batch_size == 0 or (idx + 1) == len(image_list):
                    batch_outputs = self.predictor.predict(np.array(batch_input_list))
                    for number, output in enumerate(batch_outputs):
                        result = self.postprocess(output)
                        result["filename"] = img_path_list[number]
                        total_result.append(result)
                        if self.args.pre_label_image:
                            save_prelabel_results(result["class_ids"][0], img_path_list[number],
                                                self.args.pre_label_out_idr)
                    batch_input_list = []
                    img_path_list = []
            return total_result
        else:
            logger.error("Please input legal image! The type of image supported by PaddleClas are: NumPy.ndarray and string of local path or Ineternet URL")
            return []
