...
class InvalidSRNError(SymSDKError):
    ...
    def __init__(self, srn: str, message_extra: str = None):
        base_message = f"SRN {srn} is not valid."
        message = " ".join([base_message, message_extra]) if message_extra else base_message
...
class SRNTrailingSeparatorError(InvalidSRNError):
    """Raised when a SRN contains a trailing separator."""

    def __init__(self, srn: str):
        super(InvalidSRNError, self).__init__(
            message=f"SRN {srn} is not valid because it has a trailing separator.",
            hint="SRNs must match the following structure: <ORG>:<MODEL>:<SLUG>:<VERSION>[:<IDENTIFIER>], where org, model, slug, and version are required.",
            doc_url="https://docs.symops.com/docs/sym-concepts",
        )
