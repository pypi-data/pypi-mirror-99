...
class WeChatExternalContactGroupChat(BaseWeChatAPI):
    ...
    def statistic_group_by_day(
        ...
        :param day_end_time: 结束日期的时间戳，填当天的0时0分0秒（否则系统自动处理为当天的0分0秒）。取值范围：昨天至前180天。
                            如果不填，默认同 day_begin_time（即默认取一天的数据）
        :param owner_userid_list: 群主过滤，如果不填，表示获取全部群主的数据。最多100个
        :return: 返回的 JSON 数据包
        """
        if not day_end_time:
            day_end_time = day_begin_time
        data = optionaldict(day_begin_time=day_begin_time, day_end_time=day_end_time)
        if owner_userid_list:
            data["owner_filter"] = {"userid_list": owner_userid_list}
        return self._post("externalcontact/groupchat/statistic_group_by_day", data=data)
