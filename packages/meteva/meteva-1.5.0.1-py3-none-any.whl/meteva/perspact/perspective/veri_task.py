...
def group_sta(sta,para):
    sta_set = meteva.perspact.perspective.sta_data_set(sta)
    if para["dtime"] != "fold":
...
def verification_with_complite_para(para):
    ...
    #print(station)
    time0 = meteva.base.tool.time_tools.str_to_time(para["time_range"][0])
    end_time = meteva.base.tool.time_tools.str_to_time(para["time_range"][1])
    #print(time0)
    #print(end_time)
    ...
    while time1 < end_time:
        ...
        #读取观测
        path = meteva.base.tool.path_tools.get_path(para["observation"]["path"],time1)
        #print(path)
        ob_sta = meteva.base.io.read_stadata.read_from_micaps3(path,station=station,time = time1,dtime=[0,time_type],level=0,data_name="ob")

        if ob_sta is not None:
            ob_sta= meteva.base.function.get_from_sta_data.sta_between_value_range(ob_sta, para["observation"]["valid"][0],
        ...
        for i in range(fo_type_num):
            ...
            for dtime in dtime_read:
                path = meteva.base.tool.path_tools.get_path(one_fo_para["path"],time1,dtime)
                if os.path.exists(path):
                    if one_fo_para["type"] == "sta_data":
                        sta = meteva.base.io.read_stadata.read_from_micaps3(path, station,level=0,time = time1,dtime = [dtime,time_type],data_name=data_name)
                        if sta is not None:
                            sta = meteva.base.function.get_from_sta_data.sta_between_value_range(sta,one_fo_para["valid"][0],one_fo_para["valid"][1])
                    else:
                        grd = meteva.base.io.read_griddata.read_from_nc(path)
                        if grd is not None:
                            sta = meteva.base.function.gxy_sxy.interpolation_nearest(grd,station)
                            meteva.base.set_time_dtime_level_name(sta,level=0,time = time1,dtime = dtime,data_name = data_name)
                    if sta is not None:
                        if one_fo_para["copy"] is None:
                            fo_sta_all_dict[i] = meteva.base.function.put_into_sta_data.join(fo_sta_all_dict[i],sta)
                            ...
                            for m in range(len(one_fo_para["copy"]["结束时刻向未来移动"])):
                                if one_fo_para["copy"]["预报时效增加"]:
                                    ...
                                    time_fo_veri = time_ob - datetime.timedelta(hours=dtime_veri)
                                sta1 = copy.deepcopy(sta)
                                meteva.base.set_time_dtime_level_name(sta1,time = time_fo_veri,dtime = dtime_veri)
                                fo_sta_all_dict[i]=meteva.base.function.put_into_sta_data.join(fo_sta_all_dict[i],sta1)
            ...
            if all_has:
                ...
                for key in middle_veri.keys():
                    ...
                    if "para2" in mpara.keys():
                        para2 = mpara["para2"]
                    ver_set = meteva.perspact.perspective.veri_result_set(vmethod_list=mpara["method"], para1 = para1, para2= para2, sta_data_set=sta_set)
                    middle_result_part = meteva.perspact.perspective.get_middle_veri_result(sta_set, mpara)
