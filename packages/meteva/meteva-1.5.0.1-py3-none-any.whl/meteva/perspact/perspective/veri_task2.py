...
def veri_on_grid(para):
    ...
    grid_set = meteva.base.grid(para["grid"][0],para["grid"][1])
    masker = meteva.base.io.read_griddata.read_from_nc(para["masker"],grid=grid_set)
    ...
    while time1 <= fo_end_time:
        for dtime in dtime_list:
            ...
            for i in range(dim_type_num):
                ...
                path = meteva.base.tool.path_tools.get_path(dir, time1,dtime)
                if path in path_time_dict.keys():
                    path_time_dict[path]["num"] += 1
                else:
                    path_time_dict[path] = {}
    ...
    while time1 <= fo_end_time:
        ...
        for dtime in dtime_list:
            ...
            if path_time_dict[path]["grd"] is None:
                ob_grd = meteva.base.io.read_griddata.read_from_nc(path)
                path_time_dict[path]["num"] -= 1
                if path_time_dict[path]["num"] > 0:
            ...
            for key in middle_veri.keys():
                ...
                if "para2" in mpara.keys():
                    para2 = mpara["para2"]
                for i in range(fo_type_num):
                    middle_result_part = meteva.perspact.perspective.get_middle_veri_result(grd, mpara)
