    ...
    for i in range(fo_num):
        ...
        if sta is not None and len(sta.index) > 0:
            ...
            if len(fo) > 0 and fo[0] != 9999:
                #print(fo[fo>1])
                if vmethod == "hmfn":
                    result_one_model = np.array(list(meteva.method.yes_or_no.score.hmfn(ob,fo,para1)))
                else:
                    result_one_model = np.array(list(meteva.method.yes_or_no.score.hmfn_of_sunny_rainy(ob, fo)))
...
def get_middle_veri_result1(sta_data_set,para):
    result = []
    data_names = meteva.base.get_undim_data_names(sta_data_set.sta_data)
    ...
    for vmethod in para["method"]:
        if vmethod.lower() == "hmfn":
            ...
            for sta in sta_list:
                hmfn_list = meteva.nmc_vf_product.yes_or_no.hmfn(sta, para["para1"])
                result.append(hmfn_list)
        if vmethod.lower() == "abcd":
            vmethod_list.extend(["na","nb","nc","nd"])
            for sta in sta_list:
                abcd_list = meteva.nmc_vf_product.yes_or_no.abcd(sta)
