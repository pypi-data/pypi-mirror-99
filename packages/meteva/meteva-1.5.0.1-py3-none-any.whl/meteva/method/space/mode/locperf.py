...
def locperf(X, Y, which_stats=None, alpha=0.1, k=4, a=None):
    ...
    if "qdmapdiff" in which_stats:
        diffXY = sorted(abs(dX - dY), reverse=True)
        if "qdmapdiff" in which_stats:
            if dXcheck or dYcheck:
                if k >= 1:
    ...
    if "med" in which_stats:
        out['medMiss'] = float(np.mean(Z))
        out['medFalseAlarm'] = float(np.mean(Zother))
    if "msd" in which_stats:
        out['msdMiss'] = float(np.mean(Z2))
        out['msdFalseAlarm'] = float(np.mean(Zother2))
    if "fom" in which_stats:
        out['fom'] = sum(1 / (1 + alpha * Z2)) / N
    if "minsep" in which_stats:
        if Z.size != 0:
            out['minsep'] = float(np.min(Z))
        else:
            out['minsep'] = None
    return out
