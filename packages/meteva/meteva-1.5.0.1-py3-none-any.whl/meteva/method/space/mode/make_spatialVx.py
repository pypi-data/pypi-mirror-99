...
def make_spatialVx(grd_ob, grd_fo, loc, 
    ...
    if (xdim != ydim):
        try:
            ...
            print("make.SpatialVx: dim of ob{0}  must be the same as dim of fo{1} ".format(ob.shape,fo.shape))
    ...
    if (all(thresholds) == None):
        ...
        if (("ob"not in threshnames) or ("fo" not in threshnames)):
            try:
                ...
                print("make.SpatialVx: invalid thresholds argument.  List must have components named ob and fo.")
        ...
        if(odim[0] != fdim[0]):
            try:
                ...
                print("make.SpatialVx: invalid thresholds argument.  ob must have same number of thresholds (rows) as fo.")
        nth = odim[0]
    ...
    '''
    out = {"grd_ob": ob, "grd_fo": fo, "class":"SpatialVx", "xdim":xdim, "loc":loc,
