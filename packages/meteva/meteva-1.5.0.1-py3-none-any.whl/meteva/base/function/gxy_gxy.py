...
def interpolation_linear(grd, grid, other_info='left'):
    ...
    dat0 = grd.values
    dat = np.squeeze(dat0)
    grd0 = meteva.base.basicdata.get_grid_of_data(grd)
    if (grd0.dlon * grd0.nlon >= 360):
        grd1 = meteva.base.basicdata.grid([grd0.slon, grd0.elon + grd0.dlon,grd0.dlon],[grd0.slat, grd0.elat, grd0.dlat])
        ...
        grd1 = grd0
    if other_info=='left':
        grd2 = meteva.base.basicdata.grid(grid.glon,grid.glat,grd0.gtime,grd0.dtimes,grd0.levels,grd0.members)
    else:
        grd2 = grid.copy()
    ...
    grd_new = meteva.base.basicdata.grid_data(grd2,dat2)
    return grd_new
...
def add(grd1,grd2,other_info = 'left'):
    ...
    if(grd1 is None):
        ...
        grid1 = meteva.base.basicdata.get_grid_of_data(grd1)
        grid2 = meteva.base.basicdata.get_grid_of_data(grd2)
        slon = max(grid1.slon,grid2.slon)
        elon = min(grid1.elon,grid2.elon)
        slat = max(grid1.slat,grid2.slat)
        elat = min(grid1.elat,grid2.elat)
        if other_info == 'left':
            grid_in = meteva.base.basicdata.grid([slon,elon,grid1.dlon],[slat,elat,grid1.dlat],grid1.gtime,grid1.dtimes,grid1.levels,grid1.members)
        else:
            grid_in = meteva.base.basicdata.grid([slon, elon, grid2.dlon], [slat, elat, grid2.dlat], grid2.gtime, grid2.dtimes, grid2.levels,
                              grid2.members)
        grd1_in = interpolation_linear(grd1, grid_in)
        grd2_in = interpolation_linear(grd2, grid_in)
        grd = meteva.base.basicdata.grid_data(grid_in,grd1_in.values + grd2_in.values)
        return grd
...
def mean_convolve(grd, half_window_size, skip=1):
    ...
    grd_mean_skip = meteva.base.grid_data(grid_skip, dat)
    grd_mean = meteva.base.function.gxy_gxy.interpolation_linear(grd_mean_skip, grid0)
...
def smooth(grd,times = 1):
    ...
    grid0 = meteva.base.get_grid_of_data(grd)
    grd1 = meteva.base.grid_data(grid0,dat)
    return grd1