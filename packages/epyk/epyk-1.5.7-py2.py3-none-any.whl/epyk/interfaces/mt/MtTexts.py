...
class Text:
  ...
  def field(self, html_code=None):
    ...
    label = self.page.web.mt.composite(schema, options={"reset_class": True})
  ...
  def icon(self, value, html_code=None):
    """
    Description:
    ------------
  ...
  def line(self, label="", html_code=None):
    """
    Description:
    ------------
    Floating labels display the type of input a field requires.
    Every Text Field and Select should have a label, except for full-width text fields, which use the input’s
  ...
  def floating(self, label, html_code=None):
    """
    Description:
    ------------
    Floating labels display the type of input a field requires.
  ...
  def chip(self, text, choice=False, html_code=None):
    """
    Description:
    ------------
    Chips are compact elements that allow users to enter information, select a choice, filter content, or
    trigger an action.
    ...
    for t in text:
      schema['children'].append({"type": 'div', "class": "mdc-chip", 'css': False, 'attrs': {'role': 'row'},
                                 'children': [
        {"type": 'div', "class": "mdc-chip__ripple", 'css': False},
        {"type": 'icon', "class": "material-icons mdc-chip__icon mdc-chip__icon--leading", 'css': False,
         'args': {'text': 'event'}},
  ...
  def snackbar(self, text, btn_label='RETRY', btn_action=None, type='NORMAL', dismiss=True):
    ...
    """
    display_map = {'NORMAL': 'mdc-snackbar', 'STACKED': 'mdc-snackbar mdc-snackbar--stacked',
    ...
    if dismiss:
      schema = {"type": 'div', 'class': display_map.get(type.upper(), 'mdc-snackbar'), 'css': False, 'children': [
        {"type": 'div', "class": "mdc-snackbar__surface", 'css': False, 'children': [
          {'type': 'div', 'class': 'mdc-snackbar__label', 'css': False, 'attrs': {'role': 'status'},
           'arias': {'live': 'polite'}, 'args': {'components': text}},
          {'type': 'div', 'class': 'mdc-snackbar__actions', 'css': False, 'children': [
            {'type': 'button', 'class': 'mdc-button mdc-snackbar__action', 'css': False, 'attrs': {'type': 'button'},
             'children': [
              {'type': 'div', 'class': 'mdc-button__ripple', 'css': False},
              {'type': 'span', 'class': 'mdc-button__label', 'css': False, 'args': {'text': btn_label}}
            ]},
            {'type': 'button', 'class': 'mdc-icon-button mdc-snackbar__dismiss material-icons', 'css': False, 'attrs': {
              'title': 'Dismiss'}, 'args': {'text': 'close'}},
          ]},
        ]},
      ]}
    else:
    ...
    html_snackbar = self.page.web.mt.composite(schema, options={"reset_class": True})
