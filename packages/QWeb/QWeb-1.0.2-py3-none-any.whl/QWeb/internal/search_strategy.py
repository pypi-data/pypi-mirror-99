...
class SearchStrategies:
    ALL_INPUT_ELEMENTS = '//input[@type="text" or @type="email" or @type="password" or @type="tel"]|//textarea'

    MATCHING_INPUT_ELEMENT = '//*[(self::input or self::textarea) and (normalize-space(@placeholder)="{0}" or normalize-space(@value)="{0}")]'
    CONTAINING_INPUT_ELEMENT = '//*[(self::input or self::textarea) and (contains(normalize-space(@placeholder),"{0}") or contains(normalize-space(@value),"{0}"))]'

    ACTIVE_AREA_XPATH = '//body'
    ...
    CONTAINING_TEXT_MATCH_CASE_INSENSITIVE = '//*[not(self::script) and contains(translate(normalize-space(translate(., "\u00a0", " ")), "ABCDEFGHIJKLMNOPQRSTUVWXYZÄÖÅ", \
    ...
    @staticmethod
    def all_input_elements_validation(xpath):
        return SearchStrategies.clear_xpath(xpath)

    @staticmethod
    def matching_input_element_validation(xpath):
    ...
    @staticmethod
    def text_match_validation(xpath):
        return SearchStrategies.clear_xpath(xpath)

    @staticmethod
    def containing_text_match_validation(xpath):
        return SearchStrategies.clear_xpath(xpath)
    ...
    @staticmethod
    def search_direction_validation(direction):
    ...
    @staticmethod
    def default_timeout_validation(timeout):
        return timeout

    @staticmethod
    def xhr_timeout_validation(timeout):
        return timeout

    @staticmethod
    def verify_format_string(s, placeholder_num):
