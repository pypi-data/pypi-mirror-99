...
def getCurrTime(ops):
    assets=ops['assets']
    bars=se.getBars(assets[0],1,timeFrame=se.INTRADAY)
    return bars['time'][0]
...
def startOps(ops): 
    ...
    mem=ops['mem']
    for asset in assets:
        dbar=se.getBars(asset,mem,timeFrame=se.INTRADAY)
...
def executeOrders(orders,ops,dbars):
    ...
    for asset in assets:
        ...
        if order!=None:
            ...
            continue
    total_in_shares=se.getPosition()
    if ops['verbose']==True:
...
def getCurrBars(ops,dbars):
    ...
    for asset in assets:
        dbar=dbars[asset]
        #pega nova barra    
        aux=se.getBars(asset,1,timeFrame=se.INTRADAY) # pega uma barra!
