...
def getShares(bts,asset):
    return bts['shares_'+asset]

def getBalance(bts):
    return bts['capital']

def getAfforShares(bts,dbars,assetId,money=None,volumeStep=None):
    if money==None:
        money=bts['capital']
    bars=dbars[assetId]
    price=bars['close'].iloc[-1]
    return se.getAfforShares(assetId,money,price,volumeStep)
...
def startBckt(bts): 
    ...
    dbars=dict()
    for asset in assets:
        dbars[asset]=se.getBars(asset,bts['prestart'],bts['start'],bts['type'])
        bts['shares_'+asset]=0.0
    bars=se.getBars(assets[0],bts['start'],bts['end'],bts['type'])
...
def getCurrBars(bts,dbars):
    ...
    for asset in assets:
        dbar=dbars[asset]
        #pega nova barra    
        aux=se.getBars(asset,sim_dates[bts['curr']],1,bts['type']) # pega uma barra! daily or intraday
...
def evaluateEquitySerie(serie,threshold=0.5,riskFree=0.0):
  ...
  # print("Probability that Annual Return is greater than ({:.1f}%) ={:.2f}%".format(100*l2, 100*p2,l2))
