...
def processor_type_callback(ctx: Context, processor_type: str) -> str:
    ...
    table = AsciiTable([
        ['NODE', 'CPU', 'MEMORY', 'GPU'],
        *[[n.name, f'{n.cpu_allocatable}/{n.cpu_limits}', f'{n.memory_allocatable}/{n.memory_limits}',
           f'{n.gpu_product_name}/{n.gpu_allocatable}'] for n in nodes],
    ])
...
def gpu_type_callback(ctx: Context, gpu_type: str) -> str:
    ...
    print('Inside gpu_type_callback')
    cluster = ctx.store['cluster']
    processor_type = ctx.params['processor_type']
    print(f'cluster: {cluster}, processor type: {processor_type}')
    ...
    return inquirer.prompt([inquirer.Text(  # TODO: use find_from_inquirer
        'question',
        message="GPU type",
        default="Tesla-K80",
    )]).get('question')
