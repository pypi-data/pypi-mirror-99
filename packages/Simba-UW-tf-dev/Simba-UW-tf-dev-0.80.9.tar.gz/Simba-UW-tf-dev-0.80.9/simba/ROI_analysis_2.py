        ...
        if calculate_dist:
                ...
                for animal_counter, curr_animal in enumerate(curr_shape_dict):
                    ...
                    for shape in curr_animal_dict:
                        ...
                        for entry_frame, exit_frame in zip(entry_list, exit_list):
                            inside_shape_df = current_animal_df.loc[entry_frame:exit_frame]
                            inside_shape_df_list = [inside_shape_df[i:i+currFps] for i in range(0,inside_shape_df.shape[0],currFps)]
                            for s_inside_shape in inside_shape_df_list:
                                s_inside_shape = s_inside_shape.reset_index(drop=True)
                                start_locations = (s_inside_shape.at[0, animalBodypartList[animal_counter][0]], s_inside_shape.at[0, animalBodypartList[animal_counter][1]])
                                end_locations = (s_inside_shape.at[len(s_inside_shape)-1, animalBodypartList[animal_counter][0]], s_inside_shape.at[len(s_inside_shape)-1, animalBodypartList[animal_counter][1]])
                                movement_list_in_shape.append((np.sqrt((start_locations[0] - end_locations[0]) ** 2 + (start_locations[1] - end_locations[1]) ** 2)) / pix_per_mm)
            ...
            for animal in movement_in_ROIs_dict.keys():
                ...
                for shape in animal_dict.keys():
                    ...
                    out_list_headers.append(animal + '_' + shape + '_mm_movement_inside_shape')
