...
from ..persistence.sql.nt import nt_ver
from ..persistence.sql.usda import funcs as usda_funcs
from ..persistence.sql.usda import usda_ver
...
def test_1_usda_sqlfuncs():
    ...
    result = usda_funcs.sort_foods(789, fdgrp_ids=[100])
    assert len(result) == 1

    result = usda_funcs.sort_foods_by_kcal(789)
    assert len(result) == 246
    result = usda_funcs.sort_foods_by_kcal(789, fdgrp_ids=[1100])
