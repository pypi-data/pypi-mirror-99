...
@pytest.mark.parametrize(
    "input,output",
    [
        [time_to_datetime(time_091000), time_to_datetime(time_233400)],
        [time_to_datetime(time_060300), time_to_datetime(time_091000)],
        [time_to_datetime(time_233400), time_to_datetime(time_060300) + timedelta(days=1)],
        [get_datetime(23, 59, 59), time_to_datetime(time_060300) + timedelta(days=1)],
        [get_datetime(0, 0, 0), time_to_datetime(time_060300)],
        [get_datetime(0, 0, 1), time_to_datetime(time_060300)],
        [get_datetime(6, 2, 59), time_to_datetime(time_060300)],
        [get_datetime(9, 9, 59), time_to_datetime(time_091000)],
        [get_datetime(23, 33, 59), time_to_datetime(time_233400)],
        [get_datetime(23, 34, 1), time_to_datetime(time_060300) + timedelta(days=1)],
    ],
)
def test_get_next_datetime(input, output):
    schedule = multipletime(
        timezone=timezone.utc, times=[time_060300, time_091000, time_233400,]
    )
    assert schedule.get_next_datetime(input).isoformat() == output.isoformat()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            get_datetime(0, 0, 0) + timedelta(days=1),
            get_datetime(23, 34, 0),
            False,
            subtract_time(time_060300, time(0, 0, 0)).total_seconds(),
        ],
        [get_datetime(6, 2, 59) + timedelta(days=1), get_datetime(23, 34, 0), False, 1,],
        [
            get_datetime(6, 3, 0) + timedelta(days=1),
            get_datetime(23, 34, 0),
            True,
...
def test_special_case():
    ...
    rv = schedule.is_due(last_run_at)
    assert rv.is_due is True
    assert rv.next == timedelta(days=1).total_seconds() - 1


def test_special_case2():
    schedule = multipletime(timezone=timezone.utc, times=[time(0, 6, 30)])
    last_run_at = get_datetime(0, 6, 31)
    schedule.now = lambda: get_datetime(0, 6, 32)
    rv = schedule.is_due(last_run_at)
    assert rv.is_due is False
    assert rv.next == timedelta(days=1).total_seconds() - 2
