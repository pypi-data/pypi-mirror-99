...
def display_dict(d: dict,key_color=Đ_display_key_color,arrow_color=Đ_display_arrow_color,value_color=Đ_display_value_color,clip_width=False,post_processor=identity,key_sorter=sorted) -> None:
    ...
    print((((lambda x:clip_string_width(x,max_wraps_per_line=2,clipped_suffix='………')) if clip_width else identity)(post_processor('\n'.join((key_color(key) + arrow_color(" ⟶  ") + value_color(d[key])) for key in key_sorter(d.keys()))))))  # Theres a lot of code here because we're trying to make large amounts of text user-friendly in a terminal environment. Thats why this is so complicated and possibly perceived as messy
def display_list(l: list,key_color=Đ_display_key_color,arrow_color=Đ_display_arrow_color,value_color=Đ_display_value_color) -> None:
    # also works with tuples etc
    display_dict(list_to_index_dict(l),key_color,arrow_color,value_color)
# endregion
...
def print_stack_trace(error:BaseException,full_traceback: bool = True,header='r.print_stack_trace: ERROR: '):
    from traceback import format_exception,format_exception_only
    # ⁠⁠⁠⁠        ⎧                                                                                                                                                                                                ⎫
    # ⁠⁠⁠⁠        ⎪                                  ⎧                                                                                                                                                            ⎫⎪
    # ⁠⁠⁠⁠        ⎪                                  ⎪       ⎧                                                           ⎫                               ⎧                                            ⎫           ⎪⎪
    print(fansi(header,'red','bold') + fansi(''.join(format_exception(error.__class__,error,error.__traceback__)) if full_traceback else ''.join(format_exception_only(error.__class__,error))[:-1],'red'))
# ⁠⁠⁠⁠        ⎪                                  ⎪       ⎩                                                           ⎭                               ⎩                                            ⎭           ⎪⎪
# ⁠⁠⁠⁠        ⎪                                  ⎩                                                                                                                                                            ⎭⎪
# ⁠⁠⁠⁠        ⎩                                                                                                                                                                                                ⎭
        ...
        try:
            while True:
                try:
                    ...
                    try:
                        #region  Communicate with ptpython
                        def try_eval(x):
                            try:
                                ...
                                pass
                                # print(e)
                        rp.r_iterm_comm.rp_evaluator=lambda x:try_eval(x)
                        rp.r_iterm_comm.user_created_var_names=user_created_var_names
                        #endregion
