...
KEYBOARD SHORTCUTS:
	...
	⌥ D    equivalent to \da: clears the current buffer
	...
	SUMMARY:
		ALL COMMANDS:
			...
			PIPMORE                 <RP Settings>          ?.                       CDU
			                        SET STYLE              ?v                       CDH
			<Command History>                              ?s                       CDZ
			HISTORY    (HIST)       <Shell Commands>       ?t                       CDQ
			GHISTORY   (GHIST)      !                      ?h (?/)                  CAT
			AHISTORY   (AHIST)      !!                     ?e                       NCAT
			CHISTORY   (CHIST)                             ?p                       CCAT
			DHISTORY   (DHIST)      <Simple Timer>         ?c                       ACAT
			VHISTORY   (VHIST)      TICTOC                                          CATA
			ALLHISTORY (ALLHIST)    TICTOC ON              <Others>                 NCATA
			                        TICTOC OFF             RETURN  (RET)            CCATA
			<Clipboards>                                   SUSPEND (SUS)            ACATA
			COPY                    <Profiler>             GPU                      RUN
			PASTE                   PROF                   TOP                      RUNA
			SPASTE                  PROF ON                TAB                      PWD
			WCOPY                   PROF OFF               TABA                     CPWD
			WPASTE                                         MONITOR                  APWD
			TCOPY                   <Toggle Colors>        UPDATE                   TAKE
			TPASTE                  FANSI ON               ANS PRINT ON   (APON)    OPEN
			LCOPY                   FANSI OFF              ANS PRINT OFF  (APOF)    OPENH
			LPASTE                                         ANS PRINT FAST (APFA)    OPENA
			VCOPY                   <Module Reloading>     SHELL (SH)               DISK
			VPASTE                  RELOAD ON              LEVEL                    TREE
			FCOPY                   RELOAD OFF             DITTO                    TREE ALL
			FPASTE                                         EDIT                     TREE DIR
			MLPASTE                 <Documentation>        VARS                     TREE ALL DIR
			                        HELP                   RANT                     FD SEL (FDS)
			<'ans' History>         HHELP                  FORK                     LS SEL (LSS)
			NEXT                    SHORTCUTS              WANS                     LS REL (LSR)
			PREV                                           ARG                      LS FZF (LSZ)
			PREV ON                                        VIM                      LS QUE (LSQ)
			PREV OFF                                       VIMH                     RANGER (RNG)
			PREV CLEAR                                     VIMA
			PREV ALL                                       AVIMA
			                                               GC OFF
			                                               GC ON
