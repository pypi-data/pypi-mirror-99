...
from multiprocessing import synchronize, process

try:
    from multiprocessing.connection import PipeConnection
    from multiprocessing.reduction import duplicate, steal_handle, DupHandle
...
class NonDuplexQueue(object):
    ...
    #The original multiprocessing.Queues with locks can only be initialized to a process by inheritance
    ...
    def __setstate__(self, state):
        self.reader, self.writer = self.steal_pipe(state['pid'], state['read_handle'], state['write_handle'])
        self._buffer = collections.deque()
        self._thread = None        
    ...
    def put(self, data):
        #The _notempty lock can not be pickled and shared between processes (only by inheritance)!
        #So this function should be called after the this queue has been setup between the 2 processes
        
        if self._thread is None:
            self._notempty = threading.Condition(threading.Lock())
            self._thread = threading.Thread(target=self._feed, name='NonDuplexQueueFeed' ,daemon=True)                                 
            self._thread.start()
