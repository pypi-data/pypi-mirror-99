...
class AppService(Service):
    ...
    def initial(self):
        logger.error("Initial app service")
        self.load_access_key()
        self.set_auth()
        self.valid_auth()
        self.change_to_root_org()
        logger.error("Service http auth: {}".format(self.http.auth))

    def load_access_key(self):
        logger.error("Load access key")
    ...
    def set_auth(self):
        logger.error("Set app service auth: {}".format(self.access_key))
        self.auth = self.auth_class(self.access_key)

    def valid_auth(self):
        delay = 1
        while delay < 1000:
            logger.error('self {0}'.format(self.__dict__))
            user = self.get_profile()
            logger.error('user {0}'.format(user.__dict__))
            if not user:
    ...
    def register_and_save(self):
        logger.error('bootstrap token {0}'.format(self.config['BOOTSTRAP_TOKEN']))
        wt_name = '[' + self.config['NAME'] + ']' + socket.gethostbyname(socket.gethostname())
        terminal = self.register_terminal_v2(
            wt_name, self.config['BOOTSTRAP_TOKEN']
        )
        if not terminal:
