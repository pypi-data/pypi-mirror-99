...
from .models import Application, Asset, SystemUser, Domain, CommandFilterRule
...
class AssetsMixin:
    ...
    def get_system_user_auth_info(self, system_user, asset=None):
        ...
        if resp.status_code == 200:
            ...
            logger.error('Get system user {} password: {}*** key: {}***'.format(
    ...
    def get_application(self, application_id, application_type=None):
        """获取用户应用"""
        try:
            resp = self.http.get('application', pk=(application_id, application_type))
        except(RequestError, ResponseError):
            return None
        if resp.status_code == 200:
            applications = Application.from_json(resp.json())
            return applications
        else:
            return None
