...
@click.pass_context
def sifts_mapping(ctx, input, column, sep, func, kwargs, chunksize, entry_filter, chain_filter, skip_pdbs, omit, output, iteroutput):
    def get_unp_id(args):
...
@click.pass_context
def insert_sifts_meta(ctx, input, chunksize, api_suffix):
...
@click.option('--kwargs', type=str, default='{}')
@click.pass_context
def insert_pisa_range(ctx, input, chunksize, func, kwargs):
    ...
    for ids in df:
        ...
        res = ob.run().result()
        ob.tasks = [i.get_interface_res_dict() for interfaces in res for i in interfaces if i.info['css'] > 0 and not i.use_au]
        res = ob.run().result()
