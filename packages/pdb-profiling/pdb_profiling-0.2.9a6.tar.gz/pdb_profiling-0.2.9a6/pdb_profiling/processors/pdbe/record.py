...
class SIFTS(PDB):
    ...
    async def pipe_score_base(self, sifts_df, pec_df):
        ...
        grouped_len = full_df.groupby('UniProt').new_unp_range.apply(partial(fold, add_range)).apply(range_len)
    ...
    def get_id_score_for_assembly(args):
        ...
        return -sum(ord(i) for i in struct_asym_id) - asym_id_rank - assembly_id
    ...
    async def pisa_interact_protein_else(self, sele_df, include_chains, func:str, run_as_completed:bool=False, progress_bar=None, **kwargs):
        ...
        # mask = interact_df.apply(self.check_molecule_interface_na, axis=1)
        # interact_df = interact_df[mask]
        assert frozenset(interact_df.molecule_type_1) <= frozenset({'polypeptide(L)', 'polypeptide(D)'})
        interact_df = interact_df[interact_df.interface_range_1.notnull()]
        if len(interact_df) == 0:
            return
    ...
    @staticmethod
    @unsync
    def meta_annotation(df):
        if isinstance(df, Unfuture):
            df = df.result()
