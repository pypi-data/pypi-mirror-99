...
class PDB(Base):
    ...
    async def pipe_pdbekb_annotations(self, api_suffix, merge_residue_listing:bool=True, only_protein_res: bool = False, **kwargs):
        ...
        if merge_residue_listing:
            ...
            assert all(dfrm.aa_type.str.upper() == dfrm.residue_name)
...
class SIFTS(PDB):
    ...
    async def get_mapped_pdbekb_annotaions_task(cls, pdb_id, sub_sifts_df, api_suffix, **kwargs):
        ...
        for _, record in sub_sifts_df.iterrows():
            rets.append(await cls.get_mapped_pdbekb_annotaions_task_unit(pdb_ob, record, res_df))
        return rets

    @unsync
    async def get_mapped_pdbekb_annotaions(self, api_suffix, query_filter:str='bs_score > 0', **kwargs):
        sifts_df = (await self.pipe_score())[0]
