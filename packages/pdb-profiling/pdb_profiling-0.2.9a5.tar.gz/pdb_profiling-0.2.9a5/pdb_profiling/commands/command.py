...
@click.option('--sleep', type=bool, default=True)
@click.pass_context
...
@click.option('--iteroutput', type=bool, default=True)
@click.option('--sleep', type=bool, default=True)
@click.pass_context
...
@click.option('--sleep', type=bool, default=True)
@click.pass_context
def insert_sifts_meta(ctx, input, chunksize, api_suffix, sleep):
    custom_db = ctx.obj['custom_db']
    
    @unsync
    async def insert_meta(pdb, api_suffix):
        df = await pdb.fetch_from_pdbe_api(api_suffix, PDB.to_dataframe).then(SIFTS.meta_annotation)
        if df is not None:
    ...
    for ids in df:
        ...
        with Progress(*progress_bar_args) as p:
            pdbs.fetch(insert_meta, api_suffix=api_suffix).run(p.track).result()
        done += len(pdbs)
        console.log(f'Done: {done}')
        if sleep:
            sleep(uniform(0, 3))
...
@Interface.command('pisa-range')
@click.option('--input', type=click.Path())
@click.option('--chunksize', type=int, help="the chunksize parameter", default=20)
@click.option('--func', type=str)
@click.option('--kwargs', type=str, default='{}')
@click.option('--sleep', type=bool, default=True)
@click.pass_context
def insert_pisa_range(ctx, input, chunksize, func, kwargs, sleep):
    def expand_pisa_range(df, interact_type):
        resource_i = 'PISA_interface_{}'.format(interact_type)
        resource_s = 'PISA_surface_{}'.format(interact_type)
        for _, row in df.iterrows():
            if row.molecule_type_1 == 'polypeptide(L)':
                if isinstance(row.interface_range_1, float):
                    continue
                for start, end in row.interface_range_1:
                    yield dict(pdb_id=row.pdb_id, entity_id=row.entity_id_1, 
                            struct_asym_id=row.struct_asym_id_1, chain_id=row.chain_id_1,
                            resource=resource_i, resource_id=row.resource_id, 
                            pdb_start=start, pdb_end=end)
                for start, end in row.surface_range_1:
                    yield dict(pdb_id=row.pdb_id, entity_id=row.entity_id_1, 
                            struct_asym_id=row.struct_asym_id_1, chain_id=row.chain_id_1,
                            resource=resource_s, resource_id=row.resource_id, 
                            pdb_start=start, pdb_end=end)
            if row.molecule_type_2 == 'polypeptide(L)':
                if isinstance(row.interface_range_2, float):
                    continue
                for start, end in row.interface_range_2:
                    yield dict(pdb_id=row.pdb_id, entity_id=row.entity_id_2, 
                            struct_asym_id=row.struct_asym_id_2, chain_id=row.chain_id_2,
                            resource=resource_i, resource_id=row.resource_id, 
                            pdb_start=start, pdb_end=end)
                for start, end in row.surface_range_2:
                    yield dict(pdb_id=row.pdb_id, entity_id=row.entity_id_2, 
                            struct_asym_id=row.struct_asym_id_2, chain_id=row.chain_id_2,
                            resource=resource_s, resource_id=row.resource_id, 
                            pdb_start=start, pdb_end=end)
    ...
    pisa_func_map = {
        'pipe_protein_protein_interface': 'PPI',
        'pipe_protein_nucleotide_interface': 'PNI',
        'pipe_protein_ligand_interface': 'PLI'}
    interact_type = pisa_func_map[func]
    custom_db = ctx.obj['custom_db']
    kwargs = eval(kwargs)
    df = read_csv(input, header=None, chunksize=chunksize)
    done = 0
    for ids in df:
        ob = PDBs(ids[0].unique()).fetch('pipe_interface_res_dict', func=func, **kwargs)
        res = ob.run().result()
        ob.tasks = [i.get_interface_res_dict() for interfaces in res for i in interfaces if len(i.info['chains']) == 2 and not i.use_au]
        res = ob.run().result()
        interact_df = DataFrame(j for j in res if j is not None)
        if len(interact_df) > 0:
            PDB.sqlite_api.sync_insert(PDB.sqlite_api.PISAInterfaceDict, interact_df.to_dict('records'))
            interact_df['resource_id'] = interact_df.pdb_id+'/'+interact_df.assembly_id.astype(str)+'/'+interact_df.interface_id.astype(str)
            for col in ('interface_range_1', 'interface_range_2', 'surface_range_1', 'surface_range_2'):
                if col in interact_df.columns:
                    interact_df[col] = interact_df[col].apply(lambda x: json.loads(x) if isinstance(x, str) else x)
                else:
                    assert '_2' in col
            custom_db.sync_insert(custom_db.ResidueAnnotation, tuple(expand_pisa_range(interact_df, interact_type)))
        done += len(ob)
        console.log(f'Done: {done}')
        if sleep:
            sleep(uniform(0, 3))
