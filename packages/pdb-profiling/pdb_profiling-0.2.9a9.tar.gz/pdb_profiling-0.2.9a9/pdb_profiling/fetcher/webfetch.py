...
class UnsyncFetch(Abclog):
    ...
    * Parameters of `tenacity.retry` is built-in
    ...
    async def ftp_download(cls, semaphore, method: str, info: Dict, path, rate: float):
        url = furl(info['url'])
        cls.logger.debug(f"Start to download file: {url}")
        async with semaphore:
            async with aioftp_Client.context(url.host) as client:
                if await client.exists(str(url.path)):
                    async with aiofiles_open(path, 'wb') as file_out, client.download_stream(str(url.path)) as stream:
