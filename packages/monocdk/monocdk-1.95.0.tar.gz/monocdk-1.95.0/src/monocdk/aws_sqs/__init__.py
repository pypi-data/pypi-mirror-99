...
class IQueue(_IResource_8c1dbbbd, typing_extensions.Protocol):
    ...
    def __jsii_proxy_class__() -> typing.Type["_IQueueProxy"]:
        return _IQueueProxy
...
class QueueBase(
    ...
    @builtins.staticmethod
    def __jsii_proxy_class__() -> typing.Type["_QueueBaseProxy"]:
...
class _QueueBaseProxy(
    QueueBase, jsii.proxy_for(_Resource_abff4495) # type: ignore[misc]
):
    @builtins.property # type: ignore[misc]
    @jsii.member(jsii_name="autoCreatePolicy")
    def _auto_create_policy(self) -> builtins.bool:
...
class QueueProps:
    def __init__(
        ...
        *,
        ...
        :param removal_policy: (experimental) Policy to apply when the user pool is removed from the stack. Even though queues are technically stateful, their contents are transient and it is common to add and remove Queues while rearchitecting your application. The default is therefore ``DESTROY``. Change it to ``RETAIN`` if the messages are so valuable that accidentally losing them would be unacceptable. Default: RemovalPolicy.DESTROY
