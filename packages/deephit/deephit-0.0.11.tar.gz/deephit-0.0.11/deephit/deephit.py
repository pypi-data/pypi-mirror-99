...
class Model_DeepHit:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            self.LOSS_TOTAL = self.a*self.LOSS_1 + self.b*self.LOSS_2 + self.c*self.LOSS_3  ## diff from original
...
#x        sigma1 = tf.constant(0.65, dtype=tf.float32)
    ...
    def loss_Calibration(self):
        ...
        for e in range(self.num_Event):
            ...
            tmp_eta = tf.reduce_mean((r - I_2)**2, reduction_indices=1, keepdims=True)
...
def impt_import_dataset_SYNTHETIC(norm_mode='standard'):
    ...
    # orig df = pd_train_sml
    df = load_dffromCsv('pd_train_sml.csv')
...
def impt_import_dataset_xliu(norm_mode='standard'):
    ...
    #df = pd.read_csv(in_filename, sep=',')
    # orig df = pd_test_dummy
    df = load_dffromCsv('pd_test_dummy.csv')
...
def impt_import_dataset_lb(df_label, df_time, df_features,norm_mode='standard'):
    
    label           = np.asarray(df_label + 1)  # just for consistency but +1 might not be needed !!!lb!!! 
    time            = np.asarray(df_time)
    data            = np.asarray(df_features)
    data            = impt_f_get_Normalization(data, norm_mode)
    num_Category    = int(np.max(time) * 1.2)  #to have enough time-horizon  !!!! i think we should remove 1.2 !!!lb!!!
    num_Event       = int(len(np.unique(label))) #only count the number of events (do not count censoring as an event)
