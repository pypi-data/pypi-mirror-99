...
from mkrunner import utils, exceptions
from mkrunner.client import HttpSession
from mkrunner.exceptions import ValidationFailure, ParamsError
from mkrunner.ext.uploader import prepare_upload_step
from mkrunner.loader import load_project_meta, load_testcase_file
from mkrunner.parser import build_url, parse_data, parse_variables_mapping
from mkrunner.response import ResponseObject
from mkrunner.testcase import Config, Step
from mkrunner.utils import merge_variables
from mkrunner.models import (
    TConfig,
    TStep,
    VariablesMapping,
    StepData,
...
class mkrunner(object):
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        if USE_ALLURE:
                import json
                allure.attach(json.dumps(parsed_request_dict), "request", allure.attachment_type.JSON)
        ...
        try:
            ...
            if hasattr(self.__session, "data"):
                # mkrunner.client.HttpSession, not locust.clients.HttpSession
    ...
    def test_start(self, param: Dict = None) -> "HttpRunner":
        ...
        self.__log_path = self.__log_path or os.path.join(
            ...
            # 修改日志名称
            self.__project_meta.RootDir, "logs/run.log"
        )
        # 增加日志文件格式，最大50MB、保留10天
        log_handler = logger.add(self.__log_path, level="DEBUG", rotation="50 MB", retention="10 days")
