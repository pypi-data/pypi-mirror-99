...
from mkrunner import exceptions, __version__
from mkrunner.compat import (
    ensure_testcase_v3_api,
    ensure_testcase_v3,
    convert_variables,
    ensure_path_sep,
)
from mkrunner.loader import (
    load_folder_files,
    load_test_file,
    load_testcase,
    load_testsuite,
...
from mkrunner.response import uniform_validator
from mkrunner.utils import merge_variables, is_support_multiprocessing
...
{% if parameters %}
import pytest
from mkrunner import Parameters
{% endif %}

from mkrunner import mkrunner, Config, Step, RunRequest, RunTestCase
...
#zsw allure 场景
import allure
@allure.feature("{{ features_by_stories }}")
class {{ class_name }}(mkrunner):
...
def convert_testcase_path(testcase_abs_path: Text) -> Tuple[Text, Text]:
    ...
    for index, i in enumerate(testcase_abs_path.split("/")):
        if i in ["test_cases", "test_suites"]:
            pytest_dir_name = testcase_abs_path.split("/")[index + 1]
    ...
    #testcase_python_abs_path = os.path.dirname(dir_path) + f"/pytest_file/{file_name}_test.py"
    testcase_python_abs_path = project_meta.RootDir + "/自动化用例集/" + pytest_dir_name + "/" + f"{file_name}_test.py"

    # testcase_python_abs_path = project_meta.RootDir + "/pytest_file/" + f"{file_name}_test.py"
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    if dir_path:
        ...
        #)
        project_meta = load_project_meta(dir_path)
        file_name = os.path.basename(testcase_python_abs_path)
        #zsw testsuites
        testcase_python_abs_path = project_meta.RootDir + "/自动化用例集/" + pytest_dir_name + "/" + f"{file_name}"
        # testcase_python_abs_path = project_meta.RootDir + "/pytest_file/" + os.path.basename(dir_path) + f"/{file_name}"
    ...
    #zsw
    #将用例进行分类场景，默认取得testcase testsuite下一级目录
    features_by_stories = testcase_path.split("/")[1]

    data = {
        "features_by_stories": features_by_stories,
        "version": __version__,
...
def make_testsuite(testsuite: Dict) -> NoReturn:
    ...
    # 修改suite转换路径，末尾替换yml成bySuite
    testsuite_dir = f"{testsuite_dir}_bySuite"
