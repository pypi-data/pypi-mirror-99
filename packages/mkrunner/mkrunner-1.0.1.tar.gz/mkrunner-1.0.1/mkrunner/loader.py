...
from mkrunner import builtin, utils
from mkrunner import exceptions
from mkrunner.models import TestCase, ProjectMeta, TestSuite
    ...
    with open(dot_env_path, mode="rb") as fp:
        for line in fp:
            ...
            if b"=" in line:
                ...
                variable, value = line.split(b":", 1)
            # 支持#注释和空行-fengqi
            elif len(line)==1 or line.startswith(b"#"):
                continue
            else:
...
# 增加要导入的函数文件参数
def load_debugtalk_functions(module_name) -> Dict[Text, Callable]:
    ...
    try:
        imported_module = importlib.import_module("debugtalks." + module_name)
...
def load_project_meta(test_path: Text, reload: bool = False) -> ProjectMeta:
    ...
    for root, dirs, files in os.walk(env_path):
        if files:
            for f in files:
                if f.endswith(".env"):
                    dot_env_path = os.path.join(root, f)
                    dot_env = load_dot_env_file(dot_env_path)
                    if dot_env:
                        project_meta.env = dot_env
                        project_meta.dot_env_path = dot_env_path
    ...
    # 修改debugtalk，支持多个
    debugtalks_path = project_root_directory + "/debugtalks"
    debugtalk_functions = {}
    # 遍历目录下的文件
    for root, dirs, files in os.walk(debugtalks_path):
        if files:
            for f in files:
                if f.endswith(".py"):
                    debugtalk_function = load_debugtalk_functions(f[0:-3])
                    debugtalk_functions.update(debugtalk_function)
