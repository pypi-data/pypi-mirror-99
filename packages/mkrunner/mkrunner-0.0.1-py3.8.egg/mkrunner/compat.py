...
from mkrunner import exceptions
from mkrunner.loader import load_project_meta, convert_relative_project_root_dir, load_test_file
from mkrunner.parser import parse_data
from mkrunner.utils import sort_dict_by_custom_order
...
def ensure_testcase_v3(test_content: Dict) -> Dict:
    ...
    for step in test_content["teststeps"]:
        ...
        if "request" in step:
            ...
            # 添加api参数支持
            teststep["api"] = step.pop("api")
            # 解析api路径和参数
            ref_testcase_path = __ensure_absolute(teststep["api"])
            api_content = load_test_file(ref_testcase_path)

            # 以下参数如果存在，则传给用例步骤
            if "variables" in api_content.keys():
                if "variables" in step.keys():
                    for k in api_content["variables"].keys():
                        if k not in step["variables"].keys():
                            step["variables"][k] = api_content["variables"][k]
                else:
                    step["variables"] = api_content["variables"]
            if "request" in api_content.keys():
                teststep["request"] = api_content["request"]
            if "extract" in api_content.keys():
                teststep["extract"] = api_content["extract"]
            if "validate" in api_content.keys():
                teststep["validate"] = api_content["validate"]
...
from mkrunner.utils import get_platform, ExtendJSONEncoder
