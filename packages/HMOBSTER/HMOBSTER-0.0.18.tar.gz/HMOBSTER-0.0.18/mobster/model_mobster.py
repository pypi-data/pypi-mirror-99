...
def model(data, K=1, tail=1, purity=0.96, alpha_prior_concentration = 5, alpha_prior_rate = 10, number_of_trials_clonal_mean=500., number_of_trials_k=300.,
         prior_lims_clonal=[0.1, 100000.], prior_lims_k=[0.1, 100000.], alpha_precision_concentration = 5, alpha_precision_rate=0.05):
    ...
    for kr in pyro.plate("kr", len(karyos)):
        alpha_precision = pyro.sample('alpha_precision_{}'.format(kr), dist.Gamma(concentration=alpha_precision_concentration,
        ...
        alpha = torch.abs(alpha_teo + pyro.sample("alpha_noise_{}".format(kr), dist.Normal(torch.zeros_like(alpha_teo),1/alpha_precision)))
