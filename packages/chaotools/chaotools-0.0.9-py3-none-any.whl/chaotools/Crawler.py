...
class Crawler:
    ...
    def __init__(self,filename,sqlcreate,info,threadnum=1,timeout=20,sleeptime=1,maxtrynum=5,encoding=''):
        '''
        初始化爬虫配置
        :param url:
        :param filename:
    ...
    def crawlwithsemphore(self,i,maxcount,url,stopstatus):
        if self.semphore.acquire():
            ...
            if str(r.status_code)==stopstatus:
                self.stop=True#取消整个爬虫
                self.semphore.release()
                return
            if success:
                sourcecode = r.text
    ...
    def crawl(self,urls,stopstatus="404"):
        if not os.path.exists(f'{self.filename}.db'): self.createsqlite()
        threads=[]
        for i in range(len(urls)):
            t=threading.Thread(target=self.crawlwithsemphore,args=(i,len(urls),urls[i],stopstatus))
            threads.append(t)
