...
from azureml.automl.core.shared.exceptions import (  # noqa: E402
    AutoMLException, ClientException, ErrorTypes)
...
many_model_run_properties = {'many_models_run': True}  # type: Dict[str, Any]


def compose_logs(file_name: str, model: Model, start_time: datetime) -> List[Any]:
    logs = []  # type: List[Any]
    logs.append('AutoML')
...
def run(file_path: str, args: Arguments, automl_settings: Dict[str, Any], current_step_run: Run) -> List[str]:
    timestamp_column = automl_settings.get('time_column_name', None)
    partition_column_names = automl_settings.get('partition_column_names', [])
    ...
    logs = []  # type: List[Any]
    date1 = datetime.now()
    print('start (' + file_path + ') ' + str(datetime.now()))
    ...
    try:
        ...
        for column_name in partition_column_names:
