...
1. Run `pip install LSDapi` to install dependencies
2. Make sure packages requests and bs4 are installed

How to use it
-------------

1. from LSDapi import *
2. Use as follow:
...
#1. `get_price(ticker,num)`
#2. `get_evol(ticker, num)`
#3. `get_close(ticker, num)`
#4. `get_open(ticker, num)`
#5. `get_volume(ticker, num)`
#6. `get_avg_volume(ticker, num)`
#7. `get_beta(ticker, num)`
