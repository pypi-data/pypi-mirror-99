...
#        self.possible_keys = set( [ "raw_data", "split_colums", "restrict_by_val", "table", "split_columns", "sep"] )
    ...
    def do_set_table(self, c):
        ...
        if c not in list(self.current_param_db.table_columns.keys()):
            utils.newline_msg("ERR", "table '%s' not among the ones found in the DB: (%s)"%(c, ", ".join(list(self.current_param_db.table_columns.keys()))) )
            return
    ...
    def do_conf(self,c):
        ...
        print("  + tables found: %s "%(", ".join(list(self.current_param_db.table_columns.keys()))))
