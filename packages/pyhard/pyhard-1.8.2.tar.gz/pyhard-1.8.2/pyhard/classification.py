...
_overlap_params = {
    'svc_linear': {'probability': True, 'kernel': 'linear'},
    'svc_rbf': {'probability': True, 'kernel': 'rbf'},
    'mlp': {'solver': 'lbfgs'}
}
...
class Classifier(BaseLearner):
    ...
    def update_params(new_params=None):
        if new_params is None:
            ...
            new_params['svc_linear'] = {**new_params.get('svc_linear'), **_overlap_params['svc_linear']} if \
                new_params.get('svc_linear') is not None else _overlap_params['svc_linear']
    ...
    def run(self, algo, metric='logloss', n_folds=10, n_iter=10,
        ...
        for i in range(n_iter):
            ...
            for train_index, test_index in kf.split(self.X, self.y):
                ...
                fold_accuracy = clf.score(X_test, self.y[test_index])
