...
* https://plogs.readthedocs.io
* https://github.com/metachris/plogs
...
In order to also log to a file, just use `plogs.logfile(..)`:

    plogs.logfile("/tmp/test.log")

If you want to use specific loggers instead of the global default logger, use
`setup_logger(..)`:

    logger = plogs.setup_logger(logfile="/tmp/test.log")

The default loglevel is `logging.DEBUG`. You can set it with the
parameter `level`.

See the documentation for more information: https://plogs.readthedocs.io
"""
import functools
import os
import sys
...
# Formatter defaults
DEFAULT_FORMAT = '%(color)s[%(levelname)1.3s %(asctime)s %(module)s:%(funcName)s:%(lineno)d]%(end_color)s %(message)s'
DEFAULT_DATE_FORMAT = '%Y-%m-%d %H:%M:%S'
DEFAULT_COLORS = {
    logging.DEBUG: ForegroundColors.GREEN,
    logging.INFO: ForegroundColors.CYAN,
...
PLOGS_DEFAULT_LOGGER = "plogs_default"

# Attribute which all internal loggers carry
PLOGS_INTERNAL_LOGGER_ATTR = "_is_plogs_internal"

# Attribute signalling whether the handler has a custom loglevel
PLOGS_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL = "_is_plogs_internal_handler_custom_loglevel"
...
def setup_logger(name=__name__, logfile=log_file, level=logging.DEBUG, formatter=DEFAULT_DATE_FORMAT, maxBytes=10000000, backupCount=100, fileLoglevel=None, disableStderrLogger=False, isRootLogger=False, json=False, json_ensure_ascii=False):
    ...
    for handler in list(_logger.handlers):
        if hasattr(handler, PLOGS_INTERNAL_LOGGER_ATTR):
    ...
    if disableStderrLogger:
        ...
        setattr(stderr_stream_handler, PLOGS_INTERNAL_LOGGER_ATTR, True)
    ...
    if logfile:
        ...
        setattr(rotating_filehandler, PLOGS_INTERNAL_LOGGER_ATTR, True)
...
def _stderr_supports_color():
    ...
    if os.getenv('PLOGS_FORCE_COLOR') == '1':
        return True
...
def setup_default_logger(logfile=None, level=logging.DEBUG, formatter=None, maxBytes=0, backupCount=0, disableStderrLogger=False):
    """
    Deprecated. Use `plogs.loglevel(..)`, `plogs.logfile(..)`, etc.

    Globally reconfigures the default `plogs.logger` instance.

    Usage:

    .. code-block:: python

        from plogs import logger, setup_default_logger
    ...
    """
    global logger
    logger = setup_logger(name=PLOGS_DEFAULT_LOGGER, logfile=logfile, level=level, formatter=formatter, disableStderrLogger=disableStderrLogger)
    return logger
...
def reset_default_logger():
    ...
    # Resetup
    logger = setup_logger(name=PLOGS_DEFAULT_LOGGER, logfile=_logfile, level=_loglevel, formatter=_formatter)
...
def loglevel(level=logging.DEBUG, update_custom_handlers=False):
    """
    Set the minimum loglevel for the default logger (`plogs.logger`) and all handlers.
    ...
    for handler in list(logger.handlers):
        if hasattr(handler, PLOGS_INTERNAL_LOGGER_ATTR) or update_custom_handlers:
            # Don't update the loglevel if this handler uses a custom one
            if hasattr(handler, PLOGS_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL):
                continue
...
def formatter(formatter, update_custom_handlers=False):
    """
    Set the formatter for all handlers of the default logger (``plogs.logger``).

    This reconfigures only the plogs internal handlers by default, but you can also
...
def logfile(filename, formatter=None, mode='a', maxBytes=0, backupCount=0, encoding=None, loglevel=None, disableStderrLogger=True):
    """
    ...
    setattr(rotating_filehandler, PLOGS_INTERNAL_LOGGER_ATTR, True)
    if loglevel:
        setattr(rotating_filehandler, PLOGS_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL, True)
...
def syslog(logger_to_update=logger, facility=SysLogHandler.LOG_USER, disableStderrLogger=True):
    ...
    # Setup plogs to only use the syslog handler with the specified facility
    syslog_handler = SysLogHandler(facility=facility)
    setattr(syslog_handler, PLOGS_INTERNAL_LOGGER_ATTR, True)
