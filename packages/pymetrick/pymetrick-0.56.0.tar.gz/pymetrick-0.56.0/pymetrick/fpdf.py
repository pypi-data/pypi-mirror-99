...
class FPDF(object):
    ...
    def __call__(self, orientation='P',unit='mm', format='A4'):
        """Redefine the same object as if it were new"""
        try:
            # Scale factor
            if(unit=='pt'):
                self.k=1
...
#            self.error("Don\'t alter the locale before including class file");
    ...
    def _putinfo(self):
        self._out('/Producer '+self._textstring('PyFPDF '+FPDF_VERSION+' http://https://github.com/reingart/pyfpdf'))
    ...
    def code39(self, txt, x, y, w=1.5, h=5.0):
        ...
        #self.text(n,m+h+11/self.k,txt)
        self.text(x-(len(txt)*self.k),y+h+11/self.k,'*'+txt+'*')
