...
import contrast.extern.ruamel.yaml as ruamel.yaml

But this is invalid syntax. Instead, we split this into two imports.
- the first makes sure that ruamel is in the namespace
- the second ensures that ruamel.yaml will be loaded when we try to refer to it

The second import really isn't necessary in this particular case because there are
tons of other imports, which ensure that ruamel.yaml is loaded. However, to stay
as general as possible we're being explicit here.
"""
import contrast.extern.ruamel as ruamel
import contrast.extern.ruamel.yaml

from contrast.extern.ruamel.yaml.error import UnsafeLoaderWarning, YAMLError  # NOQA

from contrast.extern.ruamel.yaml.tokens import *  # NOQA
from contrast.extern.ruamel.yaml.events import *  # NOQA
from contrast.extern.ruamel.yaml.nodes import *  # NOQA

from contrast.extern.ruamel.yaml.loader import BaseLoader, SafeLoader, Loader, RoundTripLoader  # NOQA
from contrast.extern.ruamel.yaml.dumper import BaseDumper, SafeDumper, Dumper, RoundTripDumper  # NOQA
from contrast.extern.ruamel.yaml.compat import StringIO, BytesIO, with_metaclass, PY3, nprint
from contrast.extern.ruamel.yaml.resolver import VersionedResolver, Resolver  # NOQA
from contrast.extern.ruamel.yaml.representer import (
    BaseRepresenter,
    SafeRepresenter,
    Representer,
    RoundTripRepresenter,
)
from contrast.extern.ruamel.yaml.constructor import (
    BaseConstructor,
    SafeConstructor,
    Constructor,
    RoundTripConstructor,
)
from contrast.extern.ruamel.yaml.loader import Loader as UnsafeLoader

if False:  # MYPY
    from typing import List, Set, Dict, Union, Any, Callable, Optional, Text  # NOQA
    from contrast.extern.ruamel.yaml.compat import StreamType, StreamTextType, VersionType  # NOQA
...
class YAML(object):
    ...
    def map(self, **kw):
        # type: (Any) -> Any
        if 'rt' in self.typ:
            from contrast.extern.ruamel.yaml.comments import CommentedMap
    ...
    def seq(self, *args):
        # type: (Any) -> Any
        if 'rt' in self.typ:
            from contrast.extern.ruamel.yaml.comments import CommentedSeq
