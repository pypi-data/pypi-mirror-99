...
class TizenTVDriver(BaseDriver):
    pass

    http_client = None

    """
	Starts a new Tizen TV driver session.
    ...
    """
	Stops the Tizen TV driver session and releases all assets. Should be called as the last command of every session.
    ...
    """
	Initiates the Tizen TV Remote for sending remote control commands.
	
	:returns: TizenTVRemote
	"""

    def remote(self):
        return TizenTVRemote(self.http_client, super().get_session())
