...
from src.driver.lgwebos.lg_webos_remote import LGWebOSRemote
...
class LGWebOSDriver(BaseDriver):
    ...
    """
	Starts a new LG webOS driver session.
    ...
    def __init__(self, server_url, capabilities: DeviceCapabilities):
        ...
        session_json = self.__handler(
            self.http_client.post_to_server('session', cap_json))
        super().set_session(session_json)

    """
	Stops the LG webOS driver session and releases all assets. Should be called as the last command of every session.
    ...
    """
	Initiates the LG webOS Remote for sending remote control commands.
	
	:returns: LGWebOSRemote
	"""

    def remote(self):
        return LGWebOSRemote(self.http_client, super().get_session())
