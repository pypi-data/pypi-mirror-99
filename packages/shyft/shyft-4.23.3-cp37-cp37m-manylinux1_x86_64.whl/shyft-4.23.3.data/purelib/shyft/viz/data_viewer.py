...
def get_raster_extent_from_file(f_path:str)->Tuple[np.ndarray,List[float]]:
    """ Read, interpret and return raster as numpy.array, extent  from geotiff file
    @:param f_path file-path to the file to read
    @:return np.array,extent ....,[x0,x1,y0,y1]
    """
    # currently we are OK with import here, plan to move out when the
    #  build-envs are updated
    from PIL import Image  # use pillow to read geotiff, less deps than gdal
    with Image.open(f_path) as bg:
        t_pos = bg.tag[33922]  # ref geo-tiff, this is the x0,y0,z0 def
        scale = bg.tag[33550]  # ref geo-tiff, this is the scaling
        bb = bg.getbbox() # raster dimensions
        x0 = t_pos[3]
        y1 = t_pos[4]  # due to inverted y-axis, this is the lowest point on
        dx = scale[0]
        dy = scale[1]
        x1 = x0 + dx*(bb[2]-bb[0])
        y0 = y1 - dy*(bb[3]-bb[1])
        img_data=np.array(bg)
    return (img_data,[x0,x1,y0,y1])

def plot_background(ax, f_path):
    img_data,extent = get_raster_extent_from_file(f_path)
    ax.imshow(img_data, cmap='gist_earth', extent=extent)
