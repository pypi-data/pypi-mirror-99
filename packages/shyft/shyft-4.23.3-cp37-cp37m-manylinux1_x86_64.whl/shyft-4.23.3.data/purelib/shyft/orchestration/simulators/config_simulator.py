...
from shyft.time_series import (UtcPeriod, TimeSeries, IntVector)
...
class ConfigCalibrator(simulator.DefaultSimulator):
    ...
    def _create_target_specvect(self):
        ...
        for repo in self.target_repo:
            ...
            for ts_info in repo['1D_timeseries']:
                ...
                period = UtcPeriod(ts_info['start_datetime'],
                ...
                t.uid = ts_info['uid']
                t.catchment_indexes = IntVector(ts_info['catch_id'])
                ...
                t.ts = TimeSeries(tst.to_average(ts_info['start_datetime'], ts_info['run_time_step'], ts_info['number_of_steps'], tsp[ts_info['uid']]))
                tv.append(t)
        return tv
    ...
    def save_calibrated_model(self, optim_param, outfile=None):
        ...
        model_dict = yaml.load(open(model_file), Loader=yaml.FullLoader)
        model_params = {}
