...
class YamlStateRepository(StateRepository):
    ...
    def _load_state_from_yaml_file(self, filename):
        def yaml_load(f):
            if '5.0.0' < yaml.__version__ <= '5.1.2':
                return yaml.full_load(f)
            elif yaml.__version__ >= '5.2':
                return yaml.load(f,yaml.Loader)
            return yaml.load(f)

        with open(filename, "r") as f:
            contents = yaml_load(f.read())
            return contents
    ...
    def _unique_id(self, region_model_id, utc_timestamp):
        ...
        dt = Calendar().calendar_units(utc_timestamp)
    ...
    def _state_info_from_filename(self, filename):
        ...
        try:
            ...
            si.utc_timestamp = utc_calendar.time(int(ymd[0]), int(ymd[1]), int(ymd[2]),
