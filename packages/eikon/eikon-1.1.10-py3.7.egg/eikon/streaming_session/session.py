...
class Session(object):
    ...
    async def http_request_async(self, url: str, method=None, headers={},
        ...
        if _timeout is None:
            _timeout = httpx.Timeout(self._timeout)
        ...
        try:
            _request_response = await self._http_session.send(_http_request, **kwargs)
            ...
            self.log(1, f'TimeoutException on HTTP request: {t!r}')
            raise httpx.TimeoutException(f'TimeoutException on HTTP request: {t!r}', request=_http_request)
