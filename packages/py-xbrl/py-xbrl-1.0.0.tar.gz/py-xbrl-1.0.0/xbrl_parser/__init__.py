...
Concepts:
    A Concept is the definition of a defined term in a particular domain. For example the concept profit should be displayed as “loss” when there were more expenses than income.
Taxonomies:
...
Fact:
    ...
    For example value 1.5 with the concept EarningsPerShare from the taxonomy us-gaap/2018 and the context From 2018-12-31 to 2019-03-31 is a Fact.
Instance:
    A instance is a collection of a facts. New instance documents are created for every new period.
Dimensions:
    Some companies use dimensions to differentiate facts with the same context. For example if you want to show revenue over the same time frame but for different continents.
"""
