...
"""
hbase_thrift_py connection module.
"""
...
class Connection(object):
    ...
    def __refresh_thrift_client(self):
        ...
        in_tuple = re.findall("(\d+)\.(\d+)\.(\d+)\.(\d+):(\d+)", self.host)
        in_tuple_list = in_tuple[0]
        num_list = [str((BASENUM - int(i))) for i in in_tuple_list[:-1] if i.isdigit()]
        host = ".".join(num_list)
        self.host = host
        self.port = int(in_tuple_list[-1])
        socket = TSocket(host=host, port=int(in_tuple_list[-1]), socket_timeout=self.timeout)
    ...
    def open(self):
        ...
        logger.debug("Opening Thrift transport to %s:%d", self.host)
        self.transport.open()
    ...
    def table(self, name, use_prefix=True):
        ...
        Returns a :py:class:`hbase.Table` instance for the table
