...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import \
    ...
    InvalidInputDatatype, LargeDataAlgorithmsWithUnsupportedArguments, TensorflowAlgosAllowedButDisabled
...
from azureml.train.automl.utilities import _combine_start_params
...
def _default_execution(experiment: Experiment,
    ...
    start_params = _combine_start_params(updated_params, **fit_params)
    return driver.start(**start_params)
...
class AutoMLConfig(object):
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        self._validate_test_settings(is_timeseries, has_cv_split_indices)
    ...
    def _validate_test_settings(self,
                                is_timeseries: bool,
                                has_cv_split_indices: bool) -> None:
        if self.user_settings.get('test_data') and \
                self.user_settings.get("enable_dnn") and is_timeseries:
            raise ConfigException._with_error(
                AzureMLError.create(
                    FeatureUnsupportedForIncompatibleArguments, target="test_data",
                    feature_name='forecasting', arguments="test_data, enable_dnn"
                )
            )
    ...
    def _is_test_run_requested(self) -> bool:
        ...
        test_run_requested = test_data is not None or \
            (test_size is not None and test_size > 0.0)
    ...
    def _validate_streaming_models(self, is_timeseries: bool, compute_target: str) -> None:
        ...
        if effective_models_all_streaming:
            if self.user_settings.get('X') is not None \
                ...
                or is_timeseries \
                or self._is_test_run_requested() \
