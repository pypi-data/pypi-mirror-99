...
""" from https://unicode-table.com/en/blocks/
"""
...
def is_enclosed_alphanumerics(c: Char):
    return UnicodepointIndex.ENCLOSED_ALPHANUMERICS.value == index(c)

def is_small_form_variants(c: Char):
    return UnicodepointIndex.SMALL_FORM_VARIANTS.value == index(c)

def is_halfwidth_and_fullwidth_forms(c: Char):
    return UnicodepointIndex.HALFWIDTH_AND_FULLWIDTH_FORMS.value == index(c)
