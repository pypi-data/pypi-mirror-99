...
class Serfile():
    ...
    .readFrameAtPos(n:int) : return frame numner n.
    ...
    .setCurrentPosition(n:int) : set the cursor at position n. Return 0 if OK, -1 if something's wrong.
    ...
    def __init__(self, name_of_serfile, NEW=False):
    ...
    def readFrameAtPos(self,n):
        ...
        with open(self._nameOfSerfile, 'rb') as file:
            frame = np.array([])
            offset = 178+self._cursor*self._frameDimension*self._bytesPerPixels
            file.seek(offset)
...
if __name__ == "__main__":
    ...
    serfile_ = Serfile("try_ser.ser", True)
    #test purpose####
