...
class attributes:
    ...
    def method_ticks(self):
        ...
        if self.fine_tick_locations is True:
            if not isinstance(self.x, type(None)) and isinstance(self.custom_x_tick_locations, type(None)):
                self.custom_x_tick_locations = [self.x.min(), self.x.max()]
            if not isinstance(self.y, type(None)) and isinstance(self.custom_y_tick_locations, type(None)):
