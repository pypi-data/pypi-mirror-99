...
class TonicAuthentication():
    ...
    def get_authorization_header(self, request):
        ...
        """
        logger.info(f"Request META items: {request.META.items()}")
        auth = request.META.get('HTTP_AUTHORIZATION', b'')
        if isinstance(auth, str):
            # Work around django test client oddness
            auth = auth.encode(HTTP_HEADER_ENCODING)
        logger.info(f"AUTH HEADER for this request is {auth}.")
        return auth
    ...
    def authenticate(self, request):
        ...
        if settings_auth["ALLOW_ANONYMOUS_REQUESTS"] and not authorization_header:
            logger.info(f"No AUTH HEADER found for request {request}. ANONYMOUS MODE ACTIVE.")
            return (None, None)
        """
