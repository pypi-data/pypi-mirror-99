...
class FormDataRoutingRedirect(AssertionError):
    """This exception is raised by Rakish in debug mode if it detects a
    ...
    def __init__(self, request):
        ...
        if f"{request.base_url}/" == exc.new_url.split("?")[0]:
            buf.append(
                "  The URL was defined with a trailing slash so Rakish"
...
def attach_enctype_error_multidict(request):
    """Since Rakish 0.8 we're monkeypatching the files object in case a
...
def explain_template_loading_attempts(app, template, attempts):
    ...
    if blueprint is not None and seems_fishy:
        ...
        info.append("  See https://rakish.palletsprojects.com/blueprints/#templates")
    ...
    if os.environ.get("WERKZEUG_RUN_MAIN") != "true":
        warn(
            Warning(
                ...
                " run from the rakish command line executable. Consider"
