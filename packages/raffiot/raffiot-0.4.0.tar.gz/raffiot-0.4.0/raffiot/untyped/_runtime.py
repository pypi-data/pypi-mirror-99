...
from raffiot.untyped._internal import ContTag, FiberState, IOTag, ResultTag
...
class Scheduled:
    ...
    def __init__(self, schedule, fiber):
...
class SharedState:
    ...
    def __init__(self, pool_size, nighttime):
        self.scheduled_lock = threading.Lock()
        self.scheduled = []
        self.pool_size = max(pool_size, 1)
        self.pool = [Runner(self, i) for i in range(self.pool_size)]
        self.nighttime = nighttime

    def run(self, io, context):
...
def run_callback(fiber, index, res):
...
class Runner(Thread):
    ...
    def __init__(self, shared_state, index):
        super().__init__()
        self.queue = deque()
                ...
                while True:
                    ...
                    if tag == ContTag.ID:
                        if arg_tag == ResultTag.OK:
                            ...
                            fiber.result = Error(arg_value)
                        elif arg_tag == ResultTag.PANIC:
                            fiber.result = Panic(arg_value)
                        else:
                            fiber.result = Panic(
                    ...
                    if tag == ContTag.START:
                        ...
                        break
                    arg_tag = ResultTag.PANIC
                    arg_value = Panic(MatchError(f"Invalid cont {cont + [tag]}"))
                                ...
                                with fiber._state_lock:
                                    if (
                                        ...
                                        if isinstance(r, Ok):
                                            ...
                                            fiber._arg_value = r.error
                                        elif isinstance(r, Panic):
                                            fiber._arg_tag = ResultTag.PANIC
                                            fiber._arg_value = r.exception
                                        else:
                                            fiber._arg_tag = ResultTag.PANIC
                                            fiber._arg_value = Panic(
                                                MatchError(
