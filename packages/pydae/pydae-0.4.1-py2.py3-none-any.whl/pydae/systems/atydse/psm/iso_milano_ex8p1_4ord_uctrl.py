...
class iso_milano_ex8p1_4ord_uctrl_class: 
    ...
    def __init__(self): 
        ...
        self.y_run_list = ['i_d', 'i_q', 'p_t', 'q_t', 'v_t', 'theta_t', 'v_0', 'theta_0'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['i_d', 'i_q', 'p_t', 'q_t', 'p_m', 'v_f', 'v_0', 'theta_0'] 
...
def ini(struct,mode):
    ...
    K_delta = struct[0].K_delta
    K_v = struct[0].K_v
    
    # Inputs:
    v_t = struct[0].v_t
    theta_t = struct[0].theta_t
    p_l = struct[0].p_l
    q_l = struct[0].q_l
    ...
    if mode == 3:
        ...
        struct[0].g[4,0] = p_t + v_0*v_t*sin(theta_0 - theta_t)/X_l
        struct[0].g[5,0] = q_t + v_0*v_t*cos(theta_0 - theta_t)/X_l - v_t**2/X_l
    ...
    if mode == 11:
        ...
        struct[0].Gy_ini[4,6] = v_t*sin(theta_0 - theta_t)/X_l
        struct[0].Gy_ini[4,7] = v_0*v_t*cos(theta_0 - theta_t)/X_l
        struct[0].Gy_ini[5,6] = v_t*cos(theta_0 - theta_t)/X_l
...
def run(t,struct,mode):
    ...
    p_t = struct[0].y_run[2,0]
    q_t = struct[0].y_run[3,0]
    v_t = struct[0].y_run[4,0]
    ...
    if mode == 11:
        ...
        struct[0].Gy[3,4] = i_d*cos(delta - theta_t) - i_q*sin(delta - theta_t)
        struct[0].Gy[3,5] = i_d*v_t*sin(delta - theta_t) + i_q*v_t*cos(delta - theta_t)
        struct[0].Gy[4,4] = v_0*sin(theta_0 - theta_t)/X_l
        struct[0].Gy[4,5] = -v_0*v_t*cos(theta_0 - theta_t)/X_l
        struct[0].Gy[4,6] = v_t*sin(theta_0 - theta_t)/X_l
        struct[0].Gy[4,7] = v_0*v_t*cos(theta_0 - theta_t)/X_l
        struct[0].Gy[5,4] = v_0*cos(theta_0 - theta_t)/X_l - 2*v_t/X_l
        struct[0].Gy[5,5] = v_0*v_t*sin(theta_0 - theta_t)/X_l
        struct[0].Gy[5,6] = v_t*cos(theta_0 - theta_t)/X_l
        struct[0].Gy[5,7] = -v_0*v_t*sin(theta_0 - theta_t)/X_l
        struct[0].Gy[6,4] = -v_0*sin(theta_0 - theta_t)/X_l
        ...
        struct[0].Gy[7,4] = v_0*cos(theta_0 - theta_t)/X_l
    ...
    if mode > 12:
        ...
        struct[0].Hy[1,4] = i_d*sin(delta - theta_t) + i_q*cos(delta - theta_t)
...
def nonzeros():
    ...
    Fy_ini_rows = [1, 1, 1, 2, 2, 3]

    Fy_ini_cols = [0, 1, 4, 0, 5, 1]

    Gx_ini_rows = [0, 0, 1, 1, 2, 3]

    Gx_ini_cols = [0, 2, 0, 3, 0, 0]

    Gy_ini_rows = [0, 0, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 7, 7]

    Gy_ini_cols = [0, 1, 0, 1, 0, 1, 2, 0, 1, 3, 2, 6, 7, 3, 6, 7, 6, 7, 6, 7]
