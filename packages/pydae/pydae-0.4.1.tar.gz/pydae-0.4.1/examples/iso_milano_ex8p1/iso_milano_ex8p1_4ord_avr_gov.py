...
class iso_milano_ex8p1_4ord_avr_gov_class: 
    ...
    def __init__(self): 
        ...
        self.N_z = 5 
        self.N_store = 10000 
        self.params_list = ['X_d', 'X1d', 'T1d0', 'X_q', 'X1q', 'T1q0', 'R_a', 'X_l', 'H', 'D', 'S_n', 'Omega_b', 'omega_s', 'K_a', 'T_r', 'v_pss', 'Droop', 'T_m', 'K_sec', 'K_delta'] 
        self.params_values_list  = [1.81, 0.3, 8.0, 1.76, 0.65, 1.0, 0.003, 0.05, 6.0, 1.0, 1.0, 314.1592653589793, 1.0, 100, 0.1, 0.0, 0.05, 5.0, 0.001, 0.01] 
        self.inputs_ini_list = ['P_l', 'Q_l', 'P_cig', 'Q_cig', 'v_ref'] 
        self.inputs_ini_values_list  = [0.8, 0.2, 0.1, 0.0, 1.0] 
        self.inputs_run_list = ['P_l', 'Q_l', 'P_cig', 'Q_cig', 'v_ref'] 
        self.inputs_run_values_list = [0.8, 0.2, 0.1, 0.0, 1.0] 
        self.outputs_list = ['p_m', 'P_l', 'Q_l', 'P_cig', 'Q_cig'] 
        self.x_list = ['delta', 'omega', 'e1q', 'e1d', 'v_c', 'p_m', 'xi_m'] 
        self.y_run_list = ['i_d', 'i_q', 'v_1', 'theta_1', 'P_t', 'Q_t', 'v_f', 'p_m_ref', 'v_0', 'theta_0'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['i_d', 'i_q', 'v_1', 'theta_1', 'P_t', 'Q_t', 'v_f', 'p_m_ref', 'v_0', 'theta_0'] 
...
def ini(struct,mode):
    ...
    Droop = struct[0].Droop
    T_m = struct[0].T_m
    K_sec = struct[0].K_sec
    K_delta = struct[0].K_delta
    
    # Inputs:
    P_l = struct[0].P_l
    Q_l = struct[0].Q_l
    P_cig = struct[0].P_cig
    Q_cig = struct[0].Q_cig
    ...
    P_t = struct[0].y_ini[4,0]
    Q_t = struct[0].y_ini[5,0]
    v_f = struct[0].y_ini[6,0]
    p_m_ref = struct[0].y_ini[7,0]
    v_0 = struct[0].y_ini[8,0]
    theta_0 = struct[0].y_ini[9,0]
    ...
    if mode == 2:
        ...
        struct[0].f[5,0] = (-p_m + p_m_ref)/T_m
        struct[0].f[6,0] = omega - 1
    ...
    if mode == 3:
        ...
        struct[0].g[2,0] = P_cig + P_t*S_n + v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].g[3,0] = Q_cig + Q_t*S_n + v_0*v_1*cos(theta_0 - theta_1)/X_l - v_1**2/X_l
        ...
        struct[0].g[7,0] = -K_sec*xi_m - p_m_ref - (omega - 1)/Droop
        struct[0].g[8,0] = -P_l - v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].g[9,0] = -Q_l - v_0**2/X_l + v_0*v_1*cos(theta_0 - theta_1)/X_l
    ...
    if mode == 3:
        ...
        struct[0].h[1,0] = P_l
        struct[0].h[2,0] = Q_l
        struct[0].h[3,0] = P_cig
        struct[0].h[4,0] = Q_cig
    ...
    if mode == 11:
        ...
        struct[0].Gx_ini[7,1] = -1/Droop
        struct[0].Gx_ini[7,6] = -K_sec
        ...
        struct[0].Gy_ini[2,4] = S_n
        struct[0].Gy_ini[2,8] = v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[2,9] = v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[3,2] = v_0*cos(theta_0 - theta_1)/X_l - 2*v_1/X_l
        struct[0].Gy_ini[3,3] = v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[3,5] = S_n
        struct[0].Gy_ini[3,8] = v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[3,9] = -v_0*v_1*sin(theta_0 - theta_1)/X_l
        ...
        struct[0].Gy_ini[8,2] = -v_0*sin(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[8,3] = v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[8,8] = -v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[8,9] = -v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[9,2] = v_0*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[9,3] = v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[9,8] = -2*v_0/X_l + v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy_ini[9,9] = -v_0*v_1*sin(theta_0 - theta_1)/X_l
...
def run(t,struct,mode):
    ...
    v_f = struct[0].y_run[6,0]
    p_m_ref = struct[0].y_run[7,0]
    v_0 = struct[0].y_run[8,0]
    theta_0 = struct[0].y_run[9,0]
    
    struct[0].u_run[0,0] = P_l
    struct[0].u_run[1,0] = Q_l
    struct[0].u_run[2,0] = P_cig
    struct[0].u_run[3,0] = Q_cig
    struct[0].u_run[4,0] = v_ref
    # Differential equations:
    if mode == 2:
    ...
    if mode == 11:
        ...
        struct[0].Gx[6,4] = -K_a
        struct[0].Gx[7,1] = -1/Droop
        struct[0].Gx[7,6] = -K_sec
        ...
        struct[0].Gy[2,4] = S_n
        struct[0].Gy[2,8] = v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy[2,9] = v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[3,2] = v_0*cos(theta_0 - theta_1)/X_l - 2*v_1/X_l
        struct[0].Gy[3,3] = v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy[3,5] = S_n
        struct[0].Gy[3,8] = v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[3,9] = -v_0*v_1*sin(theta_0 - theta_1)/X_l
        ...
        struct[0].Gy[8,2] = -v_0*sin(theta_0 - theta_1)/X_l
        struct[0].Gy[8,3] = v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[8,8] = -v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy[8,9] = -v_0*v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[9,2] = v_0*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[9,3] = v_0*v_1*sin(theta_0 - theta_1)/X_l
        struct[0].Gy[9,8] = -2*v_0/X_l + v_1*cos(theta_0 - theta_1)/X_l
        struct[0].Gy[9,9] = -v_0*v_1*sin(theta_0 - theta_1)/X_l
