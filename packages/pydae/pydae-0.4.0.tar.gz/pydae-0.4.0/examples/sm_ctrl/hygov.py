...
class hygov_class: 
    ...
    def __init__(self): 
        ...
        self.N_z = 1 
        self.N_store = 10000 
        self.params_list = ['Droop', 'A_t', 'T_w', 'D_t', 'K_p_gov', 'K_i_gov', 'K_servo', 'T_servo', 'V_gate_max', 'Gate_max', 'H'] 
        self.params_values_list  = [0.05, 1, 2.5, 1, 1.0, 0.1, 1.0, 1.0, 0.5, 1.0, 5.0] 
        self.inputs_ini_list = ['omega_ref', 'q_nl'] 
        self.inputs_ini_values_list  = [1.0, 0.0] 
        self.inputs_run_list = ['omega_ref', 'q_nl'] 
        self.inputs_run_values_list = [1.0, 0.0] 
        self.outputs_list = ['gate'] 
        self.x_list = ['xi_omega', 'servo', 'pos', 'q', 'omega'] 
        self.y_run_list = ['servo_u', 'gate', 'head', 'p_m'] 
        self.xy_list = self.x_list + self.y_run_list 
        self.y_ini_list = ['servo_u', 'gate', 'head', 'p_m'] 
...
def ini(struct,mode):
    ...
    T_w = struct[0].T_w
    D_t = struct[0].D_t
    K_p_gov = struct[0].K_p_gov
    K_i_gov = struct[0].K_i_gov
    K_servo = struct[0].K_servo
    T_servo = struct[0].T_servo
    V_gate_max = struct[0].V_gate_max
    Gate_max = struct[0].Gate_max
    H = struct[0].H
    
    # Inputs:
    omega_ref = struct[0].omega_ref
    q_nl = struct[0].q_nl
    
    # Dynamical states:
    xi_omega = struct[0].x[0,0]
    servo = struct[0].x[1,0]
    pos = struct[0].x[2,0]
    q = struct[0].x[3,0]
    omega = struct[0].x[4,0]
    
    # Algebraic states:
    servo_u = struct[0].y_ini[0,0]
    gate = struct[0].y_ini[1,0]
    head = struct[0].y_ini[2,0]
    p_m = struct[0].y_ini[3,0]
    
    # Differential equations:
    if mode == 2:


        struct[0].f[0,0] = -Droop*gate - omega + omega_ref
        struct[0].f[1,0] = (K_servo*(-gate + servo_u) - servo)/T_servo
        struct[0].f[2,0] = Piecewise(np.array([(V_gate_max, V_gate_max < servo), (-V_gate_max, V_gate_max < -servo), (servo, True)]))
        struct[0].f[3,0] = (1 - head)/T_w
        struct[0].f[4,0] = (-0.1*omega + p_m)/(2*H)
    ...
    if mode == 3:
        ...
        struct[0].g[0,0] = K_i_gov*xi_omega + K_p_gov*(-Droop*gate - omega + omega_ref) - servo_u
        struct[0].g[1,0] = -gate + Piecewise(np.array([(Gate_max, Gate_max < pos), (1.0e-6, pos < 1.0e-6), (pos, True)]))
        struct[0].g[2,0] = -head + q**2/(gate + 1.0e-6)**2
    ...
    if mode == 10:
        ...
        struct[0].Fx_ini[1,1] = -1/T_servo
        struct[0].Fx_ini[2,1] = Piecewise(np.array([(0, (V_gate_max < servo) | (V_gate_max < -servo)), (1, True)]))
        struct[0].Fx_ini[4,4] = -0.05/H

    if mode == 11:

        struct[0].Fy_ini[0,1] = -Droop 
        struct[0].Fy_ini[1,0] = K_servo/T_servo 
        struct[0].Fy_ini[1,1] = -K_servo/T_servo 
        struct[0].Fy_ini[3,2] = -1/T_w 
        struct[0].Fy_ini[4,3] = 1/(2*H) 

        struct[0].Gx_ini[0,0] = K_i_gov
        struct[0].Gx_ini[0,4] = -K_p_gov
        struct[0].Gx_ini[1,2] = Piecewise(np.array([(0, (Gate_max < pos) | (pos < 1.0e-6)), (1, True)]))
        struct[0].Gx_ini[2,3] = 2*q/(gate + 1.0e-6)**2
        struct[0].Gx_ini[3,3] = A_t*head

        struct[0].Gy_ini[0,1] = -Droop*K_p_gov
        struct[0].Gy_ini[2,1] = -2*q**2/(gate + 1.0e-6)**3
        struct[0].Gy_ini[3,2] = A_t*(q - q_nl)
...
def run(t,struct,mode):
    ...
    # Algebraic states:
    servo_u = struct[0].y_run[0,0]
    gate = struct[0].y_run[1,0]
    head = struct[0].y_run[2,0]
    p_m = struct[0].y_run[3,0]
    
    struct[0].u_run[0,0] = omega_ref
    struct[0].u_run[1,0] = q_nl
    # Differential equations:
    if mode == 2:
    ...
    if mode == 10:
        ...
        struct[0].Fx[1,1] = -1/T_servo
        struct[0].Fx[2,1] = Piecewise(np.array([(0, (V_gate_max < servo) | (V_gate_max < -servo)), (1, True)]))
        struct[0].Fx[4,4] = -0.05/H

    if mode == 11:

        struct[0].Fy[0,1] = -Droop
        struct[0].Fy[1,0] = K_servo/T_servo
        struct[0].Fy[1,1] = -K_servo/T_servo
        struct[0].Fy[3,2] = -1/T_w
        struct[0].Fy[4,3] = 1/(2*H)

        struct[0].Gx[0,0] = K_i_gov
        struct[0].Gx[0,4] = -K_p_gov
        struct[0].Gx[1,2] = Piecewise(np.array([(0, (Gate_max < pos) | (pos < 1.0e-6)), (1, True)]))
        struct[0].Gx[2,3] = 2*q/(gate + 1.0e-6)**2
        struct[0].Gx[3,3] = A_t*head

        struct[0].Gy[0,1] = -Droop*K_p_gov
        struct[0].Gy[2,1] = -2*q**2/(gate + 1.0e-6)**3
        struct[0].Gy[3,2] = A_t*(q - q_nl)
...
def ini_nn(struct,mode):
    ...
    if mode == 3:
        ...
        struct[0].g[1,0] = -gate + Piecewise(np.array([(Gate_max, Gate_max < pos), (1.0e-6, pos < 1.0e-6), (pos, True)]))
        struct[0].g[2,0] = -head + q**2/(gate + 1.0e-6)**2
        struct[0].g[3,0] = A_t*head*(q - q_nl) - p_m
...
def nonzeros():
    ...
    Fy_ini_rows = [0, 1, 1, 3, 4]

    Fy_ini_cols = [1, 0, 1, 2, 3]

    Gx_ini_rows = [0, 0, 1, 2, 3]

    Gx_ini_cols = [0, 4, 2, 3, 3]

    Gy_ini_rows = [0, 0, 1, 2, 2, 3, 3]

    Gy_ini_cols = [0, 1, 1, 1, 2, 2, 3]
