...
class pendulum_class: 
    ...
    def __init__(self): 
        ...
        self.inputs_run_values_list = [10] 
        self.outputs_list = ['g_1', 'PE', 'KE', 'theta'] 
        self.x_list = ['pos_x', 'pos_y', 'v_x', 'v_y'] 
...
def ini(struct,mode):
    ...
    # Inputs:
    theta = struct[0].theta
    
    # Dynamical states:
    pos_x = struct[0].x[0,0]
    pos_y = struct[0].x[1,0]
    v_x = struct[0].x[2,0]
    v_y = struct[0].x[3,0]
    
    # Algebraic states:
    lam = struct[0].y_ini[0,0]
    f_x = struct[0].y_ini[1,0]
    ...
    if mode == 2:
        ...
        struct[0].f[0,0] = v_x
        struct[0].f[1,0] = v_y
        struct[0].f[2,0] = (-K_d*v_x + f_x - 2*lam*pos_x)/M
        struct[0].f[3,0] = (-G*M - K_d*v_y - 2*lam*pos_y)/M
    ...
    if mode == 3:
        ...
        struct[0].g[0,0] = -L**2 - 1.0e-6*lam + pos_x**2 + pos_y**2
        struct[0].g[1,0] = -theta + atan2(pos_x, -pos_y)
    
    # Outputs:
    if mode == 3:

        struct[0].h[0,0] = -L**2 - 1.0e-6*lam + pos_x**2 + pos_y**2
        struct[0].h[1,0] = G*M*pos_y
    ...
    if mode == 10:
        ...
        struct[0].Fx_ini[2,2] = -K_d/M
        struct[0].Fx_ini[3,1] = -2*lam/M
        struct[0].Fx_ini[3,3] = -K_d/M

    if mode == 11:

        struct[0].Fy_ini[2,0] = -2*pos_x/M 
        struct[0].Fy_ini[2,1] = 1/M 
        struct[0].Fy_ini[3,0] = -2*pos_y/M 

        struct[0].Gx_ini[0,0] = 2*pos_x
        struct[0].Gx_ini[0,1] = 2*pos_y
        struct[0].Gx_ini[1,0] = -pos_y/(pos_x**2 + pos_y**2)
        struct[0].Gx_ini[1,1] = pos_x/(pos_x**2 + pos_y**2)
...
def run(t,struct,mode):
    ...
    # Algebraic states:
    lam = struct[0].y_run[0,0]
    theta = struct[0].y_run[1,0]
    
    struct[0].u_run[0,0] = f_x
    # Differential equations:
    if mode == 2:
    ...
    if mode == 10:
        ...
        struct[0].Fx[2,0] = -2*lam/M
        struct[0].Fx[2,2] = -K_d/M
        struct[0].Fx[3,1] = -2*lam/M
        struct[0].Fx[3,3] = -K_d/M

    if mode == 11:

        struct[0].Fy[2,0] = -2*pos_x/M
        struct[0].Fy[3,0] = -2*pos_y/M

        struct[0].Gx[0,0] = 2*pos_x
        struct[0].Gx[0,1] = 2*pos_y
        struct[0].Gx[1,0] = -pos_y/(pos_x**2 + pos_y**2)
        struct[0].Gx[1,1] = pos_x/(pos_x**2 + pos_y**2)
    ...
    if mode > 12:
        ...
        struct[0].Hx[0,0] = 2*pos_x
        struct[0].Hx[0,1] = 2*pos_y
        struct[0].Hx[1,1] = G*M
...
def ini_nn(struct,mode):
    ...
    if mode == 11:
        ...
        struct[0].Gy_ini[0,0] = -0.00000100000000000000
...
def run_nn(t,struct,mode):
    ...
    if mode == 11:
        ...
        struct[0].Gy[0,0] = -0.00000100000000000000
        struct[0].Gy[1,1] = -1
