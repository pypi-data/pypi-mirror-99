...
class AclExporter:
    ...
    def export(self, storage_filesystem, max_level, export_path):
        info_dict = {
            "storage_account": self.__storage_name,
        ...
        yaml = yaml + self.__add_active_directory_users_from_config()
        self.__logger.debug("Active Directory Table.", extra=self.__active_directory)
        ...
        yaml = yaml + self.__generate_yaml_key_value("storage_account", self.__storage_name, 0)
        ...
        df = self.__acl_string_generator.create_folder_properties_pd_table(acl_props["acl"])
        ...
        for acl_type_item in self.__get_available_acl_types(df):
            ...
            for acl_aad_oid_item in self.__get_aad_oid_list_per_type(df, acl_type_item):
                ...
                for index, row in df_filtered.iterrows():
                    ...
                    if not group_set:
                        yaml = yaml + self.__generate_yaml_key_value(
                            "name", self.__get_name_for_aad_objectid(row["acl_aad_object_id"], self.__active_directory), len_path_split + 5
                        )
                        group_set = True
        ...
        for path in paths:
            self.__logger.debug("------------------")
            if len(path.name.split("/")) <= max_level:
                _full_path, _current_folder, len_path_split, acl_props, is_folder = self.__get_folder_properties_from_sdk(
                    path, storage_filesystem
                )

                if is_folder:
                    if len_path_split <= max_level:  # variable defines how deep we want to go
                            ...
                            for acl_aad_oid_item in self.__get_aad_oid_list_per_type(df, acl_type_item):
                                ...
                                for index, row in df_filtered.iterrows():
                                    if not group_set:
                                        yaml = yaml + self.__generate_yaml_key_value(
                                            "name",
                                            self.__get_name_for_aad_objectid(row["acl_aad_object_id"], self.__active_directory),
                                    ...
                                    if row["acl_level"] == "access":
                                        yaml = yaml + self.__generate_yaml_key_value(
                                            "acl_access", row["acl_rights"], len_path_split + 5 + level_indent
                                        )
                                    elif row["acl_level"] == "default":
                                        yaml = yaml + self.__generate_yaml_key_value(
                                            "acl_default", row["acl_rights"], len_path_split + 5 + level_indent
                                        )
                                    else:
                ...
                self.__logger.debug(f"The path: {path.name} exceeding Param Max Level {[max_level]}. Skipping...")
        self.__logger.debug("------   Writing to a file ---------------")

        output_yaml_git_name = f"acl_{self.__storage_name}_{storage_filesystem}.yaml"
        self.__logger.debug(f"Yaml Git path and filename: {self.__config_dir}/{export_path}/{output_yaml_git_name}")
        self.__export_yaml(yaml, self.__config_dir + "/" + export_path + "/" + output_yaml_git_name)
        self.__logger.info("ACL Export - DONE", extra=info_dict)

    def __add_active_directory_users_from_config(self):
        content = """azure:\n  active_directory:\n"""
        print(self.__active_directory)
        for key, value in self.__active_directory.items():
    ...
    def __get_folder_properties_from_sdk(self, full_path_object, storage_filesystem):
        ...
        file_system_client = self.__service_client.get_file_system_client(storage_filesystem)
    ...
    def __get_name_for_aad_objectid(self, objectid: str, name_oid: dict) -> str:
        for key, value in name_oid.items():
            if value == objectid:
                return "'%azure.active_directory." + key + "%'"
