...
from gen2aclbundle.acl.AclConfigReader import AclConfigReader
from gen2aclbundle.acl.AclPathResolver import AclPathResolver
from gen2aclbundle.acl.set.AclStringGenerator import AclStringGenerator
...
class AclChecker:
    def __init__(
        ...
        acl_config_reader: AclConfigReader,
        acl_string_generator: AclStringGenerator,
        acl_path_resolver: AclPathResolver,
    ):
        self.__config_dir = config_dir
        self.__storage_name = storage_name
        self.__logger = logger
        self.__service_client = service_client
        self.__acl_config_reader = acl_config_reader
        self.__acl_string_generator = acl_string_generator
        self.__acl_path_resolver = acl_path_resolver

    def check(self, storage_filesystem, export_path):
        self.__logger.debug("---------- Init Parameters   ------------------------")

        self.__logger.debug("Init Connection")
        file_system_client = self.__service_client.get_file_system_client(file_system=storage_filesystem)
        self.__logger.debug("acl_init_client initialized")
        paths = file_system_client.get_paths()
        self.__logger.debug("get_paths() initialized", extra={"path": paths})

        # Static name for Git
        output_yaml_git_name = f"acl_{self.__storage_name}_{storage_filesystem}.yaml"
        yaml_path = self.__config_dir + "/" + export_path + "/" + output_yaml_git_name
        # Read Config in YAML
        config = self.__acl_config_reader.read_config(yaml_path)
    ...
    def __check_acl_for_dir(self, folder_name: str, folder_config: dict, path_current_folder: str):
        ...
        if "folders" in folder_config:
            if folder_config["folders"] is not None:
                ...
                full_path_folder, filesystem, subfolder = self.__acl_path_resolver.get_filesystem_and_path_from_root(
                    folder_name, path_current_folder
                )
                self.__logger.debug(f"full_path_folder: {full_path_folder}")
                file_system_client = self.__service_client.get_file_system_client(file_system=filesystem)
                ...
                self.__logger.debug("Getting only Default ACL from last folder in YAML")
                df = self.__acl_string_generator.create_folder_properties_pd_table(sub_acl_props["acl"])
                is_default = df["acl_level"] != "access"
                df_default = df[is_default]
                first_run = True
                acl_string = ""
                self.__logger.debug("Creating ACL string")
                for index, row in df_default.iterrows():
                    ...
                    if acl_default != "---":  # only create ACL for users that have some ACL Default defined
                        ...
                        if acl_aad_oid_item == "OWNER":
                            ...
                            acl_string = (
                                acl_string
                                + f"{acl_type}:{acl_aad_oid_item}:{acl_access},default:{acl_type}:{acl_aad_oid_item}:{acl_default}"
                            )
                        first_run = False
            ...
            if subfolders_lists != []:
                ...
                file_system_client_check = self.__service_client.get_file_system_client(file_system=filesystem)
    ...
    def __check_acl(self, storage_filesystem, storage_path, acl_string, check_files_only):
        self.__logger.debug(f"Checking ACL for FS: {storage_filesystem}, Path: {storage_path}")
        file_system_client_checker = self.__service_client.get_file_system_client(file_system=storage_filesystem)
        ...
        if is_folder:
            ...
            _path_to_file, _filename = self.__acl_path_resolver.get_path_and_filename(storage_path)
            ...
            file_acl_string = self.__acl_string_generator.create_acl_for_file(acl_string)
    ...
    def __check_acl_for_dir_real(self, folder_name: str, folder_config: dict, root_path_current_folder: str):
        self.__logger.debug("-------------------------------")
        _full_path_folder, _filesystem, _full_path_folder_to_set_acl = self.__acl_path_resolver.get_filesystem_and_path_from_root(
            folder_name, root_path_current_folder
        )
        self.__logger.debug("folder: {}".format(folder_name))
        acl_config = folder_config["acl"]
        self.__logger.debug(acl_config)
        # Create ACL string
        acl_string = self.__acl_string_generator.create_acl_string(acl_config)
        ...
        self.__check_acl(_filesystem, _full_path_folder_to_set_acl, acl_string, check_files_only=False)
    ...
    def __check_acl_orchestrator(self, storage_filesystem, storage_path, acl_string):
        ...
        try:
            ...
            if e.error_code != "DefaultAclOnFileNotAllowed":
