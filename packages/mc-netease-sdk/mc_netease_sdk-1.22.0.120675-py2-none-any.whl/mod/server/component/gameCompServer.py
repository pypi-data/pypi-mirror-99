...
class GameComponentServer(BaseComponent):
    ...
    def GetEntitiesInSquareArea(self, entityId, startPos, endPos, dimensionId=-1):
        # type: (Union[str,None], Tuple[int,int,int], Tuple[int,int,int], int) -> List[str]
        """
        获取区域内的entity列表
        """
        pass
    ...
    def PlaceStructure(self, playerId, pos, structureName, dimensionId=-1):
        # type: (Union[str,None], Tuple[float,float,float], str, int) -> bool
        """
        放置结构
        """
        pass
