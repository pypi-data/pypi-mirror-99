...
def fill_nan(arr):
    """
    :param arr: 2D numpy array, NaN's are filled by brooming
    """
...
# noinspection PyShadowingNames
@jit(nopython=True)
def nanmean_filter(inp, dd):
    """ Filter input array with mean.

    :param inp: 2D array
    :param dd: radius of mean
    :return: filtered array
    """
    bb = np.empty(inp.shape)
    bb[:, :] = np.NaN
        ...
        for jj in range(nj):
            if np.isfinite(inp[ii, jj]):
                ...
                for kk in range(i1, i2):
                    for ll in range(j1, j2):
                        if np.isfinite(inp[kk, ll]):
                            bf += inp[kk, ll]
                            nn += 1
...
def nangaussian_filter(uu, sigma):
    from scipy.ndimage import gaussian_filter  # import here to avoid static TLS ImportError
...
def itt2d(shape, dd, extend=1.00):
    ...
    vx = np.array(0.5 * (vv[:-1] + vv[1:]), dtype=int)
    dx = np.array(0.5 * (vv[1:] - vv[:-1]), dtype=int)
    vv = np.linspace(0, shape[1], dd + 1)
    vy = np.array(0.5 * (vv[:-1] + vv[1:]), dtype=int)
    dy = np.array(0.5 * (vv[1:] - vv[:-1]), dtype=int)
...
def inpaint(array, sigma=None, logger=None, update_in_place=False, fill_corners=True, fill_remaining=True,
    ...
    if np.sum(np.isnan(arr)) == np.prod(shape):
        logger.info("Array contains no valid points -> do nothing.")
        if update_in_place is False:
            return arr

    # meshgrid to get array indices right for
    yv, xv = np.meshgrid(np.arange(arr.shape[1], dtype=np.int16),  # this way memory is saved
                         np.arange(arr.shape[0], dtype=np.int16), copy=False)
    ...
    logger.info("bad/total ratio: %.5f" % (float(np.sum(bad_results) / np.prod(shape))))
    ...
    for sx, sy in itt2d(good_results.shape, dd_split, extend=extend):
        ...
        if interpolation_method is None:
            ...
            rr = bisplev(x=np.arange(xmm[0], xmm[1]),
                         ...
                         tck=tck)
            arr[xmm[0]:xmm[1], ymm[0]:ymm[1]] = rr
            ...
            if len(values) < 1:
                ...
                if len(xi) > 0:
                        ...
                        with warnings.catch_warnings():
                            ...
                            for ix, iy in product(box_x, box_y):
                                ...
                                if np.isnan(aa):  # only if corner is empty
                                    for jj in range(2, np.min(sub_shape),
                                                    2):  # incease search window fast to find stuff
                    ...
                    if max_interpolation_points is None:
                        ...
                        nn_blocks = int(np.ceil(nn / max_interpolation_points))
                        ...
                        for ii, (j1, j2) in enumerate(zip(ss[:-1], ss[1:])):
                            ...
                            grd = griddata(points=points, values=values, xi=xi[j1:j2], fill_value=np.nan,
                                           method="linear")
                            good_grd = np.isfinite(grd)  # points for which interpolation succeeded
    ...
    if fill_remaining is not None:
        ...
        logger.info("bad/total ratio: %.5f" % (float(n_bad / np.prod(shape))))
        if n_bad > 0:
            print(arr[bad_results == np.False_])
            if np.sum(np.isnan(arr)) == np.prod(arr.shape):
                logger.info("Array contains no valid points.")
            else:
    ...
    if post_processing is None:
        ...
        idx_val = np.logical_and(mask == np.True_, np.isfinite(arr))  # need '==' here
    ...
    if mask is not None:
        ...
        arr[mask == np.False_] = np.nan  # need '==' here !
