...
@jit(nopython=True, locals={'vi': float32,
                            ...
                            'b7': int8, 'b8': int8, 'b9': int8,
                            'v0': float32, 'v1': float32, 'v2': float32, 'v3': float32, 'v4': float32, 'v5': float32,
                            'v6': float32, 'v7': float32, 'v8': float32, 'v9': float32, })
                                ...
                                for i7 in d7:
                                    for i8 in d8:
                                        for i9 in d9:
                                            ...
                                            # works only in numba since the integer conversion of the b is handled
                                            # trough the interface block, without numba add int()'s here
                                            ...
                                            # when writing, numba didn't support array striding, meant is:
                                            # res[:] += vi*data[:,b0,b1,b2]
                                            for ii in range(ndim):
                                                # <- here decide weather spectral dimension is first or last
                                                dat = data[b0, b1, b2, b3, b4, b5, b6, b7, b8, b9, ii]
                                                res[ii] += vi * dat
...
class intp(object):
    ...
    def __init__(self, data, axes=None, jacobean=True, caching=False, maxsize=2000,
                 hash_pattern="%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,"):
        """
    ...
    def interpolate(self, pt):
        ...
        rr = r0 * r1 * r2 * r3 * r4 * r5 * r6 * r7 * r8 * r9
        if rr != 1:
            raise ValueError("At least one dimension is out of bounds.")
        else:
            return __intp__(self.data, [p0, p1, p2, p3, p4, p5, p6, p7, p8, p9],
                            [d0, d1, d2, d3, d4, d5, d6, d7, d8, d9], jacobean=self.__jacobean__)
