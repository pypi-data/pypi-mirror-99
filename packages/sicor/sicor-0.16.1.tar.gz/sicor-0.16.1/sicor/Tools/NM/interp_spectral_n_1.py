...
def __intp_spectral__(data, res, grad, ndim,
    ...
    for i0 in d0:
        ...
        for ii in range(ndim):
            ...
            dat = data[b0, ii]  # <- here decide weather spectral dimension is first or last
            res[ii] += vi * dat
...
def __intp__(data, pt, dv, jacobean):
    ...
    if jacobean:
        grad = np.zeros((1, ndim_spectral))
        __intp_spectral__(data, res, grad, ndim_spectral,
                          pt[0],
                          d0)  # <- here decide weather spectral dimension is first or last
...
# class intp1_dx(object):
class intp(object):
    """
    Wrapper class for 3D interpolation with intp 1 dimensions
    """

    def __init__(self, data, axes=None, jacobean=True, caching=False, maxsize=2000, hash_pattern="%.2f,"):
        """
    ...
    def interpolate(self, pt):
        ...
        if rr != 1:
            ...
            return __intp__(self.data, [p0], [d0], jacobean=self.__jacobean__)
