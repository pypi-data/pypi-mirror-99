...
@jit(nopython=True, locals={'vi': float32,
                            'b0': int8, 'b1': int8, 'b2': int8, 'b3': int8,
        ...
        for i1 in d1:
            for i2 in d2:
                for i3 in d3:
                    ...
                    v0 = sign(p0 - i0) * np.abs(1.0 * (p1 - i1) * (p2 - i2) * (p3 - i3))
                    v1 = sign(p1 - i1) * np.abs((p0 - i0) * 1.0 * (p2 - i2) * (p3 - i3))
                    v2 = sign(p2 - i2) * np.abs((p0 - i0) * (p1 - i1) * 1.0 * (p3 - i3))
                    v3 = sign(p3 - i3) * np.abs((p0 - i0) * (p1 - i1) * (p2 - i2) * 1.0)
                    ...
                    for ii in range(ndim):
                        ...
                        dat = data[b0, b1, b2, b3, ii]  # <- here decide weather spectral dimension is first or last
                        res[ii] += vi * dat
                        grad[0, ii] += v0 * dat
                        grad[1, ii] += v1 * dat
                        grad[2, ii] += v2 * dat
...
def __intp__(data, pt, dv, jacobean):
    ...
    if jacobean:
        grad = np.zeros((4, ndim_spectral))
        __intp_spectral__(data, res, grad, ndim_spectral,
                          pt[0], pt[1], pt[2], pt[3],
                          d0, d1, d2, d3)  # <- here decide weather spectral dimension is first or last
...
class intp(object):
    ...
    def interpolate(self, pt):
        ...
        if rr != 1:
            ...
            return __intp__(self.data, [p0, p1, p2, p3], [d0, d1, d2, d3], jacobean=self.__jacobean__)
