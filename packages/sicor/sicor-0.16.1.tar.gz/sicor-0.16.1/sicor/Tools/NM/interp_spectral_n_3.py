    ...
    for i0 in d0:
        for i1 in d1:
            for i2 in d2:
                ...
                vi = np.abs((p0 - i0) * (p1 - i1) * (p2 - i2))

                v0 = sign(p0 - i0) * np.abs(1.0 * (p1 - i1) * (p2 - i2))
                v1 = sign(p1 - i1) * np.abs((p0 - i0) * 1.0 * (p2 - i2))
                v2 = sign(p2 - i2) * np.abs((p0 - i0) * (p1 - i1) * 1.0)
                ...
                for ii in range(ndim):
                    dat = data[b0, b1, b2, ii]  # <- here decide weather spectral dimension is first or last
                    res[ii] += vi * dat
                    grad[0, ii] += v0 * dat
                    grad[1, ii] += v1 * dat
                    grad[2, ii] += v2 * dat
...
def __intp__(data, pt, dv, jacobean):
    ...
    if jacobean:
        grad = np.zeros((3, ndim_spectral))
        __intp_spectral__(data, res, grad, ndim_spectral,
                          pt[0], pt[1], pt[2],
                          d0, d1, d2)  # <- here decide weather spectral dimension is first or last
...
# class intp3_dx(object):
class intp(object):
    """
    Wrapper class for 3D interpolation with intp 3 dimensions
    """

    def __init__(self, data, axes=None, jacobean=True, caching=False, maxsize=2000, hash_pattern="%.2f,%.2f,%.2f,"):
        """
    ...
    def interpolate(self, pt):
        ...
        if rr != 1:
            ...
            return __intp__(self.data, [p0, p1, p2], [d0, d1, d2], jacobean=self.__jacobean__)
