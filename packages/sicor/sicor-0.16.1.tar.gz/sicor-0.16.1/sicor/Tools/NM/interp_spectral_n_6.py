...
@jit(nopython=True, locals={'vi': float32,
                            'b0': int8, 'b1': int8, 'b2': int8, 'b3': int8, 'b4': int8, 'b5': int8,
                ...
                for i3 in d3:
                    for i4 in d4:
                        for i5 in d5:
                            ...
                            vi = np.abs((p0 - i0) * (p1 - i1) * (p2 - i2) * (p3 - i3) * (p4 - i4) * (p5 - i5))
...
def __intp__(data, pt, dv, jacobean):
    ...
    if jacobean:
        grad = np.zeros((6, ndim_spectral))
        __intp_spectral__(data, res, grad, ndim_spectral,
                          pt[0], pt[1], pt[2], pt[3], pt[4], pt[5],
                          d0, d1, d2, d3, d4, d5)  # <- here decide weather spectral dimension is first or last
...
class intp(object):
    ...
    def __init__(self, data, axes=None, jacobean=True, caching=False, maxsize=2000,
                 hash_pattern="%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,"):
        """
    ...
    def interpolate(self, pt):
        ...
        if rr != 1:
            ...
            return __intp__(self.data, [p0, p1, p2, p3, p4, p5], [d0, d1, d2, d3, d4, d5], jacobean=self.__jacobean__)
