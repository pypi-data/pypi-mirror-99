...
from ....algorithms.quantization.fake_quantize import insert_fake_quantize_nodes, compute_stats_layouts, \
    get_fake_quantize_first_output
...
class MinMaxQuantization(Algorithm):
    ...
    def __get_activations_statistics_layout(self, model):
        ...
        """
        fake_quantize_config = compute_stats_layouts(self._config, model)
    ...
    @staticmethod
    def create_stats_layout(fake_quantize_config, model, for_weights=True):
        ...
        for fq in fq_nodes:
            ...
            if is_weights is True and for_weights is True:
                statistics_layout[fq.name] = get_tensor_statistics(layer_config['range_estimator'], for_weights=True)
