...
from .utils import create_metric_config, is_preset_performance, \
    get_mixed_preset_config, get_num_of_quantized_ops, evaluate_model
...
class AccuracyAwareQuantization(Algorithm):
    ...
    def __init__(self, config, engine):
        ...
        def create_quantization_algo(algo_config, name):
            algo = COMPRESSION_ALGORITHMS.get(algo_config.base_algorithm)(algo_config, engine)
            algo.name = name
            return algo

        # Create initial quantization algorithms
        self._quantization_algo = create_quantization_algo(self._config, 'AAQuantizationAlgorithm')
        self._preset_conversion_algo = create_quantization_algo(get_mixed_preset_config(self._config),
                                                                'AAConversionAlgorithm')
    ...
    def run(self, model):
        ...
        for metric_config in self._metrics_config.values():
            if metric_config.persample.is_special:
                self._stats_layout = {self._output_node_name:
                                          {'output_logits': TensorStatistic(lambda a: a)}}
                break
        ...
        if not self._baseline_metric or self._config.annotation_free:
            ...
            self._baseline_metric, self._original_per_sample_metrics = \
        ...
        self._max_drop_by_name = {name: value * self._config.maximal_drop if self._config.drop_type == 'relative'
                                        else self._config.maximal_drop for name, value in self._baseline_metric.items()}
    ...
    def _change_quantization_scope(self, model, original_accuracy_drop,
        ...
        self._quantized_layers_num = \
            get_num_of_quantized_ops(model, self._graph_transformer.fq_removal.quantize_operations)
        logger.info('The total number of quantized operations in the graph: %d', self._quantized_layers_num)

        logger.info('Removing fake quantize nodes')
        all_removed_nodes_names = []
        all_ops_in_orig_prec = set()
        ...
        for iteration in range(self._config.max_iter_num):
            ...
            fq_name_to_remove = node_importance.pop(0)
            model, removed_nodes, ops_in_orig_prec = self._graph_transformer.remove_fq_nodes(model,
                                                                                             [fq_name_to_remove])
            logger.debug('Removed a block of %d FQ layers: %s', len(removed_nodes), removed_nodes)
            logger.info('Reverted %d layers to the original precision: %s',
                        len(ops_in_orig_prec), ', '.join(ops_in_orig_prec))
            all_removed_nodes_names.append(str(removed_nodes))
            all_ops_in_orig_prec.update(ops_in_orig_prec)
            ...
            if new_accuracy_drop > accuracy_drop and self._config.use_prev_if_drop_increase:
                ...
                all_removed_nodes_names.remove(str(removed_nodes))
                all_ops_in_orig_prec.difference_update(ops_in_orig_prec)
                is_step_back = True
        ...
        if removed_all_fq or not reached_required_drop:
        ...
        if not removed_all_fq:
            logger.debug('Removed FakeQuantize nodes:\n %s', '\n'.join(all_removed_nodes_names))
            logger.info(' %d out of %d layers have been reverted back to the floating-point precision: %s',
                        len(all_ops_in_orig_prec), self._quantized_layers_num,
                        ', '.join(all_ops_in_orig_prec))

        logger.update_progress(self.total_exec_steps)

        return model

    def _get_node_importance(self, model, metric_name, qmodel_per_sample_metrics=None):
        ...
        node_importance_score = self._calculate_node_importance_scores(model,
                                                                       ranking_subset,
                                                                       metric_name)
        node_importance = sorted(node_importance_score, key=node_importance_score.get, reverse=True)
    ...
    def _calculate_node_importance_scores(self, model, ranking_subset, metric_name):
        ...
        for node in fake_quantize_nodes:
            if node.name not in cut_fqs:
                cut_model, cut_fq_layers, _ = self._modify_model_in_scope(model, [node.name])
                logger.debug('Removed a block of %d FQ layers: %s', len(cut_fq_layers), cut_fq_layers)
    ...
    def _modify_model_in_scope(self, model, node_names):
        return self._graph_transformer.remove_fq_nodes(deepcopy(model), node_names)
