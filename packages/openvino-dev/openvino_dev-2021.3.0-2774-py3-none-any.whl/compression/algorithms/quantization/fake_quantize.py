...
from ...graph.node_utils import get_all_node_outputs, get_node_input, set_node_value,\
    get_node_value, get_node_output, get_node_inputs, get_input_shape, \
    get_quantized_input_key, get_input_data_value
...
def symmetric_range(node, fq, weights_stats,
    ...
    if node.type == 'Const' or get_input_data_value(fq, 0) is not None:
        ...
        max_level = fix_zero_filters_symmetric(max_level)
        min_level = np.zeros(max_level.shape) if np.all(min_level >= 0) else \
...
def set_rescaling_factors(target_device, model, scaling_factor=2.0):
    ...
    def get_first_convolutions(pre_nodes):
        first_convolutions = []
        while pre_nodes:
            first_convolutions += [node for node in pre_nodes
                                   if node.type == 'Convolution']
            pre_nodes = [get_all_node_outputs(node) for node in pre_nodes
                         if node.type != 'Convolution']
            pre_nodes = {node for node_list in pre_nodes for node in node_list}
        return first_convolutions

    for input_node in input_nodes:
        input_convolutions += get_first_convolutions([input_node, ])
