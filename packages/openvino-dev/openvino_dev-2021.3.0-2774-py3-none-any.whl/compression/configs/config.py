...
class Config(Dict):
    ...
    def read_config(cls, path):
        ...
        cls._save_config_for_mo(config, config)
        cls._convert_mo_args(config)
        cls._configure_algo_params(config)
        cls._configure_logger_params(config)

        return config

    def _save_config_for_mo(self, config_data):
        self.model['quantization_info'] = create_quantization_info_for_mo(config_data)
