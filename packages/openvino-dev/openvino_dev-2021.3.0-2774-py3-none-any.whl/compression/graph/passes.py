...
from ..graph.node_utils import get_all_node_outputs, get_node_inputs
...
class InsertFakeQuantize(BackReplacementPattern):
    ...
    def quantize_only_input(node: Node):
        ...
        if node.type == 'Multiply' and \
                nu.get_node_output(node, 0)[0].type == 'Add' and \
                (nu.get_node_input(node, 1).type == 'Const' or nu.get_input_data_value(node, 1) is not None):
            return True
        return False
    ...
    def replace_pattern(self, graph: Graph, match: Dict[str, Node]):
        ...
        if m_op.type in ['Convolution', 'MatMul']:
...
class FakeQuantizePropagation(BackReplacementPattern):
    ...
    jump_single_branch_ops = ['ReduceMax', 'MaxPool', 'Reshape', 'Flatten', 'Squeeze', 'Unsqueeze', 'Interpolate',
                              ...
                              'VariadicSplit', 'ShuffleChannels', 'Broadcast', 'Pad']
...
def create_bias_node(graph: Graph, src_node):
    ...
    add_bias_shape = np.ones_like(bias_shape)
