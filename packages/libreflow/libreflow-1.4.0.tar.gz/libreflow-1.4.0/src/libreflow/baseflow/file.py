...
class PublishFileAction(LaunchSessionWorker):
    ...
    def launcher_exec_func_name(self):
        return "process_after_publish"
    
    def process_after_publish(self):
        if self.upload_after_publish.get():
            self.upload_published_revision()
        
        if self._file.format.get() == 'blend':
            self.save_blend_dependencies()
    ...
    def save_blend_dependencies(self):
        from blender_asset_tracer import trace, bpathlib
        from pathlib import Path
        import collections

        head = self._file.get_head_revision()
        path = Path(head.get_path())
        ...
        head.dependencies.set(dict(report))
...
TrackedFolder.open.set_related_type(OpenTrackedFolderAction)
TrackedFolder.history.set_related_type(TrackedFolderHistory)
