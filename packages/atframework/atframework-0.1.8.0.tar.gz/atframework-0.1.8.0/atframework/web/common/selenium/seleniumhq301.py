...
class Seleniumhq301(Taiji):
    ...
    def _open_browser(self, browser_name):
        ...
        if 'safari' == browser_name:
            ...
            if self.is_mac_system() is True:
                ...
                browser = webdriver.Chrome(
                    executable_path=os.path.dirname(os.__file__) +
                                    DriversSettings.LOCAL_LINUX_CHROME_DRIVER)
        else:
            if self.is_mac_system() is True:
                ...
                browser = webdriver.Firefox(
                    executable_path=os.path.dirname(os.__file__) +
                                    DriversSettings.LOCAL_LINUX_FIREFOX_DRIVER)
    ...
    def _wait_for_id_shown_and_sleep_2s(self, id_name, wait_time=60, sleep_time=2):
    ...
    def _wait_for_xpath_shown_and_sleep(self, xpath, wait_time=60, sleep_time=5):
        WebDriverWait(self.browser, wait_time).until(
    ...
    def _wait_for_xpath_shown2_and_sleep(self, xpath, wait_time=120, sleep_time=2):
    ...
    def _get_element_url_by_text(self, link_text):
        return self.browser.find_element_by_link_text(link_text).get_attribute("href")
    ...
    def _find_link_by_text(self, link_text):
        try:
            if self.browser.find_element_by_link_text(link_text).is_displayed():
                return True
            else:
                return False
        except NoSuchElementException:
