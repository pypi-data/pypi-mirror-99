...
class LatentCategorical(Model):
    params = {'K': 2, 'probs': torch.tensor([0.2, 0.3, 0.3, 0.1, 0.1]), 'hidden_dim': 5, 'a' : 1, 'b' : 100,
              ...
              "likelihood_rna" : "NB", "likelihood_atac" : "NB", 'lambda' : 1, "latent_type" : "D"}
    ...
    def likelihood(self, inf_params, mod = "rna",sum = False):
        ...
        BETA = 10000
        cc_argmax = (inf_params["CNV_probabilities"] * BETA).softmax(-1)

        segment_fact_marg = segment_fact * cc_argmax.rehsape()
