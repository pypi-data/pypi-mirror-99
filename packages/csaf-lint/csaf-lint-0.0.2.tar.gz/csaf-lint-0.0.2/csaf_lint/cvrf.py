...
"""CVRF v1.1 and v1.2 parser - format is XML."""
import os
import sys
import copy
import csv
...
CRVF_KNOWN_SEMANTIC_VERSIONS = (CRVF_DEFAULT_SEMANTIC_VERSION, '1.1')
...
class CVRF_Syntax(object):
    ...
    cvrf_versions = CRVF_KNOWN_SEMANTIC_VERSIONS
...
def parse(xml_path, version_hint='1.2', validate_schema=False):
    """Parse the document and return tuple of boolean outcome and dict."""
    cvrf_version = version_hint
    if cvrf_version not in CRVF_KNOWN_SEMANTIC_VERSIONS:
        cvrf_version =  CRVF_DEFAULT_SEMANTIC_VERSION

    logging.info('parsing document at (%s) as (v%s) CVRF' % (xml_path, cvrf_version))
    try:
        cvrf_doc = etree.parse(xml_path, etree.XMLParser(encoding=ENCODING))
        logging.info('document at (%s) successfully parsed as (v%s) CVRF' % (xml_path, cvrf_version))
    except IOError:
        message = 'I/O error: (%s) is not accessible' % xml_path
        logging.error(message)
        return False, {'error': message}
    except etree.XMLSyntaxError as err:
        message = (
            'Parsing error: document at (%s) is not well-formed.'
            ' Details %s' % (xml_path, err.error_log.filter_from_level(etree.ErrorLevels.FATAL))
        )
        logging.error(message)
        return False, {'error': message}

    # check to make sure cvrf namespace in doc matches cvrf version from command line args
    logging.info('verifying cvrf version consistency ...')
    ...
    logging.info('cvrf version requested: (%s)' % version_hint)

    if doc_cvrf_version:
        if doc_cvrf_version == version_hint:
            logging.info('OK: CVRF version matches document!')
        else:
            message = (
                'CVRF version mismatch!'
                ' Specified cvrf version does not match the namespace in the cvrf document!'
            )
            logging.error(message)
            return False, {'error': message}
    else:
    ...
    return True, cvrf_parse(cvrf_doc, parsables, output_format, output_file, args, cvrf_version)