  ...
  def check(self,p):
      ...
      for key in p:
          ...
          if k=='ovun5': 
             ...
             if p[key]<=0.001:
                p[key]= value(6.0,key)
          if k in ['bo2','bo4','bo6']:     ### scale bo
             if p[key]>=8.0:
                p[key] = value(7.95,key)
             if p[key]<=2.0:
                p[key] = value(2.05,key)
          if k in ['bo1','bo3','bo5']:     ### scale bo
             if p[key]<=-0.9:
                p[key] = value(-0.85,key)
             if p[key]>=-0.1:
                p[key] = value(-0.15,key)
          if k=='theta0':
             if p[key]>=200.0:
