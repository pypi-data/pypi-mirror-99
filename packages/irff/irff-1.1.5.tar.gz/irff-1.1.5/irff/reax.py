  ...
  def clip_parameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.01,2.5)
          elif key=='ovun5':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,999.0*self.unit)
          elif key=='ovun1':
             self.p[k] = tf.clip_by_value(self.v[k],0.09,9.0)
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.01*self.unit,8.0*self.unit)
          elif key == 'Dehb':
             ...
             if self.nn:
                self.p[k] = tf.clip_by_value(self.v[k],-0.9,-0.1)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],-50.0,-0.0001)
          elif key in ['bo2','bo4','bo6']:
             if self.nn:
                self.p[k] = tf.clip_by_value(self.v[k],2.0,8.0)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],0.0,50.0)
          elif key == 'boc1':
             ...
             rvdw_ = 1.3*self.p_['rosi_'+vn[1]]
             self.p[k] = tf.clip_by_value(self.v[k],rvdw_,2.0*self.p_['rosi_'+vn[1]])
          elif key=='val':
             sp = vn[1]
      ...
      for i in range(step+1):
          ...
          if i==0:
             ...
             if accu>accMax:
                accMax = accu
          if self.optmol:
             loss_ = loss - latol - ME_*self.mefac
          else:
             loss_ = loss - latol
