...
def text_preprocessing(dataframe,col_name, spellchecker = False):
    ...
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.cont_exp(x)) #you're -> you are; i'm -> i am
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.remove_emails(x))
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.remove_html_tags(x))
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.remove_stopwords(x))
    if spellchecker:
    	dataframe[column] = dataframe[column].progress_apply(lambda x:th.spelling_correction(x))
  
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.remove_special_chars(x))
    dataframe[column] = dataframe[column].progress_apply(lambda x: th.remove_accented_chars(x))
