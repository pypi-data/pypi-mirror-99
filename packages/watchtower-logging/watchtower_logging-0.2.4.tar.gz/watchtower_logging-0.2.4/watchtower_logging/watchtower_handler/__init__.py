...
class WatchTowerHandler(logging.Handler):
    ...
    def __init__(self, beam_id, host=DEFAULT_HOST, token=None, debug=False, flush_interval=5.0,
                 force_keep_ahead=False, protocol='https',
                 queue_size=DEFAULT_QUEUE_SIZE,
                 retry_backoff=2.0, retry_count=3, timeout=5, verify=True, version=None, dev=False):
        """
        Args:
        ...
        self.url = f'{self.protocol}://{self.host}/api/beams/{self.beam_id}'
    ...
    @property
    def user_agent(self):
        return f'watchtower-logging-python/{self.version}'