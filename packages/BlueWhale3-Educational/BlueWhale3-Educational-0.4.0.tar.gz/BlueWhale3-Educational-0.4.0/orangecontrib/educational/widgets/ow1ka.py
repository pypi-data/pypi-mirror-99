...
def __(key):
    return i18n.t('educational.ow1ka.' + key)
...
class OW1ka(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        net_error = widget.Msg(__("msg.cannot_load_data"))
        parse_error = widget.Msg(__("msg.cannot_parse_data"))
        invalid_url = widget.Msg(__("msg.invalid_url") + VALID_URL_HELP)
        data_is_anal = widget.Msg(__("msg.need_data_public_link"))

    class Information(widget.OWWidget.Information):
        response_data_empty = widget.Msg(__("msg.response_data_empty"))
    ...
    def __init__(self):
        ...
        vb = gui.vBox(self.controlArea, __("box_import_data"))
        hb = gui.hBox(vb)
        ...
        hb.layout().addWidget(QLabel(__("row_public_link_url"), hb))
        ...
        box = gui.widgetBox(self.controlArea, __("box_column"))
        ...
        with self.progressBar(3) as progress:
            try:
                ...
                try:
                    ...
                    log.exception(__("text.parsing_error"), url)
    ...
    def set_info(self):
        ...
        text += try_(lambda: __("text.data_entry").format(data[0, self.DATETIME_VAR],
