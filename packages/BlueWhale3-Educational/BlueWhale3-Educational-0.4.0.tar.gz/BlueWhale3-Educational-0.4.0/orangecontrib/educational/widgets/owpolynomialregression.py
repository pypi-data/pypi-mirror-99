...
def __(key):
    return i18n.t('educational.owpolynomialregression.' + key)
...
class OWUnivariateRegression(OWBaseLearner):
    ...
    class Inputs(OWBaseLearner.Inputs):
        learner = Input("Learner", Learner, label=i18n.t("educational.common.learner"))
    ...
    class Error(OWBaseLearner.Error):
        all_none = Msg(__("msg_feature_undefined_value"))
        no_cont_variables = Msg(__("msg_polynomial_regression_require_num_feature"))
    ...
    def add_main_layout(self):
        ...
        self.regressor_label = gui.label(
            ...
            label=__("row.regressor"))
        gui.label(widget=info_box, master=self,
                  label=__("row.mean_absolute_error"))
        gui.label(widget=info_box, master=self,
                  label=__("row.root_mean_square_error"))

        box = gui.vBox(self.controlArea, __("box_var"))

        self.x_var_model = itemmodels.VariableListModel()
        self.comboBoxAttributesX = gui.comboBox(
            box, self, value='x_var_index', label=__("row.input"),
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert((
            (__("report.polynomial_expansion"), self.polynomialexpansion),
        ))
        self.report_plot()
        if caption:
