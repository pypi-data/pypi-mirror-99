...
def __(key):
    return i18n.t('educational.owkmeans.' + key)
...
class OWKmeans(OWWidget):
    ...
    class Outputs:
        annotated_data = Output("Annotated Data", Table, default=True,
                                label=i18n.t("educational.common.annotated_data"))
        centroids = Output("Centroids", Table, label=i18n.t("educational.common.centroid"))

    class Warning(OWWidget.Warning):
        num_features = Msg(__("msg.widget_require_least_two_num_feature"))
        cluster_points = Msg(__("msg.cluster_not_exceed_point_num"))
    ...
    graph_name = 'scatter'
    output_name = "cluster"
    STEP_BUTTONS = [__("btn.reassign_membership"), __("btn.recompute_centroid")]
    AUTOPLAY_BUTTONS = [__("btn.run"), __("btn.stop")]
    ...
    def __init__(self):
        ...
        self.step_back_button = gui.button(
            self.step_box, self, __("btn.step_back"), callback=self.step_back)

        self.run_box = gui.widgetBox(self.controlArea, __("box.run"))

        self.auto_play_speed_spinner = gui.hSlider(
            self.run_box, self, 'auto_play_speed', label=__("row.speed"),
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert((
            (__("row.num_of_centroid"), self.number_of_clusters),
        ))
