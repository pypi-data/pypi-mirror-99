...
def __(key):
    return i18n.t('educational.owgradientdescent.' + key)
...
class OWGradientDescent(OWWidget):
    ...
    class Outputs:
        model = Output("Model", Model, label=i18n.t("educational.common.model"))
        coefficients = Output("Coefficients", Table, label=i18n.t("educational.common.coefficient"))
    ...
    auto_play_button_text = [__("btn.run"), __("btn.stop")]
    ...
    class Error(OWWidget.Error):
        ...
        """
        to_few_features = Msg(__("msg.num_feature_few"))
        no_class = Msg(__("msg.data_must_have_class_attr"))
        to_few_values = Msg(__("msg.class_attr_must_have_two_val"))
        all_none = Msg(__("msg.feature_no_defined_val"))
    ...
    def __init__(self):
        ...
        # info box
        self.info_box = gui.widgetBox(self.controlArea, __("box.info"))
        ...
        self.options_box = gui.widgetBox(self.controlArea, __("box.data"))
        opts = dict(
            widget=self.options_box, master=self, orientation=Qt.Horizontal,
            callback=self.change_attributes, sendSelectedValue=True,
        )
        self.cbx = gui.comboBox(value='attr_x', label=__("row.x"), **opts)
        self.cby = gui.comboBox(value='attr_y', label=__("row.y"), **opts)
        self.target_class_combobox = gui.comboBox(
            value='target_class', label=__("row.target_class"), **opts)

        self.cbx.setModel(self.x_var_model)
        self.cby.setModel(self.y_var_model)

        # properties box
        self.properties_box = gui.widgetBox(self.controlArea, __("box.property"))
        self.alpha_spin = gui.spin(
            widget=self.properties_box, master=self, callback=self.change_alpha,
            value="alpha", label=__("row.learn_rate"),
        ...
        self.auto_play_speed_spinner = gui.hSlider(
            ...
            intOnly=False, createLabel=False, label=__("row.speed"))
    ...
    def set_data(self, data):
        ...
        self.learner_label.setText(__("row.title") + self.learner_name)
    ...
    def is_logistic(self):
        return self.learner_name == __("row.name")

    def send_report(self):
        if self.data is None:
            return
        caption_items = (
            (__("caption.target_class"), self.target_class),
            (__("caption.learn_rate"), self.alpha),
            (__("caption.stochastic"), str(self.stochastic))
        )
        if self.stochastic:
            caption_items += ((__("caption.stochastic_step_size"), self.step_size),)
