...
def __(key):
    return i18n.t('educational.owpolynomialclassification.' + key)
...
class OWPolynomialClassification(OWBaseLearner):
    ...
    class Inputs(OWBaseLearner.Inputs):
        learner = Input("Learner", InputLearner, label=i18n.t("educational.common.learner"))

    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output("Coefficients", Table, default=True, label=i18n.t("educational.common.coefficient"))
    ...
    learner_name = settings.Setting(__("placeholder_name"))
    ...
    class Error(OWBaseLearner.Error):
        to_few_features = Msg(
            __("msg.polynomial_class_require_two_num_feature"))
        no_class = Msg(__("msg.data_must_have_single_class_attr"))
        all_none_data = Msg(__("msg.feature_undefined_value"))
        no_classifier = Msg(__("msg.learner_must_be_classifier"))
    ...
    def add_main_layout(self):
        ...
        # options box
        self.options_box = gui.widgetBox(self.controlArea, __("box_option"))
        opts = dict(
        ...
        self.cbx = gui.comboBox(
            value='attr_x', label=__("row.x"), callback=self.apply, **opts_combo)
        self.cby = gui.comboBox(
            value='attr_y', label=__("row.y"), callback=self.apply, **opts_combo)
        self.target_class_combobox = gui.comboBox(
            value='target_class', label=__("row.target"),
            callback=self.apply, **opts_combo)
        self.degree_spin = gui.spin(
            value='degree', label=__("row.polynomial_expansion"),
        ...
        self.contour_step_slider = gui.spin(
            ...
            label=__("row.contour_step"), decimals=2, spinType=float,
    ...
    def send_report(self):
        ...
        caption = report.render_items_vert((
            (__("report.polynomial_expansion"), self.degree),
        ))
