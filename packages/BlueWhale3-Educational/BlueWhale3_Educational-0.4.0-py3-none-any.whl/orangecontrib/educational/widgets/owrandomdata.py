...
def __(key):
    return i18n.t('educational.owrandomdata.' + key)
...
class Bernoulli(ParametersEditorDiscrete):
    name = __("box.bernoulli_distribution")
    parameters = (ParameterDef(__("row.probability"), "p", 0.5, prob_float),)
...
class ContinuousUniform(ParametersEditorContinuous):
    name = __("box.uniform_distribution")
    parameters = (
        ParameterDef(__("row.low_bound"), "loc", 0, any_float),
        ParameterDef(__("high_bound"), "scale", 1, any_float)
    )
...
class DiscreteUniform(ParametersEditorDiscrete):
    name = __("box.discrete_uniform_distribution")
    parameters = (ParameterDef(__("row.num_of_value"), "k", 6, pos_int),)
...
class Multinomial(ParametersEditorContinuous):
    name = __("box.multinomial_distribution")
    parameters = (
        ParameterDef(__("row.probabilities"), "ps", "0.5, 0.3, 0.2", str),
        ParameterDef(__("row.num_of_trial"), "n", 100, pos_int)
    )
...
class HyperGeometric(ParametersEditorContinuous):
    name = __("box.hypergeometric_distribution")
    parameters = (
        ParameterDef(__("row.num_of_object"), "M", 100, pos_int),
        ParameterDef(__("row.num_of_positive"), "n", 20, pos_int),
        ParameterDef(__("row.num_of_trial"), "N", 20, pos_int)
    )
...
class BivariateNormal(ParametersEditor):
    name = __("box.bivariate_normal_distribution")
    parameters = (
        ParameterDef(__("row.mean_x"), "mu1", 0, any_float),
        ParameterDef(__("row.variance_x"), "var1", 1, pos_float),
        ParameterDef(__("row.mean_y"), "mu2", 0, any_float),
        ParameterDef(__("row.variance_y"), "var2", 1, pos_float),
        ParameterDef(__("row.covariance"), "covar", 0.5, pos_float)
    )
    nvars = 2
...
dist_defs = [
    cd(__("box.normal_distribution"), stats.norm.rvs,
       (__("row.mean"), "loc", 0, any_float),
       (__("row.variance"), "scale", 1, pos_float)),
    Bernoulli,
    cd(__("box.binomial_distribution"), stats.binom.rvs,
       (__("row.num_of_trial"), "n", 100, pos_int),
       (__("row.probability_of_success"), "p", 0.5, prob_float)),
    ContinuousUniform,
    DiscreteUniform,
    Multinomial,
    HyperGeometric,
    cd(__("box.negative_binomial_distribution"), stats.nbinom.rvs,
       (__("row.num_of_success"), "n", 10, pos_int),
       (__("row.probability_of_success"), "p", 0.5, prob_float)),
    cd(__("box.poisson_distribution"), stats.poisson.rvs,
       (__("row.event_rate"), "mu", 5, pos_float)),
    cd(__("box.exponential_distribution"), stats.expon.rvs),
    cd(__("box.gamma_distribution"), stats.gamma.rvs,
       (__("row.shape"), "a", 2, pos_float),
       (__("row.scale"), "scale", 2, pos_float)),
    cd(__("box.student_distribution"), stats.t.rvs,
       (__("row.degree_of_freedom"), "df", 1, pos_float)),
    BivariateNormal
]
...
class OWRandomData(OWWidget):
    ...
    class Error(OWWidget.Error):
        sampling_error = \
            Msg(__("msg_sample_error_check_argument"))
    ...
    distributions = Setting([
        (__("box.normal_distribution"),
         {'number_of_vars': '10', 'name_prefix': '', 'loc': '0', 'scale': '1'}),
        (__("box.binomial_distribution"),
