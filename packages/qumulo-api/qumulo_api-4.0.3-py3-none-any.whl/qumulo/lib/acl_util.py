...
class AclEditor:
    ...
    def _find(
        ...
        if position is not None:
            if not all(
                (trustee is None, ace_type is None, rights is None, flags is None)
            ):
        ...
        if len(res) > 1 and not allow_multiple:
            raise ValueError(
                'Expected to find exactly 1 entry, but found {}'.format(len(res))
            )
    ...
    def modify(
        ...
        """
        indices = self._find(
            position, old_trustee, old_type, old_rights, old_flags, allow_multiple
        )
