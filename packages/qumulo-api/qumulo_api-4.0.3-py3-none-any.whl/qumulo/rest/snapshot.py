...
def create_snapshot(
    ...
    if path != None:
        id_ = qumulo.rest.fs.get_file_attr(conninfo, credentials, path=path).lookup(
            'file_number'
        )
    if id_ != None:
...
@request.request
def create_policy(
    conninfo, credentials, name, schedule_info, directory_id=None, enabled=None
):
...
def get_snapshot_tree_diff(
    ...
    method = 'GET'
    uri = UriBuilder(
        path='/v2/snapshots/{:d}/changes-since/{:d}'.format(newer_snap, older_snap)
    )
...
def get_all_snapshot_file_diff(
    conninfo, credentials, newer_snap, older_snap, path=None, file_id=None, limit=None
):
    uri = get_snapshot_file_diff_uri(newer_snap, older_snap, path, file_id)
