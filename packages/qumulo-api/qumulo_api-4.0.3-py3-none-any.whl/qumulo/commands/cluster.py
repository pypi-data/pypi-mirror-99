...
class SetClusterConfCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def main(conninfo, credentials, _args):
        print(cluster.put_cluster_conf(conninfo, credentials, _args.cluster_name))


class SetSSLCertificateCommand(qumulo.lib.opts.Subcommand):
    NAME = 'ssl_modify_certificate'
    SYNOPSIS = (
        'Set the SSL certificate chain and private key for the '
...
class SetClusterSlotConfigCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = (
        'Set the attributes for the given cluster slot.  '
...
class SetNodeUidLight(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            'light_state', choices=['on', 'off'], help='Should light be visible'
        )
    ...
    def main(conninfo, credentials, args):
        ...
        print(
            cluster.set_node_identify_light(
                conninfo, credentials, args.node, light_visible
            )
        )
...
class CreateCluster(qumulo.lib.opts.Subcommand):
    ...
    def options(cls, parser):
        ...
        parser.add_argument('--admin-password', '-p', help='Administrator Pasword')
        parser.add_argument(
            '--max-drive-failures',
    ...
    def main(cls, conninfo, credentials, args):
        ...
        print(
            cluster.create_cluster(
                ...
                node_ips=node_ips,
                max_drive_failures=cls.max_drive_failures_choices[
                    args.max_drive_failures
                ],
