...
class MonitorNetworkCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument(
            '--interface-id', type=int, default=1, help=argparse.SUPPRESS
        )
            ...
            if args.version == 1:
                ...
                print(
                    network.list_network_status_v2(
                        conninfo, credentials, args.interface_id
                    )
                )
...
class GetNetwork(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        print(
            network.get_network(
                conninfo, credentials, args.interface_id, args.network_id
            )
        )
...
class ModInterface(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            '--default-gateway', help='The default IPv4 gateway address'
        )
        parser.add_argument(
            '--default-gateway-ipv6', help='The default IPv6 gateway address'
        )
        parser.add_argument(
            '--bonding-mode',
    ...
    def main(conninfo, credentials, args):
        ...
        print(
            network.modify_interface(
                conninfo, credentials, args.interface_id, **attributes
            )
        )
...
class AddNetwork(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument('--assigned-by', default='STATIC', help=argparse.SUPPRESS)
    ...
    def main(conninfo, credentials, args):
        ...
        attributes['ip_ranges'] = parse_comma_deliminated_ip_ranges(
            attributes.get('ip_ranges')
        )
        floating_input = attributes.get('floating_ip_ranges', None)
        if floating_input:
            attributes['floating_ip_ranges'] = parse_comma_deliminated_ip_ranges(
                floating_input
            )
        dnsservers_input = attributes.get('dns_servers', None)
        if dnsservers_input:
            attributes['dns_servers'] = parse_comma_deliminated_ip_ranges(
                dnsservers_input
            )
        ...
        print(
            network.add_network(conninfo, credentials, args.interface_id, **attributes)
        )
