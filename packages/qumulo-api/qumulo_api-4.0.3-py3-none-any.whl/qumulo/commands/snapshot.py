...
class ModifySnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument(
            '-i', '--id', type=str_decode, required=True, help='ID of the snapshot.'
        )
        group = parser.add_mutually_exclusive_group(required=False)
        group.add_argument(
            '-e',
...
class ListAllSnapshotsCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        print(
            snapshot.list_snapshots(conninfo, credentials, include_in_delete=args.all)
        )
...
def add_hourly_specific_args(hourly_parser):
    ...
    hourly_parser.add_argument(
        '-p', '--period', type=str_decode, required=True, help=PERIOD_HELP
    )
    ...
    print(
        snapshot.modify_policy(
            ...
            schedule_info=get_schedule_info(
                get_schedule_hourly_or_less(args), args.time_to_live
            ),
            enabled=args.enabled,
        )
    )
...
def modify_daily(conninfo, credentials, args):
    print(
        snapshot.modify_policy(
            ...
            name=args.name,
            schedule_info=get_schedule_info(
                get_schedule_daily(args), args.time_to_live
            ),
            enabled=args.enabled,
        )
    )
...
def modify_monthly(conninfo, credentials, args):
    print(
        snapshot.modify_policy(
            ...
            name=args.name,
            schedule_info=get_schedule_info(
                get_schedule_monthly(args), args.time_to_live
            ),
            enabled=args.enabled,
        )
    )
...
class ModifyPolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        modify_non_schedule_fields_parser.set_defaults(func=modify_non_schedule_fields)
