...
#Spi Command 
COMMAND_READ = 0
COMMAND_READ_STATUS1 = 1
COMMAND_READ_STATUS2 = 2
COMMAND_WRITE = 4
COMMAND_WRITE_STATUS1 = 5
COMMAND_WRITE_STATUS2 = 6
COMMAND_CHIP_ERASE = 8
COMMAND_SECTOR_ERASE = 9
COMMAND_BLOCK_ERASE_32K = 10
COMMAND_BLOCK_ERASE_64K = 11
...
def WaitDone(device, spiCmd) :
    ...
    bRet = False
    while 1 :
        c = device.ReadReg1Byte(Register.ANA6706.SPICTL, byteOffset)
        if c == -1 :
...
def ReadStatus(device, command) :
    ...
    regValue[4] = spiCmd
    byteOffset=0
    isOK = device.WriteReg(Register.ANA6706.SPICTL, byteOffset, len(regValue), regValue) 
    if isOK != True :
    ...
    byteOffset = 0
    status = device.ReadReg1Byte(Register.ANA6706.SPIDATA, byteOffset)
    if status == -1 :
        raise AssertionError(ModuleName+ "." + "ReadStatus")
    return status
...
def WriteStatus(device, command, status) :
    ...
    byteOffset = 0
    bRet = device.WriteReg1Byte(Register.ANA6706.SPIDATA, byteOffset, status)
    if bRet != True :
...
def ReadDataFromHW(device, memAddr, readByteSize, dataList, dataListStartIdx=0) :
    ...
    regValue[4] = spiCmd;
    isOK = device.WriteReg(Register.ANA6706.SPICTL, 0 , len(regValue), regValue) 
    assert isOK 

    bRet = WaitDone(device, spiCmd)

    if bRet :
        isOK = device.ReadReg(Register.ANA6706.SPIDATA, 0, readByteSize, dataList, dataListStartIdx)
        assert isOK 
...
def WriteDataToHW(device, memAddr, writeByteSize, writeData, writeDataStartIdx) : 
    ...
    isOK = device.WriteReg(Register.ANA6706.SPIDATA, 0 , writeByteSize, writeData, writeDataStartIdx) 
    assert isOK 
