...
def on_message(client: mqtt.Client, userdata, msg):
    ...
    try:
        ...
        if command == 'train':
            ...
            reply_to_server({'success': True, 'command': command})
        elif command == 'search':
            ...
            if len(databases) != 0:
                ...
                for d in databases:
                    d.pop('path', None)
                msg = {'success': True, 'command': command, 'databases': databases, 'count': len(databases)}
                reply_to_server(msg)
        ...
        reply_to_server({'success': False, 'command': command, 'msg': f"Command `{command}` is not implemented"})
    except KeyError:
...
def start_mqtt_service(block=False):
    if block:
        client.loop_forever()
    else:
        client.loop_start()
