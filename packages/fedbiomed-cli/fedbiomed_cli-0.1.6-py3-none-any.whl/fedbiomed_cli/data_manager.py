...
def load_default_database(name: str, path, as_dataset=False):
...
def load_images_dataset(folder_path, as_dataset=False):
...
def load_as_dataloader(dataset):
    name = dataset['data_type']
    if name == 'default':
        return load_default_database(name=dataset['name'], path=dataset['path'], as_dataset=True)
    elif name == 'images':
        return load_images_dataset(folder_path=dataset['path'], as_dataset=True)
...
def load_data(tags: Union[tuple, list], mode: str):
    ...
    if os.path.isfile(dataset_path):
        ...
        if mode == 'torch_dataset':
            return load_as_dataloader(dataset)
        elif mode == 'torch_tensor':
