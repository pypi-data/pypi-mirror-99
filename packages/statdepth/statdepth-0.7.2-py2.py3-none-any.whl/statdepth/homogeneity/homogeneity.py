...
def _functionalhomogeneity(
    ...
    G_depths = FunctionalDepth(data=G, K=K, J=J, containment=containment, relax=relax, deep_check=deep_check, quiet=quiet)
    ...
    if len(F) == 1:
        ...
        G_deep_in_F = FunctionalDepth([F], to_compute=['g_deepest'], K=K, J=J, containment=containment, relax=relax, deep_check=deep_check, quiet=quiet).ordered().loc['g_deepest']
