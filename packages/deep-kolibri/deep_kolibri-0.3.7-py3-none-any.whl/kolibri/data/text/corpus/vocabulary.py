...
DEFAULT_NON_PADDED_NAMESPACES = ("*tags", "*y_values")
...
class Vocabulary():
    ...
    def __init__(
        ...
        self._token_to_index = defaultdict()
        self._index_to_token = defaultdict()
    ...
    def from_files(
        ...
        for namespace_filename in os.listdir(directory):
            ...
            vocab.set_from_file(filename, True, namespace=namespace, oov_token=oov_token)
    ...
    def set_from_file(
        ...
        use that vocabulary file instead of just setting the vocabulary from a dataset_train, for
    ...
    def from_instances(
        ...
        """
        logger.info("Fitting token dictionary from dataset_train.")
        padding_token = padding_token if padding_token is not None else DEFAULT_PADDING_TOKEN
        oov_token = oov_token if oov_token is not None else DEFAULT_OOV_TOKEN
    ...
    def print_statistics(self) -> None:
        if self._retained_counter:
            ...
            for namespace in self._retained_counter:
                ...
                # Now sort by token length_train, not frequency
            ...
            logger.info(
                "Vocabulary statistics cannot be printed since dataset_train instances were not used for its construction."
            )
