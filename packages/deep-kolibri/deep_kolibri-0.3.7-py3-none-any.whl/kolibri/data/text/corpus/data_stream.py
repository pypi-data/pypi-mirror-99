...
class DataStream(BaseStream):
    """ Creates a stream from a texts source.

    DataStream takes the whole texts set containing the `X` (features) and `Y` (targets) or takes `X` and `Y` separately.
    ...
    def get_y_generator(self, index):
        for y in self._y:
            yield y[index]
    ...
    def data(self):
        """
        Return the texts set used to generate the stream.
    ...
    def data(self, data):
        """
        Sets the texts set used to generate the stream.
        ...
        if isinstance(data, pd.DataFrame):
            ...
            raise ValueError("Invalid type {}, for texts".format(type(data)))
    ...
    def _load_X_y(self):
        ...
        if np.issubdtype(self.y.dtype, np.integer) or np.issubdtype(self.y.dtype, np.object):
    ...
    def get_data_info(self):
        ...
        if self.task_type == self._CLASSIFICATION:
            return "{}{} label(s), {} classes".format(name, self.n_targets, self.n_classes)
        elif self.task_type == self._REGRESSION:
            return "{} label(s)".format(name, self.n_targets)
...
def check_data_consistency(raw_data_frame, allow_nan=False):
    """
    Check texts consistency with respect to scikit-multiflow assumptions:

    * Only numeric texts types are used.
    * Missing values are, in general, not supported.

    Parameters
    ----------
    raw_data_frame: pandas.DataFrame
        The texts frame containing the texts to check.

    allow_nan: bool, optional (default=False)
        If True, allows NaN values in the texts. Otherwise, an error is raised.
