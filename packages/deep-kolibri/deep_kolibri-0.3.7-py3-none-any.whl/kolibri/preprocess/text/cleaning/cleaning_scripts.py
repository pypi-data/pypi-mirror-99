import regex as re
import unicodedata
#from kolibri.preprocess.text.scripts.email2text import EmailMessage
...
#    email = EmailMessage(text).read()
    ...
    for f in email.fragments:
        emails.append(
            {'salutation': f.salutation, 'body': f.body, 'signature': f.signature, 'disclaimer': f.disclaimer})

    text = re.sub(u"##- Please type your reply above this line -##", '', text)
    text = re.sub(
        u'(Information )?Classification:\s*(Highly Confidential|Internal|Public|Confidential|(ll\s?)?Limited Access)',
        '', text)
...
def fix_formating(text):
    ...
    text = re.sub(r'\.FW:', u'. FW:', text)
    text = re.sub(r'([a-z]\\\\:\*)|(shape\s)?\{[a-zA-Z#:; \(\)]+\}', u'', text)
    text = text.replace('...', '.')
    text = text.replace('..', '.')
...
def clean_email(email):
    return clean_text(fix_formating(parse_email(fix_contractions(email))))
...
def abstract_email(email):
    ...
    text = re.sub(
        ...
        '<EMAIL>', text)
    text = re.sub(r"\b(1(\s|-)?)?\(?[0-9]{3}\)?\s*[\.-]?[0-9]{3,4}[\.\s\-][0-9]{4}(\sext\.\s*\d+)?\b", ' <PHONENUMBER>',
                  text)
    text = re.sub(r"(\+91)\s\(?\d{2,3}\)?\s*[\.-]?[0-9]{3,4}[\.\s\-]?[0-9]{4}(\sext\.\s*\d+)?\b", ' <PHONENUMBER>',
                  text)
    text = re.sub(r"(?:[0123]?[0-9][-\./]\w+[-\./][12][0-9][0-9][0-9])(\s?\d{2}[:]\s?\d{2}[:]\s?\d{2})?(\sAE(S|D)T)?",
                  ' <DATE>', text)
    ...
    text = re.sub(r'\b((Req(uest)?\s+(ID|number)\s*[.:])|request)\s*\w{2}(\d{3,}-?)+\b', '<REQUESTID>', text,
                  flags=re.I)
    ...
    text = re.sub(
        r'\b((\d+)|(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety| ))\s+(seconds?|minutes?|hours?|days?|weeks?|months?|years?)\b',
        '<DURATION>', text)
    text = re.sub(r'\b(e|perner\.?)?\d{5,8}\b', '<EMPLOYEEID>', text, flags=re.I)
    text = re.sub(r'\b(?<!-)\d{7,9}\b\b(?!-)', '<EMPLOYEEID>', text)
    text = re.sub(r'\b(\d{1,2})(\s|-)(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s\d{1},?(\s\d{2,4})?\b',
                  '<DATE>', text, flags=re.I)

    text = re.sub(
        r'\b((1st|2nd|\d{1,2}rd)|(\d{1,2})(th|st)?)(\s|-)?(J(?:an(uary)?|u(?:ne?|ly?))|Feb(ruary)?|Ma(?:r(ch)?|y)|A(?:pr(il)?|J|ug(ust)?)|(?:(?:(?:Sept?|Nov|Dec)(em)?)|Octo?)(ber)?),?(\s|-|\')?\d{2,4}(\s\d{2}:\d{2}(\s?(AM|PM))?)?\b',
        ' <DATE>', text, flags=re.I)
    text = re.sub(
        r'\b((1st|2nd|\d{1,2}rd)|(\d{1,2})(th|st)?)(\s|-)?(J(?:an(uary)?|u(?:ne?|ly?))|Feb(ruary)?|Ma(?:r(ch)?|y)|A(?:pr(il)?|J|ug(ust)?)|(?:(?:(?:Sept?|Nov|Dec)(em)?)|Octo?)(ber)?)\b',
        ' <DATE>', text, flags=re.I)
    text = re.sub(
        r'\b(((Mon|Tues?|Wed(nes)?|Thu(rs)?|Fri|Sat(ur)?|Sun))(day)?,?\s+)?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+((1st|2nd|\d{1,2}rd)|(\d{1,2})(th)?)\b',
        '<DATE>', text, flags=re.I)
    text = re.sub(
        r'((\d{2})\s)?(J(?:anuary|u(?:ne|ly))|February|Ma(?:rch|y)|A(?:pril|ugust)|(?:(?:(?:Sept|Nov|Dec)em)|Octo)ber),?(\s(1st|2nd|\d{1,2}(th|st)?,?))?(\s\d{4})?(\s*\d{2}:\d{2})?\b',
        ' <DATE> ', text)
    text = re.sub(
        r'((Mon|Tues?|Wed(nes)?|Thu(rs)?|Fri|Sat(ur)?|Sun))(day)?\s*\d{1,2}(st|th)?([\.\\-]\d{1,2})?(\s*(a|p)\.?m\.?)?',
        '<DATE>', text)
    text = re.sub(
        r'\b(J(?:an(uary)?|u(?:ne?|ly?))|Feb(ruary)?|Ma(?:r(ch)?|y)|A(?:pr(il)?|J|ug(ust)?)|(?:(?:(?:Sept?|Nov|Dec)(em)?)|Octo?)(ber)?)(,|\s*-)?(\s*(1st|2nd|\d{1,2}(th)?,?))?(\s\d{4})?(\s*\d{2}:\d{2})?\b',
        '<DATE>', text, flags=re.I)
    text = re.sub(
        r'\b(J(?:anuary|u(?:ne|ly))|February|Ma(?:rch|y)|A(?:pril|ugust)|(?:(?:(?:Sept|Nov|Dec)em)|Octo)ber)(\s?\d{2,4})\b',
        ' <DATE> ', text, flags=re.I)
