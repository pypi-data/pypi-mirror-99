...
class ConditionalRandomField(L.Layer):
    ...
    def __init__(self, output_dim, transitions=None, **kwargs):
    ...
    def call(self, inputs, sequence_lengths=None, **kwargs):
        ...
        viterbi_sequence, _ = tfa.text.crf.crf_decode(
    ...
    def sparse_loss(self, y_true, y_pred):
        ...
        # Convert to one hot
        y_true = K.one_hot(y_true, K.shape(self.transitions)[0])
    ...
    @property
    def sparse_viterbi_accuracy(self):
        def accuracy(y_true, y_pred):
            # y_true Need to re-clarify shape and dtype
            y_true = K.reshape(y_true, K.shape(y_pred)[:-1])
            y_true = K.cast(y_true, 'int32')
        ...
        accuracy.func_name = "sparse_viterbi_accuracy"
        return accuracy
