    ...
    def predict(self, x_data, *,
        ...
        with kolibri.dnn.custom_object_scope():
            ...
            if self.multi_label:
                ...
                labels=np.array([self.label_indexer.inverse_transform([i for i in range(0, len(p))]) for p in pred])
...
#                logger.debug(f'predict output: {res}')
        ...
        return labels[np.arange(labels.shape[0])[:, None], sorted_indices], pred[np.arange(pred.shape[0])[:, None], sorted_indices]
