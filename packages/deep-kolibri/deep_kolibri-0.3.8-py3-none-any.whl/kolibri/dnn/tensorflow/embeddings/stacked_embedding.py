...
class StackedEmbedding(Embedding):
    ...
    def _load_saved_instance(cls,
        ...
        for layer in instance.embed_model.layers:
            layer.set_weights(tf_model.get_layer(layer.my_name).get_weights())
        return instance
    ...
    def __init__(self,
        ...
        if all(isinstance(embed.sequence_length, int) for embed in embeddings):
            ...
            raise ValueError('Need to set sequence length_train for all embeddings while using stacked embedding')
        ...
        self.indexer = embeddings[0].indexer
