...
class ClassifierEvaluator(object):
    ...
    def cross_validate_predict(self, classifier, X, y, folds=4, ratio_to_use=1, n_jobs=4):
        ...
        try:
            pred = cross_val_predict(classifier, X, y, cv=cv, n_jobs=-1, method='predict_proba', verbose=1)
            return pred
        except:
            self.compute_proba = False
            return cross_val_predict(classifier, X, y, cv=cv, n_jobs=-1, verbose=1)
    @staticmethod
    ...
    def get_performance_report(y_true=None, y_pred=None, target_names=None):
        ...
        report['confusion_matrix'] = ClassifierEvaluator.confusion_matrix(y_true, y_pred, target_names).to_dict()
        cr = ClassifierEvaluator.classification_report(y_true, y_pred, target_names)
