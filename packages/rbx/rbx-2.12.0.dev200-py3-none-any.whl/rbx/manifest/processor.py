...
class Creative:
    ...
    def __init__(self, manifest_url, client):
        ...
        self.client = client
        self.pixel_ratios = self.manifest.get('pixel_ratios')
    ...
    def render(self):
        ...
        for pixel_multiplier, pixel_name in self.pixel_ratios.items():
            pixel_multiplier = int(pixel_multiplier)
            root_image = make_white_image(
                height=self.height*pixel_multiplier,
                width=self.width*pixel_multiplier
            )

            ordered_images = self.root_component.compile(
                pixel_ratio=SimpleNamespace(**{
                    'name': pixel_name,
            ...
            for entry in flattened_images:
                root_image = merge_images(
                    ...
                    overlay_image=entry,
                    left=self.left*pixel_multiplier,
                    top=self.top*pixel_multiplier,
                    transparent_background=False
                )
            creatives.append(SimpleNamespace(
                filename=f'final_image{pixel_name}.png',
                name=generate_name(multiplier=pixel_multiplier),
                image=root_image
            ))
...
class LayoutComponent(Component):
    ...
    def compile(self, pixel_ratio):
        ...
        Parameters:
            pixel_ratio (SimpleNamespace): A namespace with the name and multiplier to apply.

        Returns:
            list:
                A list of ndarray images, each applied to the transparent canvas.
        """
        transparent_image = make_transparent_image(
            self.height*pixel_ratio.multiplier, self.width*pixel_ratio.multiplier)
        ...
        for entry in sorted(self.components, key=operator.attrgetter("z_index")):
            ...
            for flat_entry in flattened_entry:
                images.append(merge_images(
                    ...
                    left=entry.left*pixel_ratio.multiplier,
                    top=entry.top*pixel_ratio.multiplier,
...
class ImageComponent(Component):
    ...
    def compile(self, pixel_ratio):
        ...
        """
        return [self.render(pixel_ratio)]

    def render(self, pixel_ratio):
        ...
        Returns:
            ndarray:
                An opencv representation of the retrieved image,
        """
        return self.client.get_image(f'{self.url}{pixel_ratio.name}{self.ext}')
