...
from .processor import AssetFetcher, CreativeBuilder, zip_images
...
def build_image_from_manifest(url, filename, mode, bucket, region):
    ...
    image_data_list = builder.build()
    click.echo("Images Generated!")

    zip_images(image_data_list, filename=filename)
    click.echo(f"Images Now Available in {filename}.zip")
