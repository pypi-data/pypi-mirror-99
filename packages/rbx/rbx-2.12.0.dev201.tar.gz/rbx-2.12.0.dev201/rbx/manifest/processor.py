...
class ComponentSequence(MutableSequence):
    ...
    def append(self, component, client, modifiers):
        result = make_component(component, client, modifiers)
...
class CreativeBuilder:
    ...
    for calculating all variations, passing one set of modifiers to each Creative for rendering.
    This allows us to calculate and apply the modifier logic in a central location.
    """
    def __init__(self, manifest_url, client):
        self.manifest = client.get_manifest(manifest_url)
        self.client = client
        self.variations = self._unpack_variations()

    def _unpack_variations(self):
        """Calculates all variations that need to be generated for a given manifest.

        Returns:
            list:
                A list of SimpleNamespace objects that describe any modifiers that need to be
                applied.
        """
        variations = []
        ...
        for pixel_multiplier, pixel_name in pixel_ratios.items():
            variations.append(
                SimpleNamespace(**{
                    'name': pixel_name,
                    'multiplier': int(pixel_multiplier)
                })
            )
        return variations
    ...
    def build(self):
        ...
        Returns:
            list:
                A list of SimpleNamespace objects that describe the filename, human readable name
                and image data.
        """
        creatives = []
        ...
        for variation in self.variations:
            ...
            creatives.append(SimpleNamespace(
                filename=f'final_image{variation.name}.png',
                name=generate_name(multiplier=variation.multiplier),
...
class Creative:
    ...
    def __init__(self, client, manifest, modifiers):
        self.manifest = manifest
        self.client = client
        self.modifiers = modifiers

        root = self.manifest.get('root_component')
        self.height = px_to_int(root['css']['height']) * modifiers.multiplier
        self.width = px_to_int(root['css']['width']) * modifiers.multiplier
        self.left = px_to_int(root['css']['left']) * modifiers.multiplier
        self.top = px_to_int(root['css']['top']) * modifiers.multiplier

        self.root_component = make_component(root, self.client, modifiers)
...
class Component(ComponentSequence):
    ...
    def __init__(self, component, client, modifiers):
        super(Component, self).__init__()
        self.client = client
        self.modifiers = modifiers
        self.top = px_to_int(component['css']['top']) * modifiers.multiplier
        self.left = px_to_int(component['css']['left']) * modifiers.multiplier
        self.height = px_to_int(component['css']['height']) * modifiers.multiplier
        self.width = px_to_int(component['css']['width']) * modifiers.multiplier
...
class LayoutComponent(Component):
    ...
    def __init__(self, component, client, modifiers):
        ...
        Parameters:
            ...
            modifiers (SimpleNamespace): The modifiers to apply to the component.
        """
        ...
        for child_component in children:
            self.append(child_component, self.client, self.modifiers)
    ...
    def render(self):
        ...
        Returns:
            ndarray:
                An opencv representation of the retrieved image,
        """
        return self.client.get_image(f'{self.url}{self.modifiers.name}{self.ext}')
...
def generate_name(multiplier):
    ...
    if multiplier > 1:
        f'Retina {name} {multiplier}x Ratio'

    return name


def make_component(component, client, modifiers):
    ...
    if component_type == 'layout':
        return LayoutComponent(component, client, modifiers)

    elif component_type == 'image':
        return ImageComponent(component, client, modifiers)
...
def zip_images(images, filename=None):
    """Combine all images in the iterable to a ZIP format.
        ...
        Parameters:
            images (list):
                A list of namespaces with the filename, human name and ndarray image data.
            filename (str):
