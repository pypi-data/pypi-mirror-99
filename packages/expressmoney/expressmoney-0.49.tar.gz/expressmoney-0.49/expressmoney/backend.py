import requests
from expressmoney.constants import Status


class Api(Status):
    """Взаимодействие с api.expressmoney.com"""

    api = "https://api.expressmoney.com/{}/"
    ...
    def update(self, lookup_field, status, comment):
        ...
        if status not in [self.NEW, self.IN_PROCESS, self.SUCCESS, self.FAILURE, self.ERROR, self.CANCEL]:
