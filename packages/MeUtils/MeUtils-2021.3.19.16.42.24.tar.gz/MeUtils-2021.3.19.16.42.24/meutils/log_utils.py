...
from meutils.models.conf import WebhooksConf
...
# todo: 起个服务配置通用logger
def logger4feishu(title='这是一个标题', text='这是一条log', group='logger'):
    """http://r6d.cn/bbafq"""

    url = WebhooksConf.init().__getattribute__(group)
    return request(url, json={'title': title, 'text': text})
