...
host = url.host; port = url.port; username = url.username; password = url.password
pikadb = Client(
    host=host,
    port=port,
    username=username,
...
pikadb.jsonset = pikaset
pikadb.jsondel = pikadel
pikadb.jsonget = pikaget
pikadb.jsonarrappend = pikaseta
pikadb.jsonarrpop = pikageta
...
def pset(client, data, value):
    return pikaset("cdata", Path("{client.data}", value)

def pget(client, data):
    return pikaget("cdata", Path(f"{client.data}")

def pdel(client, value)
    return pikadel("cdata", Path(f"{client.data}")
