...
class Connect(esbmtkBase):
    """Two reservoirs connect to each other via at least one flux. This
     ...
     Example::
         ...
         Connect(source =  upstream reservoir,
                ...
                bypass :str = "upstream"/"downstream"
                )
    ...
    def __scaleflux__(self) -> None:
        ...
        if self.bypass == "upstream":
            target = self.sink
        elif self.bypass == "downstream":
            target = self.source
        elif self.bypass == "None":
            target = self.r
        else:
            raise ValueError(
                f"bypass must be None/upstream/downstream but not {self.bypass}"
            )
