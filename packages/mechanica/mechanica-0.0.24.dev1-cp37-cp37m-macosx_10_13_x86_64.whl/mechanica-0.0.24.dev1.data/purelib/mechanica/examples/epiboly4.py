...
p_cell  = m.Potential.glj(e=0.05, m=4,  min=0.3, max=1.5)
...
p_cellactin  = m.Potential.harmonic(k=1, r0=0.5, min=0.5, max=1.5)
...
#p_actin = m.Potential.power(k=20, r0=0, alpha = 1, max = 5, tol=0.01)
p_actin = m.Potential.power(k=-20, r0=0, alpha = -1, min=0.1, max = 5, tol=0.05)
...
def update(e):
    print(m.Universe.time)



def prune_extra_cells():
    cells = Cell.items().copy()

    for p in cells:
        if p.spherical_position()[2] > epiboly_percent * np.pi:
            p.destroy()
...
def create_cells():
    ...
    cell_positions = m.random_points(m.SolidSphere, count, dr=dr, phi=(0, epiboly_percent * np.pi)) \
...
def create_actin():
    ...
    parts, bonds = m.bind_sphere(p_actin, type=Actin, n=3, \
                                 phi=(0.95 * epiboly_percent * np.pi, \
