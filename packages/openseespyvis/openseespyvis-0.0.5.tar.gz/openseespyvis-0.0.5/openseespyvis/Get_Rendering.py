...
def plot_modeshape(*argv,overlap="yes",Model="none"):
	...
	modeNumber : (int) 
		Mode number to be plotted.
		
	scale      : (int) Optional 
		input to change the scale factor of the deformed shape. Default is 10.
		
	overlap    : (str) Optional 
		keyword argument to turn overlap off. Default value is "yes".
		
	Model      : (str) Optional 
		input for the name of the model used in createODB() to read the modeshape data from.
	    The default is "none" and the mode shape is plotted from the active model.
