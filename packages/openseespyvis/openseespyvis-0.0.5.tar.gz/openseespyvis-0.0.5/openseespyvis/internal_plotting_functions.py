...
def _plotEle_3D(nodes, elements, DispNodeCoordArray, fig, ax, monitorOutput, dof, limitStates, MonitorEleInfo, MonitorEleTags, MonitorEleDef, TimeStep, show_element_tags):
        ...
        if len(tempNodes) == 2:
            ...
            if monitorElementDisplay == 'yes':
                if eletag in MonitorEleTags:
                    eleColor = idbf._elementMonitorCheck(eletag, dof, monitorOutput, limitStates, limStateColors, MonitorEleInfo, MonitorEleTags, MonitorEleDef, TimeStep)
                    outputs = _plotBeam3D(iNode, jNode, ax, show_element_tags, eletag, eleColor)
                else:
                    outputs = _plotBeam3D(iNode, jNode, ax, show_element_tags, eletag, "solid")
            else:
                outputs = _plotBeam3D(iNode, jNode, ax, show_element_tags, eletag, "solid")
