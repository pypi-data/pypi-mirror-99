...
async def test_duckling_entity_extractor(component_builder):
    with aioresponses() as mocked:
        ...
        await duckling.process(message)
    ...
    with aioresponses() as mocked:
        ...
        await duckling_number.process(message)
        entities = message.get("entities")
...
async def test_duckling_entity_extractor_and_synonyms(component_builder):
    ...
    await synonyms.process(message)
...
def test_spacy_ner_extractor(component_builder, spacy_nlp):
    ...
    asyncio.get_event_loop().run_until_complete(ext.process(example, spacy_nlp=spacy_nlp))
