...
HYDRA_CHANNEL = "OMNI_FACEBOOK_00001"
...
class Messenger:
    ...
    async def postback(
        ...
        if text == "get_started":
            await self._handle_user_message('/greet{"channelUserId": "'+self.get_user_id()+'"}', self.get_user_id())
        else:
            await self._handle_user_message(text, self.get_user_id(), metadata)
    ...
    async def _handle_user_message(
        ...
        if "greet" in text:
            ...
            result = await self.startTracking(channelUserId, HYDRA_CHANNEL, sender_id)
        ...
        user_msg = UserMessage(text, out_channel, sender_id, input_channel=HYDRA_CHANNEL, metadata=metadata)
    ...
    async def startTracking(self, channelUserId, channel, sid):
...
class MessengerBot(OutputChannel):
    ...
    def send(self, recipient_id: Text, element: Any) -> None:
        ...
        logger.info(f"[{recipient_id}] - Bot response: {element.to_dict()}")
