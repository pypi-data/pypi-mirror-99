...
class DialogueStateTracker:
    ...
    def from_dict(
        ...
        if len(events_as_dict) > 0:
            session_data["total_steps"] = events_as_dict[-1].get('total_steps', 0)
            session_data["total_errors"] = events_as_dict[-1].get('total_errors', 0)

        evts = events.deserialise_events(events_as_dict)
        return cls.from_events(sender_id, evts, slots, max_event_history, session_data)
    ...
    def from_events(
        ...
        tracker = cls(sender_id, slots, max_event_history, session_data)
        for e in evts:
            tracker.update(e)
        return tracker

    def __init__(self, sender_id, slots, max_event_history=None, session_data=None):
        ...
        self._reset()
        self.total_steps = session_data.get('total_steps', 0)
        self.total_errors = session_data.get('total_errors', 0)
        self.active_form = {}
    ...
    def from_voice_channel(self):
        """Check if the input channel is a voice channel"""
        return self.get_latest_input_channel() and "VOICE" in self.get_latest_input_channel()
    ...
    def recreate_from_dialogue(self, dialogue: Dialogue) -> None:
        ...
        self.total_steps = getattr(dialogue, "total_steps", 0)
        self.total_errors = getattr(dialogue, "total_errors", 0)
        self._reset()
    ...
    def update(self, event: Event, domain: Optional[Domain] = None) -> None:
        ...
        if isinstance(event, UserUttered) and self.latest_message:
            if self.conversation_ended() and event.intent.get("name") == USER_INTENT_DISCONNECT:
                logger.info(f"[{self.sender_id}] - Conversation has been ended. Ignoring duplicate disconnect messages.")
                return
    ...
    def conversation_ended(self):
        ...
        return self.latest_message and self.latest_message.intent.get("name") == USER_INTENT_DISCONNECT
