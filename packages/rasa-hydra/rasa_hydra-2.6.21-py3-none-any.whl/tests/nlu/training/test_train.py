...
async def test_train_model(pipeline_template, component_builder, tmpdir):
    ...
    assert await loaded.parse("hello") is not None
    assert await loaded.parse("Hello today is Monday, again!") is not None
...
async def test_random_seed(component_builder, tmpdir):
    ...
    result_a = (await loaded_a.parse("hello"))["intent"]["confidence"]
    result_b = (await loaded_b.parse("hello"))["intent"]["confidence"]
...
@pytest.mark.parametrize("language, pipeline", pipelines_for_tests())
async def test_load_and_persist_without_train(language, pipeline, component_builder, tmpdir):
    _config = RasaNLUModelConfig({"pipeline": pipeline, "language": language})
...
@pytest.mark.parametrize("language, pipeline", pipelines_for_tests())
async def test_train_with_empty_data(language, pipeline, component_builder, tmpdir):
    _config = RasaNLUModelConfig({"pipeline": pipeline, "language": language})
    trainer = Trainer(_config, component_builder)
