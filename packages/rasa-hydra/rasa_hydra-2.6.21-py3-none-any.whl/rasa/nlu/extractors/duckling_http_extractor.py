...
class DucklingHTTPExtractor(EntityExtractor):
    ...
    defaults = {
        ...
        # if not set the default timeout of duckling http url is set to 30 seconds.
        "timeout": 30,
    }
    ...
    async def _duckling_parse(self, text, reference_time):
        ...
        try:
            ...
            async with self.session.post(self._url() + "/parse", data=payload, headers=headers, timeout=30) as response:
            ...
            logger.info(f"Duckling request: {round(duration, 2)}s")
            OUTGOING_REQUEST_LATENCY_SEC.labels(self._url() + "/parse", status, "POST").observe(duration)
    ...
    async def process(self, message: Message, **kwargs: Any) -> None:

        if self._url() is not None:
            reference_time = self._reference_time_from_message(message)
            matches = await self._duckling_parse(message.text, reference_time)
