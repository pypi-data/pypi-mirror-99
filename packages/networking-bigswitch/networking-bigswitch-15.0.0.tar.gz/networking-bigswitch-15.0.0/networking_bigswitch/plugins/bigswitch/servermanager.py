...
OOSM_OSP_LAST_UPDATE_PATH = OOSM_OSP_RESOURCE_PATH + "/%s/last-update"
...
def get_novaauth_cfg(conf, name):
    ...
    try:
        ...
        if isinstance(value_list[0], list):
            # api change in train, needs more investigation
...
class ServerPool(object):
    ...
    def __init__(self, timeout=False,
        ...
        # cluster state sync interval
        self.state_sync_interval = cfg.CONF.RESTPROXY.state_sync_interval
    ...
    def start_background_tasks(self):
        ...
        if self.state_sync_interval > 0:
            if 'orchestrator-state-sync' in self.get_capabilities():
                eventlet.spawn_after(60,
                                     self.state_syncer.periodic_update,
    ...
    def rest_update_osp_cluster_info(self, data):
        ...
        # nsapi does not append origination for cluster info
        data['origination'] = self.neutron_id

        errstr = _("Unable to update cluster visibility info: %s")
        self.rest_action('POST', resource, data, errstr)

    def rest_get_osp_cluster_last_update(self):
        resource = OOSM_OSP_LAST_UPDATE_PATH % self.neutron_id
        errstr = _("Failed to get last update time for the cluster: %s")
        resp = self.rest_action('GET', resource, errstr=errstr)

        return resp[3].get('last-update')
