...
class NeutronRestProxyV2Base(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _get_router_intf_details(self, context, port, subnet_id):
        """This gets router interface for 1 single subnet

        For multiple subnets, this should be called multiple times
        :param context:
        :param port:
        :param subnet_id:
        :return:
        ...
        # every port can have multiple fixed_ips
        if port.get("fixed_ips"):
            for fixed_ip in port["fixed_ips"]:
                if fixed_ip['subnet_id'] == subnet_id:
                    data['ip_address'] = fixed_ip['ip_address']
                    break
