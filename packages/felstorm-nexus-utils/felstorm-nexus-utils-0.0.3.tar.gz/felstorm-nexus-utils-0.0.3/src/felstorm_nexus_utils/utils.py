...
from jinja2 import meta, ChoiceLoader, FunctionLoader, FileSystemLoader, ModuleLoader
...
def get_asset_url(nexus_name, filename):
    ...
    return f'/apps/{nexus_name}/assets/{filename}?ts={ts}'

def get_nexus_apps():
    return next(os.walk('apps/'))[1]
...
def get_render_form_kwargs(nexus_name):
    return {
        ...
        'config': load_json(f'apps/{nexus_name}/', 'config.json'),
        'nexus': nexus_name
    }
...
def get_render_nexus_kwargs(nexus_name):
    return {
        ...
        'config': load_json(f'apps/{nexus_name}/', 'config.json'),
        'form': {
            'fields': load_json(f'apps/{nexus_name}/', 'form.json'),
...
def render_form(nexus_name, **kwargs):
    app_package = FileSystemLoader([
        f'apps/{nexus_name}/templates', 'nexus_engine/templates'
    ])
...
def compile_app_templates(nexus_name):
    env = SandboxedEnvironment(
        loader=FileSystemLoader([
            f'apps/{nexus_name}/templates'
        ]),
        autoescape=True
    )
    env.compile_templates(f'apps/{nexus_name}/templates-compiled.zip')


def render_nexus(nexus_name, **kwargs):
    app_loader = FileSystemLoader([
        'nexus_engine/templates', f'apps/{nexus_name}/templates'
    ])
    env = SandboxedEnvironment(
        loader=app_loader,
        autoescape=True
