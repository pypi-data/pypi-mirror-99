...
try:
    from trytond.modules.printing.printing import print_report_file
except ImportError:
    print_report_file = None


logger = logging.getLogger(__name__)

REDIRECT_MODEL = config.get('printer_cups', 'redirect_model')
...
class Report(URLMixin, PoolBase):
    ...
    def execute(cls, ids, data):
        ...
        # Printing facility to CUPS provided by module printing
        if print_report_file and Transaction().context.get('printer_uri'):
            print_report_file(content, file_name=action_report.name,
                ext=oext)

        # Printing facility to CUPS provided by module printer_cups
        if REDIRECT_MODEL:
            Printer = None
            try:
                Printer = pool.get(REDIRECT_MODEL)
            except KeyError:
                logger.warning('Printing Redirect Model "%s" not found.' %
                    REDIRECT_MODEL)
            if Printer:
                return Printer.send_report(oext, content, action_report.name,
                    action_report)
