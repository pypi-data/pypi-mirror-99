...
class FaustAppWrapper(ABC):
    ...
    def _init_error_counting(self):
        """
        Defines method for counting errors and warnings that we want the app to crash on.
        """
        if 'DO_ERROR_COUNTING' in os.environ and os.environ['DO_ERROR_COUNTING']:
            LOGGER.info('Custom error counting ENABLED')

            @self.app.timer(30)
            async def evaluate_error_count():
                """
                Every thirty seconds - as set in @self.app.timer() - evaluates the number of
                warnings and errors logged from loggers defined via the LOGGERS_TO_WATCH env var that
                match any string passed in via the ERROR_STRINGS env var. If the count is greater
                than zero, crash the app. It should restart and, hopefully, recover.

                Because we can't override the root logger, every error that we want to count that
                originates from an imported module must have its logger overridden with our
                custom error counter (see init_error_counter_loggers).
                """
                loggers = [logging.getLogger(logger) for logger in os.environ['LOGGERS_TO_WATCH'].split(',')]
                error_counts = [logger.error_count for logger in loggers]

                if any(error_count > 0 for error_count in error_counts):
                    os._exit(1)
        else:
            LOGGER.info('Custom error counting DISABLED')
