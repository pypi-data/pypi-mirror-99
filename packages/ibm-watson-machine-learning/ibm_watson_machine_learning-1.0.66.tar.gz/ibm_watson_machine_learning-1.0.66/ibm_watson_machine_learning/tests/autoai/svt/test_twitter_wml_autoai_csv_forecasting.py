...
from ibm_watson_machine_learning.utils.autoai.errors import NotExistingCOSResource, ForecastingUnsupportedOperation
...
class TestAutoAIRemote(unittest.TestCase):
    ...
    data_cos_path = 'data/Twitter_volume_AMZN.csv'
    ...
    OPTIMIZER_NAME = 'COVID csv wml_autoai forecasting test'
    DEPLOYMENT_NAME = "COVID AutoAI Deployment tests"
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            prediction_type=self.experiment.PredictionType.FORECASTING,
            prediction_columns=[self.prediction_column],
    ...
    def test_09_predict_using_fitted_pipeline(self):
        with self.assertRaises(ForecastingUnsupportedOperation, msg="Operation should be unsupported"):
            self.remote_auto_pipelines.predict(X=self.X_values[:5])
    ...
    def test_14__get_pipeline__load_lale_pipeline__pipeline_loaded(self):
        with self.assertRaises(ForecastingUnsupportedOperation, msg="Operation should be unsupported"):
            self.remote_auto_pipelines.get_pipeline(pipeline_name='Pipeline_4')
    ...
    def test_15_get_all_pipelines_as_lale(self):
        ...
        for pipeline_name in summary.reset_index()['Pipeline Name']:
            ...
            with self.assertRaises(ForecastingUnsupportedOperation, msg="Operation should be unsupported"):
                self.remote_auto_pipelines.get_pipeline(pipeline_name=pipeline_name)
    ...
    def test_23_score_deployed_model(self):
        predictions = self.service.score(payload=pd.DataFrame({'Cases' : []}))
        print(predictions)
    ...
    def test_33_score_batch_deployed_model(self):
        scoring_params = self.service_batch.run_job(
            payload=self.train_data.drop(columns=['timestamp']),
            background_mode=False)
