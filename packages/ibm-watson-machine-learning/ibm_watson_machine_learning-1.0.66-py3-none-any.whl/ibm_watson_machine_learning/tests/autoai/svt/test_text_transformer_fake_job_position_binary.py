...
class TestAutoAIRemote(unittest.TestCase):
    ...
    y_values = None
    elapsed_time = None
    size_dataset = environ.get('SIZE_DATASET_MB', "10")

    data_location = './autoai/data/fake_job_postings_2MB.csv'


    if size_dataset == "1":
        asset_id = "7b912366-ec5f-4e40-9091-c14fb03f9ca4" # 1MB
    else:
        asset_id = "3253ef55-bf01-4084-adeb-7efd4cef00de" # 10MB

    prediction_column = "fraudulent"

    with_text_transformer = environ.get('TT_ENABLED', 'true').lower() in (True, 'true')
    estimator = environ.get('ESTIMATOR', None)

    OPTIMIZER_NAME = f"TEXT TRANSFORMER {with_text_transformer} binary {data_location.split('/')[-1]} {size_dataset} MB {estimator}"
    DEPLOYMENT_NAME = "TEXT TRANSFORMER AutoAI Deployment tests"

    project_id = None
    space_id = None

    space_name = 'tests_sdk_space_text_transformer'
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            daub_include_only_estimators=[self.experiment.ClassificationAlgorithms.XGB],
            # text_processing_flag=self.with_text_transformer,
            test_size=0.1,
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        ...
        # TestAutoAIRemote.run_id = 'da33db18-abd5-489c-bfde-82a413563560'
        # TestAutoAIRemote.remote_auto_pipelines = self.experiment.runs.get_optimizer(self.run_id)

    def test_05b_save_logs(self):
        import json
        import datetime

        times_per_run_id = dict()
        training_details = self.wml_client.training.get_details(self.run_id)

        times_in_row = ""
        for pipeline_metrics in training_details['entity']['status']['metrics']:
            if pipeline_metrics['context']['phase'] not in times_per_run_id.keys():
                times_in_row += f"{pipeline_metrics['context']['intermediate_model']['duration'] / 1000}\t" # devide by 1000 -> converting miliseconds to seconds
            times_per_run_id[pipeline_metrics['context']['phase']] = pipeline_metrics['context']['intermediate_model'][
                                                                         'duration'] / 1000
        print(times_in_row)

        details = {"run_status": self.remote_auto_pipelines.get_run_status(),
                   "elapsed_time": self.elapsed_time,
                   "pipeline_details": self.remote_auto_pipelines.get_params(),
                   "run_details": self.remote_auto_pipelines.get_run_details(),
                   # "ml_metrics": self.remote_auto_pipelines.get_pipeline_details()['ml_metrics'].to_dict()
                   "duration_phases": times_per_run_id
                   }
        try:
            details["ml_metrics"] = self.remote_auto_pipelines.get_pipeline_details()['ml_metrics'].to_dict()
        except Exception as e:
            print("Not able to get ml_metrics, skipped")
            print(e)

        filename = f"tt_results/text_transformer_"+self.OPTIMIZER_NAME+f"{datetime.datetime.now().strftime('%H-%M-%S_%d-%m-%Y')}" + ".json"
