...
class TestAutoAIRemote(unittest.TestCase):
    ...
    # asset_id = '0296a5cf-c677-4e07-9ce4-574bd608c387'  # _test  ~1MB
    asset_id = "19f81b25-05aa-4dbd-a80a-6ca56c4f61c7"  # _train  ~10MB

    prediction_column = 'Sentiment'
    with_text_transformer = True

    OPTIMIZER_NAME = f"TEXT TRANSFORMER {with_text_transformer} multiclass {data_location.split('/')[-1]} 10MB - full"
    ...
    def test_03_initialize_optimizer(self):
        TestAutoAIRemote.remote_auto_pipelines = self.experiment.optimizer(
            ...
            daub_include_only_estimators=[self.experiment.ClassificationAlgorithms.LGBM, self.experiment.ClassificationAlgorithms.XGB],
            cognito_transform_names=[Transformers.DIFF,Transformers.SUM,Transformers.MAX,
                                     Transformers.SIN], #,Transformers.xSUM],
    ...
    def test_05_fit_run_training_of_auto_ai_in_wml(self):
        ...
        # TestAutoAIRemote.run_id = '51a3d5f2-3c72-4072-a578-f02ee5991e9b' # hist run_id
    ...
    def test_05b_save_logs(self):
        ...
        filename = "text_transformer_"+self.OPTIMIZER_NAME+f"{datetime.datetime.now().strftime('%H-%M-%S_%d-%m-%Y')}" + ".json"
