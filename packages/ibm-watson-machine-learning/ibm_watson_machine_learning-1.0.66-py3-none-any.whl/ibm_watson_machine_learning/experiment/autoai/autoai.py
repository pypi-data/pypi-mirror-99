...
class AutoAI(BaseExperiment):
    ...
    def optimizer(self,
        ...
        >>>    )
        """
        # To be removed when Text Transformer enabled officially:

            # text_processing: bool, optional
            #     If True text processing will be enabled. Default is True.
            #
        ...
        if 'tt_enabled' in self._workspace.wml_client.wml_credentials \
                or self._workspace.wml_client.wml_credentials.get('tt_enabled'):
            text_processing = kwargs.get('text_processing')
            word2vec_feature_number = kwargs.get('word2vec_feature_number')
        else:
            text_processing = None
            word2vec_feature_number = None
        try:
            del kwargs['text_processing']
        except:
            pass
        try:
            del kwargs['word2vec_feature_number']
        except:
            pass

        # - end of Text Transformer section
