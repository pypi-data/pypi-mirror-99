...
def jsonSql(dfjson,tname,sqlstr):
    for col in dfjson.columns:
        for row in dfred.take(1):
            colValue=dfred.select("`"+col+"`").take(1)[0][0]
            if isfloat(colValue):
                sqlstr+="  "+col+"  NUMERIC,"+"\n"
            else:
                try:
                    (datetime.datetime.strptime(str(colValue), '%Y-%m-%d'))==True
                    sqlstr+="  "+col+"  timestamp without time zone NOT NULL DEFAULT (current_timestamp AT TIME ZONE 'UTC'),"+"\n"
                except:
                    sqlstr+="  "+col+"  TEXT,"+"\n"
...
def jsonFlat(df,dflist):
    for types in df.dtypes:
        if types[1][:5]=='array':
            arr_df=df.withColumn(types[0], f.explode(f.col(types[0]))).select(types[0]+".*")
            dflist.append(arr_df.select([f"`{l[0]}`" for l in arr_df.dtypes if l[1]=="string"]))
            jsonFlat(arr_df,dflist)
           
        elif types[1][:6]=='struct':
            struct_df=df.withColumn(types[0],f.col(types[0])).select(f.col(types[0]+".*"))
            dflist.append(struct_df.select([f"`{l[0]}`" for l in struct_df.dtypes if l[1]=="string"]))
            jsonFlat(struct_df,dflist)
