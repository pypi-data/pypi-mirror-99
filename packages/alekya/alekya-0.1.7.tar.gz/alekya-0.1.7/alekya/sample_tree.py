...
#tables{
  ...
  <body>
    <div class="container" style='width:100%'>
        ...
        <script>
            d3.json("./static/tree2.json", function(error, json) {
                treeBoxes('', json.tree);
            });
        </script>
    </div>
...
<script>
// var filedata = JSON.parse('{{ data| safe}}')
// console.log(filedata)
// var k = Object.keys(filedata.templates)
// var v = Object.values(filedata.templates)
// console.log(k)
 $('#tdata').hide()
	...
	// 1) ******************* Update the nodes *******************
		...
		.on('click', function(d) {
					...
					if(d.typed == 'File'){
					    ...
					    if(d.nodeName.includes('.html')){
                          $('#table').text('')
                          $('#file_name').text('')
                          $('#routes').text('')
                          $('#functions').text('')
                          $('#tdata').show()
                            var keys = Object.keys(filedata['templates'][d.nodeName])
                            var values = Object.values(filedata['templates'][d.nodeName])
                              for (x=0;x<keys.length;x++){
                                  keys[x] = keys[x].replace('<', '')
                                //   str = keys[x][0]
                                //   str.replace('<' , '')
                                    $('#table').append('<tr><td>'+keys[x]+'</td><td>'+values[x]+'</td></tr>')
                                }
					    }
					     if(d.nodeName.includes('.py')){
					       let f = d.nodeName
					       $('#table').text('')
                            $('#tdata').hide()
					       $('#file_name').text('File name: '+ f)
					       $('#functions').text('Function: ' + filedata['python'][f].function)
					       $('#routes').text('Routes: ' + filedata['python'][f].route)
					        console.log(filedata['python'][f])
					    }
					}
			});
		...
		.append('xhtml').html(function(d) {
					return '<div style="width: '
							...
							+ '<p style="font-size:14px;">' + d.nodeName + '</p><br><br>'
                            +'<p style="font-size:11px;">' + d.typed + '</p><br><br>'
							+ '</div>';
				});
