...
def thermo_profiles(pressure,temperature,mixing_ratio):
    """"
    ...
    ax[0].plot(T[:lev],p[:lev],'-ob')
    ax[0].plot(Td[:lev],p[:lev],'-og')
    ax[0].plot(Tp[:lev],p[:lev],'-or')
    ax[0].set_xlabel('Temperature [C]',fontsize=12)
    ax[0].set_ylabel('Pressure [hPa]',fontsize=12)
    ax[0].invert_yaxis()
    ax[0].legend(['Temp','Temp_Dew','Temp_Parcel'],loc=1,fontsize=12)
    ax[0].grid()
    ...
    ax[1].plot(1000*q[:lev],p[:lev],'-og')
    ax[1].set_xlabel('Mixing ratio [g/kg]',fontsize=12)
    ax[1].set_ylabel('Pressure [hPa]',fontsize=12)
    ax[1].invert_yaxis()
    ax[1].grid()

    ax[2].plot(RH[:lev],p[:lev],'-og')
    ax[2].set_xlabel('Relative humiduty [%]',fontsize=12)
...
def theta_profiles(pressure,temperature,mixing_ratio,ax=None):
    """
    ...
    ax.plot(theta[:lev],p[:lev],'-ok')
    ax.plot(theta_e[:lev],p[:lev],'-ob')
    ax.plot(theta_es[:lev],p[:lev],'-or')
    ax.set_xlabel('Temperature [K]',fontsize=12)
    ax.set_ylabel('Pressure [hPa]',fontsize=12)
    ax.invert_yaxis()
    ax.legend([r'$\theta$',r'$\theta_e$',r'$\theta_{es}$'],loc=1,fontsize=12)
    ax.grid()
...
def entropy_profiles(pressure,temperature,mixing_ratio,ax=None):
    """
    plots for vertical profiles of specific entropy, moist entropy,
    and saturated moist entropy
    """
    p  = pressure*units('mbar')
    T  = temperature*units('degC')
    q  = mixing_ratio*units('kilogram/kilogram')
    ...
    if ax == None:
        fig, ax = plt.subplots(1,1,figsize=(7,7))
    ax.plot(sd[:lev],p[:lev],'-ok')
    ax.plot(sm1[:lev],p[:lev],'-ob')
    ax.plot(sm2[:lev],p[:lev],'-or')
    ax.set_xlabel('Specific entropy: sd, sm, sm_sat [J K$^{-1}$ kg$^{-1}$]',fontsize=12)
    ax.set_ylabel('Pressure [hPa]',fontsize=12)
    ax.invert_yaxis()
    ax.legend(['Sd','Sm','Sm_sat'],loc=1,fontsize=12)
    ax.grid()
...
def mse_profiles(pressure,temperature,mixing_ratio,altitude,ax=None):
    """
    plots for vertical profiles of specific entropy, moist entropy,
    and saturated moist entropy
    """
    ...
    if ax == None:
        fig, ax = plt.subplots(1,1,figsize=(7,7))
    ax.plot(dse[:lev],p[:lev],'-ok')
    ax.plot(mse[:lev],p[:lev],'-ob')
    ax.plot(mse_s[:lev],p[:lev],'-or')
    ax.set_xlabel('Specific static energies: s, h, hs [kJ kg$^{-1}$]',fontsize=12)
    ax.set_ylabel('Pressure [hPa]',fontsize=12)
    ax.invert_yaxis()
    ax.legend(['DSE','MSE','SMSE'],loc=1,fontsize=12)
    ax.grid()
...
def msed_plots(pressure,temperature,mixing_ratio,altitude,h0_std=2000,ensemble_size=20,ent_rate=np.arange(0,2,0.05),
               entrain=False,ax=None,fig=None):
    """
    ...
    p_PWtop = max(100*units.mbar, min(p) + 1*units.mbar) # integrating until 100mb
    cwv = mpcalc.precipitable_water(p,Td,top=p_PWtop) # column water vapor [mm]
    cwvs = mpcalc.precipitable_water(p,T,top=p_PWtop) # saturated column water vapor [mm]
    ...
    # filled buoyant layers in green and nonbuoyant ones in purple
    mse_bp = np.copy(mse_p); mse_nbp = np.copy(mse_p)
    for k in range(len(p)):
        if (mse_p[k] - mse_s[k].magnitude) <=0:
            mse_bp[k] = mse_s[k].magnitude
        else:
            mse_nbp[k] = mse_s[k].magnitude

    ax.fill_betweenx(p[lcl_idx:el_idx+1],mse_bp[lcl_idx:el_idx+1],mse_s[lcl_idx:el_idx+1],interpolate=True
                    ,color='green',alpha=0.3)
    ax.fill_betweenx(p[lcl_idx:el_idx+1],mse_nbp[lcl_idx:el_idx+1],mse_s[lcl_idx:el_idx+1],interpolate=True
    ...
    # Distribution of surface parcel h offsets
        ...
        for h0offset in h0offsets:
            ...
            for eps in entrainment_rates:
                ...
                hent = h4ent.copy()
                delz = np.gradient(altitude.magnitude)*units('meter')
                ...
                posboy  = (hent.magnitude > mse_s.magnitude); posboy[0] = 1  # so there is always a detrainment level
                ...
                # avoid highly entrianing parcels without buoyancy having EL at tops (Oct, 2020)
                try:
                    idx_first_boy = np.where(posboy==1)[0][1] # find the idx for the first buoyant level

                    if  (np.sum(posboy[idx_first_boy:ELindex_ent]) == ELindex_ent-idx_first_boy) and \
                        (idx_first_boy != ELindex_ent) and \
                        (p[idx_first_boy].magnitude > 850):
        ...
        NBINS = 19
        pbins = np.linspace(1000,100,num=NBINS) # pbins for detrainment levels, dp=50 hPa
        ...
        else: # no buoyant parcel in a stable environment
            det_per = 0*hist

        ax2 = fig.add_axes([0.705,0.1,0.1,0.8],facecolor=None)
        ax2.barh(pbins[1:], det_per, color='lightgrey',edgecolor='k',height=46) # dont change 46!
        ax2.set_xlim([0,100])
        ax2.set_xticks([0,20,40,60,80,100])
        ax2.set_yticks([1000,800,600,400,200])
        ax2.set_ylim([1030,120])
        ax2.set_xlabel('Parcel tops\n''[% per 50hPa]',fontsize=12)
        ax2.grid()
        ax2.set_zorder(2)

        #ax.plot( [400,400], [1100,0])
        ax.annotate('Parcel tops', xy=(362,320), color='dimgrey')
    ...
    ### surface water temperature, estimating surface fluxes
    ...
    for idT in [-2,0,2,4]:
        ...
        ssTK = ssTC.to('kelvin')
        ss = g*altitude[0] + Cp_d*ssTK
        hs = ss     + Lv*mpcalc.mixing_ratio(mpcalc.saturation_vapor_pressure(ssTK) ,p[0])

        ax.annotate(str(ssTint), xy=((ss/1000).magnitude, p[0].magnitude) , color='red', fontsize=7)
        ax.annotate(str(ssTint), xy=((hs/1000).magnitude, p[0].magnitude) , color='red', fontsize=9)
...
def entropy_plots(pressure,temperature,mixing_ratio,altitude,h0_std=2000,ensemble_size=20,ent_rate=np.arange(0,2,0.05),entrain=False):
    ...
    # Distribution of surface parcel h offsets
        ...
        # Plot a crude histogram of parcel detrainment levels
        NBINS = 20 # number of pressure levels
        pbins = np.linspace(1000,100,num=NBINS) # pbins for detrainment levels
        hist = np.zeros((len(pbins)-1))
        for x in ELps:
            for i in range(len(pbins)-1):
                if (x < pbins[i]) & (x >= pbins[i+1]):
    ...
    # label LCL and LCF
    ax.plot((sm2[lcl_idx]+(-2000,2000)*units('joule/kelvin')), lcl_pressure+(0,0)*units('mbar') ,color='orange',linewidth=3)
    ax.plot((sm2[lfc_idx]+(-2000,2000)*units('joule/kelvin')), lfc_pressure+(0,0)*units('mbar') , color='magenta',linewidth=3)
...
def add_wndbarbs(ax,pressure,uwind,vwind):
    ### adding wind barbs
    ax.barbs(395*np.ones(len(pressure)),pressure,uwind,vwind,flagcolor='dimgrey')
    ax.vlines(x=395,ymax=1050,ymin=100,linewidth=0.5)
    ax.set_xlim([280,410])
...
def indices_output(pressure,temperature,mixing_ratio,altitude):
    ...
    # Water vapor calculations
    #p_PWtop = min(p)
    p_PWtop = max(100*units.mbar, min(p) + 1*units.mbar) # integrating until 200mb
    cwv = mpcalc.precipitable_water(p,Td,top=p_PWtop) # column water vapor [mm]
    cwvs = mpcalc.precipitable_water(p,T,top=p_PWtop) # saturated column water vapor [mm]
    crh = (cwv/cwvs)*100. # column relative humidity [%]
