...
def monkey_patch_github_accessible(self, _):
    return True


def monkey_patch_github_not_accessible(self, _):
    return False
...
def test_train_default_cred(monkeypatch):
    ...
    monkeypatch.setattr(grid.Grid, '_check_github_repo_accessible',
                        monkey_patch_github_accessible)
...
def test_train_fails_if_github_repo_not_accessible(monkeypatch):
    """grid train fails and redirects to the settings page """
    ...
    monkeypatch.setattr(grid.Grid, '_check_user_github_token',
                        monkey_patch_token_check)
    monkeypatch.setattr(grid.Grid, '_check_github_repo_accessible',
                        monkey_patch_github_not_accessible)
    ...
    assert result.exit_code == 1
    assert result.exception
    assert 'Please grant access on the settings page' in result.output
    assert launch_url == os.getenv('GRID_URL') + '/#/settings'
