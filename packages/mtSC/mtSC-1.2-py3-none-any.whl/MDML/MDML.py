...
def process_data_to_same_gene(gene, root_dir, output_dir, mode):
    ...
    for l in range(len(dataset_list)):
        ...
        if '.txt' in dataset or '.csv' in dataset:
            all_data = pd.read_csv(root_dir + dataset, sep='\t')
        if '.h5' in dataset and '_processed' not in dataset:
...
def main(trainset_dir, testset_dir):
    ...
    if test_dataset_list:
        metrics_list, labels_list = get_MetricsList_and_LabelsList(model, train_dataset_list, trainset_dir)
