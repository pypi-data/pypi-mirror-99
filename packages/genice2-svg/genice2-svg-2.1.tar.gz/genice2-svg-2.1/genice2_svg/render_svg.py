    ...
    for prim in sorted(prims, key=lambda x: x[0][2]):
        ...
        if prim[1] == "L":
            if prim[4] == 0:
                ...
                svg.add(sw.shapes.Line(start=(prim[2][:2]-topleft)*zoom, end=(prim[3][:2]-topleft)*zoom, **options))
            else:
                options = {**filldefaults, **prim[5]}
                cylinder_new(svg, (prim[2]-TL0)*zoom, (prim[3]-TL0)*zoom, prim[4]*zoom, **options)
        elif prim[1] == "L2":
            if prim[3] == 0:
                ...
                cylinder_new(svg, (prim[0]+prim[2]-TL0)*zoom, (prim[0]-prim[2]-TL0)*zoom, prim[3]*zoom, **options)
        elif prim[1] == "P":
            options = prim[3]
