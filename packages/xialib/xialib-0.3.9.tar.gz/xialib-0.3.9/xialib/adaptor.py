...
class Adaptor(metaclass=abc.ABCMeta):
    ...
    def dt_sql_to_python(self, sql_format: str) -> str:
    ...
    def dt_conversion(self, type: str, src_format: str, value: str) -> str:
        """Date / Time Conversion

        Args:
            type (:obj:`str`): date / time / datetime
            src_format (:obj:`str`): Source Python Format
            value (:obj:`str`): Value to be converted
        Returns:
            :obj:`str`: Format in Python Standard
        """
        if src_format == self.dt_format.get(type, src_format) or not value:
            return value
        return datetime.strptime(value, src_format).strftime(self.dt_format[type])
