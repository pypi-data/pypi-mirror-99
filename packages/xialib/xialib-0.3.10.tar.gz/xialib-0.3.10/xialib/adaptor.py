...
class Adaptor(metaclass=abc.ABCMeta):
    ...
    @classmethod
    def dt_sql_to_python(cls, sql_format: str) -> str:
        """Date/Time Format Conversion

        Args:
            sql_format (:obj:`str`): Format in SQL Standard

        Returns:
            :obj:`str`: Format in Python Standard
        """
        return sql_format.lower().replace("yyyy", "%Y").replace("mm", "%m").replace("dd", "%d") \
                                 .replace("hh", "%H").replace("mi", "%M").replace("ss", "%S")

    @classmethod
    def dt_conversion(cls, value: str, dt_type: str, src_format: str) -> str:
        """Date / Time Conversion

        Args:
            value (:obj:`str`): Value to be converted
            src_format (:obj:`str`): Source Python Format
            dt_type (:obj:`str`): date / time / datetime

        Returns:
            :obj:`str`: Format in Python Standard
        """
        if src_format == cls.dt_format.get(dt_type, src_format) or not value:
            return value
        return datetime.strptime(value, src_format).strftime(cls.dt_format[dt_type])

    @classmethod
    def get_dt_conv_func(cls, field_line: dict):
        """Date / Time Conversion

        Args:
            field_line (:obj:`dict`): table field description

        Returns:
            :obj:`Callable`: A partial function to do the conversion which take only value as input
        """
        if "date" in field_line["type_chain"]:
            return partial(cls.dt_conversion, dt_type="date", src_format=cls.dt_sql_to_python(field_line["format"]))
        elif "time" in field_line["type_chain"]:
            return partial(cls.dt_conversion, dt_type="time", src_format=cls.dt_sql_to_python(field_line["format"]))
        elif "datetime" in field_line["type_chain"]:
            return partial(cls.dt_conversion, dt_type="datetime", src_format=cls.dt_sql_to_python(field_line["format"]))
        else:
            return None
    ...
    def load_log_data(self, log_table_id: str, table_id: str, field_data: list, meta_data: dict,
                      start_age: int, end_age: int) -> bool:
        """ To be implemented Public function
        ...
        Args:
            ...
            field_data (:obj:`list` of `dict`): Table field description
            meta_data (:obj:`dict`): Table Meta Data
