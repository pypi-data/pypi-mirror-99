...
def test_paper_space_scale_1_1():
    ...
    assert round(
        psu.from_msp(1, 'yd')) == 36, 'should be 36 inch in paper space'
    assert round(
        psu.from_msp(1, 'ft')) == 12, 'should be 12 inch in paper space'
...
def test_paper_space_to_model_space_us():
    ...
    assert isclose(psu.to_msp(1, 'in'), 2.77777777777), \
