...
def cmd_scamper(pps, tmp, write, ftype, sccmd, sudo=False):
    if sudo:
        cmd = 'sudo '
    else:
        cmd = ''
    cmd += 'scamper -O {ftype} -p {pps} -c "{sccmd}" -f {infile} {write}'.format(ftype=ftype, pps=pps, sccmd=sccmd, infile=tmp, write=write)
    return cmd
...
def main():
    ...
    parser.add_argument('-s', '--sudo', action='store_true', help='Run scamper with sudo.')
    ...
    try:
        ...
        while args.cycles == 0 or cycle < args.cycles:
            ...
            try:
                ...
                cmd = cmd_scamper(args.pps, tmp, write, ftype, sccmd, sudo=args.sudo)
                print(cmd)
                if args.debug:
