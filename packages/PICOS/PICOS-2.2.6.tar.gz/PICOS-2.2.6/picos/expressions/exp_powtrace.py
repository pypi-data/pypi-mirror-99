        ...
        if isinstance(other, AffineExpression):
            if not other.constant:
                raise NotImplementedError("You may only multiply a nonconstant "
                    "PICOS trace of a power with a constant term.")

            value = other.value

            if value < 0:
                raise NotImplementedError("You may only multiply a nonconstant "
                    "PICOS trace of a power with a nonnegative term.")
            ...
            return PowerTrace(self._x, self.p, m, self._limit)
        else:
