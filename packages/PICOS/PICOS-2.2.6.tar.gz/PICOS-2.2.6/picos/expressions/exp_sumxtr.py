...
class SumExtremes(Expression):
        ...
        if isinstance(other, AffineExpression):
            if not other.constant:
                raise NotImplementedError("You may only multiply a nonconstant "
                    "PICOS sum of extremes with a constant term.")

            if other.value < 0:
                raise NotImplementedError("You may only multiply a nonconstant "
                    "PICOS sum of extremes with a nonnegative term.")

            product = SumExtremes(
                other.value*self._x, self._k, self._largest, self._eigenvalues)
            product._typeStr = "Scaled " + product._typeStr
            product._symbStr = glyphs.clever_mul(self.string, other.string)
            return product
        else:
    ...
    def __rmul__(self, other):
        if isinstance(other, AffineExpression):
            ...
            product._symbStr = glyphs.clever_mul(other.string, self.string)
            return product
        else:
            return NotImplemented
