...
def convert_operands(
        sameShape=False, scalarLHS=False, scalarRHS=False, rMatMul=False,
        lMatMul=False, horiCat=False, vertCat=False, allowNone=False):
    ...
    *not* have arbitrary dimensions, then it must validate the element's shape
    ...
    selected  = len([arg for arg in
        (sameShape, anyScalar, lMatMul, rMatMul, horiCat, vertCat) if arg])
    if selected > 1:
...
def cvxopt_principal_root(matrix):
    ...
    """
    matrix_np = numpy.array(cvx2np(matrix))  # TODO: GitLab issue #239.
...
def sparse_quadruple(A, reshape=None, preT=False, postT=False):
    ...
    if not A:
    ...
    elif isinstance(A, cvxopt.spmatrix):
        V, I, J = A.V, A.I, A.J
        ...
        V, I, J = tuple(V), tuple(I), tuple(J)
    else:
        if reshape:
            p, q = reshape
            if preT:
                V, I, J = zip(*[
                    (v, (k // m + (k % m)*n) % p, (k // m + (k % m)*n) // p)
                    for k, v in enumerate(A) if v])
            else:
                V, I, J = zip(
                    *[(v, k % p, k // p) for k, v in enumerate(A) if v])
        else:
            if preT:
                p, q = n, m
                V, I, J = zip(
                    *[(v, k // m, k % m) for k, v in enumerate(A) if v])
            else:
                p, q = m, n
                V, I, J = zip(
                    *[(v, k % m, k // m) for k, v in enumerate(A) if v])
...
def cvx2np(A, reshape=None):
    """Convert a CVXOPT (sparse) matrix to a NumPy matrix.
    ...
    :returns: Converted :class:`NumPy matrix <numpy.matrix>`.
    """
