        ...
        if isinstance(other, AffineExpression):
            if not other.constant:
                raise NotImplementedError("You may only multiply a nonconstant "
                    "PICOS n-th root of a determinant with a constant term.")

            root = DetRootN(other.value * self._x)
            root._typeStr = "Scaled " + root._typeStr
            root._symbStr = glyphs.clever_mul(self.string, other.string)
            return root
        else:
