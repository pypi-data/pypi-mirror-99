...
class Objective:
    ...
    def value(self):
        ...
        In the case of an uncertain objective, this is the worst-case (expected)
        objective value.

        :raises picos.uncertain.IntractableWorstCase:
            When computing the worst-case (expected) value of an uncertain
    ...
    def __index__(self):
        ...
        if not value.is_integer():
            raise ValueError(
                "Cannot use the objective function {} as an index as its value "
                "of {} is not integral.".format(self._function.string, value))
...
class Problem():
    ...
    @property
    def no(self):
        """Normalized objective as an :class:`Objective` instance.

        Either a minimization or a maximization objective, with feasibility
        posed as "minimize 0".

        The same as the :attr:`~Objective.normalized` attribute of the
        :attr:`objective`.
        """
    ...
    def value(self):
        ...
        If all mutables that appear in the objective function are valued, in
    ...
    def get_valued_variable(self, name):
        ...
        variable's :attr:`values <.expression.Expression.value>` instead of the
        variable objects.
    ...
    def solve(self, **extra_options):
        ...
        access the solution as follows:
            ...
            - The variables' :attr:`~.expression.Expression.value` is set
    ...
    def minimize(self, obj, **extra_options):
        ...
        :returns: A dictionary, see :meth:`~Problem.solve`.

        .. warning::

            This is equivalent to :meth:`~Problem.set_objective`
            followed by :meth:`~Problem.solve` and will thus override
            any existing objective function and direction.
        """
    ...
    def set_var_value(self, name, value):
        """Set the :attr:`value <.expression.Expression.value>` of a variable.
