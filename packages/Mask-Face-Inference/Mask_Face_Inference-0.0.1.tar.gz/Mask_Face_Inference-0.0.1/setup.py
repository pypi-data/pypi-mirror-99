...
setuptools.setup(
    name="Mask_Face_Inference",  # 包的分发名称，使用字母、数字、_、-
    version="0.0.1",  # 版本号, 版本号规范：https://www.python.org/dev/peps/pep-0440/
    author="cangwang",  # 作者名字
    author_email="284699931@qq.com",  # 作者邮箱
    description="Mask Face detection",  # 包的简介描述
    ...
    url="http://www.github.com",  # 项目开源地址，我这里写的是同性交友官网，大家可以写自己真实的开源网址
    # packages=setuptools.find_packages(),                    # 如果项目由多个文件组成，我们可以使用find_packages()自动发现所有包和子包，而不是手动列出每个包，在这种情况下，包列表将是example_pkg
    packages=["model", "Losses"],
    # data_files=["model/shape_predictor_68_face_landmarks.dat", "model/model_V2.pt"],
    ...
    install_requires=['torch', 'numpy', 'dlib', 'cv2', "Image", "math"],  # 依赖的包
    python_requires='>=3',
)
