...
def main():
    ...
    parser.add_option(
        ...
        dest='remote',
        help=
        'remote servers. format of host1,host2,host3. Accepts host:port or user@host:port as well.'
    )
    parser.add_option('-c',
                      '--command',
                      dest='command',
    ...
    parser.add_option(
        ...
        default=False,
        help=
        'remove known_hosts item without confirmation. this is useful when a large number of target server being rebuilt frequently.'
    )
    parser.add_option('-v',
                      '--verbose',
                      action='store_true',
    ...
    def sshrun(rserver, command):
        ...
        sshbase = "ssh " + "-o ConnectTimeout=" + str(
            options.timeout) + nullinput + " " + options.option + " " + sport
        if options.debug:
        ...
        if command:
            if options.debug:
                print("# [{}] {}".format(rserver,
                                         sshbase + " \"" + command + "\""))
            child = pexpect.spawn(sshbase + " \"" + command + "\"",
        ...
        while True:
            r = child.expect([
                ...
                '[\$#]', 'key verification failed', pexpect.TIMEOUT,
                pexpect.EOF
            ],
                             timeout=options.timeout)
            if r == 0:
            ...
            if r == 1:
                ...
                if old_r == 1:
                    print(
                        "# [{}] Repeated Requesting password".format(rserver))
                    child.close()
                    good = False
                    break
            ...
            if r == 4:
                ...
                cmd = [
                    ...
                    if re.match(r"\s*ssh-keygen -f.*?-R.*", ln)
                ]
                if options.debug:
                    print(
                        "# [{}] key validation failed. target server may have been rebuilt."
                        .format(rserver))
                    print(
                        "# [{}] pls check and remove needed from known_hosts.".
                        format(rserver))
                else:
                    output += (
                        "# [{}] key validation failed. target server may have been rebuilt.\n"
                ...
                if options.rm_conflict_key:
                    ...
                    if len(cmd) > 0:
                        ...
                        os.system(
                            "ssh-keygen -f ~/.ssh/known_hosts -R {} > /dev/null 2>&1 "
                            .format(rserver))
                else:
                    break
                if options.debug:
    ...
    def sshfile(rserver, script):
        ...
        if not re.search(r"^/", options.workdir):
            o, good = sshrun(
                rserver, 'mkdir -p ~' + options.user + '/' + options.workdir)
            if not good:
                output += "# [{}] sshfile exited.\n".format(rserver)
                return output
        else:
            ...
            if not good:
                output += "# [{}] sshfile exited.\n".format(rserver)
                return output
        scpbase = "scp -q -P {} -o ConnectTimeout={} ".format(
            port, options.timeout)
        if options.pkey:
        ...
        def randstr():
            ...
            result = "".join(allchar[randrange(len(allchar))]
                             for _ in range(30))
            return result
        ...
        if mnohup or mredirect:
            res, _ = sshrun(
                rserver,
                go2workdir + "chmod -R 0755 " + tgtfile + ";  " + tgtcmd)
        else:
            res, _ = sshrun(
                rserver, go2workdir + "chmod -R 0755 " + tgtfile + ";  " +
                tgtcmd + " 2>&1 | tee " + tgtlog)
        if res:
            output += res
        if options.remove:
            if options.removeall:
                res, _ = sshrun(
                    rserver, 'rm -rf ' + '~' + luser + '/' + options.workdir +
                    '/' + '*')
            else:
    ...
    if options.script and not (os.path.isfile(options.script)
                               or os.path.isdir(options.script)):
    ...
    def worker(id=-1):
        ...
        def runsvr(rsvr):
            ...
            if options.debug:
                print("# [worker {}] rsvr={}, luser={}, port={}".format(
                    id, rsvr, luser, port))
            if not porttest(rsvr, int(port)):
                output += (
                    "# [worker {}] [{}] ssh port {} not reachable\n".format(
                        id, rsvr, port))
                return output
            if luser:
