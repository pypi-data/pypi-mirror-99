...
class Hetsar(object):
    ...
    def fit(self,m_y,a_x,m_W,exog_labels = None,mg_est = False, groupLabels = None): 
        N,T,K = a_x.shape
        m_ys = m_W@m_y
...
#                        hess=hessian,
        ...
        param_df = format_output(res,N,T,K,var,var_sand,exog_labels)
        ...
        if mg_est == True:
            if groupLabels== None:
                ...
                param_df['group'] = param_df.i.apply(lambda x: groupLabels[x])
            
            if exog_labels==None:
                exog_labels = ['x{}'.format(i) for i in range(K)]
            mg_vars = ['Wy']+exog_labels
            mg_est_list = []
            for var_hat in mg_vars:
                mg_est_list = mg_est_list + [fn_mg_est(param_df,var_hat,'group')]
                
            df_mg = reduce(lambda left,right: pd.merge(left,right,on='group'), mg_est_list)
