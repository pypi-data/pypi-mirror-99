...
class Client:
    ...
    def dataexchange_choose_dataset(self, region_name='us-east-1'):
        ...
        self.d = widgets.RadioButtons(options=self.datasets.keys(), description='Dataset: ',
                                      layout=widgets.Layout(width='100%'))
        return self.d

    def dataexchange_list_revisions(self, region_name='us-east-1'):
        self.revisions = {i['CreatedAt'].isoformat(): i['Id'] for i in
                          self.dx.list_data_set_revisions(DataSetId=self.datasets[self.d.value])['Revisions']}
        return self.revisions

    def dataexchange_choose_revision(self, region_name='us-east-1'):
        self.dataexchange_list_revisions(region_name=region_name)
        self.r = widgets.RadioButtons(options=self.revisions.keys(), description='Revision: ',
                                      layout=widgets.Layout(width='100%'))
        return self.r

    def dataexchange_runjob_download(self, bucket_name, region_name='us-east-1'):
        self.assets = [{'AssetId': i['Id'], 'Bucket': bucket_name, 'Key': i['Name']} for i in
                       self.dx.list_revision_assets(DataSetId=self.datasets[self.d.value],
                                                    RevisionId=self.revisions[self.r.value])['Assets']]
    ...
    def get_job_status(self, job_group_id):
        response = requests.get(self.trifacta_base_url + '/v4/jobGroups/{0}/status'.format(job_group_id),
                                headers={"Authorization": "Bearer " + self.trifacta_token})
    ...
    def run_job(self, wrangled_dataset_id):
        ...
        response = requests.post(self.trifacta_base_url + '/v4/jobGroups',
                                 headers={"Authorization": "Bearer " + self.trifacta_token}, json=request_body)
