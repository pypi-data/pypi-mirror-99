...
APP_STATE_VERSION_VAR_NAME (`app_state_version`). It will be called providing the version number for each version that
is higher than the version of the previously installed main config file, until the version of the bundled/new-installed
main config file get reached. So if e.g. the previously installed app state version was 3 and the new version number is
6 then this event will be fired 3 times with the argument 4, 5 and 6. It can be used e.g. to change or add app state
...
from ae.base import (                                                                                   # type: ignore
    NAME_PARTS_SEP, UNSET, duplicates, instantiate_config_parser, now_str, snake_to_camel)
from ae.files import RegisteredFile                                                                     # type: ignore
from ae.paths import (                                                                                  # type: ignore
    add_common_storage_paths, copy_file, copy_tree, norm_path, path_folders, Collector, FilesRegister)
...
from ae.console import ConsoleApp, INI_EXT  # type: ignore
...
class MainAppBase(ConsoleApp, ABC):
    ...
    def backup_config_resources() -> str:   # pragma: no cover
        ...
        coll = Collector()
        app_configs = tuple(ana + "." + ext for ana in registered_app_names() for ext in ('ini', 'cfg'))
        coll.collect("{ado}", "{usr}", "{cwd}", append=app_configs, only_first_of='prefix')
        for file in coll.files:
            copy_file(file, os.path.join(backup_root, os.path.basename(file)))

        coll = Collector(path_scanner=path_folders)
        coll.collect("{ado}", "{cwd}", append=('img', 'loc', 'snd'), only_first_of='prefix')
        for path in coll.paths:
            copy_tree(path, os.path.join(backup_root, os.path.basename(path)))
    ...
    def bundled_config_app_state_version(self) -> int:
        """ determine app state version from bundled config file (if not already moved).

        :return: value of app state variable APP_STATE_VERSION_VAR_NAME or 0 if config file got moved on first app run.
        """
    ...
    def setup_app_states(self, app_states: AppStateType):
        ...
        bundled_version = self.bundled_config_app_state_version()
        if bundled_version > current_version:
            for version in range(current_version, bundled_version + 1):
                self.call_method('on_app_state_version_upgrade', version)
            self.change_app_state(APP_STATE_VERSION_VAR_NAME, bundled_version, send_event=False, old_name=UNSET)
