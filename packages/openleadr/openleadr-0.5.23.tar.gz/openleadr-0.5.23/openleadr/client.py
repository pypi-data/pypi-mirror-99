...
class OpenADRClient:
    ...
    def add_report(self, callback, resource_id, measurement=None,
        ...
        :param boolean on_change: Whether this measurement supports reporting when the value changes.
                                  To use this functionality, call the ``client.report_on_change`` method
                                  with parameters ``report_id, r_id, value``. You get the ``report_id``
                                  and the ``r_id`` from the return of the ``client.add_report`` call.
        """
    ...
    async def request_event(self, reply_limit=None):
        """
        Requests all pending and active events from the VTN, if any.
        Returns a response_type, response_payload tuple.
        """
    ...
    def update_report_on_change(self, report_id, r_id, value):
        """
        Creates a new report after a value changed.
        """
