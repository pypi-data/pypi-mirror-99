...
def site_occupancy(wfu, ferric_choice = 'Stoichiometry'):
    ...
    for sample in wfu:
        ...
        if fc == 'Stoichiometry':
            print('Stoichiometry chosen for fc')
        ...
        elif fc == 'Assumed values':
            print('Assumed Values chosen for fc')
        ...
        # Future if statements should allow for manual set of cpx or opx Fe3+ = X% of Fe(total)
            ...
            print('Assigned Values chosen for fc')
            ms["cpx_Fe3+"] = wfu[sample]["cpx_Fe"] * (fc[0]) / 100
            ms["opx_Fe3+"] = wfu[sample]["opx_Fe"] * (fc[1]) / 100
...
#RUNNING CODE
# def main():
#     fc_choice = 'Assigned values'
#     fc_list = [2.2, 3.4]
#     xeno = "xenolith_dataTemplate.csv"
#
#     if fc_choice == 'Assigned values':
#         #fc_list = [float(fc_cpx), float(fc_opx)]
#         print(type(fc_list[0]))
#         parameters_list = initial_parameters(xeno, fc_list)  # incorporate ferric choices
#
#     elif fc_choice == 'Stoichiometry' or 'Assumed values':
#         print('DEBUG')
#         parameters_list = initial_parameters(xeno, fc_choice)  # Need to update package
#
#     global wfu, so, bp
#     wfu = parameters_list['wfu']
#     so = parameters_list['so']
