...
from pyalmanac.lib import alma_ephem as alma_ephem
from pyalmanac.lib import config as config
...
'''.format(ephem.date(da).datetime().strftime("%a"))
        ...
        if config.decf != '+':	# USNO format for Declination
            ...
            while h < 24:
                hourlydata[h] = alma_ephem.planetsGHA(da)
                da = da + ephem.hour
                h += 1
...
'''
        ...
        vd = alma_ephem.vdm_planets(date + n)
        tab = tab + r'''\hline
...
\hline
\multicolumn{{10}}{{c}}{{}}\\
'''.format(alma_ephem.ariestransit(date + n),vd[0],vd[1],vd[2],vd[3],vd[4],vd[5],vd[6],vd[7],vd[8],vd[9],vd[10],vd[11])
        n += 1
...
\hline\rule{0pt}{2.6ex}\noindent
'''
    stars = alma_ephem.stellar(date+1)
...
'''.format(datestr)
        ...
        p = alma_ephem.planetstransit(datex)
        m = m + r'''Venus & {} & {} \\
'''.format(p[0],p[1])
        m = m + r'''Mars & {} & {} \\
'''.format(p[2],p[3])
...
'''.format(ephem.date(da).datetime().strftime("%a"))
        ...
        if config.decf != '+':	# USNO format for Declination
            ...
            hourlydata = [[] for i in range(24)]
            while h < 24:
                hourlydata[h] = alma_ephem.sunmoon(da)
                da = da + ephem.hour
                h += 1
            # now print the data per hour
            da = date + n
...
'''
        ...
        vd = alma_ephem.sun_moon_SD(date + n)
        tab = tab + r'''\hline
...
'''
        ...
        twi = alma_ephem.twilight(date+1, i, hemisph)
...
'''
    ...
    for k in range(3):
        d = ephem.date(date+k)
        eq = alma_ephem.equation_of_time(d)
        if k == 2:
            tab = tab + r'''{} & {} & {} & {} & {} & {} & {}({}\%) \\[0.3ex]
...
def almanac(first_day, pagenum, dfA4):
...
\newgeometry{{nomarginpar, top={}, bottom={}, left={}, right={}}}'''.format(tm1,bm1,lm1,rm1)
    ...
    \includegraphics[clip, trim=5mm 8cm 5mm 21mm, width=0.8\textwidth]{{{}}}\\'''.format(dfA4)
