...
def test_shipments_timelines_with_tracking_dates():
    ...
    sh_df = pd.DataFrame(
        data={
            ...
            "workflow_associated_at": [day_1, day_3],
            "delivery_date": [day_2, None],
            "arrival_date": [day_1, day_5],
        }
    )
    result_should_be = pd.DataFrame(
        data={"shipment_id": ["1", "1", "2", "2", "2"], "dates": [day_1, day_2, day_3, day_4, day_5]}
...
def test_estimated_remaining_daily_workload():
    ...
    result_should_be = pd.DataFrame(
        data={
            ...
            "workflow_associated_at": [datetime.datetime(2020, 11, 15), datetime.datetime(2020, 12, 26)],
            "delivery_date": [None, None],
            "arrival_date": [None, None],
            "remaining_days": [0, 9],
...
def test_shipments_daily_workload_timelines_with_tracking_dates():
    ...
    sh_df = pd.DataFrame(
        data={
            ...
            "foresea_name": ["BATD", "BATE", "BATE"],
            "incoterm": ["fob", "ddp", "ddp"],
            "delivery_date": [datetime.datetime(2020, 11, 24).strftime("%Y-%m-%dT%H:%M:%S.%fZ"), None, None],
            "arrival_date": [
                None,
                datetime.datetime(2021, 1, 7).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                datetime.datetime(2021, 1, 7).strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
            ],
        }
    )
