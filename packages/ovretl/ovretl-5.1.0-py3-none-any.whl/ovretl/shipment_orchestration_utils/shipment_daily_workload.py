...
def find_shipment_workflow_start_date(active_shipments_data_df: pd.DataFrame) -> pd.DataFrame:
    active_shipments_data_df = active_shipments_data_df.drop_duplicates(subset=["shipment_id"]).reset_index(drop=True)
...
def get_shipment_start_date(shipment_tup: ShipmentTimelineInfos) -> date:
    return shipment_tup.workflow_associated_at
...
def shipments_timelines(active_shipments_data_df: pd.DataFrame) -> pd.DataFrame:
    shipments_timeline_infos_grouped = (
        ...
        .agg({col: "first" for col in ["workflow_associated_at", "delivery_date", "arrival_date"]})
    )
...
def shipments_daily_workload_timelines(
    ...
    active_shipments_data_df.loc[
        :, ["workflow_associated_at", "delivery_date", "arrival_date"]
    ] = active_shipments_data_df.loc[:, ["workflow_associated_at", "delivery_date", "arrival_date"]].applymap(
