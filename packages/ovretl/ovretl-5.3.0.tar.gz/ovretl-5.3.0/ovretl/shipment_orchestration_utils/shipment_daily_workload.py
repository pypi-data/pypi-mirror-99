...
def shipments_remaining_days(
    ...
    for shipment_tuple in active_shipments_data_df.itertuples():
        shipment_timeline = shipments_timelines_df.loc[shipments_timelines_df["shipment_id"] == shipment_tuple.shipment_id, :]
        shipment_future_days = shipment_timeline.loc[shipment_timeline["dates"] > shipment_tuple.workload_estimation_start_date, :]
        total_remaining_days.append(len(shipment_future_days))
    active_shipments_data_df.loc[:, "remaining_days"] = total_remaining_days
    return active_shipments_data_df


def get_shipment_start_date(shipment_tuple: ShipmentTimelineInfos) -> date:
    return shipment_tuple.created_at
...
def shipment_date_range(shipment_tuple: ShipmentTimelineInfos) -> pd.DatetimeIndex:
    start_date = get_shipment_start_date(shipment_tuple)
    end_date = get_shipment_end_date(shipment_tuple)
    if pd.isnull(end_date):
        transit_time = shipment_tuple.total_transit_time
...
def shipments_timelines(active_shipments_data_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for shipment_timeline_infos_tuple in shipments_timeline_infos_grouped.itertuples(True, "ShipmentTimelineInfos"):
        ...
        dates = shipment_date_range(shipment_tuple=shipment_timeline_infos_tuple)
...
def shipment_workload_estimation_start_date(active_shipments_data_df: pd.DataFrame, tasks_df: pd.DataFrame):
    today = date.today()
    estimation_start_date = []
    todo_tasks_df = tasks_df[tasks_df["status"] != "done"]
...
def estimated_remaining_daily_workload(
    active_shipments_data_df: pd.DataFrame, tasks_df: pd.DataFrame, shipments_timelines_df: pd.DataFrame,
) -> pd.DataFrame:
    ...
    active_shipments_data_df = shipment_workload_estimation_start_date(active_shipments_data_df, tasks_df)
    active_shipments_data_df = shipments_remaining_days(active_shipments_data_df, shipments_timelines_df)
