...
@app.route('/')
def main():
  return "Thank You For Using SilverBot, for any more help join our discord with the link in the console and to ping your bot use the pinger in the console!"
...
async def event_ready():
    ...
    print(colored('Pinger : https://pinger.dj741.repl.co/'))
    print(colored('Discord : https://discord.gg/hmXa5cQCJH', 'green'))
    print(colored('----------------', 'green'))
    member = bot.party.me
...
async def backpack(ctx, *, content = None):
    if content is None:
        await ctx.send(f'No backpack was given, try: !backpack (backpack name)')
    elif content.lower() == 'none':
