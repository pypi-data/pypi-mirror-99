...
def write_parquet_path(df: pl.DataFrame, path: Path, *, df_description: str = "dataframe", **kwargs: Any) -> None:
    ...
    :param path: `pathlib` or `s3path` path.
    :param df_description: Optional description of dataframe.
    :param kwargs: These are forwarded to `pyarrow.parquet.write_table`.
    """
...
def write_parquet_uri(df: pl.DataFrame, uri: str, *, df_description: str = "dataframe", **kwargs: Any) -> None:
    ...
    try:
        with smart_open.open(uri, "wb") as output_file:
            pq.write_table(df, where=output_file, **kwargs)
    except Exception as exception:  # pylint: disable=broad-except
