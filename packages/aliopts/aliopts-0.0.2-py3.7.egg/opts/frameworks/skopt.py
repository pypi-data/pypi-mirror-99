...
class SkoptSampler(BaseSampler):
    ...
    def __init__(
        ...
        *,
        consider_stopped_trials: bool = False,
    ) -> None:
        ...
        self._consider_stopped_trials = consider_stopped_trials

        if self._consider_stopped_trials:
            warnings.warn(
                "`consider_stopped_trials` option is an experimental feature."
    ...
    ) -> Dict[str, samples.BaseDistribution]:

        search_space = {}
        for name, distribution in self._search_space.calculate(experiment).items():
            if distribution.single():
                if not isinstance(distribution, samples.CategoricalDistribution):
    ...
    def sample_relative(
        ...
        trial: FrozenTrial,
        search_space: Dict[str, samples.BaseDistribution],
    ) -> Dict[str, Any]:
        ...
        optimizer.tell(experiment, complete_trials)
    ...
    def sample_independent(
        ...
        trial: FrozenTrial,
        param_name: str,
        param_distribution: samples.BaseDistribution,
    ) -> Any:
    ...
    def _log_independent_sampling(self, trial: FrozenTrial, param_name: str) -> None:
        ...
        logger = opts.logger.get_logger(__name__)
        logger.warning(
    ...
    def _get_trials(self, experiment: Experiment) -> List[FrozenTrial]:
        complete_trials = []
        for t in experiment.get_trials(deepcopy=False):
            if t.state == TrialState.COMPLETE:
                ...
                and self._consider_stopped_trials
            ):
...
class _Optimizer(object):
    def __init__(
        self, search_space: Dict[str, samples.BaseDistribution], skopt_kwargs: Dict[str, Any]
    ) -> None:
    ...
    def tell(self, experiment: Experiment, complete_trials: List[FrozenTrial]) -> None:
        ...
        for trial in complete_trials:
            ...
            x, y = self._complete_trial_to_skopt_observation(experiment, trial)
            xs.append(x)
            ys.append(y)
    ...
    def ask(self) -> Dict[str, Any]:
        ...
        for (name, distribution), value in zip(sorted(self._search_space.items()), param_values):
            if isinstance(distribution, samples.DiscreteUniformDistribution):
                value = value * distribution.q + distribution.low
            if isinstance(distribution, samples.IntUniformDistribution):
                value = value * distribution.step + distribution.low
            if isinstance(distribution, samples.IntLogUniformDistribution):
