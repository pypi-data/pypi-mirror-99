...
-*- coding: utf-8 -*-
@Author  : Ora
@Time    : 2020/12/15 10:57 上午
@Software: PyCharm
@File    : multivariate_parzen_estimator.py
...
class _MultivariateParzenEstimator:
    def __init__(
        ...
        for param_name, dist in search_space.items():
            if isinstance(dist, samples.CategoricalDistribution):
                low = high = q = None
            else:
    ...
    def sample(self, rng: np.random.RandomState, size: int) -> Dict[str, np.ndarray]:
        ...
        for param_name, dist in self._search_space.items():
            ...
            if isinstance(dist, samples.CategoricalDistribution):
                ...
                sample_items = _MultivariateParzenEstimator._sample_from_categorical_dist(rng, weights)
                ...
                # print("sss",samples,trunc_low,trunc_high)
            ...
            multivariate_samples[param_name] = sample_items
    ...
    def log_pdf(self, multivariate_samples: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for param_name, dist in self._search_space.items():
            sample_items = multivariate_samples[param_name]
            if isinstance(dist, samples.CategoricalDistribution):
                categorical_weights = self._categorical_weights[param_name]
                assert categorical_weights is not None
                log_pdf = np.log(categorical_weights.T[sample_items, :])
            else:
                # We restore parameters of parzen estimators.
                low = self._low[param_name]
                high = self._high[param_name]
    ...
    def _calculate_parzen_bounds(
        ...
        if isinstance(distribution, samples.UniformDistribution):
            low = distribution.low
            high = distribution.high
            q = None
        elif isinstance(distribution, samples.LogUniformDistribution):
            low = np.log(distribution.low)
            high = np.log(distribution.high)
            q = None
        elif isinstance(distribution, samples.DiscreteUniformDistribution):
            q = distribution.q
            low = distribution.low - 0.5 * q
            high = distribution.high + 0.5 * q
        elif isinstance(distribution, samples.IntUniformDistribution):
            q = distribution.step
            low = distribution.low - 0.5 * q
            high = distribution.high + 0.5 * q
        elif isinstance(distribution, samples.IntLogUniformDistribution):
    ...
    def _transform_to_uniform(
        ...
        transformed = {}
        for param_name, sample_items in multivariate_samples.items():
            distribution = self._search_space[param_name]

            assert isinstance(distribution, _DISTRIBUTION_CLASSES)
            if isinstance(
                distribution,
                (samples.LogUniformDistribution, samples.IntLogUniformDistribution),
            ):
                sample_items = np.log(sample_items)

            transformed[param_name] = sample_items
        return transformed
    ...
    def _transform_from_uniform(
        ...
        for param_name, sample_items in multivariate_samples.items():
            ...
            if isinstance(distribution, samples.UniformDistribution):
                ...
                transformed[param_name] = np.exp(sample_items)
            elif isinstance(distribution, samples.DiscreteUniformDistribution):
                q = self._q[param_name]
                sample_items = np.round((sample_items - distribution.low) / q) * q + distribution.low
                transformed[param_name] = np.clip(sample_items, distribution.low, distribution.high)
            elif isinstance(distribution, samples.IntUniformDistribution):
                q = self._q[param_name]
                sample_items = np.round(sample_items / q) * q
                transformed[param_name] = np.clip(
                    sample_items, distribution.low, distribution.high
                ).astype(int)
            elif isinstance(distribution, samples.IntLogUniformDistribution):
                sample_items = np.round(np.exp(sample_items))
                transformed[param_name] = np.clip(
                    sample_items, distribution.low, distribution.high
                ).astype(int)
            elif isinstance(distribution, samples.CategoricalDistribution):
                transformed[param_name] = sample_items
    ...
    def _calculate_categorical_params(
        ...
        prior_weight = self._parameters.prior_weight
        distribution = self._search_space[param_name]
        assert isinstance(distribution, samples.CategoricalDistribution)
