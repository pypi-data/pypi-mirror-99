...
class Urls:
    ...
    def _get_presigned_url(self, method, object_name, bucket_model=False):
        ...
        if method == 'post':
            r = requests.get(self.host + 'generate_presigned_url', data=json.dumps(to_send), headers=self.auth)
        if method == 'get':
            r = requests.get(self.host + 'generate_presigned_url', data=json.dumps(to_send), headers=self.auth)
    ...
    def _get_url_for_part(self, no_part, object_name):
        ...
        try:
            ...
            r = requests.get(self.host + 'generate_post_part_url', data=json.dumps(to_send), headers=self.auth)
            if r.status_code != 200:
    ...
    def _complete_part_upload(self, parts, object_name, network_id):
        ...
        try:
            ...
            r = requests.get(self.host + 'complete_part_upload', data=json.dumps(to_send), headers=self.auth)
