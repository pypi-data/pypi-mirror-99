...
class TransformersApp:
    ...
    def _compute_loglikelihood(self, logit):
        probs = self._softmaxbh(logit[0, 1:, :].mean(1))

        return np.sum(np.log(probs))

    def predict_loglikelihood(self, sequences: List[str]):
        ll_list = []
        for sequence in sequences:
            logit = self.predict_logits([sequence])
            ll = self._compute_loglikelihood(logit)
            ll_list.append(ll)
