...
def bump_version_from_news_files(
    current_version: str, news_files: Iterable[pathlib.Path]
) -> str:
    ...
    """
    semver_to_bump = news_ext_to_version_type(
        tuple(news.suffix for news in news_files if news.suffix != ".md")
    )
...
def news_ext_to_version_type(news_file_exts: Tuple[str]) -> str:
    ...
    if ".major" in news_file_exts:
        ...
        return "patch"
    raise ValueError(
        "No significant news file extensions found. Version will not be incremented."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        required=True,
        help="Directory where news files are located.",
    )
    parser.add_argument(
        "-v", "--verbosity", action="count", default=0, help="Set verbosity level."
    )
...
def main(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        new_version = bump_version_from_news_files(
            current_version, list(args.news_file_directory.iterdir())
        )
        print(new_version)
        return 0
