...
def get_train_test_indices(df, test_size=.2, random_state=None):
    """
    Returns index objects to separate df in train and test sets.

    Parameters
    ----------
    df : DataFrame
        DataFrame whose index will be partitioned.
    test_size : float
        Indicates the proportion of rows to be used as test set. Must be
        more between 0 and 1.
    random_state : int or None
        Used as argument of `numpy.random.seed`.

    Notes
    -----
    It was implemented for allowing 0 and 1 as `test_size` parameter,
    taking as base `train_test_split` from `sklearn.model_selection`.
    """
    assert (test_size >= 0) and (test_size <= 1), textwrap.fill(
        "`test_size` must be between 0 and 1"
    )
    index_values = df.index.unique()
    if test_size > 0:
        n_samples_test = int(round(len(index_values) * test_size))
...
class EstimatorFactory:
    ...
    predicting probabilities according to a model.
    ...
    algorithm : estimator or str in ['logistic-lasso', 'logistic']
        If an estimator, that estimator will be used instead of the
        predefined ones. The estimator should implement the 'fit'
        method. If 'logistic-lasso', a Logistic Regression with Lasso
        regularization will be use. If 'logistic', a Logistic Regression
        without any regularization will be used.
    hyperparameter_determinant : dict, str in ['grid', 'random'] or None
        ...
        search respectively. If None, then no hyperparameter search
        method is used. Defaults to None.
    ...
    train_method: str in ['train-test-split', 'cross-validation']
        Determines the training method. If set to 'train-test-split' the
        method to train will split the training set in a train and a
        test set, and will fit the estimator just with the training set.
        If set to 'cross-validation', the method will fit the estimator
        with cross validation. Defaults to 'train-test-split'.
    test_size : float
    ...
    seed : int
        ...
        to `6202`.
    """
    implemented_algorithms = ['logistic-lasso', 'logistic']
    implemented_hyperparameter_search_schemes = ['grid', 'random']
    hyperparameter_search_schemes_dict = {
        'grid': GridSearchCV,
        'random': RandomizedSearchCV
    }

    implemented_train_methods = ['train-test-split', 'cross-validation']

    def __init__(
            self, X_cols, y_col, algorithm='logistic-lasso',
            hyperparameter_determinant=None,
            hyperparameter_search_kwargs=None,
            train_method='train-test-split', test_size=.2, seed=6202):
        ...
        assert train_method in self.implemented_train_methods, \
         f"'train_method' must take values in {self.implemented_train_methods}"
    ...
    def __get_model(self):
        ...
        # define estimator
        if self.algorithm == 'logistic-lasso':
            return LogisticRegression(
                penalty="l1", solver="liblinear", random_state=self.seed)
        
        if self.algorithm == 'logistic':
            return LogisticRegression(penalty='none', random_state=self.seed)
        
        # custom algorithm
        return self.algorithm


    def __get_hyperparameter_search(self, estimator):
        hyperparameter_search_scheme = self.hyperparameter_search_schemes_dict[
                                            self.hyperparameter_determinant]
                                            
        return hyperparameter_search_scheme(
                estimator, **self.hyperparameter_search_kwargs)

    def __add_optimizer_to_model(self, estimator):

        if not self.hyperparameter_determinant:
            return estimator

        if isinstance(self.hyperparameter_determinant, dict):
            # use `hyperparameter_determinant` as parameters of the estimator
            estimator.set_params(self.hyperparameter_determinant)
            return estimator
        ...
        return self.__get_hyperparameter_search(estimator)
    
    def __train_test_split_training(self, df, estimator, return_test_index):
        # get train and test index
        train_index, test_index = get_train_test_indices(
            df, self.test_size, self.seed)
    ...
    def __cross_validation_training(self, df, estimator, cv):
        
        if not self.hyperparameter_determinant:
            self.hyperparameter_determinant = 'grid'
        
        self.hyperparameter_search_kwargs['cv'] = cv
        self.hyperparameter_search_kwargs['param_grid'] = [
            {
                **{key : [value] \
        ...
        estimator = self.__get_hyperparameter_search(estimator)

        return self.__train_test_split_training(df, estimator, False)

    def __train_model(self, df, estimator, return_test_index, cv):

        if self.train_method == 'train-test-split':
            return self.__train_test_split_training(df, estimator, 
                                                    return_test_index)

        if self.train_method == 'cross-validation':
            return self.__cross_validation_training(df, estimator, cv)
        
    def get_estimator(self, df, return_test_index=False, cv=None):
        """
        ...
        cv : None, int, iterable
            The same cv argument received by 'GridSearchCV' and
            RandomizedSearchCV classes of scikit-learn. It is only used
            in the 'cross-validation' training method. Defaults to None
        """
        
        estimator = self.__get_model()

        estimator = self.__add_optimizer_to_model(estimator)
        ...
        return self.__train_model(df, estimator, return_test_index, cv)


class ImputationSchemeInferrer(BaseEstimator):
    """
    Infers the imputation scheme that can be used later in an imputation
    ...
    transfer_strategy: list of ('index_label', {transfer_src:
        transfer_dst}) (optional)
        ...
        to None.
    verbose : bool
        If some control flow checks need to be reported. Defaults to
        False.
    force : bool
    ...
    Returns
    -------
    An imputation scheme when the 'fit' method is called
    """

    admitted_objectives = ['deficit', 'target']

    def __init__(self, target_col, candidate_col, strata_cols,
                 imputation_target_srs,
                 objective='deficit',
                 imputable_col=None, weight_col=None, transfer_strategy=None,
                 verbose=False, force=False, debug=False):
    ...
    def _infer_corrected_imputation_scheme(self, df):
        ...
        imputable_df['weighted_impute'] = imputable_df[self.candidate_col] * (
            imputable_df[
                self.weight_col] if self.weight_col is not None else 1)
    ...
    def _infer_uncorrected_imputation_scheme(self, df_, debug=False):
        ...
        deficit_srs = inferred_target_srs.subtract(
            present_per_target_sum).fillna(0)
    ...
    def _check_compatible_imputation_scheme(
        ...
        updated_imputation_scheme : None or Series
            ...
            `imputation_scheme`. If Series,
            `imputable_obs_per_strata_srs` is compared against it.
            Defaults to `None`.
        tolerance : float
...
class ScoreBasedStratifiedImputer(ImputationSchemeInferrer, TransformerMixin):
    """
    ...
    def __init__(self, target_col, candidate_col, strata_cols, score_col,
        ...
        super().__init__(target_col, candidate_col, strata_cols,
                         imputation_target_srs,
                         objective,
                         imputable_col, weight_col, transfer_strategy,
                         verbose, force, debug)
    ...
    def _get_imputation_srs(self, df, tolerance=1e-2):
        ...
        imputable_df = imputable_df.merge(
            self.imputation_scheme.to_frame(
                name='n_obs_to_impute').reset_index(
            ), on=self.strata_cols, how='left').set_index(
            imputable_df.index)

        # calculate cummulated difference on each strata
        imputable_df[
            'balance'] = imputable_df.n_obs_to_impute - imputable_df.impute_cumsum
        ...
        imputable_df['imputed'] = imputable_df['balance'] >= imputable_df[
            'balance_closest_to_zero']
    ...
    def transform(self, df):
        ...
        assert self.imputation_scheme is not None, \
            "`transform` need always to be called after `fit` method"
        assert self._assert_df_compatible_with_imputer(df_), \
            "`df` not compatible with imputer"
