...
def get_weighted_complete_randomization_series_on_subset(
        weight_srs, subset_bool_srs, success_determinant, how='prob',
        errors='warn', seed=None):
    """
    ...
    errors: str in {'warn'}, #\todo add other behaviors
        Indicates how the error of having a  number of successes greater
...
def copy_docstring(sender):
    ...
    wrappers of the agnostic functions defined in `criteriaetl.utils.
    dataload`.
...
def get_partition_bool_columns_dict(
    ...
    return {**dict(zip(conditions, choices)), **{'default': none_default}}
