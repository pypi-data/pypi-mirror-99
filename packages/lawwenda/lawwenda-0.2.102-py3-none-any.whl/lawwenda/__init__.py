...
class Share:
    ...
    def name(self) -> str:
        """
        The share name. This usually makes the last part of the url to this share. Is unique in the containing
    ...
    def title(self) -> str:
        """
        The share title. This is an arbitrary text shown in the window title. Should not contain line breaks and should
        be short.
        """
        return self.__title
    ...
    def hide_by_patterns(self) -> t.Iterable[str]:
        """
        A list of regular expressions of paths for hiding. A file or directory will be hidden if its path matches at
        least one of them. Those paths are always relative to :py:attr:`path`, always start with a `'/'`, but never end
        with a one.
    ...
    def hide_by_tags(self) -> t.Iterable[str]:
        """
        A list of tags for hiding files and directories. A file or directory will be hidden if it is tagged with at
        least one of them.
    ...
    def include_by_patterns(self) -> t.Optional[t.Iterable[str]]:
        """
        A list of regular expressions of paths for including explicitly. Those paths are always relative to
        :py:attr:`path`, always start with a `'/'`, but never end with a one.
    ...
    def exclude_by_patterns(self) -> t.Iterable[str]:
        """
        A list of regular expressions of paths for excluding. A file or directory will be excluded if its path matches
        at least one of them. Those paths are always relative to :py:attr:`path`, always start with a `'/'`, but never
        end with a one.
    ...
    def password_scrypt(self) -> t.Optional[str]:
        """
        The scrypt hash of the password for this share. Empty or `None` for disabled password protection. See also
        :py:attr:`password_salt`.
        """
    ...
    def password_salt(self) -> t.Optional[str]:
        """
        The hash salt of the password for this share, if password protected. See also :py:attr:`password_scrypt`.
        """
...
class Configuration:
    ...
    def add_share(self, path: str, *, name: str, password: t.Optional[str],
        ...
        :param path: The directory to share. See :py:meth:`Share.path`.
        :param name: The unique name of the new share. See :py:meth:`Share.name`.
        :param password: The password to protect the share with.
        :param title: The share title. See :py:meth:`Share.title`.
        :param readonly: If to share in a read-only way. See :py:meth:`Share.readonly`.
        :param hide_by_patterns:
        :param hide_by_tags:
        :param include_by_patterns:
        :param include_by_tags:
        :param exclude_by_patterns:
        :param exclude_by_tags:
