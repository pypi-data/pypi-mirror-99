...
class Filesystem:
    ...
    def try_get_fullpath(self, handle: t.Union["Filesystem.ReadHandle", "Filesystem.WriteHandle"], *,
        ...
        :param writable: Whether you might write do any write accesses to the result path.
        """
    ...
    class Node:
        ...
        def child_by_name(self, name: str) -> t.Optional["Filesystem.Node"]:
            """
            ...
            """
            result = self.__filesystem.node_by_path(f"{self.path}/{name}")
            return None if (result.path == self.path) else result
...
def create_filesystem(rootpath: str, *, readonly: bool, hide_by_patterns: t.Iterable[str] = (),
                      ...
                      exclude_hidden: bool = False) -> Filesystem:  # TODO to new module?!
    """
    ...
    import lawwenda.fs.decorators  # pylint: disable=import-outside-toplevel,redefined-outer-name
    import lawwenda.fs.local  # pylint: disable=import-outside-toplevel
