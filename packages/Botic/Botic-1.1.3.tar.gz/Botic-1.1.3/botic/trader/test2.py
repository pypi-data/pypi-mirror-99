...
class Test2(BaseTrader):
    ...
    def run_trading_algorithm(self) -> None:
        ...
        if time.time() - self._rate_limit_log > 0.5:
            ...
            self.logit(
                'wallet:{:2f} open:{} price:{} held:{} canbuy:{} total-value:{} pdiff:{}'.format(
    ...
    def _check_if_can_buy(self) -> bool:
        ...
        median_price = np.median(self._tracker_prices)
        min_price = min(self._tracker_prices)
        max_price = max(self._tracker_prices)
        if self._tracker_prices[0] > median_price and self._tracker_prices[-1] < median_price:
            can = True
        else:
            return False
