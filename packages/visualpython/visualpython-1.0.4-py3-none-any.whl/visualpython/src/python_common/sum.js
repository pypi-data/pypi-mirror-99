define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormatLine('<input type="text" id="{0}" class="{1}" />', 'vp_pySumObj', 'vp-input');
        ...
        sbTagString.clear();
        sbTagString.appendFormatLine('<div id="{0}">', 'vp_pySumArg');
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pySumObj');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pySumObj')).val());
            $(that.wrapSelector('#vp_pySumObj')).replaceWith(function() {
    ...
    PythonCommon.prototype.showTypeRow = function(type) {
        if (type == 'object') {
            $(this.wrapSelector('#vp_pySumObj')).closest('tr').show();
            $(this.wrapSelector('#vp_pySumArg')).closest('tr').hide();
        } else if (type == 'argument') {
            $(this.wrapSelector('#vp_pySumArg')).closest('tr').show();
            $(this.wrapSelector('#vp_pySumObj')).closest('tr').hide();
        }
    }
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        if (type == 'object') {
            code.appendFormat('sum({0})', $(this.wrapSelector('#vp_pySumObj')).val());
        } else if (type == 'argument') {
            code.append('sum(');
            var args = $(this.wrapSelector('#vp_pySumArg')).find('.vp-py-args');
            var validArgCount = 0;
            args.each((i, tag) => {
