define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyRound', 'vp-input');
        tblLayoutRequire.addReqRow("Input Number", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyNdigits', 'vp-input');
        tblLayoutRequire.addRow("Number Digits", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat("round({0}", $(this.wrapSelector('#vp_pyRound')).val());
        var ndigits = $(this.wrapSelector('#vp_pyNdigits')).val();
        if (ndigits != '') {
            code.appendFormat(", ndigits={0}", ndigits);
        }
        code.append(')');
