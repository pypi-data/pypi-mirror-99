define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormatLine('<input type="text" id="{0}" class="{1}" />', 'vp_pyMinObj', 'vp-input');
        ...
        sbTagString.clear();
        sbTagString.appendFormatLine('<div id="{0}">', 'vp_pyMinArg');
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pyMinObj');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pyMinObj')).val());
            $(that.wrapSelector('#vp_pyMinObj')).replaceWith(function() {
    ...
    PythonCommon.prototype.showTypeRow = function(type) {
        if (type == 'object') {
            $(this.wrapSelector('#vp_pyMinObj')).closest('tr').show();
            $(this.wrapSelector('#vp_pyMinArg')).closest('tr').hide();
        } else if (type == 'argument') {
            $(this.wrapSelector('#vp_pyMinArg')).closest('tr').show();
            $(this.wrapSelector('#vp_pyMinObj')).closest('tr').hide();
        }
    }
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        if (type == 'object') {
            code.appendFormat('min({0})', $(this.wrapSelector('#vp_pyMinObj')).val());
        } else if (type == 'argument') {
            code.append('min(');
            var args = $(this.wrapSelector('#vp_pyMinArg')).find('.vp-py-args');
            var validArgCount = 0;
            args.each((i, tag) => {
