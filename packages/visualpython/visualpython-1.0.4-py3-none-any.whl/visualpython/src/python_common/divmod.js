define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyNumToDiv', 'vp_pyDivider' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyNumToDiv', 'vp-input')
        tblLayoutRequire.addReqRow("Input Number To Divide", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyDivider', 'vp-input')
        tblLayoutRequire.addReqRow("Input Divider", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat('divmod({0}, {1})', $(this.wrapSelector('#vp_pyNumToDiv')).val(), $(this.wrapSelector('#vp_pyDivider')).val());
