define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyInt', 'vp-input');
        tblLayoutRequire.addReqRow("Input Data", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" placeholder="10" />', 'vp_pyBase', 'vp-input');
        tblLayoutRequire.addRow("Base", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat('int({0}', $(this.wrapSelector('#vp_pyInt')).val());
        var base = $(this.wrapSelector('#vp_pyBase')).val();
        if (base != '') {
            code.appendFormat(", base='{0}'", base);
        }
        code.append(')');
