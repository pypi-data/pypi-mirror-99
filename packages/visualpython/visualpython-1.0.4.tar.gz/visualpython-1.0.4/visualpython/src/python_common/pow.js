define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyBase', 'vp_pyExp' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pyBase', 'vp-input', 'base/expression');
        tblLayoutRequire.addReqRow("Input Base", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyExp', 'vp-input');
        tblLayoutRequire.addReqRow("Exponent", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        var base = $(this.wrapSelector('#vp_pyBase')).val();
        var ndigits = $(this.wrapSelector('#vp_pyExp')).val();
        code.appendFormat("pow({0}, {1})", base, ndigits);
