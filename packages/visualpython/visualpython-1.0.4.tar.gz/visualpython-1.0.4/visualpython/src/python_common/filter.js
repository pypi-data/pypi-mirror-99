define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyFunction', 'vp_pyIterable' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyFunction', 'vp-input');
        tblLayoutRequire.addReqRow("Input Function", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyIterable', 'vp-input');
        tblLayoutRequire.addReqRow("Input Iterable Data", sbTagString.toString());
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pyIterable');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pyIterable')).val());
            $(that.wrapSelector('#vp_pyIterable')).replaceWith(function() {
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.append('filter(');
        var func = $(this.wrapSelector('#vp_pyFunction')).val();
        if (func != '') {
            code.appendFormat('{0}, ', func);
        }
        code.appendFormat('{0})', $(this.wrapSelector('#vp_pyIterable')).val());
