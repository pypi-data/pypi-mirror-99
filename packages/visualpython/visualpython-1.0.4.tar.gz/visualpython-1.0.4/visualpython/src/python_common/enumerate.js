define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyEnum', 'vp-input');
        tblLayoutRequire.addReqRow("Input Data", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyStart', 'vp-input');
        tblLayoutRequire.addRow("Start Index", sbTagString.toString());
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pyEnum');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pyEnum')).val());
            $(that.wrapSelector('#vp_pyEnum')).replaceWith(function() {
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat('enumerate({0}', $(this.wrapSelector('#vp_pyEnum')).val());
        var startIdx = $(this.wrapSelector('#vp_pyStart')).val();
        if (startIdx != '') {
            code.appendFormat(', start={0}', startIdx);
        }
        code.append(')');
