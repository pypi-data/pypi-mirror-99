define([
    ...
    PandasPackage.prototype.bindOptions = function() {
        ...
        if (this.metadata) {
            var sampleType = this.getMetadata('i0');
            if (sampleType) {
    ...
    PandasPackage.prototype.generateCode = function(addCell, exec) {
        ...
        // 백그라운드 실행
        //FIXME: 백그라운드 실행 후 셀 삽입&실행, 옵션페이지 닫는 부분에 있어 공통모듈 간 조정 필요
        // this.kernelExecute(bgCode.toString(), function(result) {
        //     that.doExecute = true;
...
/*
        ...
        // 완료된 dataframe 조회
        var outputVariable = $(this.wrapSelector('#'+this.package.output[0].name)).val();
        sbCode.appendFormat('{0}', outputVariable);
*/
