define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyStr', 'vp-input');
        tblLayoutRequire.addRow("Input Data", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pyEncoding', 'vp-input', 'uft-8');
        tblLayoutRequire.addRow("Encoding", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat('str({0}', $(this.wrapSelector('#vp_pyStr')).val());
