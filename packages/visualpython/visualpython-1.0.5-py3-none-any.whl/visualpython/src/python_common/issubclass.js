define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyClass', 'vp_pyClassInfo' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyClass', 'vp-input');
        tblLayoutRequire.addReqRow("Select Object", sbTagString.toString());

        sbTagString.clear();
        // TODO: get built-in types and show as suggest input
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyClassInfo', 'vp-input');
        tblLayoutRequire.addReqRow("Class Info", sbTagString.toString());
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pyClass');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pyClass')).val());
            $(that.wrapSelector('#vp_pyClass')).replaceWith(function() {
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        var obj = $(this.wrapSelector('#vp_pyClass')).val();
        var classinfo = $(this.wrapSelector('#vp_pyClassInfo')).val();
        code.appendFormat('issubclass({0}, {1})', obj, classinfo);
