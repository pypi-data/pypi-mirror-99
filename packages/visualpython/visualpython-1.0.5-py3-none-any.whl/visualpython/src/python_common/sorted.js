define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pySorted', 'vp-input');
        tblLayoutRequire.addReqRow("Select Data", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormatLine('<select id="{0}" class="{1}">', 'vp_pyReverse', 'vp-select');
        sbTagString.appendFormatLine('<option value="{0}">{1}</option>', '', 'Default');
        sbTagString.appendFormatLine('<option value="{0}">{1}</option>', 'True', 'True');
        sbTagString.appendFormatLine('<option value="{0}">{1}</option>', 'False', 'False');
        sbTagString.append('</select>');
        tblLayoutRequire.addRow("Reverse", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyReturn', 'vp-input l');
        tblLayoutRequire.addRow("Return Variable", sbTagString.toString());
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pySorted');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pySorted')).val());
            $(that.wrapSelector('#vp_pySorted')).replaceWith(function() {
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat('sorted({0}', $(this.wrapSelector('#vp_pySorted')).val());
        var reverse = $(this.wrapSelector('#vp_pyReverse')).val();
        if (reverse != '') {
            code.appendFormat(', reverse={0}', reverse);
        }
        code.append(')');
