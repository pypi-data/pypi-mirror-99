define([
    ...
    PythonCommon.prototype.optionValidation = function() {
        var required = [ 'vp_pyObj', 'vp_pyName', 'vp_pyValue' ];
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyName', 'vp-input');
        tblLayoutRequire.addReqRow("Attribute Name", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyValue', 'vp-input');
        tblLayoutRequire.addReqRow("Attribute Value", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.appendFormat("setattr({0}, '{1}', {2})"
                        , $(this.wrapSelector('#vp_pyObj')).val()
                        , $(this.wrapSelector('#vp_pyName')).val()
                        , $(this.wrapSelector('#vp_pyValue')).val());
