define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyType', 'vp-input');
        tblLayoutRequire.addRow("Input Type", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" />', 'vp_pyObjType', 'vp-input');
        tblLayoutRequire.addRow("Input Object or Type", sbTagString.toString());
        ...
        pdGen.vp_searchVarList([], function(result) {
            ...
            suggestInput.setComponentID('vp_pyObjType');
            suggestInput.addClass('vp-input');
            suggestInput.setSuggestList(function() { return varList; });
            suggestInput.setNormalFilter(false);
            suggestInput.setValue($(that.wrapSelector('#vp_pyObjType')).val());
            $(that.wrapSelector('#vp_pyObjType')).replaceWith(function() {
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        code.append('super(');
        var func = $(this.wrapSelector('#vp_pyType')).val();
        if (func != '') {
            code.appendFormat('{0}', func);
        }
        var func = $(this.wrapSelector('#vp_pyObjType')).val();
        if (func != '') {
            code.appendFormat(', {0}', func);
        }
        code.append(')');
