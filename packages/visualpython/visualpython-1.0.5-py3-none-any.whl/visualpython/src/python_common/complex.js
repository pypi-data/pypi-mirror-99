define([
    ...
    PythonCommon.prototype.initHtml = function() {
        ...
        sbTagString.clear();
        sbTagString.appendFormat('<input type="text" id="{0}" class="{1}" placeholder="{2}"/>', 'vp_pyReal', 'vp-input', 'real/expression');
        tblLayoutRequire.addRow("Input Real", sbTagString.toString());

        sbTagString.clear();
        sbTagString.appendFormat('<input type="number" id="{0}" class="{1}" />', 'vp_pyImag', 'vp-input');
        tblLayoutRequire.addRow("Input Imag", sbTagString.toString());
    ...
    PythonCommon.prototype.generateCode = function(addCell = false, exec = false) {
        ...
        var real = $(this.wrapSelector('#vp_pyReal')).val();
        var imag = $(this.wrapSelector('#vp_pyImag')).val();
        code.appendFormat("complex(");
        if (real != '') {
            code.append(real);
            if (imag != '') {
                code.appendFormat(', {0}', imag);
            }
        }
        code.append(')');
