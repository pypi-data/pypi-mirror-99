define([
    ...
    */
     ...
     * @param {Object} apiData api block을 생성할 때 필요한 funcID값 전달 { funcID: funcID }
     */
    BlockContainer.prototype.createBlock = function(blockType, blockData = null, apiData = null) {
     ...
     *  @param {Block} thisBlock
     *  @param {boolean} isFirst true false
    */
    BlockContainer.prototype.makeBlockDom = function(thisBlock, isFirst) {
        ...
        if ( blockType == BLOCK_CODELINE_TYPE.CLASS ) {
            ...
            codeLineStr = GenerateDefCode(thisBlock);
        } else if (blockType == BLOCK_CODELINE_TYPE.IF) {
            codeLineStr = GenerateIfCode(thisBlock, BLOCK_CODELINE_TYPE.IF);
        } else if (blockType == BLOCK_CODELINE_TYPE.ELIF) {
            codeLineStr = GenerateIfCode(thisBlock, BLOCK_CODELINE_TYPE.ELIF);
        } else if (blockType == BLOCK_CODELINE_TYPE.FOR) {
            codeLineStr = GenerateForCode(thisBlock);
        } else if (blockType == BLOCK_CODELINE_TYPE.WHILE) {
            codeLineStr = GenerateWhileCode(thisBlock);
        } else if (blockType == BLOCK_CODELINE_TYPE.EXCEPT) {
            codeLineStr = GenerateExceptCode(thisBlock);
        } else if (blockType == BLOCK_CODELINE_TYPE.RETURN) {
            codeLineStr = GenerateReturnCode(thisBlock);
        } else if (blockType == BLOCK_CODELINE_TYPE.LAMBDA) {
            codeLineStr = GenerateLambdaCode(thisBlock);
    ...
    BlockContainer.prototype.makeCode = function(thisBlock) {
        ...
        blockList_thisBlockArea.some( (block,index) => {
            ...
            if ( blockType == BLOCK_CODELINE_TYPE.API ) {
                ...
                codeList.push(block.setCodeLineAndGet(indentString, false));
            }
        });
    ...
    */
    ...
    BlockContainer.prototype.resizeAPIblock = function() {
        ...
        var optionPageRectWidth = mainPageRectWidthNum - boardPageRectWidth - buttonsPageRectWidth - 50;

        if (optionPageRectWidth < NUM_OPTION_PAGE_WIDTH) {
            boardPageRectWidth = mainPageRectWidthNum - NUM_OPTION_PAGE_WIDTH - 50 - buttonsPageRectWidth;
            // optionPageRectWidth = NUM_OPTION_PAGE_WIDTH;
        } 
        
        var optionPageRectWidth_maxWidth = mainPageRectWidthNum - buttonsPageRectWidth - NUM_API_BOARD_CENTER_PAGE_MIN_WIDTH - 50;
        ...
        this.setBlockMaxWidth_blockList(boardPageRectWidth - 70);

        /** node 블럭 width 재 계산 */
        var nodeBlockList = this.getNodeBlockList();
        nodeBlockList.forEach(nodeBlock => {
            nodeBlock.renderNodeBlockWidth(boardPageRectWidth - 70);
        });
    }
