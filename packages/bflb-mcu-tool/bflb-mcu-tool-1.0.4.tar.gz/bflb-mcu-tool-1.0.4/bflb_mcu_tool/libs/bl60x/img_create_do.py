...
# update efuse info
def img_update_efuse_sp(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel,
                        sec_eng_key):
...
def img_update_efuse_cpu0(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel,
                          sec_eng_key):
...
def img_update_efuse_cpu1(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel,
                          sec_eng_key):
...
def encrypt_loader_bin_do(file, sign, encrypt, cfg):
    return False, None


def img_creat_process(img_type, flash_img, cfg):
    encrypt_blk_size = 16
    ...
    if flash_img == 1:
        ...
        if encrypt != 0:
            if encrypt == 1:
                # AES 128
                img_update_efuse_fun(cfg, sign, pk_hash, 1,
                                     encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
            if encrypt == 2:
                # AES 256
                img_update_efuse_fun(cfg, sign, pk_hash, 3,
                                     encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
            if encrypt == 3:
                # AES 192
                img_update_efuse_fun(cfg, sign, pk_hash, 2,
                                     encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
        else:
        ...
        # update efuse
        if encrypt != 0:
            img_update_efuse_fun(cfg, sign, pk_hash, 1, None, key_sel,
...
def bind_if_img(img_dir_path, cfg):
    # write whole image
...
def sign_process(cfg, data_tohash, file, privatekey_file_uecc, publickey_file, signer):
    fp = open_file(file, 'rb')
    header = bytearray(fp.read())
    header = header + bytearray(0)
    fp.close()
    pk_data, pk_hash, signature_field = img_create_sign_data(data_tohash, privatekey_file_uecc,
                                                             publickey_file)
    pk_data = pk_data + bflb_utils.get_crc32_bytearray(pk_data)
    pk_pos = 244
...
def img_create_do(args, img_dir_path=None, config_file=None):
    ...
    try:
        ...
        bflb_utils.printf(e)  # will something like "option -a not recognized")
    if img_type == "media":
        flash_img = 1
    else:
        flash_img = 0
    ...
    # deal image creation
    if cpu_type == "cpu0":
        ret0, data_tohash_cpu0 = img_creat_process("cpu0", flash_img, cfg)
    elif cpu_type == "cpu1":
        ret1, data_tohash_cpu1 = img_creat_process("cpu1", flash_img, cfg)
    elif cpu_type == "all":
        ret0, data_tohash_cpu0 = img_creat_process("cpu0", flash_img, cfg)
        ret1, data_tohash_cpu1 = img_creat_process("cpu1", flash_img, cfg)
    else:
        img_creat_process("", flash_img, cfg)
        return  # shoud return here for sp
    if ret0 == "OK" and ret1 == "OK":
        if signer == "cpu0":
            bflb_utils.printf("cpu0 sign cpu1 ")
            if flash_img == 1:
                sign_process(cfg, data_tohash_cpu1, cfg.get("Img_CPU1_Cfg", "bootinfo_file"),
                             cfg.get("Img_CPU0_Cfg", "privatekey_file_uecc"),
                             cfg.get("Img_CPU0_Cfg", "publickey_file"), "cpu0")
            else:
                sign_process(cfg, data_tohash_cpu1, cfg.get("Img_CPU1_Cfg", "whole_img_file"),
                             cfg.get("Img_CPU0_Cfg", "privatekey_file_uecc"),
                             cfg.get("Img_CPU0_Cfg", "publickey_file"), "cpu0")
        elif signer == "cpu1":
            bflb_utils.printf("cpu1 sign cpu0 ")
            if flash_img == 1:
                sign_process(cfg, data_tohash_cpu0, cfg.get("Img_CPU0_Cfg", "bootinfo_file"),
                             cfg.get("Img_CPU1_Cfg", "privatekey_file_uecc"),
                             cfg.get("Img_CPU1_Cfg", "publickey_file"), "cpu1")
            else:
                sign_process(cfg, data_tohash_cpu0, cfg.get("Img_CPU0_Cfg", "whole_img_file"),
                             cfg.get("Img_CPU1_Cfg", "privatekey_file_uecc"),
                             cfg.get("Img_CPU1_Cfg", "publickey_file"), "cpu1")
    else:
    ...
    # if user select all cpu, bind sdio img
    if cpu_type == "all" and flash_img == 0:
        bind_if_img(img_dir_path, cfg)
