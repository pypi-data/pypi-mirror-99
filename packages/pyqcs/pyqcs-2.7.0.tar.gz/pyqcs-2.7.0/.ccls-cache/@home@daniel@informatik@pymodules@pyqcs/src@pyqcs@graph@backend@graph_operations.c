...
graph_La_transform(RawGraphState * self, npy_intp i)
{
    ...
    npy_intp lM = graph_get_naXna_edge_count(self, i);
    ...
    while(ll_iter_next(iter_b, &b))
    {
        ...
        self->phase_from_SU2_gauge = (self->phase_from_SU2_gauge + 1) % 8;
    ...
    npy_intp lMp = graph_get_naXna_edge_count(self, i);
    self->phase_from_CZs -= (lM - lMp);
lat_exit:
    free(iter_b);
    free(iter_c);
    return result;
...
graph_isolated_two_qbit_CZ(RawGraphState * self, npy_intp i, npy_intp j)
{
    ...
    self->phase_from_SU2_gauge = (self->phase_from_SU2_gauge + two_qbit_vops_after_CZ[lookup_table_index][3]) % 8;
...
graph_clear_vop(RawGraphState * self, npy_intp a, npy_intp b)
{
    ...
    if(!vop_commutes_with_CZ(self->vops[a]))
    {
        vop = self->vops[a];
        ...
        //self->phase_from_SU2_gauge = (self->phase_from_SU2_gauge - 3) % 8;
    }
gcv_exit:
    return result;
}
...
{
    double phase = self->phase_from_local_gates * M_PI_4;
    ...
    {
        iter = ll_iter_t_new(self->lists[i]);
        while(ll_iter_next(iter, &j))
        {
            if(j < i)
            {
                edges++;
    ...
    phase += (M_PI * (edges - self->phase_from_CZs)) / self->length;
    phase += (M_PI_4 * self->phase_from_SU2_gauge);

    return phase;
}

npy_intp
graph_get_naXna_edge_count(RawGraphState * self, npy_intp a)
{
    if(!ll_length(self->lists[a]))
    {
        return 0;
    }
    ...
    ll_iter_t * na_iter1 = ll_iter_t_new(self->lists[a]);
    ll_iter_t * na_iter2 = ll_iter_t_new(self->lists[a]);
    npy_intp b;
    while(ll_iter_next(na_iter1, &b))
    {
        npy_intp c;
        while(ll_iter_next(na_iter2, &c))
        {
            if(c > b)
            {
                break;
            }
            if(ll_has_value(self->lists[b], c))
            {
                result++;
            }
        }
        ll_iter_reset(na_iter2);
    }
    free(na_iter1);
    free(na_iter2);
    return result;
