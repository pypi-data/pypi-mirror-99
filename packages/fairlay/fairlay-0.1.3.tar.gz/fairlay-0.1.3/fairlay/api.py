...
class FairlayPrivateAPI(object):
    ...
    def _send_request(self, endpoint, data=None):
        ...
        if output.decode("utf-8").split("|")[-1].startswith("{") and output.decode("utf-8").split("|")[-1].endswith("}"):
            try:
                otp = ujson.loads(output.decode("utf-8").split("|")[-1])
                return otp
            except ValueError:
    ...
    def get_market(self, market):
        return self._send_request('get_market', data=market)
