...
class DateWidget(widgets.DateInput):
    ...
    def build_attrs(self, *args, **kwargs):
        args = super().build_attrs(*args, **kwargs)
...
class TimeChoiceWidget(widgets.Select):
    ...
    def value_from_datadict(self, *args):
        data = super().value_from_datadict(*args)
        if data == self.NOW:
    ...
    def render(self, name, value, attrs=None, renderer=None, choices=()):
        if value:
            ...
            if value and isinstance(value, datetime.time):
                ...
                if value_str not in self.choice_values:
                    choices = list(choices)
                    choices.append((value_str, value.strftime(self.repr_format)))
        return super().render(name, value, attrs=attrs, renderer=None, choices=choices)
...
class SplitDateTime(widgets.SplitDateTimeWidget):
    ...
    def format_output(self, rendered_widgets):
        return mark_safe(f"{rendered_widgets[0]} {rendered_widgets[1]}")
...
class DateRadioInput(RadioChoiceInput):
    ...
    def render(self, name=None, value=None, attrs=None, renderer=None, choices=()):
        ...
        return mark_safe(
            f"<label{label_for}>{self.tag()} {self.label_text}: {date_widget}</label>"
        )
...
class DateRenderer(RadioFieldRenderer):
    ...
    def render(self):
        ...
        return mark_safe('<fieldset class="datetime">\n{}\n</fieldset>'.format(text))
...
class APIChoiceWidget(widgets.Input):
    ...
    * data-title: The title of the field.
    ...
    input_type = "hidden"
    template = '<div class="api-select" data-title="%(value)s" data-base-api="%(base_api_link)s"  data-api="%(link)s" data-add="%(add_link)s">%(input)s</div>'
    ...
    def render(self, name, value, attrs=None, renderer=None, choices=()):
        data = {
            ...
            "base_api_link": self._api_link,  # url with no query args (e.g. ?type=choices)
    ...
    def get_api_link(self):
        ...
        if url:
            qs = self.get_qs()
            url = f"{url}?type=choices"
            if qs:
                url = f"{url}&amp;" + "&amp;".join(
                    [
                        f"{k}={urllib.parse.quote(str(v).encode('utf8'))}"
                        for k, v in list(qs.items())
                    ]
                )

                url = f"{url}&amp;" + "&amp;".join(
                    [f"exclude={x}" for x in list(qs.keys())]
                )
        return url
...
class APIModelChoiceWidget(APIChoiceWidget):
    ...
    def label_for_value(self, value):
        return super().label_for_value(value, key="pk")
...
class APIManyChoiceWidget(APIChoiceWidget, widgets.SelectMultiple):
    ...
    template = '<div class="api-select" data-base-api="%(base_api_link)s" data-api="%(api_link)s" data-add="%(add_link)s">%(options)s</div>'
...
class HTMLWidget(widgets.HiddenInput):
    """
    WYSIWYG Widget. Adds *widget-wysiwyg* to the class attribute
    in the rendered html.
    """

    template = "cms/quillRTE.html"

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        classes = ["editor__hidden-rte"]
    ...
    def render(self, *args, **kwargs):
        ...
        return mark_safe(
            '<div class="editor">{0}{1}</div>'.format(
                text, render_to_string(self.template, context=self.attrs)
            )
        )
...
class AnnotatedHTMLWidget(widgets.MultiWidget):
    ...
    def __init__(self, attrs=None):
        html_classes = ["editor__hidden-rte", "useNormal"]
        html_attrs = {"class": " ".join(html_classes)}
        _widgets = (
            HTMLWidget(attrs=html_attrs),
            widgets.Textarea(),
        )
