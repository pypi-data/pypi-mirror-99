...
from . import fields, helpers, renders, transaction, widgets
...
class ListView(ModelCMSMixin, MultipleObjectMixin, ModelCMSView):
    ...
    :param parent_preview: Should we show 'Preview' button \
    that links to parent object. \
    Defaults to False.
    """
    ...
    # Preview button with parent's URL to preview
    parent_preview = False
    ...
    def get_filter(self, **filter_kwargs):
        ...
        q_objects = super().get_filter(**filter_kwargs)
    ...
    def get_visible_fields(self, formset):
        ...
        if formset:
            for x in formset.empty_form.visible_fields():
                if x.name not in visible_fields:
    ...
    def _get_query_string(self, request, exclude_page=True):
        ...
        return f"?{qs.urlencode()}&" if qs else "?"

    def _sort_queryset(self, object_list, sort_fields, order_types):
        queryset = self.object_list
        sort_list = []

        for field, order in zip(sort_fields, order_types):
            field = helpers.get_sort_field(field, object_list.model)

            if order and field:
                if order == helpers.AdminList.ASC:
    ...
    def get_list_data(self, request, **kwargs):
        ...
        * **parent_preview_url** - URL to preview parent object.
        """
        ...
        if self.can_sort:
            ...
            sort_fields = request.GET.get("sf", default)
            order_types = request.GET.get("ot", default_order)
        ...
        queryset = self._sort_queryset(self.object_list, sort_fields, order_types)
        ...
        adm_list = helpers.AdminList(
            formset, queryset, visible_fields, sort_fields, order_types, self.model_name
        )

        parent_preview_url = None
        if self.parent_preview:
            parent_preview_url = self.parent_object.get_absolute_url()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset and formset.is_valid():
            ...
            return self.render(
                request,
                redirect_url=url,
                message=f"{changecount} items updated",
