    ...
    def get_api_link(self):
        ...
        if url:
            ...
            if qs:
                ...
                url = f"{url}&amp;" + "&amp;".join([f"exclude={x}" for x in list(qs.keys())])
        return url
...
class HTMLWidget(widgets.HiddenInput):
    ...
    def render(self, *args, **kwargs):
        ...
        return mark_safe(u"<div class=\"editor\">{0}{1}</div>".format(text, render_to_string(self.template, context=self.attrs)))
