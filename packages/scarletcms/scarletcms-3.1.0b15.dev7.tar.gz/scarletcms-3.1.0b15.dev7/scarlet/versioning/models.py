...
from django.db.models.fields import Field, FieldDoesNotExist, related
...
class Cloneable(models.Model):
    ...
    **last_save:** Datetime that gets updated every time the object gets saved.
    ...
    def register_related(cls, related_name):
        ...
        if not isinstance(cls._clone_related, list):
            cls._clone_related = list(cls._clone_related)

        if related_name not in cls._clone_related:
...
def add_managers(attrs):
    if "objects" not in attrs and "_default_manager" not in attrs:
...
class SharedMeta(models.base.ModelBase):
    def add_to_class(cls, name, value):
        if not cls._meta.abstract:
            ...
            if isinstance(value, models.ManyToManyField) or isinstance(
                ...
                if version_model:
                    if isinstance(value, models.ManyToManyField):
                        ...
                        if not isinstance(value, M2MFromVersion):
                            raise TypeError(
                                f"The model {name} cannot contain a ManyToManyField "
                                "use M2MFromVersion instead"
                            )
                        value.db_table = version_model._meta.get_field(
                            name
                        ).m2m_db_table()
                else:
                    if (
                        value.remote_field.related_name
                        and not value.remote_field.related_name.endswith("+")
                    ):
                        value.remote_field.related_name = (
                            f"{value.remote_field.related_name}_version"
                        )
...
class VersionViewMeta(SharedMeta):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if meta and hasattr(meta, "_base_model"):
            ...
            base_name = f"{base_model._meta.app_label}.{base_model.__name__}"
        ...
        for base in bases:
            if issubclass(base, models.Model) and not base._meta.abstract:
                raise TypeError(
                    f"You cannot use a non abstract base model {base!r} "
                    f"with a VersionView model {name}."
                )

            if issubclass(base, BaseModel):
                raise TypeError(
                    f"VersionView model {cls!r} can't inherit from a BaseModel {base!r}"
                )
            ...
            if issubclass(base, BaseVersionedModel):
                if not issubclass(type(base), VersionViewMeta):
                    if custom_version:
                        raise TypeError(
                            "Django inheritence will only allow VersionView model "
                            f"{cls!r} to inherit from one and only one BaseVersionedModel"
                        )
                    else:
        ...
        vname = f"{name}VersionReferences"
        v_mod = type(
            vname,
            (object,),
            {
        ...
        version_model = SharedMeta(
            f"{name}_version", tuple(version_bases), versioned_attrs
        )
        ...
        mod = super().__new__(cls, name, tuple(new_bases), attrs)
    ...
    def add_to_class(cls, name, value):
        ...
        return super().add_to_class(name, value)
...
class VersionModelMeta(models.base.ModelBase):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if not getattr(meta, "abstract", False):
            if not getattr(meta, "_base_model", None) or not issubclass(
                meta._base_model, BaseModel
            ):
                raise TypeError(
                    f"{name} must declare a subclass of BaseModel as "
                    "it's _base_model in it's meta class"
                )
            else:
                base_model = meta._base_model
                meta._base_model = None
        ...
        mod = super().__new__(cls, name, bases, attrs)
...
class BaseVersionedModel(Cloneable, Schedulable):
    ...
    state = models.CharField(max_length=50, choices=STATES, editable=False)
    ...
    def unpublish(self):
        ...
        with xact():
            ...
            for obj in klass.normal.filter(
                object_id=self.object_id, state=self.SCHEDULED
            ):
                obj.delete()
    ...
    def publish(self, user=None, when=None):
        ...
        now = timezone.now()
        state = self.PUBLISHED if not when or when <= now else self.SCHEDULED

        with xact():
            # If this item hasn't gone live yet delete the old scheduled items
            published = False
            ...
            if self.state == self.DRAFT:
                ...
                self.date_published = when or now
            ...
            if state == self.PUBLISHED:
                self._publish(date_published=when)
            else:
                self.schedule(when=when)

    def _delete_reverse(self, reverse):
        try:
            return super()._delete_reverse(reverse)
        except KeyError:
            ...
            sub = self._meta._view_model(object_id=self.object_id, vid=self.vid)
            return sub._delete_reverse(reverse)

    def _gather_reverse(self, reverse):
        try:
            return super()._gather_reverse(reverse)
        except KeyError:
            # Could this be on the view model?
            if not hasattr(self._meta, "_view_model"):
                raise
    ...
    def make_draft(self):
        ...
        with xact():
            ...
            try:
                klass = self.get_version_class()
                old_draft = klass.normal.get(object_id=self.object_id, state=self.DRAFT)
                old_draft.delete()
    ...
    def _publish(self, published=True, **kwargs):
        with xact():
            ...
            if published:
                ...
                self.date_published = kwargs.get("date_published") or now
            else:
    ...
    def purge_archives(self):
        ...
        klass = self.get_version_class()
        qs = klass.normal.filter(
            object_id=self.object_id, state=self.ARCHIVED
    ...
    def status_line(self):
        ...
        if date:
            status = f"{status}: {formats.date_format(date, 'SHORT_DATE_FORMAT')}"
        return status
    ...
    def schedule(self, when=None, action=None, **kwargs):
        ...
        """
        action = "_publish"
        super().schedule(when=when, action=action, **kwargs)


class VersionView(BaseVersionedModel, metaclass=VersionViewMeta):
    """
    ...
    @classmethod
    def register_related(cls, related_name):
        super().register_related(related_name)
...
class VersionModel(BaseVersionedModel, metaclass=VersionModelMeta):
    """
    Abstract base model for implementations
