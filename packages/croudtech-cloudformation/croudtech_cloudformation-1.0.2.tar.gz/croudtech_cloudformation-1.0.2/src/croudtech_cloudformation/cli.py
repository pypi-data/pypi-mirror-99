...
@click.option("--cli-string-separator", required=False, default=" ", type=click.Choice([' ', ',']))
@click.argument('output_file', required=False, type=click.File('wb'), default=False)
@cli.command()
@click.pass_context
def exports_to_parameters(ctx, stack_prefix, stack_name, output_format, cli_string_separator, output_file):
    ...
    if output_format == "parameter_file":
        ...
        cli_string = cli_string_separator.join(["%s=%s" % (p['Name'],p['Value']) for p in parameters])
