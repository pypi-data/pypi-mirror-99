...
log = logging.getLogger('aurore.config')
...
class Config(UserDict):
    ...
    def load_file(self, config_file):
        try:
            ...
            # aurore knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "aurore encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('.aurore/config.yaml')
