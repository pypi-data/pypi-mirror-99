...
def norm_join(base,path):
    ...
    if os.path.isabs(full_path):
        ...
        full_base = os.path.expandvars(base)
...
def yaml_load(source, loader=yaml.Loader):
    ...
    try:
        ...
        # objects. Some aurore internal calls, sometimes in test lib, will
...
def get_themes():
    ...
    themes = {}
    builtins = pkg_resources.get_entry_map(dist='aurore', group='aurore.themes')

    for theme in pkg_resources.iter_entry_points(group='aurore.themes'):

        if theme.name in builtins and theme.dist.key != 'aurore':
