...
class InMessage(Event):
    ...
    def parse_header(self, data):
        try:
            ...
            self.parse_start_line(header_lines[0])
            return
        except:
            pass
...
class OutMessage(Event):
    def __init__(self, channel, status_line, headers=None, data=None):
        Event.__init__(self, channel.env)
...
class InRequest(InMessage):
    ...
    def parse_start_line(self, start_line):
        self.method, self.uri, self.version = start_line.split(' ', 2)
...
class InResponse(InMessage):
    def parse_start_line(self, start_line):
        self.version, self.code, self.phrase = start_line.split(' ', 2)
