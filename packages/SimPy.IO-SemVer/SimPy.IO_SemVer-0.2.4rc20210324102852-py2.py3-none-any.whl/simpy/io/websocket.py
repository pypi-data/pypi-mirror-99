...
class WebSocketHixie(object):
    ...
    packet_header_footer = struct.Struct('!B')
    ...
    def _client_handshake(self, uri, data=b''):
        ...
        headers = '\r\n'.join(['%s: %s' % header for header in headers])
        handshake = ((status_line + headers).encode('iso-8859-1') +
                b'\r\n\r\n' + key_3)
    ...
    def _reader_proc(self, data=b''):
        read = self.socket.read
        blocksize = self.blocksize
        packet_footer = self.packet_header_footer.pack(0xff)
        while True:
            event = yield self._read_ev
            self._read_ev = self.env.event()
            ...
            try:
                ...
                if header[0] & 0x80:
                    raise RuntimeError(
                        'Length indicated packages not supported yet'
                    )
                data = data[1:]
    ...
    def _writer_proc(self):
        write = self.socket.write
        packet_header = self.packet_header_footer.pack(0x00)
        packet_footer = self.packet_header_footer.pack(0xff)
