...
def append_req_file(
    ...
    if requirements_path is not None:
        with open(requirements_path) as requirements_file:
            packages = packages + tuple((line.rstrip() for line in requirements_file))
...
def install(
    ...
    append_req_file(requirements_path, packages)

    package_str_set = dedup_packages(packages)

    if game is not None:
        package_str_set.add(game)
    ...
    if enable is not None:
        ...
        if game is not None and local_db.is_world_game(enable, game.split("/")[1]):
