...
class ContentDB:
    ...
    _cache: Dict[str, Dict[str, requests.Response]]
    ...
    def _get_request(self, url: str, cache=True) -> requests.Response:
        if url in self._cache["get"]:
            response = self._cache["get"][url]
        else:
            response = requests.get(url)
            self._cache["get"][url] = response
            sleep(self._global_sleep)
        return response

    def _head_request(
        self, url: str, allow_redirects: bool = False, cache=True
    ) -> requests.Response:
        if url in self._cache["head"]:
            response = self._cache["head"][url]
        else:
