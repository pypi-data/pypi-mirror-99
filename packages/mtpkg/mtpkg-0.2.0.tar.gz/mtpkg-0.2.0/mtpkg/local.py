...
class LocalPackageDatabase:
        ...
        for mods_child in mods_path.iterdir():
            if mods_child.is_dir():
                ...
                if (mods_child / "modpack.conf").exists():
                    ...
                    for submod in filter(
                        lambda p: p.is_dir() and (p / "mod.conf").exists(),
                        mods_child.iterdir(),
                    ):
    ...
    def install_release(self, package: RemotePackage, release_id: int, src: IO[bytes]):
        ...
        with zipfile.ZipFile(src) as compressed_release:
            ...
            if len(depth1_contents) == 1 and depth1_contents[0].is_dir():
                ...
                for child in depth1_contents[0].iterdir():
                    if child.name in conf_filenames:
                        if conf_depth is None:
                            conf_name = child.name
                            conf_depth = 2
                        found_conf_filenames.add(child.name)
            ...
            if ("modpack.conf" in found_conf_filenames) and (
                "mod.conf" in found_conf_filenames
            ):
                raise LocalError(
                    f"Found a modpack.conf and mod.conf at the top-level directory of '{package.author}/{package.name}'"
                )
        ...
        if conf_name == "modpack.txt":
            conf_name = "modpack.conf"
            logging.warning(
                f"'{package.author}/{package.name}' has only a modpack.txt and no modpack.conf but did not specify a maximum Minetest version of 0.4.x, fixing locally"
            )
        conf_path = install_path / conf_name
        if not conf_path.exists():
            conf_path.touch()
        with open(conf_path, "r") as conf_file:
            new_conf_buf = io.StringIO()
            for line in conf_file:
                if line.split("=")[0].strip() == "name":
                    continue
                if not line.endswith("\n"):
                    line += "\n"
                new_conf_buf.write(line)
            new_conf_buf.write(f"name = {package.name}\n")
            new_conf_buf.write(f"author = {package.author}\n")
            new_conf_buf.write(f"release = {release_id}\n")
        with open(conf_path, "w") as conf_file:
            new_conf_buf.seek(0)
            shutil.copyfileobj(new_conf_buf, conf_file)
        new_conf_buf.close()
        ...
        # recursively add mods to mod2pack
        for submod in filter(
            lambda p: p.is_dir() and (p / "mod.conf").exists(), install_path.iterdir()
        ):
            submod_conf = parse_conf_file(submod / "mod.conf")
            submod_name = submod_conf.get("name", submod.name)
            self._mod2pack[submod_name] = local_pkg
...
def get_possible_conf_names(pkgtype: PackageType) -> Set[str]:
    if pkgtype == PackageType.MOD:
        return {"mod.conf", "modpack.conf", "modpack.txt"}
    if pkgtype == PackageType.GAME:
        return {"game.conf"}
    if pkgtype == PackageType.TEXTURE_PACK:
...
def parse_conf_file(path: Path) -> Dict[str, str]:
    ...
    with open(path) as conf_file:
        ...
        while line:
            ...
            if len(line_split) != 2:
                raise LocalError(
                    f"Failed to parse line '{line.rstrip()}' in configuration file at {path}"
                )
            ...
            if value.startswith('"""'):
                ...
                if not value.rstrip().endswith('"""'):
                    ...
                    while line and (not line.rstrip().endswith('"""')):
                        ...
                        line = conf_file.readline()
                    if not line:
                        raise LocalError(f"Couldn't parse configuration file at {path}")
                    value += line.rstrip()[:-3]
                else:
