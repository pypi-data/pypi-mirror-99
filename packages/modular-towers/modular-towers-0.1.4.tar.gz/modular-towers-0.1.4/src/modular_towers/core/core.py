...
try:
    ...
    powmod = pow
    from modular_towers.prelims import ext_gcd

def mod_tower(seq: List[int], m: int) -> int:
    """
    ...
    def _mod_tower(seq, m):
        ...
        g = gcd(b, m)
        if g == 1:
            return powmod(b, _mod_tower(e, totient(m)), m)
        ...
        h = m//n
        _, x, y = ext_gcd(n, h)
        return (h*(y%n)*powmod(b, _mod_tower(e, totient(n)), n)+
                n*(x%h)*(powmod(b, pow_list(e), h) if pow_lt(e, k) else 0))%m
    
    return _mod_tower(seq, m)