...
class getStripe:
            ...
            def main_null_calc(chr):
                with np.errstate(divide='ignore', invalid='ignore'):
                    ...
                    for it in range(itera):
                        ...
                        test_region_start0 = np.uint64(test_region_start0)
                        ...
                        if len(pool) == 0:
                            ...
                            for i in range(len(pool)):
                                ...
                                for j in range(0,400):
                                    ...
                                    if it > 0 :
                                        y = y - 400
                                    tableft[j, i] = np.nanmean(mat[(y - background_up):(y + background_down), (x - background_up-background_size):(x - background_up)])
                                    tabcenter[j, i] = np.nanmean(mat[(y - background_up):(y + background_down), (x - background_up):(x + background_down)])
                                    tabright[j, i] = np.nanmean(mat[(y - background_up):(y + background_down), (x + background_down):(x + background_down+background_size)])
                    ...
                    if depl > 0:
                        ...
                        test_region_start0 = np.uint64(test_region_start1 - (400*self.resol))
    ...
    def scoringstripes(self, df, expecVal, mask='0'):
        ...
        def iterate_idx(i,is_mask,exval):
            ...
            x_coord = str(c) + ":" + str(xs) + '-' + str(xe)
            y_coord = str(c) + ":" + str(ys) + '-' + str(ye)
            ...
            centerTotal = np.nansum(center)
            centerMean = np.nanmean(center)
    ...
    def extract(self, MP, index, perc , bgleft, bgright):
        with np.errstate(divide='ignore', invalid='ignore'):
