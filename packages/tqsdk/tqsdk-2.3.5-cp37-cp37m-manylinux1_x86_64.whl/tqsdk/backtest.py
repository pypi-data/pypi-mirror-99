...
class TqBacktest(object):
    ...
    **注意** ：如果未订阅 quote，模拟交易在下单时会自动为此合约订阅 quote ，根据回测时 quote 的更新规则，如果此合约没有订阅K线或K线周期大于分钟线 **则会自动订阅一个分钟线** 。
    ...
    async def _run(self, api, sim_send_chan, sim_recv_chan, md_send_chan, md_recv_chan):
        ...
        self._requests = {}  # 所有用户请求的 chart 序列，如果用户订阅行情，默认请求 1 分钟 Kline
                ...
                if symbols_diff:
                    ...
                    for query_id, query_result in symbols_diff.items():
                        if query_id.startswith("PYSDK_quote") and query_result.get("error", None) is None:
                            quotes = self._api._symbols_to_quotes(query_result, {"instrument_id", "price_tick"})
        ...
        if self._pending_peek:
            ...
            if not self._diffs:
                while self._requests:
                    min_request_key = min(self._requests.keys(), key=lambda serial: self._requests[serial]["timestamp"])
                    timestamp = self._requests[min_request_key]["timestamp"]  # 所有已订阅数据中的最小行情时间
                    quotes_diff = self._requests[min_request_key]["quotes"]
                    ...
                    diff = self._requests[min_request_key]["diff"]
                ...
                if not self._requests and not self._diffs:  # 当无可发送数据时则抛出BacktestFinished例外,包括未订阅任何行情 或 所有已订阅行情的最后一笔行情获取完成
    ...
    async def _ensure_serial(self, ins, dur, chart_id=None):
        if (ins, dur) not in self._requests:
            ...
            self._requests[(ins, dur)] = {
                "chart_id_set": set([chart_id]) if chart_id else set(),  # 记录当前 serial 对应的 chart_id
                "generator": self._gen_serial(ins, dur),
            }
            await self._fetch_serial((ins, dur))
        elif chart_id:
            self._requests[(ins, dur)]["chart_id_set"].add(chart_id)
    ...
    async def _fetch_serial(self, key):
        s = self._requests[key]
        try:
            s["timestamp"], s["diff"], s["quotes"] = await s["generator"].__anext__()
        except StopAsyncIteration:
            del self._requests[key]  # 删除一个行情时间超过结束时间的serial
                ...
                async for _ in update_chan:
                    ...
                    while True:
                        ...
                        if dur == 0:
                            ...
                            for chart_id in self._requests[(ins, dur)]["chart_id_set"]:
                                diff["charts"] = {
                                    chart_id: {
                            ...
                            timestamp = item["datetime"] + dur - 1000 \
                                if dur < 86400000000000 else _get_trading_day_end_time(item["datetime"]) - 999
                            if timestamp > self._end_dt:  # 超过结束时间
                                return
                            diff = {
                                "klines": {
