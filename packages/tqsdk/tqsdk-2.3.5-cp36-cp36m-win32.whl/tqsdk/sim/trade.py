...
from tqsdk.sim.utils import _get_price_range, _get_option_margin, _get_premium, _get_close_profit
...
class SimTrade(object):
    ...
    def update_quotes(self, symbol, pack):
        ...
        if position["volume_long"] > 0 or position["volume_short"] > 0:
            if position["last_price"] != quote["last_price"] or (math.isnan(underlying_last_price) or underlying_last_price != position["underlying_last_price"]):
