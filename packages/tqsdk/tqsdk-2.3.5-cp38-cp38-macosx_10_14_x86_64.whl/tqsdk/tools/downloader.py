...
class DataDownloader:
    ...
    def __init__(self, api: TqApi, symbol_list: Union[str, List[str]], dur_sec: int, start_dt: Union[date, datetime],
        ...
        self._timeout_seconds = 30  # 下载合约超时时间
        for symbol in self._symbol_list:
            if symbol in DEAD_INS:
                self._timeout_seconds = 2  # 已下市的没有交易的合约，超时时间可以设置短一点，用户不希望自己的程序因为没有下载到数据而中断
    ...
    async def _run(self):
        ...
        if self._adj_type:
            for s in self._symbol_list:
                quote = self._api.get_quote(s)  # 在Downloader初始化时，已经执行过 _ensure_symbol, 这里已经一定有合约信息, 并且这里只需要合约信息
                if quote.ins_class in ["STOCK", "FUND"]:
                    self._dividend_cache[s] = {
    ...
    def _get_data_cols(self):
        if self._dur_nano != 0:
            ...
            price_range = 1
            for symbol in self._symbol_list:
                if symbol.split('.')[0] in {"SHFE", "SSE", "SZSE"}:
                    price_range = 5
                    break
