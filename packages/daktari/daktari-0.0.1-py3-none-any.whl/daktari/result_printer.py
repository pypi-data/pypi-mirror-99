...
def get_check_status_symbol(status: CheckStatus) -> str:
...
def get_most_specific_suggestion(this_os: str, suggestions: Dict[str, str]) -> Optional[str]:
    def get_suggestion(os_to_match: str) -> Optional[str]:
        filtered = [message for os, message in suggestions.items() if os == os_to_match]
        if len(filtered) > 0:
            return filtered[0]
        else:
            return None
...
def print_check_result(result: CheckResult):
    this_os = detect_os()
    status_symbol = get_check_status_symbol(result.status)
    colour = green if result.status == CheckStatus.PASS else red
    print(f"{status_symbol} [{colour(result.check_name)}] {result.summary}")
