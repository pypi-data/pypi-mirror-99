...
class AdbTools(object):
    ...
    def __init__(self, device_id=''):
        ...
        self.__find = ''
        self.__command = r'F:\Tools\ADB\ADB\adb.exe'
    ...
    def __check_adb(self):
        """
        检查adb
        判断是否设置环境变量ANDROID_ADB
        :return:
        """
        '''
        if "ANDROID_ADB" in os.environ:
            if self.__system == "Windows":
                # path = os.path.join(os.environ["ANDROID_ADB"], "platform-tools", "adb.exe")
                path = os.path.join(os.environ["ANDROID_ADB"], "adb.exe")
                if os.path.exists(path):
                    self.__command = path
                else:
                    raise EnvironmentError(
                        "Adb not found in $ANDROID_ADB path: %s." % os.environ["ANDROID_ADB"])
            else:
                path = os.path.join(os.environ["ANDROID_ADB"], "adb")
                if os.path.exists(path):
                    self.__command = path
                else:
                    raise EnvironmentError(
    ...
    def screenshot(self, target_path=''):
        ...
        outPath = target_path
        if target_path == '':
            outPath = os.path.dirname(__file__)#os.path.expanduser('~')
        self.pull('/sdcard/%s.png' % (format_time,), outPath)
...
if __name__ == '__main__':
    adb = AdbTools()
    adb.screenshot()
    # print(zqpy.ADBService.GetAppCurr())