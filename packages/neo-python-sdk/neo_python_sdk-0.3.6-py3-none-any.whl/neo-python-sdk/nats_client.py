...
class NeoNatsClient(NATS):
    ...
    async def connect(self, loop):
        await super().connect(f'nats://{self.npq_host}:{self.npq_port}', user=self.npq_user, password=self.npq_pass, io_loop=loop)
        self.logger.info(
            f'Connected to nats://{self.npq_host}:{self.npq_port}')
    ...
    async def create(self, queue, particle={}, options={'timeout': 15000}):
        result = await super().request(queue, json.dumps(particle).encode(), timeout=options['timeout'])
