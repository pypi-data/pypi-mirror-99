    ...
    def __init__(self, ip: str, port: int, address: str,
        ...
        try:
            if mode == HTTP_TYPE.GET:
                self.requests = requests.get(url, headers=self.headers, params=json.dumps(self.params))
            elif mode == HTTP_TYPE.POST:
                self.requests = requests.post(url, headers=self.headers, data=json.dumps(self.params))
