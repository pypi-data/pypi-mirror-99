...
def parse(value, pattern='{head}{padding}{tail} [{ranges}]'):
    ...
    pattern_regex = r'^{0}$'.format(pattern_regex)
    ...
    if groups.get('range', None) is not None:
        start, end = [int(part) for part in groups['range'].split('-')]
    ...
    if groups.get('ranges', None) is not None:
        ...
        for part in parts:
            index_range = [int(range_part) for range_part in part.split('-', 2)]
    ...
    if 'holes' in groups:
        ...
        for part in parts:
            index_range = [int(hole_part) for hole_part in part.split('-', 2)]
