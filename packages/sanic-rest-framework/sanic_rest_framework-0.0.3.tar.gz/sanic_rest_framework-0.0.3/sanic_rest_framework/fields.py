...
from tortoise.fields.relational import RelationalField, ManyToManyField
from tortoise.exceptions import DoesNotExist

from sanic_rest_framework.exceptions import ValidationError
from sanic_rest_framework.validators import (
...
class Field:
    ...
    def __init__(self, read_only=False, write_only=False, required=False, allow_null=False,
                 allow_empty=False, default=empty, source=None, validators=None, error_messages=None,
    ...
    def get_external_value(self, data: Mapping) -> Any:
        ...
        if not isinstance(data, Mapping):
            raise ValidationError('传入的数据为无效数据类型，仅支持字典类型'.format(self.field_name))
    ...
    async def async_get_attribute(self, instance, attr):
        """适用于 Model 对象"""
        return await getattr(instance, attr)
    ...
    def run_validators(self, data) -> None:
        ...
        for validator in self.validators:
            try:
                ...
                if hasattr(exc, 'code') and exc.code in self.error_messages and isinstance(exc.message, ValidationError):
                    exc.message = self.error_messages[exc.code]
    ...
    def raise_error(self, _key, **kwargs):
        ...
        raise ValidationError(message_string, code=_key)
...
class CharField(Field):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.max_length is not None:
            self.validators.append(MaxLengthValidator(max_length=self.max_length, error_messages={'max_length': self.error_messages['max_length']}))
        if self.min_length is not None:
            self.validators.append(MinLengthValidator(min_length=self.min_length, error_messages={'min_length': self.error_messages['min_length']}))
...
class IntegerField(Field):
    ...
    def __init__(self, max_value=None, min_value=None, *args, **kwargs):
        ...
        if self.max_value is not None:
            self.validators.append(MaxValueValidator(max_value=self.max_value, error_messages={'max_value': self.error_messages['max_value']}))
        if self.min_value is not None:
            self.validators.append(MinValueValidator(min_value=self.min_value, error_messages={'min_value': self.error_messages['min_value']}))
...
class DateTimeField(Field):
    ...
    def __init__(self, output_format='%Y-%m-%d %H:%M:%S', input_formats='%Y-%m-%d %H:%M:%S',
    ...
    async def external_to_internal(self, data: Any) -> Any:
        ...
        if isinstance(data, str):
            try:
                data = datetime.strptime(data, self.input_formats)
            except (ValueError, TypeError):
                self.raise_error('convert')
...
class DateField(Field):
    ...
    async def external_to_internal(self, data: Any) -> Any:
        if isinstance(data, str):
            try:
                data = datetime.strptime(data, self.input_formats).date()
            except (ValueError, TypeError):
                self.raise_error('invalid', value=data)
        if isinstance(data, datetime):
...
class TimeField(Field):
    ...
    async def external_to_internal(self, data: Any) -> Any:
        if isinstance(data, str):
            try:
                data = datetime.strptime(data, self.input_formats).time()
                return data
            except (ValueError, TypeError):
...
class EnumChoiceField(Field):
    ...
    def external_to_internal(self, data: Any) -> Any:
        return self.enum_type(data) if data is not None else None
...
class SerializerMethodField(Field):
    ...
    def internal_to_external(self, data: Any) -> Any:
        method = getattr(self.parent, self.method_name)
        return method(data)

    def external_to_internal(self, data: Any) -> Any:
        raise ValidationError('SerializerMethodField 不支持反序列化')
