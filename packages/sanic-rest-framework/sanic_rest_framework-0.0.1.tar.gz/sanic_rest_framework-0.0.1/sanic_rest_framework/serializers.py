...
from sanic_rest_framework.fields import (
    empty, SkipField,
    Field, CharField, IntegerField, FloatField, DecimalField, BooleanField, DateTimeField, DateField, TimeField, ChoiceField, SerializerMethodField
)
from sanic_rest_framework.converter import ModelConverter
from .exceptions import ValidationError
from .helpers import BindingDict
...
class BaseSerializer(Field):
        ...
        if not hasattr(self, '_validated_data'):
            try:
                ...
                if hasattr(exc, 'error_dict'):
                    self._errors = exc.error_dict
                else:
                    self._errors = {self.field_name, exc.error_list}
            else:
                self._errors = {}
    ...
    async def data(self):
        """对外呈现的数据"""
        assert not self.instance is None, '调用 .data 必须先传入 instance= '
    ...
    async def internal_to_external(self, data: Any) -> Any:
        ...
        for field in fields:
            try:
                ...
                continue
            if value is None:
                res[field.field_name] = None
            else:
                res[field.field_name] = await field.internal_to_external(value)
        return res
    ...
    async def external_to_internal(self, data: Any) -> Any:
        ...
        for field in fields:
            ...
            try:
                ...
                errors[field.field_name] = exc.error_dict if hasattr(exc, 'error_dict') else exc.error_list
            except SkipField:
                pass
            else:
...
class ListSerializer(BaseSerializer):
    ...
    async def get_internal_value(self, instance: Any) -> Any:
        ...
        for attr in self.source_attrs:
            if isinstance(instance, Mapping):
                ...
                instance = await getattr(instance, attr)
    ...
    async def internal_to_external(self, data: Any) -> Any:
        ...
        :return:
        """
        iterable = await data.all() if inspect.isawaitable(data) else data
...
class ModelSerializer(Serializer):
    ...
    def get_field_kws_by_meta(self, field_name):
        ...
        if field_name in read_only_fields and field_name in write_only_fields:
            raise ValueError('字段 {} 不可用同时存在于类 ”{}“ 的 ’Meta.read_only_fields‘ '
                             '和 ’Meta.write_only_fields‘ 属性中'.format(field_name, self.__class__.__name__))
    ...
    def _clean_model_field(self, model_original_fields):
        ...
        for field_name, field_class in basis_fields.items():
            if isinstance(field_class, (tortoise_fields.relational.ForeignKeyFieldInstance, tortoise_fields.relational.OneToOneFieldInstance)):
    ...
    def _get_model_basis_fields(self, model_fields):
        ...
        :return:
        """
        return {field_name: field_class for field_name, field_class in model_fields.items() if not isinstance(field_class, tortoise_fields.relational.RelationalField)}
    #
