...
from sanic_rest_framework.fields import (
    CharField, ChoiceField, IntegerField, BooleanField,
    DecimalField, DateTimeField, DateField, TimeField,
    FloatField, EnumChoiceField
)
...
class ModelConverter(ModelConverterBase):
    ...
    def convert(self, serializer, model_field, **field_kwargs):
        ...
        if read_only and required:
            raise ValueError('{}序列化器内字段{}为必填项不能使用read_only属性'.format(
                type(serializer).__name__, model_field.model_field_name))
        if model_field.pk:
        ...
        if not isinstance(model_field, fields.relational.RelationalField):
            ...
            type_name = model_field.__class__.__name__
            if hasattr(serializer.Meta, 'depth'):
                nested_depth = serializer.Meta.depth
            else:
                nested_depth = 10
