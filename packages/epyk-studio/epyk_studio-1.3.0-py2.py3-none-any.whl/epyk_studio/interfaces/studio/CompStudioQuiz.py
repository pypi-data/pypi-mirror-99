...
class Quiz:
  ...
  def progress(self, percentage, icon="fas fa-car-side", width=(100, '%'), height=("auto", ''), options=None,
               profile=None):
    """
    Description:
    ------------
    ...
    """
    options = options or {}
    icon = self.page.ui.icons.awesome(icon, height=("auto", ''))
    progress = self.page.ui.div()
    progress.style.css.border = "1px solid %s" % self.page.theme.success[1]
    progress.style.css.display = "inline-block"
    progress.style.css.width = "calc(100% - 50px)"
    inner = self.page.ui.div("%s%%" % percentage, width=(percentage, '%'))
    inner.style.css.background = self.page.theme.success[0]
    inner.style.css.color = self.page.theme.success[1]
    inner.style.css.text_align = "center"
    progress.add(inner)
    container = self.page.ui.div([progress, icon], width=width, height=height)
    container.style.css.padding = "0 5px"
    return container

  def position(self, longitude, latitude, flag=None, width=(100, '%'), height=("auto", ''),
               icon="fas fa-map-marked-alt", options=None, profile=None):
    """
    Description:
    ------------
    ...
    :param profile: Boolean | Dictionary. Optional. A flag to set the component performance storage.
    """
    container = self.page.ui.div(width=width, height=height)
    container.icon = self.page.ui.icons.awesome(icon)
    container.icon.icon.style.css.font_factor(10)
    container.icon.icon.style.css.color = self.page.theme.greys[6]
    container.add(container.icon)
    if flag is None:
      container.button = self.page.ui.button("Check Position")
      container.button.style.css.line_height = 20
      container.add(container.button)
    elif flag:
      container.text = self.page.ui.text("Well done! Position validated")
      container.text.style.css.line_height = 20
      container.text.style.css.color = self.page.theme.success[1]
      container.text.style.css.font_factor(5)
      container.add(container.text)
    else:
      container.text = self.page.ui.text("Failed! Position not found")
      container.text.style.css.line_height = 20
      container.text.style.css.color = self.page.theme.danger[1]
      container.text.style.css.font_factor(5)
      container.add(container.text)
    return container

  def picture(self, image, question=None, icon="far fa-image", width=(100, '%'), height=("auto", ''), options=None,
              profile=None):
    """
    Description:
    ------------
    ...
    container.style.css.border = "2px solid %s" % self.page.theme.greys[3]
    container.style.css.padding = 10
    container.style.css.margin = 5
    container.style.css.width = "calc(%s%s - 10px)" % (width[0], width[1])
    container.icon = self.page.ui.icons.awesome(icon, width=(100, '%'), height=("auto", ''), profile=profile)
    container.icon.style.css.background = self.page.theme.colors[6]
    ...
    if "timer" in options:
      if options.get("timer", 0) > 0:
        container.image = self.page.ui.img(
          image, align="center", width=('auto', ''), height=(300, 'px'), profile=profile)
        chrono = self.chrono(container.image, time=options['timer'])
        container.add(chrono)
    else:
      container.image = self.page.ui.img(image, align="center", width=('auto', ''), height=(300, 'px'), profile=profile)
      container.add(container.image)
    if question is not None:
      question = self.page.ui.text(self.page.py.encode_html(question), align="center", profile=profile)
      question.style.css.font_factor(5)
      container.add(question)
    container.add(self.delimiter())
    input = self.page.ui.input(profile=profile)
    input.style.css.margin_top = 20
    input.style.css.margin_bottom = 10
    input.style.css.text_align = "left"
    container.add(input)
    button = self.page.ui.button("Validate", align="center", profile=profile)
    container.add(button)
    return container

  def question(self, question, icon="fas fa-question", width=(100, '%'), height=("auto", ''), options=None,
               profile=None):
    """
    Description:
    ------------
    ...
    container.icon.icon.style.css.color = "white"
    container.add(container.icon)
    question = self.page.ui.text(self.page.py.encode_html(question), width=(None, 'px'), profile=profile)
    question.style.css.font_factor(5)
    container.add(question)
    container.add(self.delimiter())
    input = self.page.ui.input(profile=profile)
  ...
  def charade(self, clues, question=None, icon="fas fa-book", width=(100, '%'), height=("auto", ''), options=None,
              profile=None):
    """
    Description:
    ------------
    ...
    container.add(container.icon)
    for clue in clues:
      value = self.page.ui.text(self.page.py.encode_html(clue), width=(None, 'px'), profile=profile)
  ...
  def delimiter(self, size=1):
    ...
    Attributes:
    ----------
    :param size: Integer. Optional. The hr size.
    """
  ...
  def missing(self, text, icon="fas fa-ellipsis-h", width=(100, '%'), height=("auto", ''), options=None, profile=None):
    ...
    container.add(container.icon)
    text_frg = text.split("*")
    container.add(self.page.ui.text(text_frg[0], profile=profile))
    if text.startswith("*"):
      input = self.page.ui.input("", width=(50, 'px'), profile=profile)
      input.style.css.margin = "0 5px"
      container.add(input)
    for text in text_frg[1:-1]:
      container.add(self.page.ui.text(text, profile=profile))
      input = self.page.ui.input("", width=(50, 'px'), profile=profile)
      input.style.css.margin = "0 5px"
      container.add(input)
    container.add(self.page.ui.text(text_frg[-1], profile=profile))
    if text.endswith("*"):
      container.add(self.page.ui.input("", profile=profile))
    button = self.page.ui.button("Validate", align="center", profile=profile)
    container.add(button)
    return container

  def hangman(self, question, text, proposals=None, icon="fas fa-project-diagram", width=(100, '%'),
    ...
    container.icon.icon.style.css.color = "white"
    container.add(container.icon)
    container.question = self.page.ui.text(question, width=(None, 'px'), profile=profile)
    container.question.style.css.font_factor(5)
    container.add(container.question)
    container.result = self.page.ui.text("".join(crypt_text), width=(None, 'px'), profile=profile)
  ...
  def choice(self, question, answers, status=None, icon="fas fa-question", width=(100, '%'), height=("auto", ''),
    ...
    for answer in answers:
      button = self.page.ui.button(answer, width=(100, '%'), profile=profile)
  ...
  def chrono(self, component, time=60, flag=None, icon="fas fa-stopwatch-20", width=(100, '%'), height=("auto", ''),
             options=None, profile=None):
    """
    Description:
    ------------
    ...
    """
    container = self.page.ui.div(width=width, height=height, options=options, profile=profile)
    container.timer = self.page.ui.calendars.timer(time / 60, width=(None, '%'), profile=profile)
    ...
    container.add(self.page.ui.div([self.page.ui.icons.awesome(icon), container.timer], profile=profile))
    container.add(component)
    container.timer.end(component.dom.remove())
    return container

  def estimate(self, text, tooltip=None, width=(None, '%'), height=("auto", ''), icon="fas fa-stopwatch",
               options=None, profile=None):
    """
    Description:
    ------------
    ...
    container.icon.icon.style.css.font_factor(10)
    container.icon.icon.style.css.color = self.page.theme.success[1]
    container.add(container.icon)
    container.text = self.page.ui.text(text, width=(None, 'px'), profile=profile)
  ...
  def level(self, text, tooltip, width=(None, '%'), height=("auto", ''), icon="fas fa-chess-bishop", options=None,
            profile=None):
    """
    Description:
    ------------
  ...
  def ranking(self, number=0, width=(None, '%'), height=("auto", ''), icon="fas fa-flag-checkered", options=None,
              profile=None):
    """
    Description:
    ------------
    ...
    container.icon.icon.style.css.color = self.page.theme.success[1]
    container.add(container.icon)
    container.number = self.page.ui.texts.number(number, width=(None, 'px'), profile=profile)
    container.number.style.css.line_height = 20
    container.number.style.css.color = self.page.theme.success[1]
  ...
  def interval(self, start, end, flag=None, width=(100, '%'), height=("auto", ''), icon="fas fa-hourglass-half",
               options=None, profile=None):
    """
    Description:
    ------------
  ...
  def rating(self, rating, customers, url=None, align="left", width=(300, 'px'), height=("auto", ''), options=None,
             profile=None):
    """
    Description:
    ------------
    ...
    container.stars = self.page.ui.rich.stars(rating, best=options.get("stars", 5))
    container.stars.style.display = "inline-block"
    container.stars.style.margin_right = 5
    container.link = self.page.ui.link("(%s)" % customers, url)
  ...
  def quality(self, votes, url=None, align="left", width=(300, 'px'), height=("auto", ''), options=None, profile=None):
    ...
    for i in reversed(range(dflt_options.get("stars", 5))):
      if url is not None:
        vote_records.append({"star": self.page.ui.link("%s stars" % i, url % i), "value":  votes.get(i, 0) / customers * 100})
      else:
        vote_records.append({"star": "%s stars" % i, "value":  votes.get(i, 0) / customers * 100})
    comp = self.page.ui.charts.skillbars(
      vote_records, y_column='value', x_axis='star', options=dflt_options).css({"width": "%s%s" % (width[0], width[1])})
    return comp

  def cup(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def bookmark(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def first(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def brain(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def puzzle(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def numbers(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def alphabet(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def bulb(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def road(self, fill=None, border=None, width=(30, "px"), height=(30, "px"), options=None, profile=None):
    """
    Description:
    ------------
  ...
  def summary(self, records, width=('auto', '%'), height=(None, "px"), options=None, profile=None):
    ...
    for values in records:
      ...
      if not hasattr(rec[0], 'options'):
        name = self.page.ui.text(rec[0], align="left", width=(100, 'px'), profile=profile)
    ...
    table = self.page.ui.tables.basic(
      html_records, rows=[0], cols=[1, 2, 3, 4], width=width,  height=height, options=options, profile=profile)

    table[0][-1].style.css.border_top = "2px dashed %s" % self.page.theme.success[1]
    for row in table:
      row[-1].style.css.background = self.page.theme.colors[2]
      row[-1].style.css.border_left = "2px dashed %s" % self.page.theme.success[1]
      row[-1].style.css.border_right = "2px dashed %s" % self.page.theme.success[1]
      row.style.css.border_bottom = "1px solid %s" % self.page.theme.colors[3]
    table[-1][-1].style.css.border_bottom = "2px dashed %s" % self.page.theme.success[1]
    table.set_header(["Challenge", 'Score', 'Time', 'Date', 'Ranking'], css={"padding": '5px 20px'})
    return table

  def checks(self, question, answers, html_code=None, options=None, profile=None):
    """
    Description:
    ------------
    ...
    for a in answers:
      if isinstance(a, dict):
        container.add(self.page.ui.fields.checkbox(
          a.get('value', False), a['text'], width=(100, '%'), options=options, profile=profile))
      else:
        container.add(self.page.ui.fields.checkbox(False, a, width=(100, '%'), options=options, profile=profile))
    container.submit = self.page.ui.button("Submit", align="center", profile=profile)
    container.add(container.submit)
    return container

  def yesno(self, question, labels=None, html_code=None, options=None, profile=None):
    """
    Description:
    ------------
    ...
    container.add(self.page.ui.inputs.radio(False, labels[0], group_name=container.htmlCode, profile=profile))
    container.add(self.page.ui.inputs.radio(False, labels[1], group_name=container.htmlCode, profile=profile))
    container.submit = self.page.ui.button("Submit", align="center", profile=profile)
    container.add(container.submit)
    return container
