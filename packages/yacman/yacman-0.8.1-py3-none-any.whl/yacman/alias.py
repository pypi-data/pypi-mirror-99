...
class AliasedYacAttMap(YacAttMap):
    ...
    def __init__(
        ...
        for k, v in list(self[IK][ALIASES_KEY_RAW].items()):
            for alias in v:
    ...
    def get_aliases(self, key):
        ...
        """
        aliases = []
        for a, k in list(self[IK][ALIASES_KEY].items()):
            if k == key:
                aliases.append(a)
        if aliases:
            return aliases
    ...
    def get_key(self, alias):
        ...
        if alias in list(self[IK][ALIASES_KEY].keys()):
...
def is_aliases_mapping_valid(aliases, strictness=None):
    ...
    if isinstance(aliases, Mapping):
        if all([isinstance(v, list) for k, v in list(aliases.items())]):
            return True
