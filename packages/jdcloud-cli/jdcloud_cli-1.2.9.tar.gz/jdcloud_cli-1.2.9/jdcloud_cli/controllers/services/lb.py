...
class LbController(BaseController):
    ...
    @expose(
        arguments=[
            ...
            (['--filters'], dict(help="""(array: filter) backendIds - 后端服务Id列表，支持多个; backendNames - 后端服务名字列表，支持多个; loadBalancerId - 负载均衡器Id，支持单个; agId - 可用性组Id，支持单个; loadBalancerType - 负载均衡类型，取值为：alb、nlb、dnlb，默认alb，支持单个; protocol - 后端服务的协议【alb】支持Http、Tcp，【nlb】支持Tcp，默认查询所有，支持单个;  """, dest='filters',  required=False)),
    ...
    @expose(
        arguments=[
            ...
            (['--proxy-protocol'], dict(help="""(bool) 【alb Tcp协议】获取真实ip, 取值为False(不获取)或者True(获取,支持Proxy Protocol v1版本)，默认为False """, dest='proxyProtocol', type=bool, required=False)),
    ...
    @expose(
        arguments=[
            ...
            (['--target-group-ids'], dict(help="""(array: string) 虚拟服务器组的Id列表，目前只支持一个，且与agIds不能同时存在 """, dest='targetGroupIds',  required=False)),
            (['--ag-ids'], dict(help="""(array: string) 高可用组的Id列表，目前只支持一个，且与targetGroupIds不能同时存在 """, dest='agIds',  required=False)),
            (['--proxy-protocol'], dict(help="""(bool) 【alb Tcp协议】是否启用Proxy ProtocolV1协议获取真实源ip, 取值为false(不开启)或者true(开启), 默认为false """, dest='proxyProtocol', type=bool, required=False)),
            (['--description'], dict(help="""(string) 描述,允许输入UTF-8编码下的全部字符，不超过256字符 """, dest='description',  required=False)),
            (['--session-stickiness'], dict(help="""(bool) 会话保持, 取值为false(不开启)或者true(开启)，默认为false <br>【alb Http协议，RoundRobin算法】支持基于cookie的会话保持 <br>【nlb】支持基于报文源目的IP的会话保持 """, dest='sessionStickiness', type=bool, required=False)),
    ...
    @expose(
        arguments=[
            ...
            (['--certificate-specs'], dict(help="""(array: certificateSpec) 【alb Https和Tls协议】Listener绑定的默认证书，只支持一个证书 """, dest='certificateSpecs',  required=False)),
